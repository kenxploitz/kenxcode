...
export async function resolveGatewayWsUrl(deps: ResolveGatewayWsUrlDeps, conn: GatewayWsConnection): Promise<string> {
  ...
  if (conn.authMode === 'oauth') {
    if (!mint) {
      throw new Error('This Desktop build cannot refresh OAuth WebSocket tickets. Update KenXCode Desktop and try again.')
    }
...
export interface KenXCodeWebSocketUrlOptions {
...
export function buildKenXCodeWebSocketUrl(options: KenXCodeWebSocketUrlOptions): string {
