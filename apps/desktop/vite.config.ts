...
const emojibaseAssets = () => ({
  name: 'kenxcode:emojibase-assets',
...
export default defineConfig(({ command }) => ({
  ...
  css: {
    ...
    // `C:\Users\<name>\AppData\Local\kenxcode\kenxcode-agent\apps\desktop`), so an
  ...
  resolve: {
    alias: {
      ...
      '@kenxcode/plugin-sdk': path.resolve(__dirname, './src/sdk/index.ts'),
      '@kenxcode/shared/billing': path.resolve(__dirname, '../shared/src/billing-types.ts'),
      '@kenxcode/shared': path.resolve(__dirname, '../shared/src'),
