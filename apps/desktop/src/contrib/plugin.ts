 ...
 * `KenXCodePlugin`; it never touches the registry directly — it receives a
...
import { pluginRest, type PluginRestOptions, pluginSocket } from '@/kenxcode'
...
export type { PluginRestOptions } from '@/kenxcode'
 ...
 *  under `kenxcode.plugin.<id>.` — plugins can't read or clobber each other. */
...
export interface KenXCodePlugin {
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `kenxcode.plugin.${pluginId}.${key}`
