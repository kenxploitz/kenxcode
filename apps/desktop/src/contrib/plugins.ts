 ...
 *    `KenXCodePlugin` registers automatically (vite glob — drop a folder in).
 *    None ship in-tree today; reference/demo plugins live in the companion
 *    `kenxcode-example-plugins` repo.
 *  - RUNTIME: the on-disk door (`<kenxcode home>/desktop-plugins/<name>/plugin.js`)
...
const modules = import.meta.glob<{ default: KenXCodePlugin }>('../plugins/*/plugin.{ts,tsx}', { eager: true })
...
export function discoverBundledPlugins(): void {
  ...
  for (const [path, mod] of Object.entries(modules)) {
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      console.warn(`[plugins] ${path} has no valid default KenXCodePlugin export — skipped`)
