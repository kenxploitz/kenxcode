import type { GatewayWsUrlResult } from '@kenxcode/shared'
...
declare global {
  interface Window {
    kenxcodeDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<KenXCodeConnection>
      ...
      // KenXCode Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new KENXCODE_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: KenXCodeApiRequest) => Promise<T>
      notify: (payload: KenXCodeNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<KenXCodeReadFileTextResult>
      selectPaths: (options?: KenXCodeSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<KenXCodePreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<KenXCodePreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<KenXCodePreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: KenXCodeActiveWork) => void
      setTitleBarTheme?: (payload: KenXCodeTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<KenXCodeReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<KENXCODE_HOME>/desktop-plugins),
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<KenXCodeGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<KenXCodeGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<KenXCodeGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<KenXCodeRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: KenXCodeReviewScope, baseRef?: null | string) => Promise<KenXCodeReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: KenXCodeReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<KenXCodeReviewShipInfo>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: KenXCodeTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<KenXCodeTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: KenXCodeWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: KenXCodePreviewFileChanged) => void) => () => void
...
export interface KenXCodeTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface KenXCodeTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `kenxcode update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  kenxcodeRoot?: string
...
export interface KenXCodeConnection {
  baseUrl: string
...
export interface KenXCodeTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface KenXCodeActiveWork {
  count: number
  titles: string[]
}

export interface KenXCodeWindowState {
  isFullscreen: boolean
  isMinimized?: boolean
  isVisible?: boolean
  nativeOverlayWidth: number
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a KenXCode Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted KenXCode Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteToken?: string
  remoteUrl?: string
  // For a 'cloud' connection: the selected KenXCode Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // the kenxcode gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered KenXCode Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface KenXCodeApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface KenXCodeNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface KenXCodePreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface KenXCodeReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface KenXCodePreviewWatch {
  id: string
  path: string
}
...
export interface KenXCodeGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface KenXCodeGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
...
// preselect it.
export interface KenXCodeGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface KenXCodeRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface KenXCodeRepoStatus {
  branch: null | string
...
// turn began.
export type KenXCodeReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface KenXCodeReviewFile {
  path: string
  added: number
  removed: number
...
export interface KenXCodeReviewList {
  files: KenXCodeReviewFile[]
...
export interface KenXCodeReviewPr {
  url: string
  state: string
  number: number
}
...
export interface KenXCodeReviewShipInfo {
  ghReady: boolean
  pr: KenXCodeReviewPr | null
}

export interface KenXCodeReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface KenXCodeReadDirResult {
  entries: KenXCodeReadDirEntry[]
  error?: string
}

export interface KenXCodePreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface KenXCodeSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
