...
const KENXCODE_REF_TYPES = WIRE_REFERENCE_KINDS
type KenXCodeRefType = (typeof KENXCODE_REF_TYPES)[number]
...
const KENXCODE_DIRECTIVE_RE = referenceRe()
...
export const kenxcodeDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(KENXCODE_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...kenxcodeDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing KenXCode directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.kenxcodeDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.kenxcodeDesktop?.readFileDataUrl(id)
