...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to KenXCode gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>) {
  const original = window.kenxcodeDesktop
  Object.defineProperty(window, 'kenxcodeDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config }
  })

  return () => Object.defineProperty(window, 'kenxcodeDesktop', { configurable: true, value: original })
}
