...
import type { KenXCodeReviewFile, KenXCodeReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — KenXCode' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'kenxcode.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'kenxcode.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'kenxcode.desktop.reviewTreeMode'
const SELECTED_KEY = 'kenxcode.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<KenXCodeReviewFile[]>([])
...
export const $reviewShipInfo = atom<KenXCodeReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['kenxcodeDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: KenXCodeReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly KenXCodeReviewFile[], path: string): KenXCodeReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.kenxcodeDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.kenxcodeDesktop?.openExternal?.(url)
    }
