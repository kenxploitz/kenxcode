...
import type { KenXCodeGitWorktree, KenXCodeRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: KenXCodeGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, KenXCodeRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, KenXCodeGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<KenXCodeRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<KenXCodeGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<KenXCodeRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<KenXCodeGitWorktree[]>>()
const $noRepoStatus = atom<KenXCodeRepoStatus | null>(null)
const $noWorktrees = atom<KenXCodeGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<KenXCodeRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<KenXCodeGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: KenXCodeRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: KenXCodeGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<KenXCodeRepoStatus | null>
  seq: number
  target: string
}
