import { type ConnectionState, type GatewayEvent, resolveGatewayWsUrl } from '@kenxcode/shared'
...
const isOpen = (gateway: KenXCodeGateway | null): boolean => gateway?.connectionState === 'open'
...
interface GatewayRegistryState {
  ...
  primaryGateway: KenXCodeGateway | null
  primaryProfile: string
  activeKey: string
  secondaries: Map<string, Secondary>
  $gateway: ReturnType<typeof atom<KenXCodeGateway | null>>
}

const STATE_KEY = Symbol.for('kenxcode.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<KenXCodeGateway | null>(null)
  }
}
...
export function setPrimaryGateway(gateway: KenXCodeGateway | null, profile = 'default'): void {
...
export function activeGateway(): KenXCodeGateway | null {
...
function createSecondary(profile: string): Secondary {
  const gateway = new KenXCodeGateway()
...
export async function ensureActiveGatewayOpen(): Promise<KenXCodeGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
