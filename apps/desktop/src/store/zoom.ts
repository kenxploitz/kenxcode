...
export function setZoomPercent(percent: number): void {
  window.kenxcodeDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.kenxcodeDesktop?.zoom) {
  void window.kenxcodeDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.kenxcodeDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
