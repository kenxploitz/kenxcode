...
// reaches the gateway); everything else routes through window.kenxcodeDesktop.git,
...
function file(path: string, over: Partial<KenXCodeReviewFile> = {}): KenXCodeReviewFile {
  return { path, status: 'modified', staged: false, added: 1, removed: 0, ...over } as KenXCodeReviewFile
}

type ReviewStub = Record<string, ReturnType<typeof vi.fn>>

// Install a review bridge on window.kenxcodeDesktop. Any op not supplied defaults
...
describe('ship flow', () => {
  ...
  it('createOrOpenPr opens the existing PR without creating a new one', async () => {
    const review = stubReview()
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'https://example.com/pr/9' } } as KenXCodeReviewShipInfo)

    await createOrOpenPr()

    expect(review.createPr).not.toHaveBeenCalled()
    expect(
      (window.kenxcodeDesktop as unknown as { openExternal: ReturnType<typeof vi.fn> }).openExternal
...
describe('refreshShipInfo', () => {
  it('populates ship info from the bridge', async () => {
    const info: KenXCodeReviewShipInfo = {
      ghReady: true,
      pr: { url: 'https://example.com/pr/3' }
    } as KenXCodeReviewShipInfo
  ...
  it('resets ship info when there is no bridge', async () => {
    delete (window as unknown as { kenxcodeDesktop?: unknown }).kenxcodeDesktop
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'x' } } as KenXCodeReviewShipInfo)
