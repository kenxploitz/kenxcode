...
import type { KenXCodeRepoStatus } from '@/global'
...
const sampleStatus: KenXCodeRepoStatus = {
...
const otherStatus: KenXCodeRepoStatus = {
  ...sampleStatus,
...
function stubProbe(impl: (cwd: string) => Promise<KenXCodeRepoStatus | null>) {
  ;(window as unknown as { kenxcodeDesktop?: unknown }).kenxcodeDesktop = { git: { repoStatus: impl } }
}
...
describe('refreshRepoStatus', () => {
  ...
  it('never publishes an old worktree status onto the primary after the active cwd moves', async () => {
    let resolveOld!: (status: KenXCodeRepoStatus | null) => void
    stubProbe(
      () =>
        new Promise(resolve => {
          resolveOld = resolve
  ...
  it('runs one probe at a time and coalesces overlap into one trailing refresh per drain', async () => {
    const resolvers: Array<(status: KenXCodeRepoStatus | null) => void> = []
    const calls: string[] = []
    let active = 0
    let maxActive = 0
