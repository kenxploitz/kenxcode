...
vi.mock('@/kenxcode', () => ({
  getKenXCodeConfigRecord: vi.fn(async () => ({})),
  saveKenXCodeConfig: vi.fn(async () => undefined)
}))
...
describe('applyVoiceStopPhraseFromConfig', () => {
  ...
  it('uses the first configured phrase so a custom phrase renders correctly', () => {
    applyVoiceStopPhraseFromConfig({ voice: { stop_phrases: ['goodbye kenxcode', 'stop'] } })
    expect($voiceStopPhrase.get()).toBe('goodbye kenxcode')
  })
