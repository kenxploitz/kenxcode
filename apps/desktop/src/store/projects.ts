...
import type { KenXCodeGitBaseBranch, KenXCodeGitBranch } from '@/global'
import { getKenXCodeConfig, type KenXCodeGateway } from '@/kenxcode'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/kenxcode'
...
const PROJECT_SCOPE_KEY = 'kenxcode.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('KenXCode gateway is not connected')
  }
...
async function activeProjectsContext(): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== ($activeGatewayProfile.get() || 'default')) {
    throw new Error('Active KenXCode profile changed while connecting')
  }
...
async function refreshProjectTreeOn(gateway: KenXCodeGateway): Promise<void> {
...
const repoScanStates = new WeakMap<KenXCodeGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<KenXCodeGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getKenXCodeConfig(context.profile))
...
// repo, returning where KenXCode should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<KenXCodeGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<KenXCodeGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.kenxcodeDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.kenxcodeDesktop?.writeClipboard?.(path)
  }
}
