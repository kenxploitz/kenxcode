 ...
 * app's bounds and stays visible while KenXCode is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'kenxcode.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'kenxcode.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.kenxcodeDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.kenxcodeDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.kenxcodeDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.kenxcodeDesktop?.petOverlay?.close()
}
