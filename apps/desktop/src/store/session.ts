import type { ConnectionState } from '@kenxcode/shared'
import { atom, computed } from 'nanostores'

import { lastVisibleMessageIsUser } from '@/app/chat/thread-loading'
import type { ContextSuggestion } from '@/app/types'
import type { KenXCodeConnection } from '@/global'
import type { ChatMessage } from '@/lib/chat-messages'
import { persistBoolean, persistString, storedBoolean, storedString } from '@/lib/storage'
import type { SessionInfo, UsageStats } from '@/types/kenxcode'
...
const COMPOSER_MODEL_KEY = 'kenxcode.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'kenxcode.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'kenxcode.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'kenxcode.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'kenxcode.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'kenxcode.desktop.lastSessionId'
...
const LAST_ROUTE_KEY = 'kenxcode.desktop.lastRoute'
...
function workspaceCwdKey(connection: KenXCodeConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.kenxcodeDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.kenxcodeDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<KenXCodeConnection | null>(null)
...
export const setConnection = (next: Updater<KenXCodeConnection | null>) => updateAtom($connection, next)
...
// until config loads, and re-seeded on every profile switch by useKenXCodeConfig.
