...
const kenxcode = await import('@/kenxcode')
const getKenXCodeConfig = vi.mocked(kenxcode.getKenXCodeConfig)
...
describe('project scope', () => {
  ...
  it('persists the scope to localStorage', () => {
    enterProject('p_abc')
    expect(window.localStorage.getItem('kenxcode.desktop.projectScope')).toBe('p_abc')
  })
})
...
describe('resolveNewSessionCwd', () => {
  ...
  it('inherits the focused session workspace when not drilled into a project', () => {
    ...
    $sessions.set([
      {
        archived: false,
        cwd: '/Users/me/www/kenxcode-agent',
        ended_at: null,
        id: 'sess-a',
        input_tokens: 0,
        is_active: true,
    ...
    expect(resolveNewSessionCwd()).toBe('/Users/me/www/kenxcode-agent')
  })
...
describe('repository discovery policy', () => {
  ...
  it('records disabled policy without invoking the filesystem scanner', async () => {
    ...
    getKenXCodeConfig.mockResolvedValue({
      desktop: {
  ...
  it('passes custom roots and exclusions to Electron and records on the origin gateway', async () => {
    ...
    expect(getKenXCodeConfig).toHaveBeenCalledWith('default')
  ...
  it('does not scan the local filesystem for remote connections', async () => {
    ...
    expect(scanRepos).not.toHaveBeenCalled()
    expect(getKenXCodeConfig).not.toHaveBeenCalled()
  })
})
