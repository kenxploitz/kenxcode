...
import { getProfiles, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/kenxcode'
...
import type { ProfileInfo } from '@/types/kenxcode'
...
// /api/profiles/active `current`). "default" is the root ~/.kenxcode. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'kenxcode.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'kenxcode.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.kenxcodeDesktop.api<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new KENXCODE_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.kenxcodeDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.kenxcodeDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'kenxcode.desktop.showAllProfiles'
...
// Switch to the default (root ~/.kenxcode) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.kenxcodeDesktop?.touchBackend?.(target).catch(() => undefined)
}
