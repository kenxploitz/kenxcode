...
const { getProfiles } = await import('@/kenxcode')
...
const profile = (name: string, isDefault = false): ProfileInfo => ({
  ...
  model: null,
  name,
  path: `/tmp/kenxcode/${name}`,
  provider: null,
  skill_count: 0
})

const remoteConn = (over: Partial<KenXCodeConnection> = {}): KenXCodeConnection =>
  ({ baseUrl: 'https://kenxcode-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as KenXCodeConnection

const localConn = (over: Partial<KenXCodeConnection> = {}): KenXCodeConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as KenXCodeConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<KenXCodeConnection>>()
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { kenxcodeDesktop: { getConnection } })
