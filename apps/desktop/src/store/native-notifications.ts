...
const STORAGE_KEY = 'kenxcode:native-notifications'
...
// "Backgrounded" = the user isn't on KenXCode. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.kenxcodeDesktop?.notify({
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.kenxcodeDesktop
