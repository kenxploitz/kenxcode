...
import { getKenXCodeConfigRecord, saveKenXCodeConfig } from '@/kenxcode'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getKenXCodeConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveKenXCodeConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
