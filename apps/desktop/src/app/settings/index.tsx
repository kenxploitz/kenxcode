...
import { getKenXCodeConfigDefaults, getKenXCodeConfigRecord, saveKenXCodeConfig } from '@/kenxcode'
...
export function SettingsView({ onClose, onConfigSaved, onMainModelChanged }: SettingsPageProps) {
  ...
  const exportConfig = async () => {
    try {
      const cfg = await getKenXCodeConfigRecord()
      const blob = new Blob([JSON.stringify(cfg, null, 2)], { type: 'application/json' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = 'kenxcode-config.json'
      a.click()
  ...
  const resetConfig = async () => {
    ...
    try {
      await saveKenXCodeConfig(await getKenXCodeConfigDefaults())
      triggerHaptic('success')
      onConfigSaved?.()
    } catch (err) {
