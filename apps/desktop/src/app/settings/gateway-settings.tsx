...
// KenXCode Cloud discovery lifecycle for the cloud-mode panel.
...
export function GatewaySettings({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  // --- KenXCode Cloud (cloud mode) state ---
  ...
  useEffect(() => {
    let cancelled = false
    const desktop = window.kenxcodeDesktop
  ...
  useEffect(() => {
    if (state.mode !== 'ssh' || !window.kenxcodeDesktop?.sshConfigHosts) {
      return
    }

    let cancelled = false
    void window.kenxcodeDesktop
      .sshConfigHosts()
      .then(result => {
        if (!cancelled) {
  ...
  const payload = () => ({
    ...
    sshRemoteKenXCodePath: state.sshRemoteKenXCodePath.trim(),
  ...
  const save = async (apply: boolean) => {
    ...
    try {
      const next = apply
        ? await window.kenxcodeDesktop.applyConnectionConfig(payload())
        : await window.kenxcodeDesktop.saveConnectionConfig(payload())
      ...
      const errors = {
        ...
        'kenxcode-not-found': g.sshErrNotInstalled,
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const saved = await window.kenxcodeDesktop.saveConnectionConfig({
        mode: state.mode,
        profile: scope ?? undefined,
      ...
      const result = await window.kenxcodeDesktop.oauthLoginConnectionConfig(trimmedUrl)
      ...
      if (result.connected) {
        const refreshed = await window.kenxcodeDesktop.getConnectionConfig(scope)
  ...
  const signOut = async () => {
    ...
    try {
      await window.kenxcodeDesktop.oauthLogoutConnectionConfig(trimmedUrl || undefined)
      const refreshed = await window.kenxcodeDesktop.getConnectionConfig(scope)
  ...
  const resolveSshHost = async (host: string) => {
    if (!host || !window.kenxcodeDesktop?.sshResolveHost) {
      return
    }
    ...
    try {
      const resolved = await window.kenxcodeDesktop.sshResolveHost(host)
  ...
  const testSsh = async () => {
    ...
    try {
      const result = await window.kenxcodeDesktop.testConnectionConfig(payload())
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.kenxcodeDesktop.testConnectionConfig({
        mode: 'remote',
        profile: scope ?? undefined,
        remoteAuthMode: authMode,
        remoteToken: authMode === 'token' ? remoteToken.trim() || undefined : undefined,
  ...
  if (!window.kenxcodeDesktop?.getConnectionConfig) {
  ...
  return (
    <SettingsContent bare={embedded}>
      ...
      {/* KenXCode Cloud panel: one portal sign-in, then a discovered-agent picker
          ...
          <ListRow
            action={
              <Input
                ...
                placeholder="https://gateway.example.com/kenxcode"
        ...
        <div className="mt-5 grid gap-1">
          ...
          <ListRow
            action={
              <Input
                className={cn('h-8 font-mono', CONTROL_TEXT)}
                onChange={event => setState(current => ({ ...current, sshRemoteKenXCodePath: event.target.value }))}
                placeholder={g.sshKenXCodePathPlaceholder}
                value={state.sshRemoteKenXCodePath}
              />
            }
            description={g.sshKenXCodePathDesc}
            title={g.sshKenXCodePathTitle}
          />
          {scope !== null ? (
            <ListRow
              action={
      ...
      {embedded ? null : (
        <div className="mt-6 grid gap-1">
          <ListRow
            action={
              <Button onClick={() => void window.kenxcodeDesktop?.revealLogs()} size="sm" variant="textStrong">
                <FileText />
                {g.openLogs}
              </Button>
            }
