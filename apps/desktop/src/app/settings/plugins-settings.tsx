...
function reveal(file: string) {
  void window.kenxcodeDesktop?.revealPath?.(file)?.catch(() => undefined)
}

async function revealPluginsDir() {
  try {
    // Electron owns the local plugin root — deriving it from the backend's
    // kenxcode_home breaks against a remote backend (#66899).
    const dir = await window.kenxcodeDesktop?.desktopPluginsRoot?.()
    ...
    const result = await window.kenxcodeDesktop?.openDir?.(dir)
