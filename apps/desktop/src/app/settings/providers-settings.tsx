...
import { disconnectOAuthProvider, listOAuthProviders } from '@/kenxcode'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/kenxcode'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.kenxcodeDesktop?.terminal)
...
//      in kenxcode_cli/provider_catalog.py) — the SAME provider identity
//      `kenxcode model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // KenXCode can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider KenXCode can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // KenXCode never deletes creds another tool owns behind a silent API call.
