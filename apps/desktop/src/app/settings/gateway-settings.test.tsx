...
beforeEach(() => {
  profiles.set([
    ...
    },
    {
      ...
      model: null,
      name: 'work',
      path: '/tmp/kenxcode/profiles/work',
      provider: null,
      skill_count: 0
    }
  ])
...
describe('GatewaySettings', () => {
  it('labels local mode as default inheritance for a named profile', async () => {
    ...
    expect(
      screen.getByText('Start a private KenXCode backend on localhost. This is the default and works offline.')
    ).toBeTruthy()
    ...
    expect(
      screen.queryByText('Start a private KenXCode backend on localhost. This is the default and works offline.')
    ).toBeNull()
  })
    ...
    getConnectionConfig.mockImplementation(async profile =>
      profile === 'work'
        ? {
            ...
            sshPort: 22,
            sshKeyPath: '',
            sshRemoteKenXCodePath: '/opt/kenxcode/bin/kenxcode',
