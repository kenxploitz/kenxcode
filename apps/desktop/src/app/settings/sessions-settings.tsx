...
import type { KenXCodeConfigRecord, SessionInfo } from '@/types/kenxcode'
...
function AutoArchiveSetting() {
  ...
  useEffect(() => {
    ...
    if (!window.kenxcodeDesktop) {
      return
    }

    let alive = true

    void getKenXCodeConfigRecord()
      .then(record => {
        if (!alive) {
          return
        }
  ...
  const persist = useCallback(
    async (autoArchive: boolean, archiveDays: number) => {
      ...
      try {
        await saveKenXCodeConfig(updated)
      } catch (err) {
...
// / Program Files), which buried any files KenXCode wrote there.
function DefaultProjectDirSetting() {
  ...
  useEffect(() => {
    ...
    // / Storybook / non-Electron context `window.kenxcodeDesktop` is
    ...
    const settings = window.kenxcodeDesktop?.settings
