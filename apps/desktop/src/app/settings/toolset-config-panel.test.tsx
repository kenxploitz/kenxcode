...
import type { ToolsetConfig } from '@/types/kenxcode'
...
const getKenXCodeConfigSchema = vi.fn()
...
vi.mock('@/kenxcode', () => ({
  ...
  getKenXCodeConfigSchema: () => getKenXCodeConfigSchema(),
...
beforeEach(() => {
  ...
  getKenXCodeConfigRecord.mockResolvedValue({
    tts: {
      provider: 'edge',
  ...
  getKenXCodeConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveKenXCodeConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveKenXCodeConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
