...
import { getElevenLabsVoices, getKenXCodeConfigSchema, saveKenXCodeConfig } from '@/kenxcode'
...
import type { ConfigFieldSchema, KenXCodeConfigRecord } from '@/types/kenxcode'

import { setKenXCodeConfigCache, useKenXCodeConfigRecord } from '../hooks/use-config-record'
...
export function voiceFieldVisible(key: string, config: KenXCodeConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const [config, setConfig] = useState<KenXCodeConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useKenXCodeConfigRecord()
  ...
  const {
    ...
    queryKey: ['kenxcode-config-schema'],
    queryFn: getKenXCodeConfigSchema,
    staleTime: 5 * 60 * 1000
  })
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          const result = await saveKenXCodeConfig(config)
  ...
  const updateConfig = (next: KenXCodeConfigRecord) => {
