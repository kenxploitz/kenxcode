...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { kenxcodeDesktop: { readDir: typeof readDir } }).kenxcodeDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): KenXCodeReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { kenxcodeDesktop: unknown }).kenxcodeDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: KenXCodeReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<KenXCodeReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { kenxcodeDesktop: unknown }).kenxcodeDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.kenxcodeDesktop is missing', async () => {
    delete (window as unknown as { kenxcodeDesktop?: unknown }).kenxcodeDesktop
