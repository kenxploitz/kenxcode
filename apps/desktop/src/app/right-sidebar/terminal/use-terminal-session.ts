...
const KENXCODE_PATHS_MIME = 'application/x-kenxcode-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(KENXCODE_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(KENXCODE_PATHS_MIME)
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.kenxcodeDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (kenxcode --tui, vim) grab the mouse, so a plain drag
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.kenxcodeDesktop?.readClipboard?.()) ?? ''
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[kenxcode-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.kenxcodeDesktop?.terminal?.write(sessionId, `${command}\r`)
