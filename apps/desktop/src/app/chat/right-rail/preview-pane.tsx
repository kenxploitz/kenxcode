        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.kenxcodeDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask KenXCode to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.kenxcodeDesktop?.watchPreviewFile ||
      !window.kenxcodeDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.kenxcodeDesktop.onPreviewFileChanged(payload => {
    ...
    void window.kenxcodeDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.kenxcodeDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.kenxcodeDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:kenxcode-preview')
