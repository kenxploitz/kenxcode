...
import type { SessionInfo } from '@/kenxcode'
...
// zero KenXCode sessions, and those arrive continuously — so once the user dragged
...
export function useRepoWorktreeMap(
  repoPaths: string[],
  enabled: boolean
): [Record<string, KenXCodeGitWorktree[]>, boolean] {
  const [map, setMap] = useState<Record<string, KenXCodeGitWorktree[]>>({})
