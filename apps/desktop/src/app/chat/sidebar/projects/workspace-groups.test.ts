...
import type { KenXCodeGitWorktree } from '@/global'
import type { ProjectInfo, SessionInfo } from '@/types/kenxcode'
...
describe('baseName', () => {
  it('returns the final path segment, ignoring trailing slashes and separators', () => {
    expect(baseName('/www/kenxcode-agent/')).toBe('kenxcode-agent')
...
describe('mergeRepoWorktreeGroups (visual enhancer)', () => {
  it('injects a linked worktree lane discovered by git that has no sessions yet', () => {
    ...
    const discovered: KenXCodeGitWorktree[] = [
  ...
  it('surfaces a user-named "New worktree" under .worktrees/ as its own lane', () => {
    const discovered: KenXCodeGitWorktree[] = [
      {
        branch: 'kenxcode/test-gui-stuff',
        detached: false,
        isMain: false,
        locked: false,
    ...
    expect(merged.map(g => g.label)).toContain('kenxcode/test-gui-stuff')
  })

  it('relabels a dir-named linked worktree lane to its live checked-out branch', () => {
    // Backend labels the lane by the worktree dir (`kenxcode-agent-ci`); the live
...
describe('overlayLiveLanes', () => {
  ...
  it('places a session into an out-of-tree (sibling) worktree lane by its path', () => {
    // `kenxcode-agent-ci` is a linked worktree living BESIDE the repo, not under
