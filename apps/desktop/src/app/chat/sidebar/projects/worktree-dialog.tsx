...
import type { KenXCodeGitBranch } from '@/global'
...
const branchActionLabel = (branch: KenXCodeGitBranch, copy: BranchActionCopy) => {
...
export function WorktreeDialog({ repoPath, onStarted, open, onOpenChange, initialBase }: WorktreeDialogProps) {
  ...
  const [branches, setBranches] = useState<KenXCodeGitBranch[]>([])
  ...
  const convert = async (branch: KenXCodeGitBranch) => {
    if (pending || !repoPath || !branch) {
      return
    }
