import type { BillingBlock } from '@kenxcode/shared'
import type { KenXCodeSkin } from '@kenxcode/shared/skin'
 ...
 * billing-specific toast — never the generic "KenXCode error" — with a smart CTA
...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  ...
  // refreshKenXCodeConfig — two REST calls (config + defaults) per event, per
  ...
  return useCallback(
    (event: RpcEvent) => {
      ...
      if (event.type === 'gateway.ready') {
        ...
        ingestBackendSkin((payload as { skin?: KenXCodeSkin } | undefined)?.skin, { apply: false })
        ...
        // A runtime skin switch (KenXCode activating an authored skin, or `/skin`
        ...
        if (fromActiveProfile) {
          ingestBackendSkin(payload as KenXCodeSkin | undefined, { apply: true })
        }
        ...
        if (apply) {
          ...
          // everything refreshKenXCodeConfig applies is either active-session
        ...
        const failure =
          payload?.status === 'error'
            ? {
                error: coerceGatewayText(payload.error).trim() || finalText || 'KenXCode reported an error',
        ...
        // native OS notification when KenXCode is backgrounded; dispatch is gated
        ...
        const errorMessage = payload?.message || 'KenXCode reported an error'
