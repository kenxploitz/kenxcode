import { skillInvocationText } from '@kenxcode/shared'
import { type MutableRefObject, useCallback, useRef } from 'react'

import { getProfiles } from '@/kenxcode'
...
// 120s RPC budget (KENXCODE_TUI_RPC_TIMEOUT_MS default) instead.
...
const renderWakeStatus = (status: WakeStatusResponse): string => {
  const lines = [
    ...
    `Phrase: "${status.phrase?.trim() || 'hey kenxcode'}"`,
  ...
  return useCallback(
    async (rawCommand: string, options?: { sessionId?: string; recordInput?: boolean }) => {
      ...
      const actionHandlers: Record<DesktopActionId, (ctx: SlashActionCtx) => Promise<void>> = {
        ...
        // lease. Sending it through slash.exec creates a separate KenXCodeCLI in
