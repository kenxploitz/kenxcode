...
describe('renderRpcResult', () => {
  ...
  describe('session.save', () => {
    it('echoes the saved file path', () => {
      expect(renderRpcResult({ file: '/home/user/.kenxcode/sessions/saved/x.json' }, 'save')).toBe(
        'Saved transcript to /home/user/.kenxcode/sessions/saved/x.json'
      )
    })
  })
  ...
  describe('session.status', () => {
    it('passes through the multi-line plain-text output verbatim', () => {
      const output = 'KenXCode TUI Status\n\nSession ID: s-1\nModel: nous-kenxcode-3 (unknown)'
