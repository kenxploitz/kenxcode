...
import { getKenXCodeConfig } from '@/kenxcode'
...
import { useKenXCodeConfig } from './use-kenxcode-config'

vi.mock('@/kenxcode', () => ({
  getKenXCodeConfig: vi.fn(),
  getKenXCodeConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'kenxcode.desktop.workspace-cwd'
...
const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getKenXCodeConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getKenXCodeConfig>>)

describe('useKenXCodeConfig refreshKenXCodeConfig', () => {
  beforeEach(() => {
  ...
  // `agent.reasoning_effort: high` to KenXCode' built-in medium.
  it('publishes the profile default effort even when a manual pick blocks the composer reseed', async () => {
    ...
    const { result } = renderHook(() => useKenXCodeConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshKenXCodeConfig()
    })
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useKenXCodeConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getKenXCodeConfig>>>()
    vi.mocked(getKenXCodeConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useKenXCodeConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshKenXCodeConfig(true)
    })
    expect(getKenXCodeConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getKenXCodeConfig>>)
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getKenXCodeConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getKenXCodeConfig>>>()
    vi.mocked(getKenXCodeConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshKenXCodeConfig(true)
      refreshC = result.current.refreshKenXCodeConfig(true)
    })
