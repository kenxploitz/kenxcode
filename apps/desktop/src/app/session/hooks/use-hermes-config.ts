...
import { getKenXCodeConfig, getKenXCodeConfigDefaults } from '@/kenxcode'
...
interface KenXCodeConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useKenXCodeConfig({ activeSessionIdRef }: KenXCodeConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshKenXCodeConfig = useCallback(
    async (force = false) => {
      if (force) {
      ...
      try {
        const [config, defaults] = await Promise.all([getKenXCodeConfig(), getKenXCodeConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying KenXCode' built-in medium over the user's config.
  ...
  return { refreshKenXCodeConfig, sttEnabled, voiceMaxRecordingSeconds }
}
