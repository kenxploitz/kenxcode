...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.kenxcodeDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    window.kenxcodeDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.kenxcodeDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.kenxcodeDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // kenxcode:// deep links -> a reviewable /blueprint command in the composer.
  useEffect(() => {
    const unsubscribe = window.kenxcodeDesktop?.onDeepLink?.(payload => {
    ...
    void window.kenxcodeDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.kenxcodeDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.kenxcodeDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
