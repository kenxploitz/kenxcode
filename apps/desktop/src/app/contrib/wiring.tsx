...
import { getSessionMessages, triggerCronJob } from '@/kenxcode'
...
import { useKenXCodeConfig } from '../session/hooks/use-kenxcode-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const { refreshKenXCodeConfig, sttEnabled, voiceMaxRecordingSeconds } = useKenXCodeConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('kenxcode:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('kenxcode:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshKenXCodeConfig(true)
    void refreshActiveProfile()
  }, [activeGatewayProfile, refreshCurrentModel, refreshKenXCodeConfig])
