...
// bug that drives the REAL useGatewayBoot hook + REAL KenXCodeGateway through a
...
function Harness({
  ...
  useGatewayBoot({
    ...
    refreshKenXCodeConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { kenxcodeDesktop?: unknown }).kenxcodeDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { kenxcodeDesktop?: unknown }).kenxcodeDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForKenXCode) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startKenXCode()'s remote branch awaits waitForKenXCode() for 45s before it
    ...
    ;(window as { kenxcodeDesktop?: unknown }).kenxcodeDesktop = desktop
    ...
    // After ~45s waitForKenXCode gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('KenXCode backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
  ...
  it('FIX: a failed session-list fetch during boot is non-fatal — the app still boots', async () => {
    ...
    // failDesktopBoot's "KenXCode couldn't start" overlay — the socket is open
