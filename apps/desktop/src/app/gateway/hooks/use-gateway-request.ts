...
import type { KenXCodeGateway } from '@/kenxcode'
...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as KenXCodeGateway | null
  const gatewayRef = useRef<KenXCodeGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.kenxcodeDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<KenXCodeGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as KenXCodeGateway | null
      }),
    []
  )
