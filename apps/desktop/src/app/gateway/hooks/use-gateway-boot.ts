import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@kenxcode/shared'
import { useEffect, useRef } from 'react'

import type { KenXCodeConnection } from '@/global'
import { KenXCodeGateway } from '@/kenxcode'
...
import type { RpcEvent } from '@/types/kenxcode'
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.kenxcodeDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: KenXCodeGateway | null) => void
  refreshKenXCodeConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: KenXCodeConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting KenXCode...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting KenXCode…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to KenXCode gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshKenXCodeConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshKenXCodeConfig().catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startKenXCode re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new KenXCodeGateway()
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshKenXCodeConfig(),
          ...
          // retry it), not brick boot behind the "KenXCode couldn't start"
