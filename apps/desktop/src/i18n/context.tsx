...
import { getKenXCodeConfigRecord, type KenXCodeConfigRecord, saveKenXCodeConfig } from '@/kenxcode'
...
export interface I18nConfigClient {
  getConfig: () => Promise<KenXCodeConfigRecord>
  saveConfig: (config: KenXCodeConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.kenxcodeDesktop?.api) {
      return Promise.resolve({})
    }

    return getKenXCodeConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.kenxcodeDesktop?.api) {
    ...
    return saveKenXCodeConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: KenXCodeConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: KenXCodeConfigRecord, locale: Locale): KenXCodeConfigRecord {
