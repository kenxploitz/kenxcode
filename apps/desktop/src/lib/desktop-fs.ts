import type {
  KenXCodeConnection,
  KenXCodeReadDirResult,
  KenXCodeReadFileTextResult,
  KenXCodeSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: KenXCodeConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export async function readDesktopDir(path: string): Promise<KenXCodeReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<KenXCodeReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<KenXCodeReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<KenXCodeReadFileTextResult>(fsPath('read-text', path))
}
...
export async function selectDesktopPaths(options?: KenXCodeSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
