...
export async function resolveMediaDisplaySrc(path: string): Promise<string> {
  ...
  if (window.kenxcodeDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.kenxcodeDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.kenxcodeDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
export async function resolveMediaPlaybackSrc(path: string): Promise<string> {
  ...
  if (window.kenxcodeDesktop && ['audio', 'video'].includes(mediaKind(path))) {
...
export function mediaStreamUrl(path: string): string {
  return `kenxcode-media://stream/${encodeURIComponent(filePathFromMediaPath(path))}`
}
...
// (workspace, skills, ~/.kenxcode/cache, etc.); /api/media is intentionally
