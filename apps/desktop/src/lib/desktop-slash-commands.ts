...
/** Why a known KenXCode command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active KenXCode profile', surface: action('profile') },
  {
    name: '/skin',
  ...
  { name: '/version', description: 'Show KenXCode Agent version', surface: exec() },
...
function isKnownKenXCodeSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * KenXCode' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownKenXCodeSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with KenXCode and were never asked for. It is
