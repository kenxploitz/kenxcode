...
function installDesktopBridge(partial: Partial<Window['kenxcodeDesktop']> = {}) {
  desktopWindow.kenxcodeDesktop = {
    fetchLinkTitle: vi.fn().mockResolvedValue(''),
    openExternal: vi.fn().mockResolvedValue(undefined),
    ...partial
  } as unknown as Window['kenxcodeDesktop']
}
...
function installTitleBridge(title: string) {
  ...
  installDesktopBridge({ fetchLinkTitle: bridge as unknown as Window['kenxcodeDesktop']['fetchLinkTitle'] })
...
afterEach(() => {
  ...
  if (initialKenXCodeDesktop) {
    desktopWindow.kenxcodeDesktop = initialKenXCodeDesktop
  } else {
    delete desktopWindow.kenxcodeDesktop
  }
})
...
describe('external link helpers', () => {
  ...
  it('opens links via the desktop bridge', () => {
    ...
    installDesktopBridge({ openExternal: openExternal as unknown as Window['kenxcodeDesktop']['openExternal'] })
  ...
  it('prefixes a pretty link to a known host with its brand glyph', () => {
    ...
    const url = 'https://github.com/NousResearch/kenxcode-agent/pull/123'
