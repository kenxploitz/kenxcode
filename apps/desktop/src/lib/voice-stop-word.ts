...
//
// When someone is in a hands-free "Hey KenXCode" voice chat, the natural way to
...
// phrase (optionally addressed to KenXCode), so a real turn that merely contains
...
// Optional address prefixes so "kenxcode stop" / "ok stop" / "hey kenxcode, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey kenxcode', 'hey kenxcode,', 'kenxcode', 'kenxcode,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("kenxcode") is not a stop command on its own.
      continue
    }
 ...
 * to KenXCode). Returns false for anything that merely contains "stop" as part of
