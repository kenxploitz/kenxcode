...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'kenxcode:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.kenxcodeDesktop?.saveImageFromUrl) {
        if (await window.kenxcodeDesktop.saveImageFromUrl(src)) {
