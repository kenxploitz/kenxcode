...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when KenXCode doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in kenxcode_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `kenxcode model` uses — instead of
...
export interface KenXCodeConfig {
  agent?: {
...
export type KenXCodeConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `kenxcode model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// folders. Mirrors kenxcode_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with KenXCode, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `kenxcode tools` active-provider detection). */
  is_active: boolean
 ...
 *  not KenXCode); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/kenxcode/update/check` — the backend's own update state.
