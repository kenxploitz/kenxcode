...
const SKIN_KEY = 'kenxcode-desktop-theme-v2'
const MODE_KEY = 'kenxcode-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'kenxcode-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'kenxcode-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'kenxcode-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.kenxcodeTheme = skinName
  root.dataset.kenxcodeMode = rendered
  ...
  window.kenxcodeDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('kenxcode-boot-background', chromeBg)
    window.localStorage.setItem('kenxcode-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.kenxcodeThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.kenxcodeDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (KenXCode authoring/activating a skin from a
