/**
 * Live skin sync from the KenXCode backend.
 *
 * The backend resolves the active skin (built-in or `$KENXCODE_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (KenXCode
 ...
 */
export function ingestBackendSkin(skin: KenXCodeSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as KenXCodeSkin)
