...
import { buildDesktopBackendEnv, kenxcodeManagedNodePathEntries, normalizeKenXCodeHomeRoot } from './backend-env'
import { isReauthRequiredError, waitForKenXCodeReady } from './backend-health'
import {
  canImportKenXCodeCli,
  execProbeSync,
  PROBE_TIMEOUT_MS,
  shouldTrustKenXCodeOverride,
  verifyKenXCodeCli
} from './backend-probes'
...
import {
  ...
  resolveVenvKenXCodeCommand
} from './windows-kenxcode-path'
import {
...
const USER_DATA_OVERRIDE = process.env.KENXCODE_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.KENXCODE_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.KENXCODE_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with KENXCODE_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[kenxcode] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[kenxcode] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. KENXCODE_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[kenxcode] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[kenxcode] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// never go through `kenxcode desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose KenXCode sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[kenxcode] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[kenxcode] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[kenxcode] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[kenxcode] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[kenxcode] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('kenxcode:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized KenXCode burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[kenxcode] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[kenxcode] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[kenxcode] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[kenxcode] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// KENXCODE_HOME — the user-facing root for everything KenXCode-related. Mirrors
// scripts/install.ps1's $KenXCodeHome and scripts/install.sh's $KENXCODE_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\kenxcode (matches install.ps1)
//   macOS / Linux: ~/.kenxcode (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.kenxcode directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\kenxcode yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// KENXCODE_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// KENXCODE_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.kenxcode / %LOCALAPPDATA%\kenxcode.
function resolveKenXCodeHome() {
  if (process.env.KENXCODE_HOME) {
    return normalizeKenXCodeHomeRoot(process.env.KENXCODE_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'kenxcode-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a KENXCODE_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\kenxcode and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('KENXCODE_HOME')

    if (fromRegistry) {
      return normalizeKenXCodeHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'kenxcode')
    const legacy = path.join(app.getPath('home'), '.kenxcode')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.kenxcode setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.kenxcode')
}

const KENXCODE_HOME = resolveKenXCodeHome()

function pathWithKenXCodeManagedNode(...entries) {
  const managed = kenxcodeManagedNodePathEntries(KENXCODE_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_KENXCODE_ROOT — the canonical mutable KenXCode install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_KENXCODE_ROOT = path.join(KENXCODE_HOME, 'kenxcode-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_KENXCODE_ROOT, 'venv')
...
// boots, going straight to `resolveKenXCodeBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_KENXCODE_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_KENXCODE_ROOT, '.kenxcode-bootstrap-complete')
...
// active-profile.json records which KenXCode profile the desktop launches its
// local backend as. When set, startKenXCode() passes `kenxcode --profile <name>
// dashboard …`, which deterministically pins KENXCODE_HOME (see
// _apply_profile_override in kenxcode_cli/main.py) and bypasses the sticky
// ~/.kenxcode/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors kenxcode_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// kenxcodeDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under KENXCODE_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by kenxcode_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(KENXCODE_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (kenxcode_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.KENXCODE_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.KENXCODE_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.KENXCODE_DESKTOP_SKIP_QUIT_CONFIRM === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.KENXCODE_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.KENXCODE_DESKTOP_APP_NAME || 'KenXCode'
...
// 'kenxcode:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.nousresearch.kenxcode) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.kenxcode')
}

// Seed the native About panel with the live KenXCode version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `kenxcode update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveKenXCodeVersion(),
...
const MEDIA_PROTOCOL = 'kenxcode-media'

// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(kenxcode-media://…)`.
...
// startKenXCode(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.KENXCODE_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.KENXCODE_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startKenXCode() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning kenxcode serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start KenXCode backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  const lines = text.split(/\r?\n/).map(line => `[kenxcode] ${line}`)
  kenxcodeLog.push(...lines)

  if (kenxcodeLog.length > 300) {
    kenxcodeLog.splice(0, kenxcodeLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-kenxcode-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('kenxcode:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the kenxcode:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('kenxcode:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_KENXCODE_ROOT
  })
}
...
// The Tauri updater writes KENXCODE_HOME/.kenxcode-update-in-progress for the whole
...
// (`force_kill_other_kenxcode`, taskkill /IM kenxcode.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(KENXCODE_HOME)),
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — KenXCode will start automatically when it completes…',
        12
      )
    },
...
function findOnPath(command) {
  ...
  // shell-script shim named `kenxcode` — must not shadow `kenxcode.cmd`/`kenxcode.exe`.
...
function unwrapWindowsVenvKenXCodeCommand(command, backendArgs) {
  return resolveVenvKenXCodeCommand(command, backendArgs, {
...
// spawns `kenxcode serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `kenxcode` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'kenxcode_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      // `kenxcode --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, KENXCODE_HOME, ...(backend.env || {}) },
        timeout: PROBE_TIMEOUT_MS,
        stdio: 'ignore',
        // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
        // (see resolveKenXCodeBackend step 4); execFileSync of a .cmd without
...
function isKenXCodeSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'kenxcode_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.KENXCODE_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several KenXCode deps (notably pywinpty's Rust-built
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the kenxcode-import
...
// findGitBash — locate bash.exe on Windows. Resolves KENXCODE_GIT_BASH_PATH
...
// normal KENXCODE_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\kenxcode\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'kenxcode', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'kenxcode', 'git', 'bin', 'git.exe'))
  }
...
function recentKenXCodeLog() {
  return kenxcodeLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's kenxcode root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_KENXCODE_ROOT.
// KENXCODE_DESKTOP_KENXCODE_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.KENXCODE_DESKTOP_KENXCODE_ROOT && path.resolve(process.env.KENXCODE_DESKTOP_KENXCODE_ROOT),
    !IS_PACKAGED && isKenXCodeSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isKenXCodeSourceRoot(ACTIVE_KENXCODE_ROOT) ? ACTIVE_KENXCODE_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_KENXCODE_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('kenxcode:updates:progress', merged)
  }
}
...
// Windows that binary owns ALL repo mutation — running `kenxcode update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(KENXCODE_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `kenxcode update` can write
// fresh entry points. On Windows this is the file the running backend
// `kenxcode.exe` holds open; on POSIX it's never mandatory-locked.
function venvKenXCodeShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'kenxcode.exe')
    : path.join(updateRoot, 'venv', 'bin', 'kenxcode')
}
...
// only signals the direct child, so on Windows a backend `kenxcode.exe` that
// spawned its own grandchildren (a `kenxcode` REPL, a pty terminal session, the
...
// `kenxcodeProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `kenxcode.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a kenxcode
// REPL, a pty terminal, the gateway) keep `kenxcode.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  const kenxcodeProcess = backendConnectionState.getProcess()

  if (kenxcodeProcess && Number.isInteger(kenxcodeProcess.pid)) {
    pids.push(kenxcodeProcess.pid)
  }
  ...
  if (kenxcodeProcess && !kenxcodeProcess.killed) {
    try {
      kenxcodeProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvKenXCodeShimPath(updateRoot)
  ...
  while (Date.now() < deadlineMs) {
    ...
    const currentKenXCodeProcess = backendConnectionState.getProcess()

    if (currentKenXCodeProcess && Number.isInteger(currentKenXCodeProcess.pid)) {
      stragglers.push(currentKenXCodeProcess.pid)
    }
...
// `kenxcode update`). Instead we spawn the staged KenXCode-Setup binary with
// --update and quit, so it can run `kenxcode update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux: never hand off, staged kenxcode-setup or not — the resolver
      ...
      // `kenxcode update` (backend) + `kenxcode desktop --build-only` (OS-aware GUI
    ...
    if (!updater) {
      ...
      // `kenxcode desktop`, never the Tauri installer that self-copies
      // kenxcode-setup.exe into KENXCODE_HOME). They DO have a working `kenxcode`
      ...
      // checkout they're on — bare `kenxcode update` defaults to main and would
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') {
            command = `kenxcode update --branch ${branch}`
          }
        }
      } catch {
        // Best-effort: fall back to bare `kenxcode update` if branch detection fails.
      }

      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })

      return { ok: true, manual: true, command, kenxcodeRoot: updateRoot }
    }

    const handoffConflict = updateHandoffConflict(KENXCODE_HOME)
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating KenXCode — this window will close and the updater will open. Don’t reopen KenXCode yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    preflightStateDb(KENXCODE_HOME, rememberLog)

    // Stop our own backend(s) and wait for the venv shim to unlock BEFORE we
    // spawn the updater. Without this the updater races a still-locked
    // kenxcode.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running kenxcode, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the KenXCode install open ' +
        '(a second KenXCode window or a terminal running kenxcode?). Close it and retry.'
    ...
    // KenXCode processes running from this venv.  The updater normally refuses
    ...
    // `kenxcode update` will run (the venv shim is locked while we live).
    const child = spawnUpdaterProcess(updater, updaterArgs, {
      cwd: KENXCODE_HOME,
      env: {
        ...process.env,
        KENXCODE_HOME,
        PATH: pathWithKenXCodeManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore'
    })
    ...
    // abort with "Another KenXCode update is already running (PID <itself>)" —
    ...
    if (Number.isInteger(child.pid) && stagedUpdaterSupportsPrewrittenMarker(updater)) {
      writeUpdateMarker(KENXCODE_HOME, child.pid)
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvKenXCode = path.join(venvBin, IS_WINDOWS ? 'kenxcode.exe' : 'kenxcode')
  ...
  // not just the `kenxcode.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvKenXCode) || fileExists(path.join(updateRoot, '.kenxcode-bootstrap-complete'))
...
// Resolve the kenxcode CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `kenxcode` on PATH.
function resolveKenXCodeCliBinary(updateRoot) {
  const venvKenXCode = path.join(updateRoot, 'venv', 'bin', 'kenxcode')

  if (fileExists(venvKenXCode)) {
    return venvKenXCode
  }

  return findOnPath('kenxcode') || null
}
...
// Python-level pre-update snapshot inside `kenxcode update`.
function preflightStateDb(kenxcodeHome, rememberLog) {
  const stateDbPath = path.join(kenxcodeHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(kenxcodeHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(kenxcodeHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(kenxcodeHome, old))
            } catch {
              void 0
            }
          }
...
// macOS/Linux in-app update: backend (`kenxcode update`) + OS-aware GUI rebuild
// (`kenxcode desktop --build-only`), then atomically swap the running .app bundle
...
async function applyUpdatesPosixInApp(opts: any) {
  const updateRoot = resolveUpdateRoot()
  const kenxcode = resolveKenXCodeCliBinary(updateRoot)

  if (!kenxcode) {
    emitUpdateProgress({ stage: 'manual', message: 'kenxcode update', percent: null })

    return { ok: true, manual: true, command: 'kenxcode update', kenxcodeRoot: updateRoot }
  }

  // ── Pre-flight state.db integrity guard (#68474) ──
  preflightStateDb(KENXCODE_HOME, rememberLog)

  // Put the KenXCode-managed Node and the venv on PATH so `kenxcode desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\\kenxcode\\node, not node\\bin.
  // PYTHONUNBUFFERED: `kenxcode update` writes to a pipe here, so CPython
  ...
  const env: Record<string, string> = {
    KENXCODE_HOME,
    PYTHONUNBUFFERED: '1',
    PATH: pathWithKenXCodeManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `kenxcode update` reaps stale `kenxcode serve` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads KENXCODE_DESKTOP_CHILD_PID and excludes
  ...
  if (kenxcodeProcess && Number.isInteger(kenxcodeProcess.pid)) {
    desktopChildPids.push(kenxcodeProcess.pid)
  }
  ...
  if (desktopChildPids.length) {
    env.KENXCODE_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating KenXCode (git + dependencies)…', percent: 10 })

  const updated = (await runStreamedUpdate(kenxcode, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })) as any
  ...
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'kenxcode update failed.', error: updated.error || 'update-failed' })

    return { ok: false, error: 'kenxcode update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(kenxcode, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })

  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart KenXCode to retry.',
  ...
  // Linux in-app update terminal state (#45205). `kenxcode desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting KenXCode…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `kenxcode-desktop-update-${Date.now()}.sh`)
      ...
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen KenXCode to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the KenXCode desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen KenXCode to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'KenXCode.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'KenXCode.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart KenXCode to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.kenxcode-update-new"; then
    rm -rf "$DST.kenxcode-update-old" 2>/dev/null || true
    mv "$DST" "$DST.kenxcode-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.kenxcode-update-new" "$DST"
    rm -rf "$DST.kenxcode-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart KenXCode to load the new version.',
      percent: 100
    })
...
// Marker-independent: is the canonical install at ACTIVE_KENXCODE_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isKenXCodeSourceRoot(ACTIVE_KENXCODE_ROOT) &&
    fileExists(venvPython) &&
    canImportKenXCodeCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_KENXCODE_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `kenxcode
...
function resolveWebDist() {
  const override = process.env.KENXCODE_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set KENXCODE_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: kenxcode desktop --force-build`
  )
...
function resolveKenXCodeCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/KenXCode.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.KENXCODE_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveKenXCodeCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'kenxcode_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_KENXCODE_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `KenXCode at ${ACTIVE_KENXCODE_ROOT}`,
    command,
    args: ['-m', 'kenxcode_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      kenxcodeHome: KENXCODE_HOME,
      pythonPathEntries: [ACTIVE_KENXCODE_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_KENXCODE_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveKenXCodeBackend(backendArgs) {
  // 1. Explicit override -- KENXCODE_DESKTOP_KENXCODE_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.KENXCODE_DESKTOP_KENXCODE_ROOT && path.resolve(process.env.KENXCODE_DESKTOP_KENXCODE_ROOT)

  if (overrideRoot && isKenXCodeSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `KenXCode source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `kenxcode` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isKenXCodeSourceRoot.)
  if (!IS_PACKAGED && isKenXCodeSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `KenXCode source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_KENXCODE_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\kenxcode\\kenxcode-agent (Windows) or ~/.kenxcode/kenxcode-agent.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active KenXCode runtime at ${ACTIVE_KENXCODE_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `kenxcode` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    KENXCODE_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.KENXCODE_DESKTOP_IGNORE_EXISTING !== '1') {
    let kenxcodeCommand = null
    const kenxcodeOverride = process.env.KENXCODE_DESKTOP_KENXCODE

    if (kenxcodeOverride) {
      const resolvedOverride = findOnPath(kenxcodeOverride)

      if (resolvedOverride) {
        kenxcodeCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(kenxcodeOverride, { isWsl: IS_WSL })) {
        kenxcodeCommand = kenxcodeOverride
      } else {
        rememberLog(`Ignoring Windows KenXCode override under WSL: ${kenxcodeOverride}`)
      }
    } else {
      kenxcodeCommand = findOnPath('kenxcode')
    }

    if (kenxcodeCommand) {
      if (looksLikeDesktopAppBinary(kenxcodeCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving KenXCode CLI: ${kenxcodeCommand}`)
        kenxcodeCommand = null
      }
    }

    if (kenxcodeCommand) {
      const unwrapped = unwrapWindowsVenvKenXCodeCommand(kenxcodeCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `kenxcode` shim
      ...
      const shellForProbe = isCommandScript(kenxcodeCommand)

      // KENXCODE_DESKTOP_KENXCODE is an explicit deployment override (used by
      ...
      if (shouldTrustKenXCodeOverride(kenxcodeOverride) || verifyKenXCodeCli(kenxcodeCommand, { shell: shellForProbe })) {
        ...
        // unwrapWindowsVenvKenXCodeCommand here: the second call repeats the
        // same un-memoized import probe, costing up to another full probe
        // timeout on the boot path for an answer we already have.
        return {
          label: `existing KenXCode CLI at ${kenxcodeCommand}`,
          command: kenxcodeCommand,
          args: backendArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing KenXCode CLI at ${kenxcodeCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed kenxcode_cli module via system Python.
  ...
  if (python) {
    ...
    // having kenxcode_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\kenxcode\kenxcode-agent\venv.
    if (canImportKenXCodeCli(python)) {
      return {
        kind: 'python',
        label: `installed kenxcode_cli module via ${python}`,
        command: python,
        args: ['-m', 'kenxcode_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: kenxcode_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveKenXCodeBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'KenXCode Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
    ...
    activeRoot: ACTIVE_KENXCODE_ROOT,
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveKenXCodeBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no KenXCode install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'KenXCode recovery was handed off to KenXCode Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      kenxcodeHome: KENXCODE_HOME,
      logRoot: path.join(KENXCODE_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('KenXCode install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `KenXCode bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(KENXCODE_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startKenXCode() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // kenxcode:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveKenXCodeBackend(backend.args))
  }
  ...
  if (!isKenXCodeSourceRoot(ACTIVE_KENXCODE_ROOT)) {
    throw new Error(
      `KenXCode install at ${ACTIVE_KENXCODE_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. KenXCode' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\kenxcode\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `kenxcode` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for KenXCode on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch KenXCode.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isKenXCodeSourceRoot()
    // plus an importable kenxcode_cli before it hands back the active runtime.
    // If we hit this, the user (or a deleted venv) broke the invariant; tell
    // them to re-run the install.
    throw new Error(
      `KenXCode venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `KenXCode at ${ACTIVE_KENXCODE_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'KenXCode runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----kenxcode-${crypto.randomBytes(12).toString('hex')}`
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported KenXCode backend URL protocol: ${parsed.protocol}`))
    ...
    const req = client.request(
      parsed,
      {
        ...
        headers: {
          ...
          'X-KenXCode-Session-Token': token,
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the KenXCode backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to KenXCode backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-KenXCode-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('kenxcode:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveKenXCodeCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('kenxcode:preview-file-changed', payload)
}
...
// waitForKenXCodeReady how to read a 401 — rejected session vs gated route.
...
async function waitForKenXCode(baseUrl, token, signal?, authMode?) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForKenXCodeReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('kenxcode:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('kenxcode:close-preview-requested')
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('kenxcode:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting KenXCode...".
function sendPowerResume() {
  ...
  webContents.send('kenxcode:power-resume')
}
...
// 'kenxcode:power-battery' push below.
ipcMain.handle('kenxcode:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('kenxcode:power-battery', next)
    }
  }
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('kenxcode:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('kenxcode:window-state-changed', state)
}
...
//
// Hosted KenXCode gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``kenxcode_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / kenxcode #37247).
//     Both are set as HttpOnly cookies (``kenxcode_session_at`` ~15 min,
//     ``kenxcode_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:kenxcode-remote-oauth'
...
// "not signed in", which on the initial boot path (startKenXCode → the renderer's
// single-shot boot() with no retry) surfaced as the "KenXCode couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "KenXCode
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to KenXCode Cloud agent…' : 'Sign in to KenXCode gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to KenXCode backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in kenxcode_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- KenXCode Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:kenxcode-remote-oauth`:
...
// convention (kenxcode_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every KenXCode surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.nousresearch.com'

function resolvePortalBaseUrl() {
  const raw = process.env.KENXCODE_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
// authenticates via PRIVY, not the KenXCode gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for kenxcode_session_at/rt that the portal
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a KenXCode Cloud sign-in.'))
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to KenXCode Cloud',
        autoHideMenuBar: true,
        webPreferences: {
          contextIsolation: true,
          nodeIntegration: false,
...
// Discover the hosted (KenXCode Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to KenXCode Cloud. Open Settings → Gateway, choose KenXCode Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (error && error.statusCode === 401) {
      const err = new Error('Your KenXCode Cloud session has expired. Open Settings → Gateway and sign in again.') as any
      err.needsCloudLogin = true
      err.cause = error
      throw err
    }
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your KenXCode Cloud session has expired. Sign in to KenXCode Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the KenXCode Cloud org tag on cloud-mode entries so Settings can
...
// a valid stored value (pins the root KENXCODE_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.KENXCODE_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.KENXCODE_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted KenXCode Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteKenXCodePath: (ssh || savedSsh)?.remoteKenXCodePath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by KENXCODE_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the KenXCode Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered KenXCode Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteKenXCodePath: input.sshRemoteKenXCodePath ?? existingBlock.remoteKenXCodePath,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (kenxcode #37247), and the gateway middleware transparently
    ...
    if (
      ...
      const err = new Error(
        'Remote KenXCode gateway uses OAuth, but you are not signed in. ' +
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote KenXCode gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote KenXCode gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget() {
  ...
  if (process.env.KENXCODE_DESKTOP_REMOTE_URL) {
    return null
  }
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, fingerprint, lease) {
  ...
  try {
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteKenXCodePath || '')
    ...
    result = await lifecycle({
      ...
      remoteKenXCodePath: sshConfig.remoteKenXCodePath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForKenXCode: (baseUrl, token) => waitForKenXCode(baseUrl, token, lease.signal, 'token'),
  ...
  sshConnections.set(scope, {
    ...
    host: sshConfig.host,
    hostLabel,
    kenxcodeVersion: result.kenxcodeVersion || '',
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.kenxcodeVersion || 'kenxcode (version unknown)'} at ${result.kenxcodePath || '?'}`
  )
  ...
  return { ...connection, remoteKenXCodeVersion: result.kenxcodeVersion || '' }
}
...
//   2. env override (KENXCODE_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.KENXCODE_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.KENXCODE_DESKTOP_REMOTE_TOKEN

  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'KENXCODE_DESKTOP_REMOTE_URL is set but KENXCODE_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote KenXCode backend.'
      )
    }
...
// env → global) so a startKenXCode() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every KenXCode
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteKenXCodePath: input.sshRemoteKenXCodePath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteKenXCodePath || '')
          let kenxcodePath
          let kenxcodeVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            kenxcodePath = runtime.kenxcodePath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.kenxcodePath])
            kenxcodeVersion = inspection.version
            supported = inspection.supported
          } else {
            kenxcodePath = await remoteLifecycle.locateKenXCode(ssh, sshConfig.remoteKenXCodePath || '')
            kenxcodeVersion = await remoteLifecycle.probeKenXCodeVersion(ssh, kenxcodePath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, kenxcodePath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update KenXCode on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteKenXCodePath: kenxcodePath,
            remoteKenXCodeVersion: kenxcodeVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startKenXCode())
  ...
  // connect to KenXCode gateway". Mirror the renderer's connect here so the test
...
function resetKenXCodeConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  const kenxcodeProcess = backendConnectionState.invalidate()
  stopBackendChild(kenxcodeProcess)
...
// startKenXCode() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetKenXCodeConnection() invalidates it.
  const kenxcodeProcess = backendConnectionState.getProcess()
  const dying = kenxcodeProcess && !kenxcodeProcess.killed ? kenxcodeProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('kenxcode:connection:applied')
}
...
async function ensureBackend(profile) {
  ...
  if (route.backend === 'primary') {
    const connection = await startKenXCode()
...
// local-spawn portion of startKenXCode() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForKenXCode(remote.baseUrl, remote.token, undefined, remote.authMode)
  ...
  // --profile wins over the inherited KENXCODE_HOME env (see _apply_profile_override
  // step 3 in kenxcode_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveKenXCodeBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const kenxcodeCwd = resolveKenXCodeCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting KenXCode backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: kenxcodeCwd,
        KENXCODE_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        KENXCODE_DESKTOP: '1',
        KENXCODE_WEB_DIST: webDist,
        ...(readyFile ? { KENXCODE_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`KenXCode backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`KenXCode backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)

    if (!ready) {
      rejectStart?.(
        new Error(`KenXCode backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForKenXCode(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `KenXCode backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `KenXCode backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
// backend whose ensure_kenxcode_home() recreates the deleted profile directory.
//
...
async function startKenXCode() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startKenXCode() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving KenXCode backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      await advanceBootProgress('backend.remote', `Connecting to remote KenXCode backend at ${remote.baseUrl}`, 24)
      await waitForKenXCode(remote.baseUrl, remote.token, undefined, remote.authMode)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote KenXCode backend is ready',
        progress: 94,
        running: true,
        error: null
      })
      ...
      return {
        ...
        remoteKenXCodeVersion: remote.remoteKenXCodeVersion,
        token: remote.token,
        wsUrl: remote.wsUrl,
        logs: kenxcodeLog.slice(-80),
        ...getWindowState()
    ...
    // deterministic (it wins over the sticky ~/.kenxcode/active_profile file) and
    // resolves KENXCODE_HOME the same way `kenxcode -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving KenXCode runtime', 28)

        return resolveKenXCodeBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting KenXCode backend via ${backend.label}`, 84)
    rememberLog(`Starting KenXCode backend via ${backend.label}`)

    const kenxcodeProcess = spawn(
      backend.command,
      backend.args,
      hiddenWindowsChildOptions({
        cwd: kenxcodeCwd,
        env: {
          ...process.env,
          // Explicitly pin KENXCODE_HOME for the child so Python's get_kenxcode_home()
          // resolves to the SAME location our resolveKenXCodeHome() picked. Without
          // this pin, Python falls back to ~/.kenxcode on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\kenxcode, which differs from C:\Users\<u>\.kenxcode.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets KENXCODE_HOME via setx; the desktop
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, kenxcodeProcess)

    if (!processOwner) {
      stopBackendChild(kenxcodeProcess)
      throw new Error('KenXCode backend start was superseded by a newer connection attempt.')
    }

    kenxcodeProcess.stdout.on('data', rememberLog)
    kenxcodeProcess.stderr.on('data', rememberLog)
    ...
    kenxcodeProcess.once('error', error => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale KenXCode backend error: ${error.message}`)
        rejectBackendStart?.(new Error('KenXCode backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`KenXCode backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `KenXCode backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    kenxcodeProcess.once('exit', (code, signal) => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale KenXCode backend exit (${signal || code})`)
      ...
      rememberLog(`KenXCode backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `KenXCode backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `KenXCode backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentKenXCodeLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for KenXCode backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(kenxcodeProcess, { readyFile }),
      backendStartFailed
    ])
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for KenXCode backend to become ready', 90)
    await Promise.race([waitForKenXCode(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => kenxcodeProcess.exitCode === null && !kenxcodeProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local KenXCode backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'KenXCode backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while KenXCode is
...
// pushes pet state over IPC (kenxcode:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via kenxcode:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main KenXCode app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the KenXCode icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see kenxcode:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep KenXCode in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('kenxcode:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('kenxcode:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('kenxcode:quick-entry:shown')
}
...
function createWindow() {
  ...
  startKenXCode().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('kenxcode:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting KenXCode…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('kenxcode:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: resetKenXCodeConnection,
...
ipcMain.handle('kenxcode:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('kenxcode:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('kenxcode:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('kenxcode:window:openInstance', async () => {
  createInstanceWindow()

  return { ok: true }
})
ipcMain.handle('kenxcode:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('kenxcode:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('kenxcode:zoom:get', event => {
...
ipcMain.on('kenxcode:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('kenxcode:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('kenxcode:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('kenxcode:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('kenxcode:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('kenxcode:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('kenxcode:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('kenxcode:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('kenxcode:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('kenxcode:pet-overlay:control', payload)
})
ipcMain.handle('kenxcode:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startKenXCode() call restarts the
...
ipcMain.handle('kenxcode:bootstrap:repair', async () => {
  // Forceful repair: force the next startKenXCode() through the full installer
  ...
  // "bypass active runtime" path inside startKenXCode() and fall through
  ...
  resetKenXCodeConnection()

  return { ok: true }
})
ipcMain.handle('kenxcode:bootstrap:continue-local', async () => {
  rememberLog('[bootstrap] local install selected by renderer; continuing first-launch bootstrap')
  continueFirstRunLocalBootstrap()

  return { ok: true }
})
ipcMain.handle('kenxcode:bootstrap:cancel', async () => {
...
ipcMain.handle('kenxcode:boot-progress:get', async () => bootProgressState)
ipcMain.handle('kenxcode:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('kenxcode:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('kenxcode:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('kenxcode:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('kenxcode:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('kenxcode:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('kenxcode:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startKenXCode() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('kenxcode:connection-config:oauth-logout', async (_event, rawUrl) => {
...
// --- KenXCode Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('kenxcode:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('kenxcode:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('kenxcode:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('kenxcode:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('kenxcode:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new KENXCODE_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('kenxcode:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('kenxcode:requestMicrophoneAccess', async () => {
...
ipcMain.handle('kenxcode:api', async (_event, request) => {
  ...
  // backend calls ensure_kenxcode_home() which recreates the profile directory,
...
ipcMain.handle('kenxcode:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))

ipcMain.handle('kenxcode:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('kenxcode:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('kenxcode:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
...
ipcMain.handle('kenxcode:data-url-read-max:get', () => ({
...
ipcMain.handle('kenxcode:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('kenxcode:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('kenxcode:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('kenxcode:readFileText', async (_event, filePath) => {
...
ipcMain.handle('kenxcode:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('kenxcode:writeClipboard', (_event, text) => {
...
ipcMain.handle('kenxcode:readClipboard', () => clipboard.readText())

ipcMain.handle('kenxcode:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('kenxcode:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('kenxcode:saveClipboardImage', async () => {
...
ipcMain.handle('kenxcode:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('kenxcode:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('kenxcode:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('kenxcode:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('kenxcode:active-work', (event, payload) => {
...
ipcMain.on('kenxcode:titlebar-theme', (_event, payload) => {
...
ipcMain.on('kenxcode:native-theme', (_event, mode) => {
...
ipcMain.on('kenxcode:translucency', (_event, payload) => {
...
ipcMain.on('kenxcode:keep-awake', (_event, on) => {
...
ipcMain.handle('kenxcode:quick-entry:settings:get', async () => {
...
ipcMain.handle('kenxcode:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('kenxcode:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('kenxcode:quick-entry:submit', {
...
ipcMain.on('kenxcode:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('kenxcode:quick-entry:state', payload)
  }
})

ipcMain.on('kenxcode:quick-entry:dismiss', () => hideQuickEntryWindow())

ipcMain.handle('kenxcode:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `kenxcode:window:openSession`).
...
ipcMain.handle('kenxcode:find-in-page', (event, query, options) => {
...
ipcMain.handle('kenxcode:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('kenxcode:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveKenXCodeCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('kenxcode:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveKenXCodeCwd()
}))

ipcMain.handle('kenxcode:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('kenxcode:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('kenxcode:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('kenxcode:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('kenxcode:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('kenxcode:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: kenxcodeLog.slice(-200) }))
...
function terminalShellCommand() {
  // KENXCODE_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.KENXCODE_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip KenXCode' TUI into
  ...
  env.TERM_PROGRAM = 'KenXCode'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a kenxcode/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from KENXCODE_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.KENXCODE_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `kenxcode:terminal:${id}:${suffix}`
}
...
ipcMain.handle('kenxcode:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('kenxcode:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('kenxcode:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('kenxcode:fs:openDir', async (_event, dirPath) => {
...
// The LOCAL Desktop runtime-plugin root: `<KENXCODE_HOME>/desktop-plugins`,
// resolved from the main-process KENXCODE_HOME (see resolveKenXCodeHome) — NOT from
// the connected backend. A remote backend reports its own `kenxcode_home` over
...
ipcMain.handle('kenxcode:fs:desktopPluginsRoot', async () => {
  // Profile-aware: a named Desktop profile gets its own plugin root under
  // profiles/<name>/, matching the profile-scoped kenxcode_home the backend
  // reported before this resolver existed. 'default'/unset pins the global root.
  const profile = readActiveDesktopProfile()
  const base = profile && profile !== 'default' ? path.join(KENXCODE_HOME, 'profiles', profile) : KENXCODE_HOME
...
ipcMain.handle('kenxcode:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('kenxcode:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('kenxcode:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('kenxcode:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('kenxcode:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('kenxcode:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('kenxcode:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('kenxcode:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))

ipcMain.handle('kenxcode:git:baseBranchList', async (_event, repoPath) => listBaseBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('kenxcode:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('kenxcode:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('kenxcode:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('kenxcode:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('kenxcode:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('kenxcode:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('kenxcode:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('kenxcode:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('kenxcode:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('kenxcode:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('kenxcode:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('kenxcode:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('kenxcode:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('kenxcode:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('kenxcode:terminal:start', async (event, payload = {}) => {
...
ipcMain.handle('kenxcode:terminal:write', (_event, id, data) => {
...
ipcMain.handle('kenxcode:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('kenxcode:terminal:cwd', async (_event, id) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('kenxcode:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('kenxcode:updates:check', async () =>
...
ipcMain.handle('kenxcode:updates:apply', async (_event, payload) =>
...
ipcMain.handle('kenxcode:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('kenxcode:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical KenXCode version (the one `release.py` bumps in
// kenxcode_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real KenXCode version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveKenXCodeVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'kenxcode_cli', '__init__.py')
...
// Re-resolve the live KenXCode version and push it into the native About panel
// just before showing it, so an in-place `kenxcode update` is reflected without
...
ipcMain.handle('kenxcode:version', async () => ({
  appVersion: resolveKenXCodeVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  kenxcodeRoot: resolveUpdateRoot()
}))
...
// the actual removal via `kenxcode uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (kenxcode_cli/uninstall.py + kenxcode_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_KENXCODE_ROOT
  ...
  const fallback = () => ({
    kenxcode_home: KENXCODE_HOME,
    agent_installed: isKenXCodeSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'kenxcode_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, KENXCODE_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no KenXCode agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import kenxcode_cli` resolves from
  ...
  if (modeRemovesAgent(mode)) {
    ...
    if (sysPy) {
      py = sysPy
      pythonPath = ACTIVE_KENXCODE_ROOT
    } else if (IS_WINDOWS) {
      rememberLog(
  ...
  try {
    await releaseBackendLock(ACTIVE_KENXCODE_ROOT, 'uninstall')
  } catch (error) {
  ...
  const scriptArgs = {
    ...
    pythonExe: py,
    pythonPath,
    agentRoot: ACTIVE_KENXCODE_ROOT,
    uninstallArgs,
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `kenxcode-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `kenxcode-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('kenxcode:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('kenxcode:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('kenxcode:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('kenxcode:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// kenxcode:// deep links (e.g. kenxcode://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const KENXCODE_PROTOCOL = 'kenxcode'
...
function _extractDeepLink(argv) {
  ...
  return argv.find(a => typeof a === 'string' && a.startsWith(`${KENXCODE_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // kenxcode://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('kenxcode:deep-link', payload)
...
ipcMain.handle('kenxcode:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${KENXCODE_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(KENXCODE_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(KENXCODE_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `kenxcode://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching kenxcode:// URL is in our own argv.
...
app.on('before-quit', event => {
  ...
  // quitting KenXCode never keeps another app's chord hostage.
