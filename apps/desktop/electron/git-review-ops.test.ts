...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'kenxcode-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'kenxcode-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'KenXCode Test'], { cwd: dir })
