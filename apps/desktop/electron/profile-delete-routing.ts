// Profile-delete routing logic for the `kenxcode:api` IPC handler.
//
...
// pool backend -- spawning a fresh one would call ensure_kenxcode_home() and
...
/**
 * Parse a `kenxcode:api` request into the profile name a DELETE targets, or
...
/**
 * Route the next `kenxcode:api` request away from the primary/window backend
 ...
 * ensure_kenxcode_home() recreates the directory the delete just removed.
 */
