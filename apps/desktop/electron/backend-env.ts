...
/**
 * KenXCode-managed Node.js directories, in preferred lookup order.
 *
 * There are two on-disk layouts. `scripts/install.ps1` unpacks portable Node
 * straight into `%LOCALAPPDATA%\kenxcode\node` (node.exe at the root, no `bin\`);
 * `scripts/install.sh` and the node-bootstrap helper use the POSIX
 * `$KENXCODE_HOME/node/bin`. Emit BOTH on every platform so mixed and migrated
 ...
 * `iter_kenxcode_node_dirs()` in kenxcode_constants.py, which the Electron main
 * process cannot import.
 */
function kenxcodeManagedNodePathEntries(
  kenxcodeHome,
  ...
  const root = pathModule.join(kenxcodeHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const kenxcodeNodeDirs = kenxcodeManagedNodePathEntries(kenxcodeHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([kenxcodeNodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizeKenXCodeHomeRoot(kenxcodeHome, { pathModule = pathModuleForPlatform(process.platform) }: any = {}) {
  if (!kenxcodeHome) {
    return kenxcodeHome
  }

  const resolved = pathModule.resolve(String(kenxcodeHome))
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). kenxcode_bootstrap sets this inside the child too, but only
