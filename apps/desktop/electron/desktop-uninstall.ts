 ...
 * the three user-facing uninstall modes to the `kenxcode uninstall` CLI flags,
 ...
 *              `kenxcode uninstall --gui --yes`
 *   - 'lite' → remove the GUI + agent code, KEEP user data (config / sessions
 *              / .env) for a future reinstall. `kenxcode uninstall --yes`
 *   - 'full' → remove everything: GUI + agent + all user data.
 *              `kenxcode uninstall --full --yes`
 *
 * Why a detached cleanup script: 'lite'/'full' delete the very venv the
 * `kenxcode` command runs from, and every mode may need to delete the running
...
/**
 * Map an uninstall mode to the `python -m kenxcode_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `kenxcode_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'kenxcode_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/KenXCode.app/Contents/MacOS/KenXCode  → …/KenXCode.app
 *   Windows: …\KenXCode\KenXCode.exe                 → …\KenXCode  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/KenXCode.app/Contents/MacOS/KenXCode → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/KenXCode.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs KenXCode.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]KenXCode$/i.test(dir) || /[\\/]kenxcode-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/kenxcode
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import kenxcode_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, kenxcodeHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export KENXCODE_HOME=${q(kenxcodeHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import kenxcode_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but KenXCode install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "KENXCODE_HOME=${String(kenxcodeHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
