...
const SCAN_MODULE = 'kenxcode_cli._scan_venv_blockers'
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another KenXCode process is using this installation.',
    '',
...
export function formatProbeFailedMessage(): string {
  return (
    'Update aborted: Desktop could not verify the KenXCode installation is free.\n' +
    '\n' +
    'Close other KenXCode windows and terminals, then retry.  If the problem\n' +
    'persists, run `kenxcode update` in a terminal for detailed diagnostics.'
  )
}
