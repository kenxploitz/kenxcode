 ...
 *   1. KENXCODE_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\kenxcode\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect KENXCODE_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.KENXCODE_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'kenxcode', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'kenxcode', 'git', 'usr', 'bin', 'bash.exe'))
  }
