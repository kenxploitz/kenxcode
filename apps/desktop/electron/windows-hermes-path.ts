/**
 * windows-kenxcode-path.ts
 *
 * Pure, dependency-injected pieces of Windows `kenxcode` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvKenXCodeCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `kenxcode` shim shadowed the real
 *      kenxcode.cmd/kenxcode.exe; the shim then failed the --version probe and
 ...
 *      venv\Scripts\kenxcode.exe (the console-script shim, written at the END
 ...
 *   3. resolveVenvKenXCodeCommand() — unwrapWindowsVenvKenXCodeCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `kenxcode`) must
 * not shadow `kenxcode.cmd`/`kenxcode.exe`. The empty entry is kept LAST so
 ...
 * signals (venv python interpreter, venv kenxcode shim, bootstrap-complete
 * marker) — gating on just the kenxcode.exe console-script shim alone is the
...
export interface ResolveVenvKenXCodeCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportKenXCodeCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `kenxcode`/`kenxcode.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/kenxcode(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m kenxcode_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportKenXCodeCli(). A
 * venv whose update died mid-`pip install` still has python.exe + kenxcode.exe
 ...
 *
 * Returns null when `command` is not a venv kenxcode shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvKenXCodeCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvKenXCodeCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^kenxcode(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv KenXCode at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing KenXCode Python at ${python}`,
    command: python,
