 ...
 * Background
 * ----------
 * After `kenxcode update` + `kenxcode desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything KENXCODE_DESKTOP_* is preserved
// plus KENXCODE_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['KENXCODE_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['KENXCODE_DESKTOP_']
