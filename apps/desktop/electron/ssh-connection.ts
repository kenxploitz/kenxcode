...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(KENXCODE_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-KenXCode-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.kenxcode/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'kenxcode-desktop-ssh')
  }

  return path.join(os.homedir(), '.kenxcode', 'desktop-ssh')
}
