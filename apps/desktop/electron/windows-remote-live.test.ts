...
//   KENXCODE_WIN_SSH_HOST   ssh alias/host of the Windows box
//   KENXCODE_WIN_SSH_USER   remote user
//   KENXCODE_WIN_SSH_KENXCODE absolute path to the remote kenxcode.exe under test
const liveHost = process.env.KENXCODE_WIN_SSH_HOST || ''
const liveUser = process.env.KENXCODE_WIN_SSH_USER || ''
const configuredKenXCode = process.env.KENXCODE_WIN_SSH_KENXCODE || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-KenXCode-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredKenXCode)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredKenXCode.replace('kenxcode.exe', 'python.exe')}' -m kenxcode_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredKenXCode.replace('kenxcode.exe', 'python.exe')
        const terminate = `& '${python}' -m kenxcode_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.kenxcodePath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m kenxcode_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m kenxcode_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
