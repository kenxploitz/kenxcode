...
export interface KenXCodeReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForKenXCodeReady`).
   */
...
export async function waitForKenXCodeReady(baseUrl: string, options: KenXCodeReadyOptions): Promise<void> {
  ...
  throw new Error(`KenXCode backend did not become ready: ${detail}`)
}
