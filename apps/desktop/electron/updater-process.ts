 ...
 *
 * The Tauri installer self-copies into KENXCODE_HOME on *every* platform
 * (`kenxcode-setup.exe` on Windows, `kenxcode-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_kenxcode_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `kenxcode update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(kenxcodeHome, 'kenxcode-setup.exe')
 ...
 *
 * `copy_self_to_kenxcode_home` deliberately no-ops during `--update`
 ...
 * "Another KenXCode update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `kenxcode update`, which
 ...
 * the installer is written to KENXCODE_HOME at install/repair time, making mtime
