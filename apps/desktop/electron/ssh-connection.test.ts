...
test('redactSecrets scrubs the spawn-time session token env var', () => {
  const line = 'setsid env KENXCODE_DASHBOARD_SESSION_TOKEN=abc123deadbeef KENXCODE_DESKTOP=1 kenxcode dashboard'
  const out = redactSecrets(line)
  assert.ok(!out.includes('abc123deadbeef'))
  assert.match(out, /KENXCODE_DASHBOARD_SESSION_TOKEN=<redacted>/)
  // non-secret env vars are preserved
  assert.match(out, /KENXCODE_DESKTOP=1/)
})
...
test('redactSecrets scrubs Authorization and X-KenXCode-Session-Token headers', () => {
  ...
  assert.match(redactSecrets('X-KenXCode-Session-Token: hdr_888'), /X-KenXCode-Session-Token: ?<redacted>/)
  assert.ok(!redactSecrets('X-KenXCode-Session-Token: hdr_888').includes('hdr_888'))
})
...
test('controlSocketPath default base stays under sun_path even with the temp-listener suffix', () => {
  ...
  const p = controlSocketPath('kenxcode', 'remote-build-server', 22) // no baseDir → default
...
test('buildExecArgs ends with host then the remote command', () => {
  ...
  const args = buildExecArgs(conn, 'command -v kenxcode', 15000)
  assert.equal(args[args.length - 1], 'command -v kenxcode')
...
test('close() removes the control socket when -O exit fails', async () => {
  const dir = path.join(os.tmpdir(), `kenxcode-ssh-close-${process.pid}-${Date.now()}`)
...
test('open() creates the control-socket directory if it does not exist', async () => {
  const dir = path.join(os.tmpdir(), `kenxcode-ssh-test-${process.pid}-${Date.now()}`)
