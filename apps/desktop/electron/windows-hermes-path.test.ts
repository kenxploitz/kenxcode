// Unit tests for the pure Windows `kenxcode` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvKenXCodeCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `kenxcode` shim shadows
//      the real kenxcode.cmd/kenxcode.exe.
//   2. chooseUpdaterArgs() — must gate on haveRealInstall (any real-install
//      signal), not just the kenxcode.exe console-script shim, or healthy
//      installs get forced into a destructive --repair.
//   3. resolveVenvKenXCodeCommand() — must probe the venv python via
//      canImportKenXCodeCli() before trusting it, or a broken venv gets
...
function makeDeps(overrides: Partial<Parameters<typeof resolveVenvKenXCodeCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportKenXCodeCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    kenxcodeHome: '/fake/kenxcode-home',
...
test('resolveVenvKenXCodeCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvKenXCodeCommand('/root/venv/Scripts/kenxcode.exe', [], deps), null)
})

test('resolveVenvKenXCodeCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvKenXCodeCommand('/root/venv/Scripts/kenxcode.cmd', [], deps), null)
})

test('resolveVenvKenXCodeCommand: returns null when the basename is not kenxcode/kenxcode.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvKenXCodeCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvKenXCodeCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvKenXCodeCommand('/root/venv/bin/kenxcode.exe', [], deps), null)
})

test('resolveVenvKenXCodeCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvKenXCodeCommand('/root/venv/Scripts/kenxcode.exe', [], deps), null)
})

test('resolveVenvKenXCodeCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportKenXCodeCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvKenXCodeCommand('/root/venv/Scripts/kenxcode.exe', ['serve'], deps)
...
test('resolveVenvKenXCodeCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvKenXCodeCommand('/root/venv/Scripts/kenxcode.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'kenxcode_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvKenXCodeCommand: is case-insensitive on kenxcode.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvKenXCodeCommand('/root/venv/Scripts/KENXCODE.EXE', [], deps))
  assert.ok(resolveVenvKenXCodeCommand('/root/venv/SCRIPTS/kenxcode.exe', [], deps))
})
