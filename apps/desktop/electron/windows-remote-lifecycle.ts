...
const READY_RE = /^KENXCODE_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitKenXCodePath = '') {
  const explicit = psLiteral(explicitKenXCodePath)

  const script = [
    '$ErrorActionPreference="Stop"',
    `$explicit=${explicit}`,
    '$kenxcodeHome=$env:KENXCODE_HOME',
    'if(-not $kenxcodeHome){$kenxcodeHome=Join-Path $env:LOCALAPPDATA "kenxcode"}',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command kenxcode.exe -ErrorAction SilentlyContinue',
    'if($cmd){$candidates+=$cmd.Source}',
    '$candidates+=(Join-Path $kenxcodeHome "kenxcode-agent\\venv\\Scripts\\kenxcode.exe")',
    '$candidates+=(Join-Path $HOME "kenxcode-agent\\.venv\\Scripts\\kenxcode.exe")',
    '$kenxcode=$candidates|Where-Object{Test-Path -LiteralPath $_ -PathType Leaf}|Select-Object -First 1',
    'if(-not $kenxcode){throw "KenXCode is not installed on the remote Windows host."}',
    'if($explicit -and $kenxcode -ne $explicit){throw "The configured KenXCode path is not an executable file."}',
    '$python=Join-Path (Split-Path $kenxcode) "python.exe"',
    'if(-not (Test-Path -LiteralPath $python -PathType Leaf)){throw "The remote KenXCode Python runtime was not found."}',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;kenxcodeHome=$kenxcodeHome;kenxcodePath=$kenxcode;python=$python}|ConvertTo-Json -Compress'
  ].join(';')
...
async function detectRemotePlatform(ssh, explicitKenXCodePath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitKenXCodePath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'kenxcode_cli.windows_ssh_runtime', operation, ...args]
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.kenxcodePath === 'string' &&
    typeof lock.kenxcodeHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.kenxcodePath === runtime.kenxcodePath &&
    lock.kenxcodeHome === runtime.kenxcodeHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertCurrent(signal)
  const runtime = await probeWindowsRemote(ssh, remoteKenXCodePath)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.kenxcodePath])

  if (!inspection.supported) {
    const error: any = new Error('Update KenXCode on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.kenxcodePath = inspection.path
  const kenxcodeVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located kenxcode at ${runtime.kenxcodePath}`)
  ...
  try {
    spawned = await helper(
      ...
      'spawn',
      [],
      JSON.stringify({ ownershipId, spawnNonce, profile, kenxcodePath: runtime.kenxcodePath })
    )
  } catch (error) {
  ...
  const owned = {
    ...
    profile,
    kenxcodePath: runtime.kenxcodePath,
    kenxcodeHome: runtime.kenxcodeHome,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForKenXCode(baseUrl, token)
    assertCurrent(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="KenXCode SSH"', 'powershell.exe -NoLogo')
