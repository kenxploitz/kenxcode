...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/kenxcode', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/kenxcode/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/kenxcode'), 'https://gw.example.com/kenxcode/auth/native/refresh')
})
