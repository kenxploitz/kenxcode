...
export function markerPath(kenxcodeHome) {
  return path.join(kenxcodeHome, '.kenxcode-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(kenxcodeHome)
  let raw
 ...
 *
 * The Tauri-based kenxcode-setup.exe takes several seconds to initialise its
 ...
 */
export function writeUpdateMarker(kenxcodeHome, pid, { now = Date.now } = {}) {
  const file = markerPath(kenxcodeHome)
 ...
 * KenXCode to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(kenxcodeHome, opts)
