 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of KenXCode Agent by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.kenxcode-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(kenxcodeHome) {
  return path.join(kenxcodeHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.kenxcode/kenxcode-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(kenxcodeHome) {
  if (!kenxcodeHome) {
    return null
  }

  const candidate = path.join(kenxcodeHome, 'kenxcode-agent', 'scripts', installScriptName())
...
function cachedScriptPath(kenxcodeHome, commit) {
  return path.join(bootstrapCacheDir(kenxcodeHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/NousResearch/kenxcode-agent/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(kenxcodeHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(kenxcodeHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, kenxcodeHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass KENXCODE_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          KENXCODE_HOME: kenxcodeHome || process.env.KENXCODE_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, kenxcodeHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, kenxcodeHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--kenxcode-home', kenxcodeHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, kenxcodeHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, kenxcodeHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, kenxcodeHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(kenxcodeHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, kenxcodeHome, emit })
