...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'kenxcode-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/kenxcode-runner-test',
    sourceRepoRoot: null,
    kenxcodeHome: '/tmp/kenxcode-runner-test',
    logRoot: '/tmp/kenxcode-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'kenxcode-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'kenxcode-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/kenxcode-agent',
      kenxcodeHome: '/tmp/kenxcode'
    }),
    ['--dir', '/tmp/kenxcode-agent', '--kenxcode-home', '/tmp/kenxcode', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/kenxcode-agent', '--kenxcode-home', '/tmp/kenxcode', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/kenxcode',
      kenxcodeHome: '/tmp/home'
    }),
    ['--dir', '/tmp/kenxcode', '--kenxcode-home', '/tmp/home', '--branch', 'main']
  )
})
