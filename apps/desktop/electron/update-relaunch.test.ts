 ...
 *      internals; KENXCODE_HOME / KENXCODE_DESKTOP_* env + cwd preserved) and is
 *      safely shell-quoted.
...
const ROOT = '/home/u/.kenxcode/kenxcode-agent'
...
test('resolveUnpackedRelease returns the dir for a binary UNDER release/<plat>-unpacked', () => {
  const exec = path.join(UNPACKED, 'kenxcode')
...
test('resolveUnpackedRelease is null for AppImage / .deb / .rpm / dev / unresolved paths', () => {
  // AppImage mount
  assert.equal(resolveUnpackedRelease('/tmp/.mount_KenXCode12345/AppRun', ROOT, 'linux'), null)
  // .deb / .rpm system install
  assert.equal(resolveUnpackedRelease('/usr/lib/kenxcode/kenxcode', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease('/opt/KenXCode/kenxcode', ROOT, 'linux'), null)
  // dev electron
  assert.equal(
    resolveUnpackedRelease('/home/u/.kenxcode/kenxcode-agent/node_modules/electron/dist/electron', ROOT, 'linux'),
    null
  )
  // empty / missing
  assert.equal(resolveUnpackedRelease('', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease(path.join(UNPACKED, 'kenxcode'), '', 'linux'), null)
})
...
test('resolveUnpackedRelease is not fooled by a sibling prefix dir', () => {
  ...
  const sneaky = path.join(ROOT, 'apps', 'desktop', 'release', 'linux-unpacked-evil', 'kenxcode')
...
test('collectRelaunchArgs drops Electron internals, keeps user/launcher args', () => {
  const argv = [
    ...
    '--lang=en-US',
    'kenxcode://open/agent/42', // deep link — keep
    '--profile=work', // app flag — keep
    '--remote-debugging-port=9222' // internal — drop
  ]

  assert.deepEqual(collectRelaunchArgs(argv), ['--no-sandbox', 'kenxcode://open/agent/42', '--profile=work'])
  assert.deepEqual(collectRelaunchArgs(undefined), [])
})

test('collectRelaunchEnv preserves KENXCODE_HOME + KENXCODE_DESKTOP_* + sandbox opt-out only', () => {
  const env = {
    KENXCODE_HOME: '/home/u/.kenxcode',
    KENXCODE_DESKTOP_REMOTE_URL: 'http://box:9119',
    KENXCODE_DESKTOP_REMOTE_TOKEN: 'secret',
    KENXCODE_DESKTOP_KENXCODE_ROOT: '/home/u/dev/kenxcode',
    KENXCODE_DESKTOP_APP_NAME: 'KenXCodeSandbox',
...
test('buildRelaunchScript embeds pid/exec/args/env/cwd and is valid bash', () => {
  const script = buildRelaunchScript({
    pid: 4242,
    execPath: '/home/u/.kenxcode/kenxcode-agent/apps/desktop/release/linux-unpacked/KenXCode',
    args: ['kenxcode://open/agent/42', "--note=it's fine"],
    env: { KENXCODE_HOME: '/home/u/.kenxcode', KENXCODE_DESKTOP_REMOTE_URL: 'http://box:9119' },
    cwd: '/home/u/work dir'
  })
  ...
  assert.match(script, /export KENXCODE_HOME='\/home\/u\/\.kenxcode'/)
  assert.match(script, /export KENXCODE_DESKTOP_REMOTE_URL='http:\/\/box:9119'/)
  assert.match(script, /cd '\/home\/u\/work dir'/)
  assert.match(script, /exec '.*\/linux-unpacked\/KenXCode' 'kenxcode:\/\/open\/agent\/42' '--note=it'\\''s fine'/)

  // It must be syntactically valid bash (`bash -n`). Write to a temp file and lint.
  const tmp = path.join(os.tmpdir(), `kenxcode-relaunch-test-${Date.now()}.sh`)
...
test('buildRelaunchScript with no args/env still lints clean', () => {
  const script = buildRelaunchScript({
    pid: 1,
    execPath: '/opt/KenXCode/KenXCode',
    args: [],
    env: {},
    cwd: ''
  })

  const tmp = path.join(os.tmpdir(), `kenxcode-relaunch-test2-${Date.now()}.sh`)
  fs.writeFileSync(tmp, script)
  ...
  assert.match(script, /exec '\/opt\/KenXCode\/KenXCode'\n/)
})
