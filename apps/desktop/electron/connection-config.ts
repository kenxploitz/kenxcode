 ...
 *     `X-KenXCode-Session-Token` header; WS uses `?token=`.
...
// kenxcode_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - kenxcode_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - kenxcode_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / kenxcode #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-kenxcode_session_at', '__Secure-kenxcode_session_at', 'kenxcode_session_at']
const RT_COOKIE_VARIANTS = ['__Host-kenxcode_session_rt', '__Secure-kenxcode_session_rt', 'kenxcode_session_rt']

// The Nous portal (NAS) does NOT use KenXCode gateway session cookies — it is a
...
// Keep this aligned with kenxcode_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['kenxcode', 'test', 'tmp', 'root', 'sudo'])
...
// KenXCode Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteKenXCodePath = String(entry.remoteKenXCodePath || '').trim()

  if (remoteKenXCodePath) {
    out.remoteKenXCodePath = remoteKenXCodePath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote KenXCode installation. Preserve an explicit mapping
  // when it is a valid KenXCode profile identifier; otherwise fall back to the
 ...
 *     `--profile`, so its `KENXCODE_HOME` scopes it.
 *
...
function pathWithGlobalRemoteProfile(path, profile, opts: ProfileRouteOptions = {}) {
  ...
  try {
    parsed = new URL(rawPath, 'http://kenxcode.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a kenxcode session ACCESS-token cookie
 ...
 * the KenXCode gateway session cookies, so cloud sign-in / discovery liveness
