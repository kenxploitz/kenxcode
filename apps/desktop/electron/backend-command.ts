// Backend subcommand routing for the desktop-managed KenXCode process.
//
// The desktop app launches its own headless backend via `kenxcode serve` — it
...
// app hasn't updated yet, or an older `kenxcode` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional KenXCode profile to pin via `--profile`.
 */
 ...
 * `-m kenxcode_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `kenxcode_cli/subcommands/dashboard.py` source registers
