 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote KenXCode dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the KenXCode install on the remote (login-shell probe),
 ...
 *     `KENXCODE_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForKenXCode in.
 *
 * The minted KENXCODE_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const READY_RE = /^KENXCODE_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
const REMOTE_LOCK_DIR = '~/.kenxcode/desktop-ssh'
...
// Resolve the remote kenxcode executable. An EXPLICIT path is honored strictly
...
async function locateKenXCode(ssh, remoteKenXCodePath) {
  ...
  if (remoteKenXCodePath) {
    if (await isExecutable(remoteKenXCodePath)) {
      return resolveLauncher(remoteKenXCodePath)
    }

    const err: any = new Error(
      `The KenXCode path you set is not an executable on the remote host: "${remoteKenXCodePath}". ` +
        'Check the path (it must be the full path to the `kenxcode` binary on the remote, e.g. ' +
        '~/kenxcode-agent/.venv/bin/kenxcode), or clear it to auto-detect.'
    )

    err.kind = 'kenxcode-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v kenxcode')}`)).trim()
  ...
  candidates.push('~/.local/bin/kenxcode')
  candidates.push('/usr/local/bin/kenxcode')
  candidates.push('~/.kenxcode/kenxcode-agent/venv/bin/kenxcode')
  ...
  const err: any = new Error(
    'KenXCode is not installed on the remote host (could not find a `kenxcode` executable). ' +
      'Install it on the remote with:  curl -fsSL https://kenxcode-agent.nousresearch.com/install.sh | sh  ' +
      '— or set the KenXCode path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<kenxcode> --version`,
// e.g. "KenXCode Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH kenxcode a
// connection uses, so a stale/unexpected install is visible.
async function probeKenXCodeVersion(ssh, kenxcodePath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(kenxcodePath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". KenXCode Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The KENXCODE_HOME the remote dashboard will use (explicit env wins, else
// ~/.kenxcode). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteKenXCodeHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${KENXCODE_HOME:-$HOME/.kenxcode}"')).trim().split('\n').pop()

    return out || '~/.kenxcode'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote KenXCode home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'kenxcodePath', 'kenxcodeHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(ssh, pid, spawnNonce, kenxcodePath = '') {
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !kenxcodePath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(kenxcodePath)})\n` +
...
async function cleanupStale(ssh, ownershipId, lock, pidAlive = true) {
  if (pidAlive && lock && (await pidIsOurDashboard(ssh, lock.pid, lock.spawnNonce, lock.kenxcodePath))) {
    try {
      const result = (
        await ssh.exec(
...
function buildSpawnCommand(kenxcodePath, profile, opts: any = {}) {
  const kenxcode = expandRemotePath(kenxcodePath)
  ...
  const dashCmd = `env KENXCODE_DESKTOP=1 ${kenxcode} ${profileArgs}${subCmd}`
...
async function remoteSupportsSshOwnership(ssh, kenxcodePath) {
  const kenxcode = expandRemotePath(kenxcodePath)

  const out = await ssh.exec(
    `help="$(${kenxcode} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(ssh, { kenxcodePath, profile, token, ownershipId }) {
  if (!(await remoteSupportsSshOwnership(ssh, kenxcodePath))) {
    const err: any = new Error(
      'The remote KenXCode install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update KenXCode on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    out = await ssh.exec(buildSpawnCommand(kenxcodePath, profile, { spawnNonce, tokenFilePath, logPath }))
  } catch (error) {
    try {
      await ssh.exec(`rm -f ${expandRemotePath(tokenFilePath)}`)
    } catch {
 ...
 * opened SshConnection, forward/pickLocalPort/waitForKenXCode, a token-gated
...
async function connect(deps) {
  ...
  const kenxcodePath = await locateKenXCode(ssh, remoteKenXCodePath)
  log(`located kenxcode at ${kenxcodePath}`)
  const kenxcodeVersion = await probeKenXCodeVersion(ssh, kenxcodePath)

  if (kenxcodeVersion) {
    log(`remote kenxcode version: ${kenxcodeVersion}`)
  }

  const reuseToken = deps.reuseToken || ''
  const kenxcodeHome = await probeRemoteKenXCodeHome(ssh)
  const lock = await readLockfile(ssh, ownershipId)

  if (lock) {
    const pidAlive = await remotePidAlive(ssh, lock.pid)
    const owned = pidAlive && (await pidIsOurDashboard(ssh, lock.pid, lock.spawnNonce, lock.kenxcodePath))
    ...
    const reusable =
      ...
      lock.kenxcodePath === kenxcodePath &&
      lock.kenxcodeHome === kenxcodeHome
  ...
  try {
    ...
    assertNotAborted(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForKenXCode(baseUrl, spawnToken)
    assertNotAborted(signal)
...
export {
  ...
  ownershipDirectory,
  pidIsOurDashboard,
  probeKenXCodeVersion,
  probeRemoteKenXCodeHome,
  probeRemotePlatform,
  PROTOCOL_VERSION,
  readLockfile,
  READY_RE,
