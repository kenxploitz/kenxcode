...
test('stagedUpdaterSupportsPrewrittenMarker rejects installers predating the self-adopt fix', () => {
  ...
  // refreshed because copy_self_to_kenxcode_home no-ops during --update.
  assert.equal(
    stagedUpdaterSupportsPrewrittenMarker('C:\\KenXCode\\kenxcode-setup.exe', {
...
test('resolveStagedUpdaterBinary still returns a stale staged updater on Windows', () => {
  ...
  assert.equal(
    resolveStagedUpdaterBinary('C:\\KenXCode', {
      fileExists: () => true,
      isWindows: true,
      stagedMtimeMs: () => MARKER_SELF_ADOPT_EPOCH_MS - 60 * DAY_MS
    }),
    path.join('C:\\KenXCode', 'kenxcode-setup.exe')
  )
})
...
test('spawnUpdaterProcess hides the updater console and detaches the child on Windows', () => {
  ...
  const result = spawnUpdaterProcess(
    'kenxcode-setup.exe',
    ['--update', '--branch', 'main'],
    { cwd: 'C:\\KenXCode', detached: true, stdio: 'ignore' },
    {
      isWindows: true,
  ...
  assert.deepEqual(calls, [
    {
      ...
      command: 'kenxcode-setup.exe',
      options: { cwd: 'C:\\KenXCode', detached: true, stdio: 'ignore', windowsHide: true }
    }
  ])
})
...
test('resolveStagedUpdaterBinary hands Windows the staged installer it finds', () => {
  const home = 'C:\\Users\\kenxcode\\AppData\\Local\\kenxcode'
  const staged = path.join(home, 'kenxcode-setup.exe')
  const probed: string[] = []
...
test('resolveStagedUpdaterBinary returns null off Windows even when kenxcode-setup is staged (#74836)', () => {
  const home = '/Users/kenxcode/.kenxcode'
  let probes = 0

  const resolved = resolveStagedUpdaterBinary(home, {
    // The installer stages kenxcode-setup on macOS/Linux too, so "it exists" is
...
test('resolveStagedUpdaterBinary returns null on Windows when nothing is staged', () => {
  const resolved = resolveStagedUpdaterBinary('C:\\Users\\kenxcode\\AppData\\Local\\kenxcode', {
    fileExists: () => false,
    isWindows: true
  })
