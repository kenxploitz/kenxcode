/**
 * Shared E2E fixtures for the KenXCode desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → kenxcode serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `kenxcode desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `kenxcode-e2e-${prefix}-${Math.random()}`))
  const kenxcodeHome = path.join(root, 'kenxcode-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(kenxcodeHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(kenxcodeHome, 'config.yaml')
 ...
 */
export function writeEnvFile(kenxcodeHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(kenxcodeHome, '.env')
 ...
 */
function writeEmptyConfig(kenxcodeHome: string): void {
  const configPath = path.join(kenxcodeHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - KENXCODE_HOME → sandbox kenxcode-home (isolated config/sessions)
 *  - KENXCODE_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - KENXCODE_DESKTOP_IGNORE_EXISTING=1 → don't pick up `kenxcode` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - KENXCODE_DESKTOP_KENXCODE_ROOT → REPO_ROOT (dev checkout resolution)
 *  - KENXCODE_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    KENXCODE_HOME: sandbox.kenxcodeHome,
    KENXCODE_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    KENXCODE_DESKTOP_IGNORE_EXISTING: '1',
    KENXCODE_DESKTOP_KENXCODE_ROOT: REPO_ROOT,
    KENXCODE_DESKTOP_APP_NAME: `KenXCodeE2E-${Date.now()}`,
    ...
    KENXCODE_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated KENXCODE_HOME + userData
 * @param env      - the process environment (already has KENXCODE_HOME etc.)
...
export async function setupMockBackend(options: MockBackendOptions = {}): Promise<MockBackendFixture> {
  ...
  writeMockProviderConfig(
    sandbox.kenxcodeHome,
    mock.url,
    options.extraDisplayConfig,
    options.extraConfig,
    options.modelContextLength,
  )
  writeEnvFile(sandbox.kenxcodeHome)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.kenxcodeHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via KENXCODE_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`kenxcode serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.kenxcodeHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { KENXCODE_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to KenXCode backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'KenXCode.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'KenXCode.app', 'Contents', 'MacOS', 'KenXCode')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'kenxcode')
}
 ...
 * progress without spawning a real KenXCode backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * KENXCODE_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    KENXCODE_DESKTOP_BOOT_FAKE: '1',
    KENXCODE_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + kenxcode-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).KENXCODE_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).KENXCODE_DESKTOP_KENXCODE
  delete (env as Record<string, string | undefined>).KENXCODE_DESKTOP_KENXCODE_ROOT
