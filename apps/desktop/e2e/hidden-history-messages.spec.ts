...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  writeMockProviderConfig(sandbox.kenxcodeHome, mock.url)
  fs.appendFileSync(
    path.join(sandbox.kenxcodeHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.kenxcodeHome)
  const builder = await RealSessionBuilder.start(sandbox.kenxcodeHome)
  try {
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.kenxcodeHome, mock.url)
  fs.appendFileSync(path.join(sandbox.kenxcodeHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
  writeEnvFile(sandbox.kenxcodeHome)
  const { app, page } = await launchDesktop(buildAppEnv(sandbox))
