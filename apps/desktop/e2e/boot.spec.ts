 ...
 * packaged binary) with a real `kenxcode serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → kenxcode serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with KenXCode title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('KenXCode')
  })
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → kenxcode serve is spawned → WS connects → config
