 ...
 * AIAgent loop, using the E2E mock provider configured in `kenxcodeHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(kenxcodeHome: string) {
    this.child = spawn('uv', ['run', '--active', '--no-sync', 'python', '-m', 'tui_gateway.entry'], {
      cwd: REPO_ROOT,
      env: {
        ...process.env,
        KENXCODE_HOME: kenxcodeHome,
  ...
  static async start(kenxcodeHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(kenxcodeHome)
