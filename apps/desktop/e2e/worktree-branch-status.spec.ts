...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'KenXCode E2E'], { cwd: repo })
...
function configureRepoCwd(kenxcodeHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(kenxcodeHome, mockUrl)
  fs.appendFileSync(path.join(kenxcodeHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(kenxcodeHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.kenxcodeHome, mock.url, repo)
