...
How to build KenXCode Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another KenXCode surface can also
...
- A **runtime home change** (switching the underlying `KENXCODE_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across KenXCode — do not assume
