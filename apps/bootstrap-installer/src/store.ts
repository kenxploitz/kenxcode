...
/// desktop app handing off via `KenXCode-Setup.exe --update`.
...
export const $kenxcodeHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.kenxcode/logs/bootstrap-installer.log')
    $kenxcodeHome.set('~/.kenxcode')
  ...
  try {
    const [logPath, kenxcodeHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_kenxcode_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $kenxcodeHome.set(kenxcodeHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch KenXCode" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (KenXCode-Setup.exe --update);
...
export async function launchKenXCodeDesktop(): Promise<void> {
  ...
  await invoke('launch_kenxcode_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'KenXCode repository' },
