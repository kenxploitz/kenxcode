...
if [ "$$" -eq 1 ]; then
    exec /init /opt/kenxcode/docker/main-wrapper.sh "$@"
fi

echo "[kenxcode] WARNING: container entrypoint is not PID 1; skipping s6-overlay /init and falling back to direct bootstrap. Supervised services are unavailable in this runtime, but the requested command will still run." >&2
...
/opt/kenxcode/docker/stage2-hook.sh
exec /opt/kenxcode/docker/main-wrapper.sh "$@"
