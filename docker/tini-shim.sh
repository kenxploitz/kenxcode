...
# KenXCode WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: KENXCODE_TINI_SHIM_TARGET overrides the `/init` path (and
# KENXCODE_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${KENXCODE_TINI_SHIM_TARGET:-/init}"
WRAPPER="${KENXCODE_TINI_SHIM_WRAPPER:-/opt/kenxcode/docker/main-wrapper.sh}"
