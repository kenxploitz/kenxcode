...
# by /etc/cont-init.d/01-kenxcode-setup (installed by the Dockerfile). This
...
# on the pre-s6 contract "entrypoint.sh sets up state then execs kenxcode"
...
echo "[kenxcode] WARNING: docker/entrypoint.sh is a deprecated shim under " \
...
exec /opt/kenxcode/docker/stage2-hook.sh "$@"
