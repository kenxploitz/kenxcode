...
#
# Wired into the image as /etc/cont-init.d/01-kenxcode-setup by the
...
KENXCODE_HOME="${KENXCODE_HOME:-/opt/data}"
INSTALL_DIR="/opt/kenxcode"

# Drop to kenxcode via s6-setuidgid, but skip it when already non-root.
as_kenxcode() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid kenxcode "$@"; }
...
# starts non-root. The baked install tree under /opt/kenxcode is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $KENXCODE_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass KENXCODE_UID/KENXCODE_GID — or the PUID/PGID aliases — which the
...
# `id -u kenxcode` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# kenxcode UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u kenxcode)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-kenxcode UID).
...
and the baked /opt/kenxcode install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e KENXCODE_UID=\$(id -u) -e KENXCODE_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the kenxcode user to that UID/GID at boot and chowns the data
...
# --- Bootstrap KENXCODE_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid kenxcode mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom KENXCODE_HOME paths whose parents only
# root can create (e.g. `KENXCODE_HOME=/home/kenxcode/.kenxcode` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for KENXCODE_UID/KENXCODE_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  KENXCODE_UID/
# KENXCODE_GID still win when both are set.  See #15290, salvages #25872.
KENXCODE_UID="${KENXCODE_UID:-${PUID:-}}"
KENXCODE_GID="${KENXCODE_GID:-${PGID:-}}"

if [ -n "${KENXCODE_UID:-}" ] && validate_uid_gid "$KENXCODE_UID" && [ "$KENXCODE_UID" != "$(id -u kenxcode)" ]; then
    echo "[stage2] Changing kenxcode UID to $KENXCODE_UID"
    usermod -u "$KENXCODE_UID" kenxcode
fi
if [ -n "${KENXCODE_GID:-}" ] && validate_uid_gid "$KENXCODE_GID" && [ "$KENXCODE_GID" != "$(id -g kenxcode)" ]; then
    echo "[stage2] Changing kenxcode GID to $KENXCODE_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$KENXCODE_GID" kenxcode 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised kenxcode user (UID 10000)
...
# kenxcode process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes kenxcode. Idempotent across container
...
#   - kenxcode is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G kenxcode 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] kenxcode already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" kenxcode 2>/dev/null; then
        echo "[stage2] Added kenxcode to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group kenxcode failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When KENXCODE_UID is remapped or the top-level $KENXCODE_HOME isn't owned by
# the runtime kenxcode UID, restore ownership to kenxcode — but ONLY for the
# directories kenxcode actually writes to. The full $KENXCODE_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of kenxcode-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_kenxcode_uid=$(id -u kenxcode)

path_has_symlink_component() {
    path="$1"
    root="${2:-$KENXCODE_HOME}"
...
chown_kenxcode_tree() {
    ...
    chown -R kenxcode:kenxcode "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_kenxcode_owner() {
    target="$1"
    find "$target" \( ! -user kenxcode -o ! -group kenxcode \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$KENXCODE_HOME" 2>/dev/null)" != "$actual_kenxcode_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $KENXCODE_HOME (targeted) to kenxcode ($actual_kenxcode_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $KENXCODE_HOME: chown the directory itself (not its contents)
    # so kenxcode can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$KENXCODE_HOME"; then
        :
    else
        chown kenxcode:kenxcode "$KENXCODE_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $KENXCODE_HOME failed (rootless container?) — continuing"
    fi
    # KenXCode-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by kenxcode (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$KENXCODE_HOME/$sub" ] && tree_has_non_kenxcode_owner "$KENXCODE_HOME/$sub"; then
            chown_kenxcode_tree "$KENXCODE_HOME/$sub"
        fi
    done
fi
...
# kenxcode user. Hosted/container instances keep mutable state under
# $KENXCODE_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# KENXCODE_DISABLE_LAZY_INSTALLS=1. Keeping /opt/kenxcode root-owned and
...
# $KENXCODE_HOME/lazy-packages dir on the data volume (Dockerfile sets
# KENXCODE_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to kenxcode
...
# Always reset ownership of $KENXCODE_HOME/profiles to kenxcode on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> kenxcode …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as kenxcode and walks
...
if [ -d "$KENXCODE_HOME/profiles" ] && tree_has_non_kenxcode_owner "$KENXCODE_HOME/profiles"; then
    chown_kenxcode_tree "$KENXCODE_HOME/profiles"
fi

# Always reset ownership of $KENXCODE_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged kenxcode runtime even
...
# gate as profiles/).
if [ -d "$KENXCODE_HOME/cron" ] && tree_has_non_kenxcode_owner "$KENXCODE_HOME/cron"; then
    chown_kenxcode_tree "$KENXCODE_HOME/cron"
fi

# Always ensure logs/gateways is kenxcode-owned (#45258). Formerly healed by
...
# top-level $KENXCODE_HOME is mis-owned, so a warm volume with kenxcode-owned
# KENXCODE_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid kenxcode mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as kenxcode.
if [ -d "$KENXCODE_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$KENXCODE_HOME/logs/gateways"; then
        :
    else
        chown kenxcode:kenxcode "$KENXCODE_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# kenxcode pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged kenxcode gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $KENXCODE_HOME is
...
# and cron/.
if [ -d "$KENXCODE_HOME/platforms/pairing" ] && tree_has_non_kenxcode_owner "$KENXCODE_HOME/platforms/pairing"; then
    chown_kenxcode_tree "$KENXCODE_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$KENXCODE_HOME/pairing" ] && tree_has_non_kenxcode_owner "$KENXCODE_HOME/pairing"; then
    chown_kenxcode_tree "$KENXCODE_HOME/pairing"
fi

# Reset ownership of kenxcode-owned top-level state files on every boot.
# The targeted data-volume chown above only covers kenxcode-owned
# *subdirectories*; loose state files living directly under $KENXCODE_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> kenxcode …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged kenxcode runtime then hits
...
# sweep so host-owned files in a bind-mounted $KENXCODE_HOME are never
...
# entries of kenxcode_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$KENXCODE_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$KENXCODE_HOME/$f"; then
            :
        else
            chown kenxcode:kenxcode "$KENXCODE_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the kenxcode runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$KENXCODE_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$KENXCODE_HOME/config.yaml"; then
        :
    else
        chown kenxcode:kenxcode "$KENXCODE_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$KENXCODE_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as kenxcode user ---
# Run as kenxcode via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $KENXCODE_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_kenxcode mkdir -p \
    "$KENXCODE_HOME/backups" \
    "$KENXCODE_HOME/cron" \
    "$KENXCODE_HOME/sessions" \
    "$KENXCODE_HOME/logs" \
    "$KENXCODE_HOME/logs/gateways" \
    "$KENXCODE_HOME/hooks" \
    "$KENXCODE_HOME/memories" \
    "$KENXCODE_HOME/skills" \
    "$KENXCODE_HOME/skins" \
    "$KENXCODE_HOME/plans" \
    "$KENXCODE_HOME/workspace" \
    "$KENXCODE_HOME/home" \
    "$KENXCODE_HOME/pairing" \
    "$KENXCODE_HOME/platforms/pairing" \
    "$KENXCODE_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/kenxcode/.install_method (see Dockerfile), NOT written here into
# $KENXCODE_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $KENXCODE_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.kenxcode:/opt/data) and sometimes shared with a
...
# 'kenxcode update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $KENXCODE_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/kenxcode stamp, so deleting the data-dir copy is safe).
if [ -f "$KENXCODE_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$KENXCODE_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$KENXCODE_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$KENXCODE_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$KENXCODE_HOME/$dest"; then
            :
        else
            as_kenxcode cp "$INSTALL_DIR/$src" "$KENXCODE_HOME/$dest"
        fi
    fi
}
...
if [ -f "$KENXCODE_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$KENXCODE_HOME/.env"; then
        :
    else
        chown kenxcode:kenxcode "$KENXCODE_HOME/.env" 2>/dev/null || true
        chmod 600 "$KENXCODE_HOME/.env" 2>/dev/null || true
    fi
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $KENXCODE_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `kenxcode update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set KENXCODE_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$KENXCODE_HOME/config.yaml" ]; then
    s6-setuidgid kenxcode "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$KENXCODE_HOME/auth.json" ] && [ -n "${KENXCODE_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$KENXCODE_HOME/auth.json"; then
        :
    else
        printf '%s' "$KENXCODE_AUTH_JSON_BOOTSTRAP" > "$KENXCODE_HOME/auth.json"
        chown kenxcode:kenxcode "$KENXCODE_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$KENXCODE_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via KENXCODE_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$KENXCODE_HOME/auth.json" ] && [ -n "${KENXCODE_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$KENXCODE_HOME/auth.json"; then
        :
    else
        s6-setuidgid kenxcode "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$KENXCODE_HOME/auth.json" \
...
# can set KENXCODE_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# KENXCODE_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$KENXCODE_HOME/gateway_state.json" ] && \
        [ "${KENXCODE_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$KENXCODE_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$KENXCODE_HOME/gateway_state.json"
        chown kenxcode:kenxcode "$KENXCODE_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$KENXCODE_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_kenxcode "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/kenxcode/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI KenXCode spawns for the
...
# main-wrapper.sh propagates it into the supervised ``kenxcode`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-kenxcode, dashboard, etc.).
