#!/bin/sh
# shellcheck shell=sh
# /opt/kenxcode/bin/kenxcode — `docker exec` privilege-drop shim.
#
# Background
# ----------
# The s6 image runs the supervised gateway/main process as the unprivileged
# `kenxcode` user (UID 10000). When an operator runs `docker exec <c> kenxcode ...`
# the default UID is root (0), and any file the command writes under
# $KENXCODE_HOME — auth.json, .env, config.yaml — ends up root-owned and
# unreadable to the supervised gateway. The most common manifestation: the
# user runs `docker exec <c> kenxcode login`, this writes
# /opt/data/auth.json as root:root mode 0600, and from then on the gateway
# returns "Provider authentication failed: KenXCode is not logged into Nous
# Portal" on every incoming message — even though `docker exec <c> kenxcode
...
# Fix
# ---
# This shim sits at /opt/kenxcode/bin/kenxcode and is placed earliest on PATH.
# When invoked as root, it drops to the kenxcode user (via s6-setuidgid)
# before exec'ing the real venv binary, so anything that writes under
# $KENXCODE_HOME is uid-aligned with the supervised processes. When invoked
# as any non-root UID — including the supervised processes themselves,
# `docker exec --user kenxcode`, kanban subagents, etc. — it short-circuits
...
# (/opt/kenxcode/.venv/bin/kenxcode), so the second hop cannot re-enter this
# shim regardless of PATH state. No sentinel env var needed.
#
# Opt-out: set KENXCODE_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
...
# state via the kenxcode CLI. Default is to drop.

set -e

REAL=/opt/kenxcode/.venv/bin/kenxcode

# Defensive: if the venv binary is missing (corrupted image, partial
# install), fail loudly rather than silently masking it.
if [ ! -x "$REAL" ]; then
    echo "kenxcode-shim: $REAL not found or not executable" >&2
    exit 127
fi

# Already non-root? Just exec the real binary. This is the hot path for
# supervised processes (uid 10000) and for `docker exec --user kenxcode`.
if [ "$(id -u)" != "0" ]; then
    exec "$REAL" "$@"
fi

# Root, with opt-out set? Honor it.
case "${KENXCODE_DOCKER_EXEC_AS_ROOT:-}" in
    1|true|TRUE|True|yes|YES|Yes)
        exec "$REAL" "$@"
        ;;
esac

# Root, no opt-out. Drop to the kenxcode user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "kenxcode-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "kenxcode-shim: re-run with --user kenxcode or set KENXCODE_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the kenxcode user's home before dropping privileges. Without
