...
const logDir = join(process.env.KENXCODE_HOME?.trim() || join(homedir(), '.kenxcode'), 'logs')
...
// a real ~/.kenxcode (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('kenxcode-tui: parent lifecycle log unavailable\n')
    }
  }
}
