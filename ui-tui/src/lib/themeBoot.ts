 ...
 * `kenxcode-boot-background` / `kenxcode-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports KENXCODE_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.kenxcode matches get_kenxcode_home()'s default.
const bootFilePath = () => join(process.env.KENXCODE_HOME ?? join(homedir(), '.kenxcode'), 'tui-theme-boot.json')

// Never touch the user's real ~/.kenxcode from test runs (the TS suite has no
// KENXCODE_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into KENXCODE_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.KENXCODE_TUI_THEME || env.KENXCODE_TUI_LIGHT) {
    return result
  }
  ...
  if (boot.mode) {
    env.KENXCODE_TUI_THEME = boot.mode
  ...
  if (
    ...
    !env.KENXCODE_TUI_BACKGROUND
  ) {
    env.KENXCODE_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.KENXCODE_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.KENXCODE_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into KENXCODE_TUI_THEME.
