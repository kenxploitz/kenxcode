...
import { Box, Text } from '@kenxcode/ink'
...
/**
 * User widget apps — KenXCode authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$KENXCODE_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.kenxcode/plugins/`: files under KENXCODE_HOME execute
...
const widgetsDir = () => join(process.env.KENXCODE_HOME?.trim() || join(homedir(), '.kenxcode'), 'tui-widgets')
 ...
 *  every change, so a widget KenXCode writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or KenXCode) ever writes must hot-load too — a 10s poll here read
