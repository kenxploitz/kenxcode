...
import { terminalBackgroundHex } from '@kenxcode/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $KENXCODE_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see KENXCODE_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['KENXCODE_TUI_BACKGROUND', process.env.KENXCODE_TUI_BACKGROUND ?? '(unset)'],
            ['KENXCODE_TUI_THEME', process.env.KENXCODE_TUI_THEME ?? '(unset)'],
