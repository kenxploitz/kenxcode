import type { RunExternalProcess } from '@kenxcode/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`kenxcode ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching kenxcode: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`kenxcode ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
