import type { InputEvent, Key } from '@kenxcode/ink'
import * as Ink from '@kenxcode/ink'
 ...
 * but that accepts `key.meta` on macOS — and kenxcode-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.KENXCODE_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
