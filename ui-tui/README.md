...
React + Ink terminal UI for KenXCode. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `KENXCODE_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `kenxcode setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.kenxcode/.kenxcode_history` or under `KENXCODE_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `kenxcode setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/kenxcode-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external kenxcode setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.kenxcode/tui-parent.log
    ...
    types/
      kenxcode-ink.d.ts            type declarations for @kenxcode/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent KenXCodeCLI subprocess for slash commands
```
