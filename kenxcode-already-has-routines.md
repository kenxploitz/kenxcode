# KenXCode Agent Has Had "Routines" Since March
...
KenXCode equivalent — works today:
```bash
...
KenXCode equivalent — works today:
```bash
kenxcode webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working KenXCode implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | KenXCode Agent |
|---|---|---|
...
## Things KenXCode Does That Routines Can't
...
KenXCode has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
KenXCode Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
kenxcode gateway setup    # enable webhooks
kenxcode webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation blueprints gallery: [kenxcode-agent.nousresearch.com/docs/reference/automation-blueprints-catalog](https://kenxcode-agent.nousresearch.com/docs/reference/automation-blueprints-catalog)

Documentation: [kenxcode-agent.nousresearch.com](https://kenxcode-agent.nousresearch.com)

GitHub: [github.com/NousResearch/kenxcode-agent](https://github.com/NousResearch/kenxcode-agent)

---

*KenXCode Agent is built by [Nous Research](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
