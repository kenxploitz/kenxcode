"""Windows UTF-8 bootstrap for KenXCode entry points.
...
should be imported at the very top of every KenXCode entry point
(``kenxcode``, ``kenxcode-agent``, ``kenxcode-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``kenxcode_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``kenxcode_bootstrap``, never ``kenxcode_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing KenXCode modules.

    KenXCode ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real KenXCode source root
    ...
    """
    root = src_root or os.environ.get("KENXCODE_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``KENXCODE_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("KENXCODE_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import kenxcode_bootstrap``
# (or ``from kenxcode_bootstrap import apply_windows_utf8_bootstrap``) at
