...
interface KenXCodeConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function KenXCodeConsoleModal({ open, onClose }: KenXCodeConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("kenxcode> ");
  const inputPromptRef = useRef("kenxcode> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "kenxcode> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to KenXCode Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="kenxcode-console-title"
    >
      <div
        className={cn(
          themedBody,
