...
const SIDEBAR_COLLAPSED_KEY = "kenxcode-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="kenxcode-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="kenxcode-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateKenXCode = status?.can_update_kenxcode === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'kenxcode update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "kenxcode update";
    return (
      t.status.updateKenXCodeConfirmMessage ??
      `This will run 'kenxcode update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateKenXCodeConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateKenXCode) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateKenXCode,
      runningLabel: t.status.updatingKenXCode,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the KenXCode gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateKenXCodeConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateKenXCodeConfirmTitle ?? `${t.status.updateKenXCode}?`}
    />
    </>
  );
}
