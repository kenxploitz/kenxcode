"""CLI entry point for the kenxcode-agent ACP adapter.

Loads environment variables from ``~/.kenxcode/.env``, configures logging
...
try:
    ...
    # shadowing KenXCode's own modules — ``kenxcode acp`` can be started from any
    # cwd, including a project that has same-named packages on its path.
    kenxcode_bootstrap.harden_import_path()
...
def _load_env() -> None:
    """Load .env from KENXCODE_HOME (default ``~/.kenxcode``)."""
    from kenxcode_cli.env_loader import load_kenxcode_dotenv

    kenxcode_home = get_kenxcode_home()
    loaded = load_kenxcode_dotenv(kenxcode_home=kenxcode_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", kenxcode_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="kenxcode-acp",
        description="Run KenXCode Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print KenXCode version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive KenXCode provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.kenxcode/node/ "
...
def _print_version() -> None:
    ...
    print(kenxcode_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import KenXCodeACPAgent  # noqa: F401

    print("KenXCode ACP check OK")


def _run_setup() -> None:
    from kenxcode_cli.main import main as kenxcode_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "kenxcode", "model"]
        kenxcode_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting kenxcode-agent ACP adapter")
    ...
    import acp
    from .server import KenXCodeACPAgent
    ...
    if os.environ.get("KENXCODE_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
