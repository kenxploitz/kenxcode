"""ACP auth helpers — detect and advertise KenXCode authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "kenxcode-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active KenXCode runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if KenXCode can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for KenXCode.
    ...
    not have KenXCode provider credentials configured yet, so KenXCode always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate KenXCode using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure KenXCode provider",
            description=(
                "Open KenXCode' interactive model/provider setup in a terminal. "
                "Use this when KenXCode has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
