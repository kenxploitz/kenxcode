"""ACP session manager — maps ACP sessions to KenXCode AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.kenxcode/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when KenXCode itself is running in WSL.

    Windows ACP clients can launch ``kenxcode acp`` inside WSL while still sending
...
def _normalize_cwd_for_compare(cwd: str | None) -> str:
    ...
    from kenxcode_constants import windows_path_to_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch KenXCode from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["kenxcode-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed KenXCode agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by KenXCode AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current KenXCode runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.kenxcode/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``KENXCODE_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_kenxcode_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=kenxcode_home / "state.db")
            return self._db_instance
        except Exception:
    ...
    def _restore(self, session_id: str) -> Optional[SessionState]:
        ...
        # for the rest of the session (see kenxcode_state.get_messages_as_conversation).
        try:
    ...
    def _make_agent(
        ...
        *,
        ...
        # the automatic late-refresh (see KenXCodeACPAgent._schedule_mcp_late_refresh).
        try:
        ...
        # editor/session cwd instead of the KenXCode daemon's process cwd.
