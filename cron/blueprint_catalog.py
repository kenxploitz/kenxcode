...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``kenxcode://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``kenxcode://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"kenxcode://blueprint/{quote(blueprint.key)}{qs}"
