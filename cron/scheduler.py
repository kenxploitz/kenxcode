...
Uses a file-based lock (~/.kenxcode/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `kenxcode update` reloads
# the module) fail with ModuleNotFoundError for kenxcode_time et al.
...
from kenxcode_time import now as _kenxcode_now
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy kenxcode_cli import at cron module load (matches
    # _resolve_cron_enabled_toolsets' fallback) and share one MCP-membership
    # computation with the gateway/CLI platform resolver.
    from kenxcode_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``kenxcode tools`` config for the ``cron`` platform.
    ...
    try:
        from kenxcode_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``kenxcode update`` / ``kenxcode gateway stop`` / systemd restart, or an
...
_kenxcode_home: Path | None = None


def _get_kenxcode_home() -> Path:
    """Resolve KenXCode home dynamically while preserving test monkeypatch hooks.

    Cron is per-profile by design (#4707): the in-process ticker runs inside a
    profile-scoped gateway, so resolving the active KENXCODE_HOME at call time
    ...
    """
    return _kenxcode_home or get_kenxcode_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    kenxcode_home = _get_kenxcode_home()
    lock_dir = kenxcode_home / "cron"
...
def _open_continuable_cron_thread(
    ...
    thread_name = f"KenXCode — {task_name}"
    try:
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("KENXCODE_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid KENXCODE_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(
    ...
    Scripts must reside within KENXCODE_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other KenXCode-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against KENXCODE_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_kenxcode_home() / "scripts"
    ...
    # escape — scripts MUST reside within KENXCODE_HOME/scripts/.
    try:
...
def run_job(
    ...
    if job.get("no_agent"):
        ...
        now_iso = _kenxcode_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    #
    # Bounded with its own timeout (separate from KENXCODE_CRON_TIMEOUT, which
    ...
    try:
        ...
        _raw_env_timeout = os.getenv("KENXCODE_CRON_SESSION_DB_TIMEOUT", "").strip()
        if _raw_env_timeout:
            try:
                _session_db_timeout = float(_raw_env_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid KENXCODE_CRON_SESSION_DB_TIMEOUT=%r; using config/default",
                    _raw_env_timeout,
                )
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_kenxcode_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    _cron_session_id = f"cron_{job_id}_{_kenxcode_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    # gateway message. Do not seed KENXCODE_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads KENXCODE_SESSION_PLATFORM
    #     and KENXCODE_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via KENXCODE_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     KENXCODE_SESSION_PLATFORM == "telegram".
    ...
    #     send_message gate read KENXCODE_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the KENXCODE_CRON_AUTO_DELIVER_* vars set
    # below, so clearing KENXCODE_SESSION_* here does not affect delivery.
    ...
    _ctx_tokens = set_session_vars(
        ...
        # KENXCODE_SESSION_* routing keys just below, so an async delegation's
        ...
        # os.environ KENXCODE_SESSION_KEY, which risks routing a cron subagent's output
    ...
    _cron_delivery_vars = (
        "KENXCODE_CRON_AUTO_DELIVER_PLATFORM",
        "KENXCODE_CRON_AUTO_DELIVER_CHAT_ID",
        "KENXCODE_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    _cron_session_var = _VAR_MAP["KENXCODE_CRON_SESSION"]
    ...
    try:
        ...
        # load_kenxcode_dotenv (not a bare load_dotenv) and reset the secret-
        # source cache first: startup already applied external secrets and
        # recorded this KENXCODE_HOME in _APPLIED_HOMES, so a naive reload would
        ...
        # forced re-pull off the network. load_kenxcode_dotenv also handles the
        # utf-8/latin-1 encoding fallback internally.
        from kenxcode_cli.env_loader import (
            load_kenxcode_dotenv,
            reset_secret_source_cache,
        )
        reset_secret_source_cache()
        load_kenxcode_dotenv(kenxcode_home=_get_kenxcode_home())

        delivery_target = _resolve_delivery_target(job)
        if delivery_target:
            _VAR_MAP["KENXCODE_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["KENXCODE_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["KENXCODE_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
        ...
        # cron-fleet default) > KENXCODE_MODEL env > config.yaml ``model:``
        ...
        model = job.get("model") or os.getenv("KENXCODE_MODEL") or ""
        ...
        try:
            ...
            _cfg_path = str(_get_kenxcode_home() / "config.yaml")
        ...
        if not (isinstance(model, str) and model.strip()):
            raise RuntimeError(
                ...
                f"KENXCODE_MODEL={os.getenv('KENXCODE_MODEL', '')!r}, "
                "config.yaml model.default missing or empty). "
                f"Set a per-job model via "
                f"`cronjob action=update job_id={job_id} model=<name>` or set a "
                "default with `kenxcode model <name>`."
            )

        # Apply IPv4 preference if configured.
        try:
            from kenxcode_constants import apply_ipv4_preference
        ...
        prefill_file = (
            os.getenv("KENXCODE_PREFILL_MESSAGES_FILE", "")
        ...
        if prefill_file:
            ...
            if not pfpath.is_absolute():
                pfpath = _get_kenxcode_home() / pfpath
            if pfpath.exists():
                try:
        ...
        try:
            # Do not inject KENXCODE_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        agent = AIAgent(
            ...
            # KENXCODE_HOME. When a workdir is configured, also inject project
        ...
        # override via KENXCODE_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("KENXCODE_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid KENXCODE_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
**Job ID:** {job_id}
**Run Time:** {_kenxcode_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        if _session_db:
            ...
            try:
                ...
                _cron_title = f"{_title_base} · {_kenxcode_now().strftime('%b %d %H:%M')}"
...
def run_one_job(job: dict, *, adapters=None, loop=None, verbose: bool = False) -> bool:
    ...
    try:
        ...
        _scope_token = set_secret_scope(
            build_profile_secret_scope(_get_kenxcode_home())
        )
    ...
    *,
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _kenxcode_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _kenxcode_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set KENXCODE_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("KENXCODE_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid KENXCODE_CRON_MAX_PARALLEL value; defaulting to unbounded")
