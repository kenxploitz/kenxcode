"""
LINE Messaging API platform adapter for KenXCode Agent.
...
to KenXCode Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``KENXCODE_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            kenxcode_home = Path(get_kenxcode_home()).resolve()
        except Exception:
            kenxcode_home = Path.home().joinpath(".kenxcode").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``kenxcode status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``kenxcode status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``kenxcode setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.kenxcode/.env`` via ``kenxcode_cli.config``.
    """
    print()
    ...
    try:
        from kenxcode_cli.config import get_env_var, set_env_var
    except ImportError:
        print("kenxcode_cli.config not available; set LINE_* vars manually in ~/.kenxcode/.env")
        return
