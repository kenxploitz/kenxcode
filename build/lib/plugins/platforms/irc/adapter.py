"""
IRC Platform Adapter for KenXCode Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the KenXCode agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "kenxcode-bot")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :KenXCode Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :KenXCode Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: kenxcode_, kenxcode_1, kenxcode_2...
...
def check_requirements() -> bool:
    ...
    # kenxcode setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `kenxcode gateway setup` flow for the IRC platform.

    Lazy-imports ``kenxcode_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect KenXCode to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #kenxcode — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.kenxcode/.env")
    print_info("Restart the gateway for changes to take effect: kenxcode gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "kenxcode-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "kenxcode-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :KenXCode Agent (cron)")
