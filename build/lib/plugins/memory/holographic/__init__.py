"""kenxcode-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $KENXCODE_HOME/config.yaml (profile-scoped):
  plugins:
    kenxcode-memory-store:
      db_path: $KENXCODE_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "kenxcode-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, kenxcode_home):
        """Write config to config.yaml under plugins.kenxcode-memory-store."""
        from pathlib import Path
        config_path = Path(kenxcode_home) / "config.yaml"
        try:
            import yaml
            ...
            existing["plugins"]["kenxcode-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_kenxcode_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _kenxcode_home = str(get_kenxcode_home())
        _default_db = _kenxcode_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $KENXCODE_HOME in user-supplied paths so config values like
        # "$KENXCODE_HOME/memory_store.db" or "~/.kenxcode/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$KENXCODE_HOME", _kenxcode_home)
            db_path = db_path.replace("${KENXCODE_HOME}", _kenxcode_home)
