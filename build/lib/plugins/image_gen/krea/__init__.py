...
# KenXCode uses 3 abstract aspect ratios. Map to Krea's enum (which is wider).
        ...
        **kwargs: Any,
        ...
        if managed is not None:
            ...
            if not auth_token:
                return error_response(
                    error=(
                        "KREA_API_KEY not set. Run `kenxcode tools` → Image "
        ...
        headers = {
            ...
            "User-Agent": "KenXCode-Agent/1.0 (krea-image-gen)",
        }
        if managed is not None:
            ...
            if managed is not None and 400 <= status < 500:
                hint = (
                    ...
                    if status == 429
                        ...
                        "`kenxcode tools` → Image Generation."
                    )
                )
                return error_response(
                    error=(
