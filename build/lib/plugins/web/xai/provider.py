...
``{title, url, description, position}`` rows every other KenXCode web
provider produces.
...
prefers KenXCode-managed xAI Grok OAuth (via ``kenxcode auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.kenxcode/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``kenxcode tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `kenxcode auth` to sign in with "
