...
_KENXCODE_CORE_TOOLS = [
    ...
    # pane, and react to a message with an emoji (all gated on KENXCODE_DESKTOP
    ...
    # spawned as a kanban worker (KENXCODE_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `kenxcode tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "video_gen": {
        "description": (
            ...
            "``kenxcode tools`` → Video Generation."
        ),
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (KENXCODE_KANBAN_TASK env "
    ...
    # Coding posture (base KenXCode — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in kenxcode_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full KenXCode toolsets (CLI + messaging platforms)
    #
    ...
    # the `kenxcode send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "kenxcode-cli": {
        ...
        "tools": _KENXCODE_CORE_TOOLS,
        "includes": []
    },

    "kenxcode-cron": {
        # Mirrors kenxcode-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `kenxcode tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as kenxcode-cli; gated by `kenxcode tools`",
        "tools": _KENXCODE_CORE_TOOLS,
        "includes": []
    },
    ...
    "kenxcode-bluebubbles": {
        "description": "BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server",
        "tools": _KENXCODE_CORE_TOOLS,
        "includes": []
    },

    "kenxcode-homeassistant": {
    ...
    "kenxcode-email": {
        "description": "Email bot toolset - interact with KenXCode via email (IMAP/SMTP)",
        "tools": _KENXCODE_CORE_TOOLS,
        "includes": []
    },
    ...
    "kenxcode-sms": {
        "description": "SMS bot toolset - interact with KenXCode via SMS (Twilio)",
        "tools": _KENXCODE_CORE_TOOLS,
        "includes": []
    },
    ...
    "kenxcode-webhook": {
        ...
        "tools": _KENXCODE_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "kenxcode-gateway": {
        ...
        "tools": [],
        "includes": ["kenxcode-telegram", "kenxcode-discord", "kenxcode-whatsapp", "kenxcode-slack", "kenxcode-signal", "kenxcode-bluebubbles", "kenxcode-homeassistant", "kenxcode-email", "kenxcode-sms", "kenxcode-mattermost", "kenxcode-matrix", "kenxcode-dingtalk", "kenxcode-feishu", "kenxcode-wecom", "kenxcode-wecom-callback", "kenxcode-weixin", "kenxcode-qqbot", "kenxcode-webhook", "kenxcode-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``kenxcode-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_KENXCODE_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``kenxcode-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (kenxcode-<name>).
        # Gives them _KENXCODE_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("kenxcode-"):
            platform_name = name[len("kenxcode-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_KENXCODE_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
