...
def _(rid, params: dict) -> dict:
    from kenxcode_cli.input_sanitize import sanitize_user_prompt_text
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from kenxcode_cli.clipboard import has_clipboard_image, save_clipboard_image
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not KenXCode Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the KenXCode/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not KenXCode Desktop. If it serves KenXCode/Desktop UI or another unrelated app, stop that process and report failure.",
