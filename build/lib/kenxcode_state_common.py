...
Extracted verbatim from kenxcode_state.py so the SessionDB mixin modules
(kenxcode_state_search / kenxcode_state_schema / kenxcode_state_portability) can
reference them without importing kenxcode_state (which would be a cycle).
kenxcode_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``kenxcode sessions optimize-storage``. A legacy DB sits at
...
# `kenxcode sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `kenxcode db optimize`. This is
