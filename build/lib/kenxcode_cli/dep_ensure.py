...
from kenxcode_constants import agent_browser_runnable, find_node_executable
from tools.environments.local import kenxcode_subprocess_env

_IS_WINDOWS = platform.system() == "Windows"

_DEP_CHECKS = {
    # find_node_executable() rather than a bare which(): $KENXCODE_HOME/node is
    ...
    "browser": lambda: (
        ...
        or _has_kenxcode_agent_browser()
    ),
...
def _has_kenxcode_agent_browser() -> bool:
    ...
    # install.sh installs globally into $KENXCODE_HOME/node/bin/ via npm -g --prefix
...
def ensure_dependency(
    ...
    if shell == "powershell":
        ...
        cmd = [
            ...
            "-File", str(script),
            "-Ensure", dep,
            "-KenXCodeHome", str(get_kenxcode_home()),
        ]
    else:
        cmd = ["bash", str(script), "--ensure", dep]

    run_env = kenxcode_subprocess_env(inherit_credentials=False)
