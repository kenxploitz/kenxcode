"""
Single source of truth for provider identity in KenXCode Agent.
...
2. **KenXCode overlays** — transport type, auth patterns, aggregator flags,
...
# -- KenXCode overlay ----------------------------------------------------------
# KenXCode-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class KenXCodeOverlay:
    """KenXCode-specific provider metadata layered on top of models.dev."""
...
KENXCODE_OVERLAYS: Dict[str, KenXCodeOverlay] = {
    "moa": KenXCodeOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": KenXCodeOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENROUTER_BASE_URL",
    ),
    "nous": KenXCodeOverlay(
        transport="openai_chat",
        auth_type="oauth_device_code",
        base_url_override="https://inference-api.nousresearch.com/v1",
    ),
    "openai-codex": KenXCodeOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": KenXCodeOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": KenXCodeOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": KenXCodeOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="KENXCODE_QWEN_BASE_URL",
    ),
    "lmstudio": KenXCodeOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": KenXCodeOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": KenXCodeOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": KenXCodeOverlay(
    ...
    "kimi-for-coding": KenXCodeOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": KenXCodeOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": KenXCodeOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": KenXCodeOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": KenXCodeOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": KenXCodeOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": KenXCodeOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": KenXCodeOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "vercel": KenXCodeOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ),
    "opencode": KenXCodeOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": KenXCodeOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_GO_BASE_URL",
    ),
    "kilo": KenXCodeOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="KILOCODE_BASE_URL",
    ),
    "huggingface": KenXCodeOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="HF_BASE_URL",
    ),
    "novita": KenXCodeOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nvidia": KenXCodeOverlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": KenXCodeOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": KenXCodeOverlay(
        transport="openai_chat",
        base_url_env_var="TOKENHUB_BASE_URL",
    ),
    "arcee": KenXCodeOverlay(
        transport="openai_chat",
    ...
    "azure-foundry": KenXCodeOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": KenXCodeOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "kenxcode", "user-config"
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. KenXCode overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + KenXCode overlay
    ...
    overlay = KENXCODE_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + kenxcode extra
    ...
    if overlay is not None:
        # KenXCode-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    *model* is optional but required for dual-wire providers (Nous) whose
    ...
    # chat_completions. The KenXCode overlay still advertises openai_chat
    ...
    # Direct provider checks for providers not in KENXCODE_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
...
def resolve_provider_full(
    ...
    # 0.5 Exact KenXCode provider IDs must win over LOSSY alias collapsing.
    ...
    if canonical != raw:
        try:
            from kenxcode_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
