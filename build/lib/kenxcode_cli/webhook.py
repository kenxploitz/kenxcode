"""kenxcode webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.kenxcode/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _kenxcode_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: kenxcode gateway run
"""
...
def webhook_command(args):
    """Entry point for 'kenxcode webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: kenxcode webhook {subscribe|list|remove|test}")
        print("Run 'kenxcode webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (kenxcode gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: kenxcode webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from kenxcode webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (kenxcode gateway run)")
