"""Bounded product contract for the first KenXCode shared-metrics slice."""
...
SCHEMA_KEY = "kenxcode.metrics.schema_version"
SCHEMA_VERSION = "kenxcode.metrics.event.v1"
MODEL_CALL_SCOPE = "kenxcode.model_call"
TASK_SCOPE = "kenxcode.task_run"
SUBSCRIBER_NAME = "kenxcode.nemo_relay.shared_metrics"
PRIMARY_MODEL_CALL_ROLE = "primary"
MODEL_CALL_METRIC = "kenxcode.model_call.count"
TASK_STARTED_METRIC = "kenxcode.task_run.started"
TASK_FINISHED_METRIC = "kenxcode.task_run.finished"
...
# in KenXCode's execution metadata because that flag has narrower routing/catalog
# semantics there.
...
# KenXCode intentionally resolves these local runtimes through the generic custom
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from kenxcode_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map KenXCode terminal state to bounded task outcome dimensions."""
...
def provider_family(kwargs: dict[str, Any]) -> str:
    """Map a KenXCode provider to a bounded product category."""
...
def _provider_metadata(provider: str) -> tuple[str, bool, bool]:
    ...
    try:
        ...
        from kenxcode_cli.providers import KENXCODE_OVERLAYS, normalize_provider
...
def _known_provider_ids() -> frozenset[str]:
    """Cache KenXCode's static provider catalog for the process lifetime."""
    try:
        from kenxcode_cli.provider_catalog import provider_catalog_by_slug
