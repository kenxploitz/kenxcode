"""Relay subscriber for the persisted KenXCode shared-metrics slice."""
...
class SharedMetricsSubscriber:
    """Persist validated KenXCode counters from Relay lifecycle events."""
    ...
    def __init__(
        ...
        *,
        ...
        self.store = store
        self._kenxcode_version = kenxcode_version or "unknown"
        ...
        with self._lock:
            ...
            try:
                self.store.record_counter(
                    metric_name,
                    dimensions,
                    self._kenxcode_version,
                )
            except Exception:
                logger.warning(
                    "Unable to persist the KenXCode shared metric: %s",
                    metric_name,
                    exc_info=True,
                )
