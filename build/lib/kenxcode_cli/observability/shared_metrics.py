"""Durable aggregation and local export for KenXCode shared metrics."""
...
from kenxcode_cli.sqlite_util import write_txn
...
_PACKAGE_SCHEMA_VERSION = "kenxcode.shared_metrics.v1"
...
class SharedMetricsStore:
    ...
    def __init__(
        ...
        root = get_kenxcode_home() / "telemetry" / "shared_metrics"
    ...
    def record_model_call(
        ...
        kenxcode_version: str,
    ) -> None:
        """Increment the terminal model-call counter for the current UTC day."""
        self.record_counter(MODEL_CALL_METRIC, dimensions, kenxcode_version)
    ...
    def counter_snapshot(self) -> list[dict[str, Any]]:
        ...
        with self._connection() as connection:
            rows = connection.execute(
                ...
                ORDER BY period_start, kenxcode_version, metric_name, dimensions_json
                """
            ).fetchall()
        return [
            {
                ...
                "kenxcode_version": row["kenxcode_version"],
    ...
    def _ensure_schema(self) -> None:
        with self._connection(busy_timeout_ms=_SCHEMA_BUSY_TIMEOUT_MS) as connection:
            # Serialize first-run creation and upgrades across KenXCode processes.
    ...
    def _ensure_schema_in_transaction(connection: sqlite3.Connection) -> None:
        ...
        connection.execute(
            ...
            CREATE TABLE IF NOT EXISTS counter_aggregates (
                ...
                kenxcode_version TEXT NOT NULL,
    ...
    def _pending_period_count(self) -> int:
        with self._connection() as connection:
            row = connection.execute(
                ...
                FROM (
                    SELECT period_start, kenxcode_version
                    FROM counter_aggregates
                    WHERE value > packaged_value
                    GROUP BY period_start, kenxcode_version
                )
                """
            ).fetchone()
    ...
    def _create_package_in_transaction(
        ...
        period_row = connection.execute(
            """
                ...
                FROM counter_aggregates
                WHERE value > packaged_value
                ORDER BY period_start, kenxcode_version
                LIMIT 1
                """
        ).fetchone()
        ...
        rows = connection.execute(
            """
                ...
                FROM counter_aggregates
                WHERE period_start = ?
                  AND kenxcode_version = ?
                  AND value > packaged_value
                ORDER BY metric_name, dimensions_json
                """,
            (period_value, period_row["kenxcode_version"]),
        ).fetchall()
        ...
        payload = {
            ...
            "resource": {"kenxcode_version": period_row["kenxcode_version"]},
