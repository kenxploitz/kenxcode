...
entries via ``kenxcode mcp catalog`` or the interactive ``kenxcode mcp picker``,
and install them with ``kenxcode mcp install <name>`` (or by toggling in the
picker, which flows them through any required env/OAuth setup).

Catalog policy:
- Entries are added only by merging a PR into kenxcode-agent. Presence in the
...
- Manifests pin transport details (commands, args, refs). Pins follow the
  ...
  auto-updated; users explicitly re-run ``kenxcode mcp install <name>`` to
  pull a new manifest version after a repo update.
- Secrets prompted at install time go to ``~/.kenxcode/.env`` (the
...
from kenxcode_constants import get_kenxcode_home, get_optional_mcps_dir
...
from kenxcode_cli.cli_output import prompt as _prompt_input
...
class TransportSpec:
    ...
    # auth.env so they are prompted for and land in ~/.kenxcode/.env.
...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this KenXCode install."""
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this KenXCode understands version {_MANIFEST_VERSION})"
        )
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their KenXCode is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this KenXCode
        understands. Update KenXCode to install this entry.
...
def _install_root() -> Path:
    ...
    root = get_kenxcode_home() / "mcp-installs"
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.kenxcode/mcp-installs/<name>`` and run
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.kenxcode/.env via save_env_value()."""
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    try:
        ...
        from kenxcode_cli.mcp_config import _probe_single_server
...
def _apply_tool_selection(
    ...
    Probe-fail path:
      ...
      - Either way, point the user at ``kenxcode mcp configure <name>``.
    """
    print()
    ...
    if probed is None:
        ...
        if manifest_default:
            ...
            print(color(
                ...
                f"Run `kenxcode mcp configure {entry.name}` after the server "
            ...
            print(color(
                ...
                f"Run `kenxcode mcp configure {entry.name}` after first "
                "connect to prune.",
                Colors.YELLOW,
            ))
        return
    ...
    if not chosen_indices:
        ...
        print(color(
            f"  No tools selected. Run `kenxcode mcp configure {entry.name}` "
            "to change.",
            Colors.YELLOW,
        ))
        return
    ...
    if len(chosen_indices) == len(probed):
        ...
        # the user can re-run `kenxcode mcp configure <name>` and unselect a
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            # the existing `kenxcode auth <provider>` flow. Surface guidance
            ...
            print(color(
                ...
                f"`kenxcode auth {entry.auth.provider}` if you have not "
    ...
    from kenxcode_cli.mcp_config import _save_mcp_server
    ...
    print(color(
        ...
        f"Start a new KenXCode session to load its tools.",
        Colors.GREEN,
    ))
