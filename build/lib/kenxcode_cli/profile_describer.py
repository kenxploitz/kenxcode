...
Used by ``kenxcode profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``kenxcode_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the KenXCode Agent kanban board.
...
Rules:
  ...
  - Never write "KenXCode Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_kenxcode_home())
        else:
