"""Default configuration data for KenXCode Agent.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from kenxcode_cli/config.py. Must not import from kenxcode_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["kenxcode-cli"],
    # SQLite journal mode used by every KenXCode database opener. WAL is the
    ...
    "agent": {
        ...
        # the KenXCode-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps KenXCode
        ...
        # identity slot (SOUL.md). Empty by default. The KENXCODE_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, KenXCode adds a coding operating brief
        ...
        # ``KENXCODE_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        #             KENXCODE_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force KENXCODE_HOME/home when it exists (old strict
        ...
        # skipped. When empty, KenXCode auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), KenXCode sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when KenXCode
        ...
        # bundled KenXCode image, which drops to the `kenxcode` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, KenXCode sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and KenXCode should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``kenxcode chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.kenxcode/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: kenxcode sweeps the checkpoint base at startup
        ...
        # ``kenxcode checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .kenxcode.md, or .cursorrules before KenXCode applies
    ...
    # Single-query (``kenxcode -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, KenXCode truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so KenXCode' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); kenxcode =
                                      # KenXCode' compression threshold triggers
    ...
    "auxiliary": {
        ...
        # Invoked by ``kenxcode kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``kenxcode kanban decompose`` and the kanban
        ...
        # ``kenxcode profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `kenxcode model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `kenxcode` (and `kenxcode chat`) launches by default:
        ...
        # REPL and `--tui` (or KENXCODE_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `kenxcode --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `kenxcode -c` muscle memory.  Default off so existing
        # users aren't surprised.  KENXCODE_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `kenxcode --tui` drops a one-time hint
        ...
        # model — see kenxcode_cli/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to KENXCODE_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/kenxcode`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `kenxcode pets`. Disabled until
        ...
        "pet": {
            ...
            # get_kenxcode_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``KENXCODE_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``KENXCODE_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``KENXCODE_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``KENXCODE_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``KENXCODE_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/kenxcode``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; KenXCode appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.kenxcode/cache/piper-voices/
    ...
    # "Hey KenXCode" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey kenxcode",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_kenxcode" (the bundled, works-out-of-the-box default) OR a
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.kenxcode/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.kenxcode/cache/delegation/
    ...
    # response. If not, KenXCode feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before KenXCode auto-pauses the goal and
    ...
    "moa": {
        ...
        # <kenxcode_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.kenxcode/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${KENXCODE_SKILL_DIR} and ${KENXCODE_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `kenxcode curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `kenxcode curator run --consolidate` overrides
        ...
        # normally restored on every `kenxcode update`, so pruning them only
        ...
        # skipped), snapshot ~/.kenxcode/skills/ into
        # ~/.kenxcode/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `kenxcode curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for kenxcode-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚡ *KenXCode Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (KENXCODE_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace KenXCode' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.kenxcode/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or KENXCODE_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `kenxcode doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``kenxcode_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow KenXCode to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $KENXCODE_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via KENXCODE_CRON_MAX_PARALLEL env var.
        ...
        # KENXCODE_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
    ...
    # todos to ready, and fires `kenxcode -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `kenxcode` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `kenxcode kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     kenxcode-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core KenXCode tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.kenxcode/logs/.
    ...
    # update model picker lists without shipping a kenxcode-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://kenxcode-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `kenxcode model` invocation; network failures
    ...
    "gateway": {
        ...
        # internal KENXCODE_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.kenxcode/sessions/sessions.json entirely.
        ...
        # the KENXCODE_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.kenxcode.gateway` that defenses 1-2 don't
        ...
        # ``KENXCODE_GATEWAY_MAX_STARTS`` / ``KENXCODE_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.kenxcode/.env,
        ...
        # behavior: files must live under the KenXCode cache, under
        ...
        # user. Bridged to KENXCODE_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the KenXCode
        # cache (~/.kenxcode/cache/{documents,images,audio,video,screenshots})
        ...
        # to KENXCODE_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # KENXCODE_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to KENXCODE_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.kenxcode/state.db.
    ...
    "sessions": {
        ...
        # the default of ``kenxcode sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.kenxcode/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `kenxcode sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `kenxcode update` prints a one-line notice with
        ...
        # ~/.kenxcode/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to KENXCODE_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # KENXCODE_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
    },
    ...
    "updates": {
        ...
        #     DBs) into <KENXCODE_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``kenxcode backup``-style zip
        #     of KENXCODE_HOME into <KENXCODE_HOME>/backups/, restorable with
        #     ``kenxcode import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `kenxcode update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<KENXCODE_HOME>/lsp/bin/`` on first use.
    ...
    # `kenxcode tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.kenxcode/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.kenxcode/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.kenxcode/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, KenXCode may use this encrypted
            ...
            # ~/.kenxcode/bin/ on first use.  When False you must install
            ...
            # `kenxcode secrets bitwarden setup`.
            "server_url": "",
        },
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.kenxcode/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. KenXCode disables it for our users unless they
        # explicitly opt in here. When false (default), KenXCode sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; KenXCode also calls
    ...
    #
    # Configure with `kenxcode egress setup`.  Disabled by default — the rest of
    # KenXCode works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.kenxcode/bin/ on
    ...
    # KenXCode Desktop (Electron app) launch options. These only affect
    # `kenxcode desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the KENXCODE_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials KenXCode mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'kenxcode model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the KenXCode auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey KenXCode' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use KenXCode, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @kenxcode:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. KENXCODE_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #kenxcode)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: kenxcode-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'kenxcode-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote KenXCode API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote KenXCode API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote KenXCode API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # KENXCODE_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to KENXCODE_TOOL_PROGRESS_MODE for backward
    ...
    # KENXCODE_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor retired its only consumer (the v3→4 migration).
    "KENXCODE_PREFILL_MESSAGES_FILE": {
    ...
    "KENXCODE_EPHEMERAL_SYSTEM_PROMPT": {
