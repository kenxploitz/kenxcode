"""``kenxcode sync`` subcommand parser — Skill Sync.

Cloned from ``kenxcode_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    kenxcode sync status                 show gate/opt-in/head state
    kenxcode sync pull                   pull and materialize opted-in skills
    kenxcode sync push                   push opted-in skills
    kenxcode sync now                    reconcile: pull then push
    kenxcode sync enable <skill>         opt a skill into sync
    kenxcode sync disable <skill>        opt a skill out of sync
    kenxcode sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    kenxcode sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  kenxcode sync status            what is synced, and from where\n"
            "  kenxcode sync enable my-skill   include a skill in your sync\n"
            "  kenxcode sync now               pull, then push\n"
            "  kenxcode sync propose my-skill  share a skill with your team\n"
        ),
