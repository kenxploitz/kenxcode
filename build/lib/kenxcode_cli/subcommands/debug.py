"""``kenxcode debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for KenXCode Agent. Use 'kenxcode debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    kenxcode debug share              Upload debug report (asks for confirmation)
    kenxcode debug share --yes        Skip confirmation (for scripts/CI)
    kenxcode debug share --lines 500  Include more log lines
    kenxcode debug share --expire 30  Keep paste for 30 days
    kenxcode debug share --local      Print report locally (no upload)
    kenxcode debug share --no-redact  Disable upload-time secret redaction
    kenxcode debug share --nous       Upload to Nous-internal storage (private)
    kenxcode debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'kenxcode debug share'",
    )
