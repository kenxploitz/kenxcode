"""``kenxcode dashboard`` / ``kenxcode serve`` subcommand parsers.
...
``kenxcode_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `kenxcode dashboard` / `kenxcode serve` cmdlines and SIGTERM them directly —
    # the same path `kenxcode update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running KenXCode web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running KenXCode web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the KenXCode Agent web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older KenXCode desktop app shells (<= 0.15.x) spawn the
    # backend as `kenxcode dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "KenXCode couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the KenXCode Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the KenXCode backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the KenXCode backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # (cmd_dashboard exports KENXCODE_SERVE_HEADLESS=1). `dashboard` leaves it
    # unset and serves the browser UI as before.
    serve_parser.set_defaults(func=cmd_dashboard, no_open=True, headless_backend=True)

    # `kenxcode dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.kenxcode/.env.
    # Nested subparser so bare `kenxcode dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "KENXCODE_DASHBOARD_OAUTH_CLIENT_ID into ~/.kenxcode/.env, and prints "
            "how to engage the login gate. Requires being logged in (kenxcode setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://kenxcode.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via KENXCODE_DASHBOARD_PORTAL_URL. Mainly for "
