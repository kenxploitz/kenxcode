...
``CommandDef.execute`` (kenxcode_cli/commands.py) names a key in
...
for a fixed context (enforced by tests/kenxcode_cli/test_commands_execute.py).

Import discipline: this module imports nothing heavy at module level and
``kenxcode_cli.commands`` does NOT import this module (the ``execute`` field is
...
def _exec_version(ctx: CommandContext) -> CommandReply:
    ...
    from kenxcode_cli.banner import format_banner_version_label
...
def _exec_profile(ctx: CommandContext) -> CommandReply:
    ...
    if not home_display:
        ...
        home_display = display_kenxcode_home()
...
def _exec_bundles(ctx: CommandContext) -> CommandReply:
    ...
    if not bundles:
        return CommandReply(
            ...
            "Create one with: kenxcode bundles create <name> --skill <s1> --skill <s2>\n"
...
def _exec_help(ctx: CommandContext) -> CommandReply:
    ...
    from kenxcode_cli.commands import gateway_help_lines
