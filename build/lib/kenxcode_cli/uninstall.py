"""
KenXCode Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.kenxcode/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove KenXCode PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing kenxcode-agent or kenxcode PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# KenXCode Agent" comment and following line
                if '# KenXCode Agent' in line or '# kenxcode-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('kenxcode' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing kenxcode
                if 'kenxcode' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the kenxcode wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "kenxcode",
        Path.home() / ".local" / "bin" / "kenxcode-acp",
        Path.home() / ".local" / "bin" / "kenxcode-agent",
        Path("/usr/local/bin/kenxcode"),
        Path("/usr/local/bin/kenxcode-acp"),
        Path("/usr/local/bin/kenxcode-agent"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains kenxcode_cli reference)
                content = wrapper.read_text(encoding="utf-8")
                if 'kenxcode_cli' in content or 'kenxcode-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(kenxcode_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``kenxcode`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this KenXCode home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (kenxcode_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``kenxcode gateway run`` processes
    ...
    try:
        from kenxcode_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from kenxcode_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``KENXCODE_HOME`` and ``KENXCODE_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\kenxcode\git\cmd``, ``%LOCALAPPDATA%\kenxcode\git\bin``,
#      ``%LOCALAPPDATA%\kenxcode\git\usr\bin``, ``%LOCALAPPDATA%\kenxcode\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\kenxcode\git\`` and Node to
#      ``%LOCALAPPDATA%\kenxcode\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``kenxcode dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\kenxcode\gateway-service\`` and sometimes
...
def _kenxcode_path_markers(kenxcode_home: Path) -> list[str]:
    """Path-entry substrings that identify KenXCode-owned User-PATH entries."""
    root = str(kenxcode_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare kenxcode-agent install dir.
    markers = [root + "\\kenxcode-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if KENXCODE_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "kenxcode".  We don't want to catch
    # unrelated entries like "ckenxcode-foo" or "ephermeral", so we look for
    # backslash-kenxcode as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(kenxcode_home: Path) -> list[str]:
    """Strip KenXCode-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _kenxcode_path_markers(kenxcode_home)
            kept: list[str] = []
...
def remove_kenxcode_env_vars_windows() -> list[str]:
    """Delete KENXCODE_HOME and KENXCODE_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("KENXCODE_HOME", "KENXCODE_GIT_BASH_PATH"):
                try:
...
def _is_default_kenxcode_home(kenxcode_home: Path) -> bool:
    """Return True when ``kenxcode_home`` points at the default (non-profile) root."""
    try:
        from kenxcode_constants import get_default_kenxcode_root
        return kenxcode_home.resolve() == get_default_kenxcode_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its KENXCODE_HOME directory.

    We shell out to ``kenxcode -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current KENXCODE_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m kenxcode_cli.main` so we don't depend on a `kenxcode`
    #    wrapper that may be half-removed mid-uninstall.
    kenxcode_invocation = [_sys.executable, "-m", "kenxcode_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                kenxcode_invocation + ["gateway", subcmd],
                capture_output=True,
    ...
    # 3. Wipe the profile's KENXCODE_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``kenxcode uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$KENXCODE_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from kenxcode_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(kenxcode_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚡ KenXCode Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No KenXCode Chat GUI installation was found.")
        print(f"  Checked: {kenxcode_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The KenXCode agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(kenxcode_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The KenXCode agent at {kenxcode_home / 'kenxcode-agent'}")
        print(f"  • Your config, sessions, and secrets under {kenxcode_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The KenXCode agent is still installed. Run 'kenxcode' to use the CLI,")
    print("or 'kenxcode uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.kenxcode/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.kenxcode/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie KENXCODE_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_kenxcode_home(kenxcode_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.kenxcode data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚡ KenXCode Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {kenxcode_home / 'config.yaml'}")
    print(f"  Secrets: {kenxcode_home / '.env'}")
    print(f"  Data:    {kenxcode_home / 'cron/'}, {kenxcode_home / 'sessions/'}, {kenxcode_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their KENXCODE_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL KenXCode data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the KenXCode code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, kenxcode_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • KenXCode PATH entries from shell configs / Windows User PATH")
    print("  • KenXCode wrapper scripts and KenXCode-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • KenXCode config/data: {kenxcode_home}")
        if _is_default_kenxcode_home(kenxcode_home):
        ...
        print(f"  • Keep KenXCode config/data: {kenxcode_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``kenxcode`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$KENXCODE_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in kenxcode_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\kenxcode\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(kenxcode_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No KenXCode-owned PATH entries in User environment")

        log_info("Removing KENXCODE_HOME / KENXCODE_GIT_BASH_PATH User env vars...")
        removed_env = remove_kenxcode_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No KenXCode-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing kenxcode command...")
    ...
    #     (only when they still point into this KenXCode home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing KenXCode-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(kenxcode_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No KenXCode-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(kenxcode_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE KENXCODE_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from kenxcode_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(kenxcode_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.kenxcode/, just remove the kenxcode-agent subdir
            if kenxcode_home in project_root.parents or project_root.parent == kenxcode_home:
    ...
    #     under KENXCODE_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(kenxcode_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(kenxcode_home)
    ...
    # 5. Optionally remove ~/.kenxcode/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile KENXCODE_HOME dirs live under
        ...
        try:
            if kenxcode_home.exists():
                shutil.rmtree(kenxcode_home)
                log_success(f"Removed {kenxcode_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {kenxcode_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {kenxcode_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {kenxcode_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  iex (irm https://kenxcode-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://kenxcode-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using KenXCode Agent! ⚡")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m kenxcode_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import kenxcode_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``kenxcode_constants`` + ``kenxcode_cli.colors``
    (and lazily ``kenxcode_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m kenxcode_cli.uninstall")
    parser.add_argument(
        "--mode",
