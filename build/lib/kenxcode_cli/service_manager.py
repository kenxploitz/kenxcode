...
the existing module-level functions in kenxcode_cli.gateway and
kenxcode_cli.gateway_windows directly. This protocol is a thin facade
...
``kenxcode gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``kenxcode gateway start/stop/restart``).
    """
    ...
    # `kenxcode gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged kenxcode
    ...
    detection always fails. Since every KenXCode runtime call inside the
    container drops to kenxcode via ``s6-setuidgid``, that silent failure
...
# in ``kenxcode_cli.gateway`` (systemd/launchd) and ``kenxcode_cli.gateway_windows``
...
# active (set via the ``kenxcode -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in kenxcode_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from kenxcode_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from kenxcode_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from kenxcode_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from kenxcode_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in kenxcode_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from kenxcode_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from kenxcode_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from kenxcode_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from kenxcode_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``kenxcode_cli.gateway_windows`` (Scheduled Task /
...
#
# Per-profile gateways are registered dynamically when `kenxcode profile create`
# runs inside the container (Phase 4). Static services (main-kenxcode, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's KENXCODE_HOME blindly;
    derive the shared profile root from the current KENXCODE_HOME and map the
    ...
    kenxcode_home = Path(os.environ.get("KENXCODE_HOME", "/opt/data"))
    if kenxcode_home.parent.name == "profiles":
        root = kenxcode_home.parent.parent
    else:
        root = kenxcode_home
...
# UID/GID of the in-image ``kenxcode`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the kenxcode user.
    ...
    (the ``kenxcode`` user — UID 10000 — running every KenXCode runtime
    ...
    is dispatched as the kenxcode user.
    ...
    with kenxcode ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           kenxcode:kenxcode, 0755 (parent must already exist)
    ``svc_dir/event/``                     kenxcode:kenxcode, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 kenxcode:kenxcode, 0755
    ``svc_dir/supervise/event/``           kenxcode:kenxcode, 03730
    ``svc_dir/supervise/control``          kenxcode:kenxcode, 0660    (FIFO)
    ...
    slot's supervise/ was kenxcode-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _KENXCODE_UID, _KENXCODE_GID)
        except PermissionError:
            # Running as the kenxcode user already — directory is kenxcode-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _KENXCODE_UID, _KENXCODE_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is kenxcode-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _KENXCODE_UID, _KENXCODE_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``kenxcode -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`kenxcode profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-kenxcode, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources KENXCODE_HOME (and any extra env) via with-contenv —
             so e.g. ``-e KENXCODE_HOME=/data/kenxcode`` is honored at run
          ...
          4. Drops to the kenxcode user and exec's
             ``kenxcode -p <profile> gateway run`` (or just ``kenxcode
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``kenxcode gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root KENXCODE_HOME profile" (the implicit profile that exists at
        the top of $KENXCODE_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``kenxcode_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$KENXCODE_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``KENXCODE_HOME``,
        ...
        allocator in ``kenxcode_cli.profiles._allocate_gateway_port``).
        ...
        lines = [
            ...
            "cd /opt/data",
            ". /opt/kenxcode/.venv/bin/activate",
        ]
        ...
        # guard.
        lines.append("export KENXCODE_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's KENXCODE_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``kenxcode gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-KENXCODE_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The KENXCODE_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own KENXCODE_HOME and s6 guarantees a
        # single supervised instance per slot, so there is no legitimate
        # supervised sibling for ``--replace`` to clobber.
        if profile == "default":
            gateway_cmd = "kenxcode gateway run --replace"
        else:
            gateway_cmd = f"kenxcode -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid kenxcode {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${KENXCODE_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the KENXCODE_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e KENXCODE_HOME=/data/kenxcode``
        gets its logs under /data/kenxcode/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``kenxcode logs`` reads and what
        ...
        return (
            ...
            f': "${{KENXCODE_HOME:=/opt/data}}"\n'
            f'log_dir="$KENXCODE_HOME/logs/gateways/{prof}"\n'
            # Create the leaf and clear a stale s6-log lock as kenxcode when
            # this script starts as root. Never chown or unlink kenxcode-writable
            # volume paths from this restartable root-context script:
            # log/supervise/control is kenxcode-owned, so an unprivileged user
            # can race a pathname op through a symlink swap (CWE-59 /
            # CWE-367). Parent logs/gateways is seeded kenxcode-owned at stage2
            # boot (#45258; tests/docker/test_log_dir_seed.py).
            f'if [ "$(id -u)" = 0 ]; then\n'
            f'  s6-setuidgid kenxcode mkdir -p "$log_dir"\n'
            f'  s6-setuidgid kenxcode rm -f "$log_dir/lock"\n'
            f'else\n'
            f'  mkdir -p "$log_dir"\n'
            f'  rm -f "$log_dir/lock"\n'
            f'fi\n'
            ...
            f'exec s6-setuidgid kenxcode s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``kenxcode gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``kenxcode -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with kenxcode ownership
            ...
            # the kenxcode user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is kenxcode-owned (see ``_seed_supervise_skeleton``), and on
