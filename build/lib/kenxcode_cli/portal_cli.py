"""``kenxcode portal`` — the human-readable entry point for Nous Portal.

Running ``kenxcode portal`` with no subcommand performs the one-shot Portal
...
``kenxcode auth add nous --type oauth`` (which still works), is identical to
``kenxcode setup --portal``, and runs the same Nous flow as the first-time quick
setup.
...
already in ``kenxcode auth`` or ``kenxcode tools``. It's the onboarding + discovery
...
DOCS_URL = "https://kenxcode-agent.nousresearch.com/docs/user-guide/features/tool-gateway"
...
def _cmd_status(args) -> int:
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `kenxcode model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `kenxcode portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    ...
    This is the human-readable front door for `kenxcode auth add nous --type
    oauth`. It reuses the exact wiring behind `kenxcode setup --portal` (which in
    ...
    """
    from kenxcode_cli.setup import _run_portal_one_shot
...
def portal_command(args) -> int:
    """Top-level dispatch for `kenxcode portal <subcommand>`."""
    sub = getattr(args, "portal_command", None)
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `kenxcode portal` is the
        # human-readable alias for `kenxcode auth add nous --type oauth` /
    ...
    print("Run `kenxcode portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `kenxcode portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up Nous Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `kenxcode portal` with no subcommand to log in to Nous Portal "
            "and set it up — pick a model, set Nous as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `kenxcode auth add "
            "nous --type oauth`, identical to `kenxcode setup --portal`). "
