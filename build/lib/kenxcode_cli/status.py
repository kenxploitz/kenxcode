"""
Status command for kenxcode CLI.

Shows the status of all KenXCode Agent components.
"""
...
from kenxcode_cli.auth import AuthError, resolve_provider
from kenxcode_cli.colors import Colors, color
from kenxcode_cli.config import get_env_path, get_env_value, get_kenxcode_home, load_config
from kenxcode_cli.models import provider_label
...
from kenxcode_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``kenxcode config``'s
...
def show_status(args):
    """Show status of all KenXCode Agent components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚡ KenXCode Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        # Read-only display: use the refresh-free snapshot so `kenxcode status`
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: kenxcode portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: kenxcode model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: kenxcode auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: kenxcode auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: kenxcode model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from kenxcode_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'kenxcode-agent[vercel]')"
    ...
    try:
        from kenxcode_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   kenxcode gateway")
    ...
    jobs_file = get_kenxcode_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    if _session_count is not None and _session_count > 0:
        ...
        sessions_file = get_kenxcode_home() / "sessions" / "sessions.json"
    ...
    try:
        from kenxcode_cli.active_sessions import (
    ...
    print(color("  Run 'kenxcode doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'kenxcode setup' to configure", Colors.DIM))
    print()
