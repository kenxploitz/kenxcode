"""Dependency-light venv recovery that runs BEFORE kenxcode_cli.main's imports.

The ``kenxcode`` console entry point is ``kenxcode_cli.main:main``.  Importing
``kenxcode_cli.main`` pulls in third-party packages at module level (``dotenv``
via ``kenxcode_cli.env_loader``, ``yaml`` via ``kenxcode_cli.config``, ...).  In
...
a corrupted venv.  ``kenxcode_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``kenxcode_cli.main`` to become
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``kenxcode acp`` speaks JSON-RPC on stdout);
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``kenxcode_cli.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``kenxcode_cli.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls.  Only acts when a
    recovery marker from a prior ``kenxcode update`` exists AND an import probe
