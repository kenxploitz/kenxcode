"""
Profile management for multiple isolated KenXCode instances.

Each profile is a fully independent KENXCODE_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.kenxcode/profiles/<name>/`` by default.

The "default" profile is ``~/.kenxcode`` itself — backward compatible,
zero migration needed.

Usage::

    kenxcode profile create coder          # fresh profile + bundled skills
    kenxcode profile create coder --clone  # also copy config, .env, SOUL.md, skills
    kenxcode profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    kenxcode -p coder chat                 # or via flag
    kenxcode profile use coder             # set as sticky default
    kenxcode profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See kenxcode_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.kenxcode``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   kenxcode-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `kenxcode backup` archives
...
# Marker file written by `kenxcode profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `kenxcode
...
# `kenxcode skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         KenXCode infrastructure directories that only the default profile
         (``~/.kenxcode``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_kenxcode_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.kenxcode) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "kenxcode-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when KENXCODE_HOME equals the
...
# into the export. The set below identifies the *known KenXCode profile
# artifacts* at the root of KENXCODE_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``kenxcode_constants``.
...
_RESERVED_NAMES = frozenset({
    "kenxcode", "default", "test", "tmp", "root", "sudo",
})

# KenXCode subcommands that cannot be used as profile names/aliases
_KENXCODE_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the kenxcode root, NOT to the current KENXCODE_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where KENXCODE_HOME points outside
    ``~/.kenxcode``, profiles live under ``KENXCODE_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_kenxcode_home() / "profiles"


def _get_default_kenxcode_home() -> Path:
    """Return the default (pre-profile) KENXCODE_HOME path.

    In standard deployments this is ``~/.kenxcode``.
    In Docker/custom deployments where KENXCODE_HOME is outside ``~/.kenxcode``
    (e.g. ``/opt/data``), returns KENXCODE_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_kenxcode_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the KenXCode installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its KENXCODE_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_kenxcode_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, kenxcode subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _KENXCODE_SUBCOMMANDS:
        return f"'{canon}' conflicts with a kenxcode subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            kenxcode_exe = shutil.which("kenxcode") or "kenxcode"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(kenxcode_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running KenXCode. If we
    ...
    try:
        ...
        # not be migrated. The next `kenxcode doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``kenxcode -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``kenxcode profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``kenxcode -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``kenxcode profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``KENXCODE_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing KenXCode config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_kenxcode_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, kenxcode_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own KENXCODE_HOME.
    ...
    The returned ``kenxcode_home`` is the path to pass to
    ``set_kenxcode_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_kenxcode_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``kenxcode update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.kenxcode)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.kenxcode/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `kenxcode -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this KenXCode profile.\n"
    ...
    if not soul_path.exists():
        try:
            from kenxcode_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `kenxcode update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`kenxcode profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running KenXCode (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `kenxcode profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `kenxcode -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches KENXCODE_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``kenxcode profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "KENXCODE_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_kenxcode_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running KenXCode *backends* bound to this profile.
    ...
    names it, so find it by inspection: a KenXCode backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``KENXCODE_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `kenxcode -p <canon> profile
    ...
    kenxcode_markers = ("kenxcode_cli.main", "kenxcode-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a KenXCode process: either an entrypoint marker in argv, or
            # a resolved executable named `kenxcode`.
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_kenxcode = (
                any(marker in joined for marker in kenxcode_markers)
                or exe_name == "kenxcode"
                or exe_name.startswith("kenxcode")
            )
            if not is_kenxcode:
                continue
            ...
            # ...or by KENXCODE_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("KENXCODE_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.kenxcode).\n"
            "To remove everything, use: kenxcode uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_kenxcode_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``KENXCODE_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from kenxcode_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set KENXCODE_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("KENXCODE_HOME")
    try:
        os.environ["KENXCODE_HOME"] = str(profile_dir)
        from kenxcode_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["KENXCODE_HOME"] = old_home
        elif "KENXCODE_HOME" in os.environ:
            del os.environ["KENXCODE_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.kenxcode/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: kenxcode profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from KENXCODE_HOME.

    Returns ``"default"`` if KENXCODE_HOME is not set or points to ``~/.kenxcode``.
    Returns the profile name if KENXCODE_HOME points into ``~/.kenxcode/profiles/<name>``.
    Returns ``"custom"`` if KENXCODE_HOME is set to an unrecognized path.
    """
    from kenxcode_constants import get_kenxcode_home
    kenxcode_home = get_kenxcode_home()
    resolved = kenxcode_home.resolve()

    default_resolved = _get_default_kenxcode_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      KENXCODE_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-KenXCode file the
      user may have lying alongside KENXCODE_HOME (#58394).
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.kenxcode itself — its parent is ~/ and its
        # directory name is ".kenxcode", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: kenxcode profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.kenxcode itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.kenxcode). "
            "Specify a different name: kenxcode profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="kenxcode_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"kenxcode_{old_name}"
    legacy_old_host = f"kenxcode.{old_name}"
    new_host = f"kenxcode_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_kenxcode_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("kenxcode_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a KENXCODE_HOME path string.

    Called early in the CLI entry point, before any kenxcode modules
    are imported, to set the KENXCODE_HOME environment variable.
    """
