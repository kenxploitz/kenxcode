"""
Dump command for kenxcode CLI.

Outputs a compact, plain-text summary of the user's KenXCode setup
...
from kenxcode_cli.config import get_kenxcode_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.kenxcode/.env.
    ...
    exports. ``kenxcode debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.kenxcode_build_sha`` by the Dockerfile's
    ``KENXCODE_GIT_SHA`` build-arg (see ``kenxcode_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``kenxcode dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from kenxcode_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(kenxcode_home: Path) -> int:
...
def _cron_summary(kenxcode_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = kenxcode_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"kenxcode_home:      {display_kenxcode_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.kenxcode/.env: a managed
        ...
        # A credential added via `kenxcode auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `kenxcode auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["kenxcode-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(kenxcode_home)}")
    lines.append(f"  skills:             {_count_skills(kenxcode_home)}")
