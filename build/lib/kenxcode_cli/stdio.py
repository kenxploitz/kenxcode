...
on Japanese locales, etc.).  KenXCode's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``kenxcode_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. KenXCode is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``KENXCODE_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("KENXCODE_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the KenXCode-managed Git install directories so
    ...
    # already-running shells don't see the change — which means kenxcode
    ...
    # install without KenXCode-managed PortableGit).
    ...
    # stdin is re-configured for completeness; KenXCode's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and KenXCode's
    ``kenxcode config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching KenXCode (User env var in Windows Settings, or
    export in a PowerShell profile) and KenXCode picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known KenXCode-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\kenxcode\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``kenxcode`` from right after install) retain their old PATH.

    Any subprocess KenXCode spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known KenXCode-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "kenxcode", "git", "cmd"),
        os.path.join(local_appdata, "kenxcode", "git", "bin"),
        os.path.join(local_appdata, "kenxcode", "git", "usr", "bin"),
        # KenXCode venv Scripts directory — host of the kenxcode.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes kenxcode, but harmless to include.
        os.path.join(local_appdata, "kenxcode", "kenxcode-agent", "venv", "Scripts"),
