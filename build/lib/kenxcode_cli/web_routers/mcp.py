...
web_server - reached via the late-binding seam in :mod:`kenxcode_cli.web_deps`
...
from kenxcode_cli.web_deps import late, LateState
from kenxcode_cli.web_models import (
    MCPCatalogInstall,
    MCPEnabledToggle,
    MCPServerCreate,
    MCPServersReplace,
...
_log = logging.getLogger("kenxcode_cli.web_server")
...
_spawn_kenxcode_action = late("_spawn_kenxcode_action")
...
async def replace_mcp_servers(body: MCPServersReplace, profile: Optional[str] = None):
    ...
    """
    from kenxcode_cli.mcp_config import _replace_mcp_servers
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from kenxcode_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    def _probe_scoped():
        ...
        # The probe touches no skills globals; it only needs the KENXCODE_HOME
...
async def auth_mcp_server(name: str, request: Request, profile: Optional[str] = None):
    ...
    process_home = str(get_kenxcode_home().expanduser().resolve(strict=False))
    with _profile_scope(profile):
        servers = _get_mcp_servers()
        flow_home = str(get_kenxcode_home().expanduser().resolve(strict=False))
    if name not in servers:
        raise HTTPException(status_code=404, detail=f"Server '{name}' not found")
    ...
    with _mcp_oauth_flows_lock:
        ...
        if any(
            ...
            and flow.kenxcode_home == flow_home
...
async def mcp_oauth_callback(
    ...
    if flow is None:
        return HTMLResponse("<h1>OAuth flow expired</h1><p>Return to KenXCode and try again.</p>", status_code=404)
    try:
    ...
    if error:
        return HTMLResponse("<h1>Authorization failed</h1><p>Return to KenXCode for details.</p>", status_code=400)
    return HTMLResponse("<h1>Authorization received</h1><p>You can close this tab and return to KenXCode.</p>")
...
async def list_mcp_catalog(profile: Optional[str] = None):
    ...
    `kenxcode mcp catalog` / `kenxcode mcp install` read.  ``profile`` scopes
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds KENXCODE_HOME-derived paths in the child.
