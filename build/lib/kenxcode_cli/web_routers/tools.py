...
late-binding seam in :mod:`kenxcode_cli.web_deps` (``late`` for callables,
...
async def get_toolsets(profile: Optional[str] = None):
    ...
    from kenxcode_cli.platforms import platform_label
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``kenxcode tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `kenxcode tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``kenxcode tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.kenxcode/.env`` via ``save_env_value`` —
    the same store ``kenxcode tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``kenxcode tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``kenxcode -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same KENXCODE_HOME the rest of the
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``kenxcode computer-use permissions grant`` as a background action.
