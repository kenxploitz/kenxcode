"""``kenxcode gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh Nous Portal access token from the existing login
     (``~/.kenxcode/auth.json``) — the same path ``kenxcode dashboard register``
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.kenxcode/.env``. The per-gateway secret authenticates the WS upgrade;
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "token could not be verified — try `kenxcode auth add nous` and retry."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def cmd_gateway_enroll(args) -> None:
    ...
    from kenxcode_cli.config import is_managed, save_env_value
    ...
    if is_managed():
        print(
            "✗ `kenxcode gateway enroll` is not available in a managed/hosted install.\n"
    ...
    #    save_env_value writes them to ~/.kenxcode/.env (0600 dir) and never logs.
    to_write = {
