"""
KenXCode Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``kenxcode desktop`` / ``kenxcode gui``)
     Built inside the agent checkout under ``$KENXCODE_HOME/kenxcode-agent/``:
       ...
       - ``$KENXCODE_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/KenXCode.app`` or ``~/Applications/KenXCode.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\KenXCode`` (NSIS per-user)
...
the app name ("KenXCode"), separate from ``$KENXCODE_HOME``:
  - macOS:   ``~/Library/Application Support/KenXCode``
  - Windows: ``%APPDATA%\\KenXCode``
  - Linux:   ``$XDG_CONFIG_HOME/KenXCode`` (default ``~/.config/KenXCode``)
...
``kenxcode uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(kenxcode_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return kenxcode_home / "kenxcode-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "KenXCode"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "KenXCode"
...
def source_built_gui_artifacts(kenxcode_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``kenxcode desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``kenxcode-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(kenxcode_home)
    ...
    return [
        ...
        kenxcode_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "KenXCode" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/KenXCode.app"),
            home / "Applications" / "KenXCode.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\KenXCode).
            local_base / "Programs" / "KenXCode",
            # Older / alternate layout some builds used.
            local_base / "kenxcode-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "KenXCode")
    else:
        ...
        paths += [
            data_base / "applications" / "kenxcode.desktop",
            data_base / "applications" / "KenXCode.desktop",
        ]
    return paths


def agent_is_installed(kenxcode_home: Path) -> bool:
    """Return True when a usable Python agent install exists under KENXCODE_HOME.
    ...
    if (agent_root / "kenxcode_cli").is_dir():
        return True
...
def gui_is_installed(kenxcode_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(kenxcode_home):
        if p.exists():
            return True
...
def gui_install_summary(kenxcode_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = kenxcode_home if kenxcode_home is not None else get_kenxcode_home()
...
def uninstall_gui(kenxcode_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``kenxcode-agent/kenxcode_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$KENXCODE_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove kenxcode' or "
            "'sudo dnf remove kenxcode'). AppImage builds are a single file you "
