...
``kenxcode gateway restart`` (or ``launchctl kickstart ai.kenxcode.gateway``
or ``systemctl restart kenxcode-gateway``).  When the cron fires, the
...
``kenxcode cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``kenxcode gateway``, ``launchctl ... kenxcode-gateway``,
``systemctl ... kenxcode-gateway``, ``pkill`` against the gateway) so it
...
commands and shell scripts they reference when ``_KENXCODE_GATEWAY=1``. It also
rejects ``launchctl submit`` in gateway sessions because launchd treats that
primitive as a persistent KeepAlive job, not a one-shot task. ``kenxcode gateway
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `kenxcode gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:kenxcode\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a kenxcode-gateway label. macOS launchd
    # labels look like `ai.kenxcode.gateway` / `kenxcode-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated kenxcode services (e.g.
    # `launchctl unload ai.kenxcode.update-checker.plist`).
    # `submit` and `bootstrap` are included alongside the direct verbs
    # (kickstart/etc.): `launchctl submit -l ai.kenxcode.gateway-<suffix> --
    ...
    # Branch C: systemctl ops on a kenxcode-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bkenxcode[.\-]?gateway)"
    # Branch D: pkill / kill targeting the kenxcode gateway process. Both
...
# lines (e.g. `launchctl submit \` / `  -l ai.kenxcode.gateway-... \` / `  -- ...`,
...
def contains_launchctl_submit_command(command: str) -> bool:
    ...
    chosen by whoever writes it, so a neutral name (``ai.kenxcode.svc-reload-tmp``)
...
def _iter_referenced_shell_scripts(
    ...
    *,
    ...
    for segment in _iter_command_segments(command):
        ...
        # (e.g. `Path.home() / ".kenxcode"`), not an executable reference.
...
def _resolve_script_path(script_path: str) -> Path:
    ...
    under ``<KENXCODE_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.kenxcode/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    return get_kenxcode_home() / "scripts" / raw
...
def _read_script_for_scanning(script_path: str) -> str:
    ...
    if unsafe:
        return "kenxcode gateway restart"
...
def check_gateway_lifecycle(
    ...
    if python_script:
        ...
        # `kenxcode gateway restart` embedded in a .py script is still
    ...
    if unsafe:
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `kenxcode gateway restart` from a shell outside "
