...
it is automatically downloaded from GitHub releases to $KENXCODE_HOME/bin/tirith.
...
def _get_kenxcode_home() -> str:
    """Return the KenXCode home directory, respecting KENXCODE_HOME env var."""
    return str(get_kenxcode_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_kenxcode_home(), ".tirith-install-failed")
...
def _kenxcode_bin_dir() -> str:
    """Return $KENXCODE_HOME/bin, creating it if needed."""
    d = os.path.join(_get_kenxcode_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $KENXCODE_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_kenxcode_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $KENXCODE_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $KENXCODE_HOME/bin/tirith
    ...
    kenxcode_bin = os.path.join(_kenxcode_bin_dir(), "tirith")
    if os.path.isfile(kenxcode_bin) and os.access(kenxcode_bin, os.X_OK):
        _resolved_path = kenxcode_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return kenxcode_bin
