...
<url>``) is spawned as a direct child of the KenXCode process. KenXCode's own
...
KenXCode process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next KenXCode startup's opt-in
