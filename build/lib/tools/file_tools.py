...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``kenxcode_constants.get_subprocess_home()`` so that ``~`` resolves
    ...
    if not path or "~" not in path:
        return path
    try:
        from kenxcode_constants import get_subprocess_home
...
def _truncate_to_char_budget(content: str, max_chars: int) -> tuple[str, int, bool]:
    ...
    ``read_file``). Where kenxcode previously hard-rejected an oversized read
...
def _file_ops_uses_host_paths(file_ops) -> bool:
    """Return True when *file_ops* targets the same host filesystem as KenXCode.
...
_kenxcode_config_resolved: str | None = None
_kenxcode_config_resolved_loaded = False


def _get_kenxcode_config_resolved() -> str | None:
    """Return the resolved absolute path of the KenXCode config file (cached)."""
    global _kenxcode_config_resolved, _kenxcode_config_resolved_loaded
    if _kenxcode_config_resolved_loaded:
        return _kenxcode_config_resolved
    _kenxcode_config_resolved_loaded = True
    try:
        from kenxcode_cli.config import get_config_path
        _kenxcode_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _kenxcode_config_resolved = str(Path(_expand_tilde("~/.kenxcode/config.yaml")).resolve())
        except Exception:
            _kenxcode_config_resolved = None
    return _kenxcode_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the KenXCode config file directly.
    # approvals.mode and other security settings live here; a malicious or
    # prompt-injected agent could silently disable exec approval by writing to
    # this file.
    kenxcode_config = _get_kenxcode_config_resolved()
    if kenxcode_config and (resolved == kenxcode_config or normalized == kenxcode_config):
        return (
            f"Refusing to write to KenXCode config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.kenxcode/config.yaml directly or use 'kenxcode config' instead."
        )
    return None


def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side KenXCode mirror prefix for Docker file tools."""
    try:
        from tools.terminal_tool import (
            _active_environments,
            _env_lock,
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    """Return a soft-guard warning when ``filepath`` lands in another KenXCode
    profile's scoped area, a host-side sandbox-mirror of authoritative profile
    state, or the Docker container's sandbox mirror of KenXCode state.
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.kenxcode/…`` mirror created by a
      non-local terminal backend (Docker, Daytona, etc.), where the host
      KenXCode process never reads the mirror and the authoritative file is
      left untouched.
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.kenxcode/…`` path.

    Returns ``None`` when the write is in-scope or outside KenXCode scope.
    ...
    # in a session that cd'd into ``~/.kenxcode/profiles/other/`` is
...
def read_file_tool(path: str, offset: int = 1, limit: int = 2000, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── KenXCode internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under KENXCODE_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == KENXCODE_HOME (e.g. "auth.json") still hits the
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    ``cross_profile`` opts out of the soft cross-KenXCode-profile guard. The
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    ``cross_profile`` opts out of the soft cross-KenXCode-profile guard for
...
WRITE_FILE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another KenXCode profile's skills/plugins/cron/memories — by default these writes are blocked with a warning because they affect a different profile than the one this session is running under.",
                "default": False,
            },
        },
...
PATCH_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another KenXCode profile's skills/plugins/cron/memories.",
                "default": False,
            },
        },
        "required": ["mode"],
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with kenxcode_tools.write_file() for very "
            "large files."
        )
