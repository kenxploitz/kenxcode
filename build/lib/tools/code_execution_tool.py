...
Lets the LLM write a Python script that calls KenXCode tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `kenxcode_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `kenxcode_tools.py` with file-based RPC stubs
...
# available on every platform KenXCode itself runs on.
...
# match a safe prefix, the operational KENXCODE_ allowlist, or (on Windows) an
# OS-essential name.  Delegate-task child context is also an exact-name
# operational marker: without it, a sandbox script that spawns/imports KenXCode
# code can lose the DB-layer Kanban mutation guard while still inheriting
# KENXCODE_HOME.
#
# NB: the broad "KENXCODE_" prefix was deliberately removed (#27303) — it leaked
# KENXCODE_*-named config that lacks a secret substring (e.g. KENXCODE_BASE_URL,
# KENXCODE_KANBAN_DB, KENXCODE_*_WEBHOOK).  The child only needs the few
# location/profile vars in _KENXCODE_CHILD_ALLOWED below; KENXCODE_RPC_SOCKET /
# KENXCODE_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational KENXCODE_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set kenxcode_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_KENXCODE_CHILD_ALLOWED = frozenset({
    "KENXCODE_HOME",
    "KENXCODE_PROFILE",
    "KENXCODE_CONFIG",
    "KENXCODE_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational KENXCODE_* vars (_KENXCODE_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret KENXCODE_* vars dropped by the tightened allowlist (#27303). The
    # broad "KENXCODE_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like KENXCODE_KANBAN_DB / KENXCODE_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _KENXCODE_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("KENXCODE_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped KENXCODE_* var.
            _dropped_kenxcode.append(k)
    if _dropped_kenxcode:
        logger.debug(
            "execute_code: dropped %d non-allowlisted KENXCODE_* var(s) from the "
            ...
            len(_dropped_kenxcode),
            ", ".join(sorted(_dropped_kenxcode)),
        )
...
# ---------------------------------------------------------------------------
# kenxcode_tools.py code generator
# ---------------------------------------------------------------------------
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-KenXCode-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-KenXCode-profile soft guard."""',
...
def _sandbox_failure_hint(stderr_text: str, enabled_tools=None) -> Optional[str]:
    ...
    kenxcode_tools import misuse (importing tools that aren't in the sandbox,
    ...
    try:
        m = re.search(
            r"cannot import name '(\w+)' from 'kenxcode_tools'", window
        )
        if m:
            missing = m.group(1)
...
def generate_kenxcode_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the kenxcode_tools.py stub module.
...
"""Auto-generated KenXCode tools RPC stubs."""
...
def _connect():
    ...
    KENXCODE_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["KENXCODE_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("KENXCODE_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
"""Auto-generated KenXCode tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("KENXCODE_RPC_DIR") or os.path.join(tempfile.gettempdir(), "kenxcode_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated kenxcode_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/kenxcode_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_kenxcode_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/kenxcode_tools.py", tools_src)
        ...
        env_prefix = (
            f"KENXCODE_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"KENXCODE_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("KENXCODE_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    # --- Set up temp directory with kenxcode_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="kenxcode_sandbox_")
    ...
    # selector from KENXCODE_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"kenxcode_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated kenxcode_tools module.
        ...
        tools_src = generate_kenxcode_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "kenxcode_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  KENXCODE_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["KENXCODE_RPC_SOCKET"] = rpc_endpoint
        child_env["KENXCODE_RPC_TOKEN"] = rpc_token
        ...
        # Ensure the kenxcode-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from kenxcode_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _kenxcode_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _kenxcode_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # KENXCODE_TIMEZONE is an internal KenXCode setting and must not leak
        # into child processes.
        _tz_name = os.getenv("KENXCODE_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("KENXCODE_TIMEZONE", None)
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.kenxcode/.env')).
...
def _load_config() -> dict:
    ...
    ``kenxcode --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (kenxcode-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``kenxcode tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and kenxcode-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.kenxcode/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that calls KenXCode tools programmatically. "
        ...
        f"Available via `from kenxcode_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from kenxcode_tools import {import_str}` "
