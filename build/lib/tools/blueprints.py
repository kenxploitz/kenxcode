...
`kenxcode skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.kenxcode.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.kenxcode.blueprint`` is a mapping containing
    ...
    kenxcode = meta.get("kenxcode") if isinstance(meta, dict) else None
    blueprint = kenxcode.get("blueprint") if isinstance(kenxcode, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.kenxcode.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.kenxcode.blueprint`` block) they can hand
    to ``kenxcode skills publish`` to share. ``body`` is the plain-language
