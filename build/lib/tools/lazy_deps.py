"""
Lazy dependency installer for opt-in KenXCode Agent backends.

Many KenXCode features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``kenxcode-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``kenxcode tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``KENXCODE_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/kenxcode`` read-only), setting
  ``KENXCODE_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
  ...
  installed package cannot brick KenXCode, which is what made it safe to seal
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # ─── Wake word ("Hey KenXCode") engines ──────────────────────────────────
    ...
    # (KENXCODE_DISABLE_LAZY_INSTALLS=1) and lazy installs are redirected to the
    ...
    # HF Agent Trace Viewer upload (kenxcode trace upload / /upload-trace).
    #
    ...
    # presence, the `kenxcode update` lazy-refresh pass re-asserts THIS pin on
...
_LAZY_TARGET_ENV = "KENXCODE_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``KENXCODE_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
    ...
    if os.environ.get("KENXCODE_DISABLE_LAZY_INSTALLS") == "1":
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    ``kenxcode update`` lazy-refresh pass.
    """
    if sys.platform == "win32" and feature == "platform.matrix":
        return (
            ...
            "from sdist. Run KenXCode under WSL to use Matrix on Windows."
        )
    return None
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``kenxcode update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _core_constraints_file() -> Optional[Path]:
    ...
    * A backend that *requires* a version conflicting with core fails loudly
    ...
    try:
        ...
        if not lines:
            return None
        fd, path = tempfile.mkstemp(prefix="kenxcode-core-constraints-", suffix=".txt")
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    Mirrors the strategy in ``kenxcode_cli.tools_config._pip_install`` but
    ...
    try:
        ...
        uv_env = kenxcode_subprocess_env(inherit_credentials=False)
        uv_env["VIRTUAL_ENV"] = str(venv_root)

        # Tier 1: uv (preferred — fast, doesn't need pip in the venv)
        # Managed uv first: $KENXCODE_HOME/bin is never on PATH, so a bare
        # which() misses the uv KenXCode installed and falls through to the
...
def install_specs(specs: list[str] | tuple[str, ...], *, timeout: int = 300) -> InstallSpecsResult:
    ...
    * **Durable-target on immutable images** — when the deployment seals the
      agent venv (``KENXCODE_DISABLE_LAZY_INSTALLS=1``) and sets
      ``KENXCODE_LAZY_INSTALL_TARGET``, installs are redirected to the writable
...
def active_features() -> list[str]:
    ...
    Used by ``kenxcode update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``kenxcode update``. Never raises; lazy-install failures
