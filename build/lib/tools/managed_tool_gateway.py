...
def auth_json_path():
    """Return the KenXCode auth store path, respecting KENXCODE_HOME overrides."""
    return get_kenxcode_home() / "auth.json"
...
def peek_nous_access_token() -> Optional[str]:
    ...
    Availability scans (`kenxcode tools`, banner/status paint, provider
...
# managed vendor's gateway URL is pinned: adding one is a KenXCode release, and
