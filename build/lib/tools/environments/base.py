"""Base class for all KenXCode execution environment backends.
...
# KENXCODE_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
# every is_interrupted() state change from _wait_for_process.  Off by default
# to avoid flooding production gateway logs.
_DEBUG_INTERRUPT = bool(os.getenv("KENXCODE_DEBUG_INTERRUPT"))
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {KENXCODE_HOME}/sandboxes/.
    """
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    if custom:
        p = Path(custom)
    else:
        p = get_kenxcode_home() / "sandboxes"
...
def _cwd_marker(session_id: str) -> str:
    return f"__KENXCODE_CWD_{session_id}__"
...
# the FIRST session's KENXCODE_SESSION_ID into the snapshot makes every LATER
...
# KENXCODE_SESSION_ID leak via the shared snapshot). Stripping them from the
...
# set), not KenXCode' per-turn session identity.
#
# Kept in sync with gateway.session_context._VAR_MAP: every bridged name starts
# with one of these prefixes (or is KENXCODE_UI_SESSION_ID). Used by unit tests
# as the Python-side contract for the exclusion set; the dump path unsets by
# name/prefix instead of grepping declare lines (see below / issue #71296).
_SNAPSHOT_EXCLUDED_ENV_REGEX = (
    "^declare -x (KENXCODE_SESSION_|KENXCODE_UI_SESSION_ID|KENXCODE_CRON_AUTO_DELIVER_|KENXCODE_CRON_SESSION)"
)
...
def _export_dump_excluding_session_vars(
    ...
    Matrix room/display name via ``KENXCODE_SESSION_CHAT_NAME``) land in the
    ...
    return (
        "{ ( "
        "unset ${!KENXCODE_SESSION_*} ${!KENXCODE_CRON_AUTO_DELIVER_*} "
        f"KENXCODE_UI_SESSION_ID{extra_unset} 2>/dev/null; "
        "export -p; "
        ") || true; } "
        f"> {tmp_path}"
    )
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all KenXCode backends.
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/kenxcode-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/kenxcode-cwd-{self._session_id}.txt"
    ...
    def init_session(self):
        ...
        bootstrap = (
            ...
            f"__kenxcode_fns=$(declare -F | awk '{{print $3}}' | grep -vE '^_[^_]') || true\n"
            f"[ -n \"$__kenxcode_fns\" ] && declare -f $__kenxcode_fns "
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        for name in passthrough_names:
            marker = f"_KENXCODE_RUNTIME_PASSTHROUGH_{name}"
        ...
        parts.append("__kenxcode_ec=$?")
        # Restrict KenXCode metadata files without changing the user's command
        ...
        parts.append("exit $__kenxcode_ec")
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"KENXCODE_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(
        ...
        if bounded_capture:
            ...
            try:
                spill_dir = get_kenxcode_home() / "cache" / "terminal-output"
        ...
        # --- Debug tracing (opt-in via KENXCODE_DEBUG_INTERRUPT=1) -------------
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __KENXCODE_CWD_{session}__ marker from stdout output.
