#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the KenXCode Skills Hub.
...
Used by kenxcode_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _kenxcode_home() -> Path:
    return get_kenxcode_home()


def _skills_dir() -> Path:
    forced = _override("SKILLS_DIR")
    return Path(forced) if forced is not None else _kenxcode_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            kenxcode_meta = metadata.get("kenxcode", {})
            if isinstance(kenxcode_meta, dict):
                tags = kenxcode_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            kenxcode_meta = metadata.get("kenxcode", {})
            if isinstance(kenxcode_meta, dict):
                raw_tags = kenxcode_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.kenxcode/skills/ during setup.  They are discoverable via the Skills Hub
    (search / install / inspect) and labelled "official" with "builtin" trust.
    """

    OFFICIAL_REPO = "NousResearch/kenxcode-agent"
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                kenxcode_meta = meta_block.get("kenxcode", {})
                if isinstance(kenxcode_meta, dict):
                    tags = kenxcode_meta.get("tags", [])
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``kenxcode_cli.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
# ---------------------------------------------------------------------------
# KenXCode centralized index source
# ---------------------------------------------------------------------------

KENXCODE_INDEX_URL = "https://kenxcode-agent.nousresearch.com/docs/api/skills-index.json"
KENXCODE_INDEX_TTL = 6 * 3600  # 6 hours


def _kenxcode_index_cache_file() -> Path:
    return _index_cache_dir() / "kenxcode-index.json"


def _load_kenxcode_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for KENXCODE_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    kenxcode_index_cache_file = _kenxcode_index_cache_file()
    if kenxcode_index_cache_file.exists():
        try:
            age = time.time() - kenxcode_index_cache_file.stat().st_mtime
            if age < KENXCODE_INDEX_TTL:
                return json.loads(kenxcode_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("KenXCode index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "KenXCode index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("KenXCode index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        kenxcode_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        kenxcode_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
...
class KenXCodeIndexSource(SkillSource):
    """Skill source backed by the centralized KenXCode Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_kenxcode_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "kenxcode-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "kenxcode-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        KenXCodeIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if _effective_filter == "all":
        for src in sources:
            if (src.source_id() == "kenxcode-index"
