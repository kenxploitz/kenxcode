...
2. An event-loop heartbeat file at ``<KENXCODE_HOME>/state/gateway.heartbeat`` so
...
def _process_kenxcode_home() -> Path:
    """KENXCODE_HOME for process-level identity files (ignore profile overrides)."""
...
def get_loop_heartbeat_path(home: Optional[Path] = None) -> Path:
    """Return ``<KENXCODE_HOME>/state/gateway.heartbeat``."""
    base = home if home is not None else _process_kenxcode_home()
    ...
    *,
    ...
    def _watchdog() -> None:
        ...
        try:
            ...
            pass
        try:
            from kenxcode_logging import drain_log_queue
