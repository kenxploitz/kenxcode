...
1. ``flush_pending_to_file()`` — called BEFORE ``_pending_messages.clear()``
   ...
   under ``<kenxcode_home>/pending_messages/``.
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active KENXCODE_HOME."""
    from kenxcode_constants import get_kenxcode_home

    flush_dir = get_kenxcode_home() / "pending_messages"
