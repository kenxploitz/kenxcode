...
Defenses 1 and 2 (the ``_KENXCODE_GATEWAY`` guard on ``kenxcode gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.kenxcode.gateway")``,
...
State lives in ``<KENXCODE_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_kenxcode_home() / "gateway" / "restart_loop.json"
