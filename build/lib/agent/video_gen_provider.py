...
as ``kind: backend``) or ``~/.kenxcode/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``kenxcode tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$KENXCODE_HOME/cache/videos/``, creating parents as needed."""
    from kenxcode_constants import get_kenxcode_home

    path = get_kenxcode_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$KENXCODE_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$KENXCODE_HOME/cache/videos/``.
