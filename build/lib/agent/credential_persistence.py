...
``auth.json``.  They intentionally have no dependency on ``kenxcode_cli.auth`` so
...
# Sources KenXCode owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and KenXCode-owned OAuth/device-code state)
