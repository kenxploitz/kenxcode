...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `kenxcode backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - kenxcode_home (str): The active KENXCODE_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.kenxcode``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "kenxcode").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'kenxcode memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], kenxcode_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'kenxcode memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``kenxcode_home`` is the active KENXCODE_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE KENXCODE_HOME.

        ``kenxcode backup`` only walks KENXCODE_HOME, so any provider state kept
        ...
        of the archive, and ``kenxcode import`` restores them to their original
