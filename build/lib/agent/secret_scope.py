...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the KENXCODE_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # KenXCode runtime / deployment
    "KENXCODE_HOME", "KENXCODE_PROFILE", "KENXCODE_GATEWAY_LOCK_DIR",
    "KENXCODE_MAX_ITERATIONS", "KENXCODE_MAX_TOKENS", "KENXCODE_API_TIMEOUT",
    "KENXCODE_REDACT_SECRETS", "KENXCODE_NOUS_TIMEOUT_SECONDS",
    "_KENXCODE_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "KENXCODE_KANBAN_DB", "KENXCODE_KANBAN_WORKSPACES_ROOT", "KENXCODE_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "KENXCODE_KANBAN_",
    "KENXCODE_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset KenXCode writes
    ...
    ``kenxcode_cli.config._parse_env_value``) but never mutates the process
    ...
    # Parse values with the canonical KenXCode parser: save_env_value
    ...
    from kenxcode_cli.config import _parse_env_value
...
def build_profile_secret_scope(kenxcode_home: Path) -> Dict[str, str]:
    ...
    """
    home = Path(kenxcode_home)
    secrets = load_env_file(home / ".env")

    try:
        from kenxcode_cli.env_loader import get_secret_source_values
