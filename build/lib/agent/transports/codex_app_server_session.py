...
Owns one Codex thread per KenXCode session. Drives `turn/start`, consumes
...
# KenXCode' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_KENXCODE_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse KenXCode/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per KenXCode session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _KENXCODE_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("KENXCODE_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="kenxcode",
            client_title="KenXCode Agent",
            client_version=_get_kenxcode_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but KenXCode' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into KenXCode' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own kenxcode-tools callback we
            # auto-accept — the user already approved KenXCode' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO KenXCode
        approval-mode/timeout logic. The KenXCode-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; KenXCode approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map KenXCode approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    KenXCode returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the KenXCode approval mode/timeout resolution
    ...
    # "prompt expired"; the KenXCode-side messaging already distinguishes them.
    return "decline"
...
def _get_kenxcode_version() -> str:
    """Best-effort KenXCode version string for codex's userAgent line."""
    try:
