...
appropriate.  All installs go to a KenXCode-owned bin staging dir,
``<KENXCODE_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``kenxcode lsp
  status``.
...
# ``<KENXCODE_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # `kenxcode lsp status` reports the host's presence.
...
def kenxcode_lsp_bin_dir() -> Path:
    """Return the KenXCode-owned bin staging dir for LSP servers."""
    from kenxcode_constants import get_kenxcode_home

    p = get_kenxcode_home() / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(kenxcode_lsp_bin_dir() / name):
...
def _install_npm(
    ...
    """
    # Managed npm first: $KENXCODE_HOME/node is not on an arbitrary process's
    # PATH, so a bare which() misses the Node that KenXCode installed and
    ...
    if npm is None:
        ...
        return None
    staging = kenxcode_lsp_bin_dir().parent  # <KENXCODE_HOME>/lsp/
    ...
    for c in _native_binary_candidates(nm_bin):
        if c.exists():
            ...
            link = kenxcode_lsp_bin_dir() / c.name
            if not link.exists():
                try:
                    link.symlink_to(c)
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a kenxcode-owned target dir.
    ...
    console script.
    """
    pip_target = kenxcode_lsp_bin_dir().parent / "python-packages"
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = kenxcode_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``kenxcode lsp status`` CLI to give users a quick
