...
def _fetch_with_timeout(
    ...
    an unbounded hang on every ``kenxcode`` invocation.
    """
...
def _active_profile_name(home_path: Optional[Path]) -> str:
    ...
    A named profile's KENXCODE_HOME is ``~/.kenxcode/profiles/<name>``; the
    default profile (``~/.kenxcode``) returns "".
    """
    if home_path is not None:
        resolved = Path(home_path)
        if resolved.parent.name == "profiles" and resolved.name:
            return resolved.name
    for env_name in ("KENXCODE_PROFILE_NAME", "KENXCODE_PROFILE"):
