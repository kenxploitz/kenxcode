...
cache problem must never block KenXCode startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the KenXCode home used for cache paths.

    ``home_path`` is whatever ``load_kenxcode_dotenv()`` already resolved;
    falling back to ``$KENXCODE_HOME`` / ``~/.kenxcode`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from kenxcode_constants import get_kenxcode_home

        home_path = get_kenxcode_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<kenxcode_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if KENXCODE_HOME is ever made traversable.
            try:
