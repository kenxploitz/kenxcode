...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time KenXCode starts and finds ``~/.openclaw/``.

    Points users at ``kenxcode claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``kenxcode claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to KenXCode, run "
        "`kenxcode claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `kenxcode claw cleanup` (renames it to ~/.openclaw.pre-migration — "
