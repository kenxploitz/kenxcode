...
Plugins built on KenXCode Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing KenXCode
...
:class:`~kenxcode_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors KenXCode' main config
...
handles every provider, fallback chain, and per-task override KenXCode
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`kenxcode_cli.plugins.PluginContext`
