...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into KenXCode with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.kenxcode/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of KenXCode
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``kenxcode tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``kenxcode setup`` for
