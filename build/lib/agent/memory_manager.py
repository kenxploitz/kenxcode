...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_KENXCODE_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, KenXCode expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``kenxcode_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_kenxcode_home()`` themselves.
        """
        if "kenxcode_home" not in kwargs:
            from kenxcode_constants import get_kenxcode_home
            kwargs["kenxcode_home"] = str(get_kenxcode_home())
        for provider in self._providers:
            try:
