...
Data resolution order:
  ...
  2. Disk cache (~/.kenxcode/models_dev_cache.json — any age; stale data is
...
# ---------------------------------------------------------------------------
# Provider ID mapping: KenXCode ↔ models.dev
# ---------------------------------------------------------------------------

# KenXCode provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → KenXCode (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_kenxcode_home() / "models_dev_cache.json"
...
def fetch_models_dev(
    ...
    When ``force_refresh=True`` (used by ``kenxcode config refresh``, the
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a KenXCode provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal KenXCode agent traffic.
...
def get_provider_info(
    ...
    Accepts either a KenXCode provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve KenXCode ID → models.dev ID
...
def get_model_info(
    ...
    Accepts KenXCode or models.dev provider ID.  Tries exact match then
