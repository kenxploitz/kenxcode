...
`kenxcode config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.kenxcode/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `KENXCODE_HOME`, secret scope, and
