...
The canonical transcript store is SQLite via `SessionDB` (from `kenxcode_state`). The
...
Counts consecutive restarts via a JSON file (`{KENXCODE_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `kenxcode update`, `kenxcode gateway restart`,
or `/restart`.
