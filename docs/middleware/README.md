...
KenXCode middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing KenXCode'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before KenXCode calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving KenXCode retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving KenXCode guardrails,
...
- `telemetry_schema_version`: currently `kenxcode.observer.v1`
- `middleware_schema_version`: currently `kenxcode.middleware.v1`
...
KenXCode stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, KenXCode runs
them as a nested chain in registration order. Middleware failures are fail-open:
KenXCode logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, KenXCode applies middleware in this order:
...
For each tool call, KenXCode applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal KenXCode pre-execution path against the effective arguments:
...
```bash
kenxcode plugins enable <plugin-name>
```

For isolated local testing, use one `KENXCODE_HOME` for plugin enablement and the
agent run:

```bash
export KENXCODE_HOME=/tmp/kenxcode-middleware-test
mkdir -p "$KENXCODE_HOME"
kenxcode plugins enable <plugin-name>
kenxcode chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run kenxcode plugins enable <plugin-name>
uv run kenxcode chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["kenxcode_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/kenxcode-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape KenXCode expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, KenXCode treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, KenXCode preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. KenXCode does not convert downstream
