...
has to be thrown away or summarized. KenXCode has always done this in one batch:
...
KenXCode folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. KenXCode's normal session flush is
...
With `micro_compact` unset or `false` KenXCode behaves exactly as it always has:
...
Defaults to `$KENXCODE_HOME/logs/agent.log`. It reports passes, outcome mix, net
