...
When running KenXCode inside Docker, the default `network_mode: host` gives the
...
The KenXCode [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
│                                             │
│   ┌──────────────┐   ┌──────────────────┐   │
│   │ kenxcode-agent │   │ kenxcode-dashboard │   │
│   └──────┬───────┘   └────────┬─────────┘   │
│          │                    │              │
│          ▼                    │              │
│   ┌──────────────┐            │              │
...
#
# Usage:
#   KENXCODE_UID=$(id -u) KENXCODE_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=kenxcode,kenxcode-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://kenxcode-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — KenXCode trust model and vulnerability reporting
