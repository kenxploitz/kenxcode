...
This document is the formal interface between the **KenXCode gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `kenxcode gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`kenxcode gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `kenxcode` CLI.
