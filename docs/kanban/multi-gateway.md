...
KenXCode supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.kenxcode/config.yaml`:
...
Or set the env var: `KENXCODE_KANBAN_DISPATCH_IN_GATEWAY=false`
