...
KenXCode includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into KenXCode and does not require `kenxcode plugins enable
observability/nemo_relay`. KenXCode remains importable without Relay on other
native targets. Those targets use an explicit reduced-capability no-op host:
KenXCode execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `kenxcode-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

KenXCode requires NeMo Relay 0.6.0 or later within the 0.6 release line. That
...
KenXCode installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the KenXCode process so configured interceptors can
...
Collection remains off unless KenXCode policy enables it:
...
KenXCode core owns one Relay host and one isolated Relay session scope per KenXCode
session. Core lifecycle producers use
`kenxcode_cli.observability.relay_runtime` to obtain the shared session handle or
run Relay scope, LLM, tool, and mark APIs in that session context. New product
marks do not require KenXCode plugin registration. Shared-metrics marks must
...
```text
KenXCode turn, API, and tool hooks
  -> Relay session, task, and LLM lifecycle
  -> KenXCode shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

KenXCode sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
Each task run is a Relay `Function` scope named `kenxcode.task_run`, parented to
the owning KenXCode session. The start counter contains only bounded execution
...
provider attempts for the same KenXCode API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their KenXCode tool-call
...
cancellations. Active task ownership follows the task ID if KenXCode rotates its
conversation session during context compression.

Local state is written under:

```text
$KENXCODE_HOME/telemetry/shared_metrics/metrics.sqlite3
$KENXCODE_HOME/telemetry/shared_metrics/outbox/*.json
```
...
schema field name, its current scope is one `KENXCODE_HOME`, so it is more
...
Deleting `$KENXCODE_HOME/telemetry/shared_metrics` resets the identifier together
...
Run a real KenXCode CLI turn against the deterministic local model server:
