...
KenXCode gateway daemon, exported over OTLP/HTTP to an operator-configured
...
(`plugins/observability/nemo_relay/`) and its KenXCode-owned subscribers.

## What gets exported

| Signal | OTLP route | Content |
| --- | --- | --- |
| Gateway gauges | `/v1/metrics` | `kenxcode.gateway.up/state/busy/drainable/active_agents/background_work/background_delegations/restart_requested`, `kenxcode.platform.up/degraded` with bounded `error_code` attributes |
...
`kenxcode.gateway.active_agents`, `kenxcode.gateway.background_work`, and
`kenxcode.gateway.background_delegations` are complementary. `active_agents`
...
```bash
kenxcode monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install 'kenxcode-agent[otlp]'`),
...
a plain `kenxcode gateway run`: the exporter lives in the gateway process, so
...
`kenxcode.gateway.up`, `kenxcode.platform.up`, and `kenxcode.platform.degraded`.
...
absent_over_time(kenxcode_gateway_up[5m])

# Locally owned bridge is explicitly down.
kenxcode_platform_up == 0

# Scheduler thread is stale even though the gateway may still be alive.
kenxcode_cron_scheduler_heartbeat_age_seconds > 180

# Ticker loops but has not completed a successful tick recently.
kenxcode_cron_scheduler_last_success_age_seconds > 300

# One or more jobs are beyond their existing scheduler grace window.
kenxcode_cron_jobs_overdue > 0

# Catch-up counter increased, proving at least one stale occurrence was
# collapsed and run once after a delay.
increase(kenxcode_cron_scheduler_catch_up_occurrences[15m]) > 0
```

Cron execution lifecycle records arrive as `kenxcode.cron_execution` spans.
Alert or derive events from bounded attributes such as:

```text
kenxcode.status = failed|unknown
kenxcode.delivery_outcome = failed|not_configured
kenxcode.error_class = auth_failed|rate_limited|timeout|network_error|
...
3. a cron lifecycle feed keyed only by opaque `kenxcode.job_key`;
...
KenXCode Agent-owned Relay transport health remains in scope. A separate gateway
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log /tmp/kenxcode_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log /tmp/kenxcode_otel_capture.jsonl --wait 8
...
`kenxcode.error_class = ...` list in this file's alert section and the enum's unit
...
```bash
kenxcode monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
...
The `kenxcode monitoring` CLI intentionally exposes `status` only. This first
release covers only KenXCode Agent-owned service-health and operational-diagnostic
signals, including KenXCode Agent-owned Relay transport health. Team Gateway's
