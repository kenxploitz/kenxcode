# KenXCode Observer Hooks

KenXCode observer hooks are the read-only telemetry contract for plugins that
...
They do not replace KenXCode' planner, model providers, memory, tool registry,
...
KenXCode also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "kenxcode.observer.v1"
```

Hook callbacks are fail-open. KenXCode catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so KenXCode only builds
