...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from kenxcode_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_kenxcode_home), "display": display_kenxcode_home()})
    if key == "project":
    ...
    if key == "mtime":
        cfg_path = _kenxcode_home / "config.yaml"
        try:
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No KenXCode provider is configured.",
                },
            )
