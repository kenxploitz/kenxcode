...
def _default_registry_path() -> Path:
    return get_kenxcode_home() / "state" / _REGISTRY_NAME
...
class HostSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        expected_kenxcode_home: str | None = None,
        autostart: bool = True,
    ) -> None:
        ...
        self.expected_kenxcode_home = expected_kenxcode_home if expected_kenxcode_home is not None else str(get_kenxcode_home())
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        if self.env:
            env.update(self.env)
        env["KENXCODE_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("kenxcode_home") or "")
        if got_home and got_home != self.expected_kenxcode_home:
            raise RuntimeError(f"compute host KENXCODE_HOME mismatch: {got_home} != {self.expected_kenxcode_home}")
