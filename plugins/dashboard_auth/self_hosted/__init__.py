...
A standards-compliant OpenID Connect Relying Party for the ``kenxcode dashboard``
...
:class:`~kenxcode_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``kenxcode_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~kenxcode_cli.dashboard_auth.Session`.
    ...
    dashboard:
      oauth:
        ...
        self_hosted:
          ...
          # credential — prefer the env var / ~/.kenxcode/.env over config.yaml.

    # Environment overrides (Docker/Fly secret injection)
    KENXCODE_DASHBOARD_OIDC_ISSUER
    KENXCODE_DASHBOARD_OIDC_CLIENT_ID
    KENXCODE_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
    KENXCODE_DASHBOARD_OIDC_CLIENT_SECRET # optional; set for a confidential client
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        cookie_payload = {
            "kenxcode_session_pkce": f"state={state};verifier={code_verifier}",
        }
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — KenXCode does not call
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``KENXCODE_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "KENXCODE_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    client_id = _resolve_setting(
        "KENXCODE_DASHBOARD_OIDC_CLIENT_ID", oidc_cfg.get("client_id")
    )
    scopes = (
        _resolve_setting("KENXCODE_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    # Optional — set only for a confidential client. A credential, so the
    # canonical home is the env var / ~/.kenxcode/.env; config.yaml is supported
    # for precedence symmetry. Empty ⇒ public client (unchanged behaviour).
    client_secret = _resolve_setting(
        "KENXCODE_DASHBOARD_OIDC_CLIENT_SECRET", oidc_cfg.get("client_secret")
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "(KENXCODE_DASHBOARD_OIDC_ISSUER + KENXCODE_DASHBOARD_OIDC_CLIENT_ID) "
