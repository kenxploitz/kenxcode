...
``_check_spotify_available()`` — when the user has not run ``kenxcode auth
spotify``, the tools remain registered (so they appear in ``kenxcode
...
The Spotify auth flow (``kenxcode auth spotify``), CLI plumbing, and docs
