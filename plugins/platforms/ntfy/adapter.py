"""ntfy platform adapter (KenXCode plugin).
...
a KenXCode dependency.

This adapter ships as a KenXCode platform plugin under
``plugins/platforms/ntfy/``. The KenXCode plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "kenxcode-in"              # subscribe topic (incoming)
          publish_topic: "kenxcode-out"     # optional — defaults to topic
...
_ECHO_TAG = "kenxcode-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``kenxcode cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a KenXCode dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `kenxcode gateway status` without
