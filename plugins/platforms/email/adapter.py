"""
Email platform adapter for the KenXCode gateway.

Allows users to interact with KenXCode by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "kenxcode-agent" "version" "{_kenxcode_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `kenxcode config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `kenxcode gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "KenXCode Agent")
        ...
        msg_id = f"<kenxcode-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# kenxcode_cli/gateway.py, and the _send_email dispatch in
