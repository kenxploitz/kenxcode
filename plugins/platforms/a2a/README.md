# A2A — Agent-to-Agent protocol for KenXCode
...
peer (another KenXCode, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
kenxcode gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, KenXCode serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.kenxcode/a2a_audit.jsonl`.
- Conversations persist to `~/.kenxcode/a2a_conversations/` — they survive context
