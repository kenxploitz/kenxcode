...
# ``model_tools`` import time, so ``kenxcode status``, ``kenxcode chat``, etc.
...
# ``kenxcode_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "KenXCode is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a kenxcode MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "kenxcode-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${KENXCODE_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _kenxcode_home = _get_kenxcode_home()
        except (ModuleNotFoundError, ImportError):
            _kenxcode_home = _Path.home() / ".kenxcode"
        self._thread_count_store = _ThreadCountStore(
            _kenxcode_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "KenXCode is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("KENXCODE_HOME", str(_Path.home() / ".kenxcode"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${KENXCODE_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a kenxcode MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'KenXCode is thinking…' marker message.
        ...
        "KenXCode is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "KenXCode is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "KenXCode is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "KenXCode is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``kenxcode setup``.

    The setup wizard at ``kenxcode_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.kenxcode/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. kenxcode-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.kenxcode/.env")
    print_info("Restart the gateway: kenxcode gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``kenxcode cron`` running as a
    separate process from ``kenxcode gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the KenXCode plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="Run `kenxcode setup` to install Google Chat support.",
        ...
        # the "KenXCode is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'KenXCode is thinking…' marker message "
