...
This plugin connects KenXCode Agent to iMessage (and other Spectrum
...
TypeScript-only, KenXCode runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other KenXCode channel,
...
Run `kenxcode photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.kenxcode/.env` (the same place every other
...
Management metadata lives in `~/.kenxcode/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `kenxcode photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, KenXCode
  ...
  `.pluginPayloadAttachment` images immediately after the URL; KenXCode coalesces
...
- **Native polls are supported.** KenXCode posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `kenxcode send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<kenxcode-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `kenxcode photon status`, a DM and a group roundtrip,
