...
Small Node helper that bridges KenXCode Agent to Photon's Spectrum SDK
(`spectrum-ts`).  KenXCode is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-KenXCode-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and KenXCode can receive inbound messages
...
The KenXCode plugin's `kenxcode photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. KenXCode is Python, so replies go through this sidecar
