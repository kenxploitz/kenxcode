"""SimpleX Chat platform adapter (KenXCode plugin).
...
This adapter ships as a KenXCode platform plugin under
``plugins/platforms/simplex/``. The KenXCode plugin loader scans the
...
Optional environment variables:
    ...
    KENXCODE_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``kenxcode setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside KenXCode — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("KENXCODE_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``kenxcode setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.kenxcode/.env`` via
    ``kenxcode_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "kenxcode_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.kenxcode/.env"
        )
        return
