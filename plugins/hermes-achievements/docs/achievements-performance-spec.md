# KenXCode Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: kenxcode-achievements plugin
...
### `GET /api/plugins/kenxcode-achievements/achievements`
...
### `POST /api/plugins/kenxcode-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/kenxcode-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/kenxcode-achievements/overview`
...
- Persist plugin data under:
  - `~/.kenxcode/plugins/kenxcode-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.kenxcode/plugins/kenxcode-achievements/scan_checkpoint.json`
...
- `~/.kenxcode/plugins/kenxcode-achievements/`
