...
Optional KenXCode observability plugin that configures exporters and maps
KenXCode-specific observer hooks to NeMo Relay marks and ATIF state. KenXCode core
...
not replace KenXCode Agent's planner, tools, memory, model provider routing, or
CLI UX. KenXCode core emits NeMo Relay lifecycle events for provider and tool
...
With this plugin enabled, KenXCode Agent can:

- Export the Relay scopes and LLM/tool lifecycles emitted by KenXCode core.
- Add KenXCode session, turn, approval, and subagent mark events.
...
```bash
kenxcode plugins enable observability/nemo_relay
```

The `KENXCODE_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `KENXCODE_HOME` that the
agent run will use:

```bash
env KENXCODE_HOME=/tmp/kenxcode-nemo-relay-test \
  kenxcode plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`KENXCODE_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`KENXCODE_HOME` is the KenXCode profile/config home used by both
`kenxcode plugins enable ...` and the later `kenxcode chat ...` run. If unset,
KenXCode uses the user's default home, usually `~/.kenxcode`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export KENXCODE_HOME=/tmp/kenxcode-nemo-relay-test
kenxcode plugins enable observability/nemo_relay
kenxcode chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `kenxcode` command you run is built from the
checkout that contains this plugin. A globally installed older CLI will not see
new bundled plugins from your working tree.

```bash
uv sync
uv run kenxcode plugins enable observability/nemo_relay
uv run kenxcode chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
wheel, KenXCode installs its supported NeMo Relay runtime as a normal dependency:

```bash
uv build --wheel
python -m pip install --force-reinstall dist/kenxcode_agent-*.whl
kenxcode plugins enable observability/nemo_relay
```
...
behavior; it does not control KenXCode shared client metrics.

## Export Configuration

The plugin can configure exporters directly from `KENXCODE_NEMO_RELAY_*`
...
```bash
export KENXCODE_NEMO_RELAY_ATOF_ENABLED=1
export KENXCODE_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export KENXCODE_NEMO_RELAY_ATIF_ENABLED=1
export KENXCODE_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `KENXCODE_NEMO_RELAY_ATOF_FILENAME`
- `KENXCODE_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `KENXCODE_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `KENXCODE_NEMO_RELAY_ATIF_AGENT_NAME`
- `KENXCODE_NEMO_RELAY_ATIF_AGENT_VERSION`
- `KENXCODE_NEMO_RELAY_ATIF_MODEL_NAME`
- `KENXCODE_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point KenXCode at it:

```bash
export KENXCODE_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
agent_name = "KenXCode Agent"
agent_version = "local"
```

When `KENXCODE_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`KENXCODE_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`KENXCODE_NEMO_RELAY_ATIF_*` fallback setup is also skipped so KenXCode does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
KenXCode keeps the direct env-var fallbacks active for that run.

KenXCode core routes provider and tool execution through NeMo Relay managed APIs
...
KenXCode uses the dynamic-plugin activation API available in NeMo Relay 0.6 and
later. Configure native or worker plugins with KenXCode-owned
...
with this KenXCode-owned section. The gateway combines those records with
separate lifecycle state for enablement, trust policy, and worker environments;
the Python binding does not yet expose that resolver. KenXCode rejects
...
KenXCode activates these plugins before registering its managed LLM and tool
...
For the full generic KenXCode middleware contract, see
...
with KenXCode and a local Ollama model served through the OpenAI-compatible API.

```bash
export KENXCODE_HOME=/tmp/kenxcode-nemo-relay-docs/kenxcode-home
mkdir -p "$KENXCODE_HOME"

cat > "$KENXCODE_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent KenXCode session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export KENXCODE_NEMO_RELAY_ATOF_ENABLED=1
export KENXCODE_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/kenxcode-nemo-relay-docs/subagent/atof
export KENXCODE_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export KENXCODE_NEMO_RELAY_ATOF_MODE=overwrite
export KENXCODE_NEMO_RELAY_ATIF_ENABLED=1
export KENXCODE_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/kenxcode-nemo-relay-docs/subagent/atif
export KENXCODE_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export KENXCODE_NEMO_RELAY_ATIF_AGENT_NAME='KenXCode Agent E2E'
export KENXCODE_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export KENXCODE_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"kenxcode.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "KenXCode Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. KenXCode dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/kenxcode-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/kenxcode-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/kenxcode-nemo-relay-docs/workdir/beta.txt
cd /tmp/kenxcode-nemo-relay-docs/workdir

export KENXCODE_NEMO_RELAY_ATOF_ENABLED=1
export KENXCODE_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/kenxcode-nemo-relay-docs/parallel/atof
export KENXCODE_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export KENXCODE_NEMO_RELAY_ATOF_MODE=overwrite
export KENXCODE_NEMO_RELAY_ATIF_ENABLED=1
export KENXCODE_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/kenxcode-nemo-relay-docs/parallel/atif
export KENXCODE_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export KENXCODE_NEMO_RELAY_ATIF_AGENT_NAME='KenXCode Agent E2E'
export KENXCODE_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- KenXCode sessions map to `agent` scopes.
- KenXCode core managed provider calls map to `llm` scope start/end events.
- KenXCode core managed tool calls map to `tool` scope start/end events.
...
KenXCode core owns the LLM and tool boundaries and enters NeMo Relay managed
execution while a KenXCode-managed Relay consumer is active. With no shared
metrics subscriber or explicitly configured Relay plugin, KenXCode calls the
...
Enable it for KenXCode:

```bash
export KENXCODE_NEMO_RELAY_PLUGINS_TOML=/tmp/kenxcode-middleware-test/plugins.toml
```

Execution follows these boundaries with or without an adaptive component:

```text
KenXCode provider call
  -> nemo_relay.llm.execute(...)
    -> KenXCode provider adapter callback(...)

KenXCode tool call
  -> nemo_relay.tools.execute(...)
    -> KenXCode authorization and dispatch callback(...)
```
...
middleware for a local KenXCode run. This path requires a NeMo Relay runtime that
supports `[components.config.tool_parallelism]`, as provided by NeMo Relay 0.6
and later.

```bash
export KENXCODE_HOME=/tmp/kenxcode-middleware-test/kenxcode-home
mkdir -p "$KENXCODE_HOME" /tmp/kenxcode-middleware-test/nemo-relay
...
cat > /tmp/kenxcode-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/kenxcode-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/kenxcode-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "KenXCode Middleware E2E"
agent_version = "local"
...
export KENXCODE_NEMO_RELAY_PLUGINS_TOML=/tmp/kenxcode-middleware-test/nemo-relay/plugins.toml
