...
_DEFAULT_CONTAINER_TAG = "kenxcode"
...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=kenxcode"
...
def _load_supermemory_config(kenxcode_home: str) -> dict:
    config = _default_config()
    config_path = Path(kenxcode_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, kenxcode_home: str) -> None:
    config_path = Path(kenxcode_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "kenxcode"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes KenXCode writes into the "KenXCode" Space in the Supermemory
        ...
        merged = {"sm_source": "kenxcode", **(metadata or {})}
...
def _resolve_container_tag_for_setup(kenxcode_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(kenxcode_home)
...
def _probe_supermemory_connection(api_key: str, kenxcode_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(kenxcode_home)
    base_url = _resolve_base_url(config["base_url"])
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(kenxcode_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `kenxcode memory setup`.
        # All other options are documented for $KENXCODE_HOME/supermemory.json
    ...
    def save_config(self, values, kenxcode_home):
        ...
        _save_supermemory_config(sanitized, kenxcode_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, kenxcode_home)
    ...
    def post_setup(self, kenxcode_home: str, config: dict) -> None:
        ...
        from kenxcode_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(Path(kenxcode_home) / ".env", env_writes)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from kenxcode_constants import get_kenxcode_home
        self._kenxcode_home = kwargs.get("kenxcode_home") or str(get_kenxcode_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._kenxcode_home)
