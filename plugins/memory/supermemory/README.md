...
- Hosted: API key from [app.supermemory.ai/integrations?connect=kenxcode](http://app.supermemory.ai/integrations?connect=kenxcode)
...
Before running `kenxcode memory setup`, add the local endpoint to
...
Then run `kenxcode memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$KENXCODE_HOME/supermemory.json`
...
| `container_tag` | `kenxcode` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `kenxcode-{identity}` → `kenxcode-coder`). |
...
`https://api.supermemory.ai`. KenXCode resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: kenxcode`, and document writes stamp
`metadata.sm_source: kenxcode`. This is a **functional routing key, not telemetry**:
it groups KenXCode-written memories into a dedicated "KenXCode" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per KenXCode profile:

```json
{
  "container_tag": "kenxcode-{identity}"
}
```

For a profile named `coder`, this resolves to `kenxcode-coder`. The default profile resolves to `kenxcode-default`. Without `{identity}`, all profiles share the same container.
