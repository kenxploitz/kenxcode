...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/kenxcode>
...
```bash
kenxcode memory setup honcho   # configure Honcho directly (works on a fresh install)
kenxcode memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
kenxcode config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.kenxcode/.env
```

> `kenxcode honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `kenxcode memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `kenxcode model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$KENXCODE_HOME/honcho.json` | Profile-local (isolated KenXCode instances) |
| 2 | `~/.kenxcode/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active KenXCode profile: `kenxcode` (default) or `kenxcode_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `kenxcode honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | KenXCode session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`kenxcode-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`kenxcode`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-kenxcode-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening kenxcode in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — KenXCode session ID (timestamp + hex). Every `kenxcode` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple KenXCode profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`kenxcode`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active KenXCode profile: `kenxcode` (default) or `kenxcode_<profile>` (e.g. `kenxcode -p coder` -> host key `kenxcode_coder`). Older `kenxcode.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `KENXCODE_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `kenxcode-agent`) |
...
|---------|-------------|
| `kenxcode memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `kenxcode honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `kenxcode memory setup`) |
| `kenxcode honcho status` | Show resolved config for active profile |
| `kenxcode honcho enable` / `disable` | Toggle Honcho for active profile |
| `kenxcode honcho mode <mode>` | Change recall or observation mode |
| `kenxcode honcho peer --user <name>` | Update user peer name |
| `kenxcode honcho peer --ai <name>` | Update AI peer name |
| `kenxcode honcho tokens --context <N>` | Set context token budget |
| `kenxcode honcho tokens --dialectic <N>` | Set dialectic max chars |
| `kenxcode honcho map <name>` | Map current directory to a session name |
| `kenxcode honcho sync` | Create host blocks for all KenXCode profiles |
