...
``kenxcode://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the KenXCode OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "kenxcode-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``kenxcode-desktop`` / ``kenxcode-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to KenXCode.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local KENXCODE_HOME override can't reach.
