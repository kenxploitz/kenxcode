...
```bash
kenxcode memory setup    # select "mem0"
```
...
Behavioral settings live in `$KENXCODE_HOME/mem0.json` (set them via `kenxcode memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.kenxcode/.env`.
...
| `user_id` | `kenxcode-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh KenXCode session and call `mem0_search` — it connects to your server.
...
```bash
kenxcode memory setup mem0 --mode oss \
...
```bash
kenxcode memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$KENXCODE_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.kenxcode/mem0_qdrant"}}
  }
}
```
...
```bash
kenxcode memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$KENXCODE_HOME/mem0.json`).
