...
Configuration
-------------
Secret (lives in $KENXCODE_HOME/.env or the environment):
...
Behavioral settings (live in $KENXCODE_HOME/mem0.json, set via `kenxcode memory
setup`):
  ...
  agent_id           — Agent identifier (default: kenxcode)
...
_DEFAULT_USER_ID = "kenxcode-user"
...
def _load_config() -> dict:
    """Load config from env vars, with $KENXCODE_HOME/mem0.json overrides.
    ...
    config = {
        ...
        "agent_id": os.environ.get("MEM0_AGENT_ID", "kenxcode"),
        "oss": {},
    }
    ...
    config_path = get_kenxcode_home() / "mem0.json"
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, kenxcode_home):
        """Write config to $KENXCODE_HOME/mem0.json."""
        import json
    ...
    def post_setup(self, kenxcode_home: str, config: dict) -> None:
        ...
        post_setup(kenxcode_home, config)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $KENXCODE_HOME/mem0.json) —
        ...
        self._agent_id = self._config.get("agent_id", "kenxcode")
