...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — KenXCode peer ID in OpenViking (default: kenxcode)
...
_DEFAULT_AGENT = "kenxcode"
_AGENT_PROMPT_LABEL = "KenXCode peer ID in OpenViking"
...
_SYNC_TRACE_ENV = "KENXCODE_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip KenXCode slash-skill scaffolding before sending content to OpenViking.
...
def _load_kenxcode_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("KENXCODE_HOME", "")).expanduser() if os.environ.get("KENXCODE_HOME") else Path.home() / ".kenxcode"
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory disabled for this KenXCode run."
    )
...
def _save_kenxcode_only_config(
    *,
    config: dict,
    provider_config: dict,
    env_path: Path,
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new KenXCode session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in KenXCode only", "write values only to KenXCode .env"),
    ...
    _save_kenxcode_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to KenXCode .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See kenxcode-agent#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_kenxcode_openviking_config()
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "KenXCode peer ID in OpenViking, sent as the actor peer and "
    ...
    def post_setup(self, kenxcode_home: str, config: dict) -> None:
        ...
        from kenxcode_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        kenxcode_home_path = Path(kenxcode_home)
        env_path = kenxcode_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        settings = _resolve_connection_settings(_load_kenxcode_openviking_config())
        ...
        self._turn_count = 0
        kenxcode_home = str(kwargs.get("kenxcode_home") or "").strip()
        if not kenxcode_home:
            try:
                from kenxcode_constants import get_kenxcode_home
                kenxcode_home = str(get_kenxcode_home())
            except Exception:
                kenxcode_home = str(Path.home() / ".kenxcode")
        self._kenxcode_home = kenxcode_home
        ...
        try:
            ...
            if health_state == "unreachable":
                ...
                _emit_runtime_warning(
                    f"{health_message} OpenViking memory disabled for this KenXCode run.",
                    warning_callback,
                )
                self._client = None
        except ImportError:
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        ``~/.kenxcode/.env`` after startup never reach the live client and tools
        keep running against stale auth until the user restarts kenxcode (#21130).
    ...
    def _pending_session_dir(self) -> Optional[Path]:
        if not self._kenxcode_home:
            return None
        return Path(self._kenxcode_home) / _PENDING_SESSIONS_RELATIVE_DIR
    ...
    def _run_lock_dir(self) -> Optional[Path]:
        if not self._kenxcode_home:
            return None
        return Path(self._kenxcode_home) / _RUN_LOCKS_RELATIVE_DIR
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of KenXCode' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert KenXCode canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See kenxcode-agent#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/kenxcode/.overview.md).
