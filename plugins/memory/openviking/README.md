...
- OpenViking server running and reachable from KenXCode
...
connection values into KenXCode, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.kenxcode/.env`; for a named profile use
...
OpenViking's server config is separate from KenXCode:
...
KenXCode-side provider config is read from environment variables in the active
profile's `.env`:
...
| `OPENVIKING_AGENT` | `kenxcode` | KenXCode peer ID in OpenViking, used for peer-scoped memories |

When `OPENVIKING_API_KEY` is set, KenXCode lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
KenXCode sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
...
KenXCode built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| KenXCode action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under the configured peer memory namespace |

Built-in `replace` and `remove` operations are not mirrored because KenXCode
...
file URIs, such as
`viking://user/peers/kenxcode/memories/preferences/mem_abc123.md` or the canonical
`viking://user/default/peers/kenxcode/memories/preferences/mem_abc123.md`. Files
