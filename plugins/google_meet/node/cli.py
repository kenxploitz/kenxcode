"""`kenxcode meet node ...` subcommand tree.

Wired into the existing ``kenxcode meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``kenxcode meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="kenxcode-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``kenxcode meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             kenxcode meet node approve <name> ws://<host>:{args.port} {token}")
        try:
