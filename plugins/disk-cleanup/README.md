...
Auto-tracks and cleans up ephemeral files created during KenXCode Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$KENXCODE_HOME` and `/tmp/kenxcode-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `KENXCODE_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `KENXCODE_HOME` or `/tmp/kenxcode-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$KENXCODE_HOME/disk-cleanup/` is itself excluded
- `$KENXCODE_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
