"""disk_cleanup — ephemeral file cleanup for KenXCode Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under KENXCODE_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly KENXCODE_HOME and /tmp/kenxcode-*
Never touches: ~/.kenxcode/logs/ or any system directory.
"""
...
try:
    ...
    def get_kenxcode_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("KENXCODE_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".kenxcode").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$KENXCODE_HOME/logs/``."""
    return get_kenxcode_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$KENXCODE_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under KENXCODE_HOME or ``/tmp/kenxcode-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    kenxcode_home = get_kenxcode_home()
    try:
        path.resolve().relative_to(kenxcode_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/kenxcode-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("kenxcode-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "kenxcode-agent", "backups", "profiles", ".worktrees",
...
# Paths under $KENXCODE_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so KENXCODE_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        kenxcode_home = get_kenxcode_home()
        for parent in ("cron", "cronjobs"):
            base = kenxcode_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside KENXCODE_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under KENXCODE_HOME, but never recurse into known
    # durable state trees.  Some installs place the KenXCode checkout, venv,
    # and desktop build under KENXCODE_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    kenxcode_home = get_kenxcode_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in kenxcode_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    kenxcode_home = get_kenxcode_home()
    try:
        rel = path.resolve().relative_to(kenxcode_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "kenxcode-agent",
        ...
        # Path isn't under KENXCODE_HOME (e.g. /tmp/kenxcode-*) — fall through.
        pass
