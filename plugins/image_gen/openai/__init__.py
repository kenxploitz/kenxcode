...
three virtual model IDs so the ``kenxcode tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `kenxcode tools` → Image "
                    "Generation → OpenAI to configure, or `kenxcode setup` "
                    "to add the key."
                ),
