...
storage discipline as the other indexes). On a populated database, run
    ...
    kenxcode sessions optimize-storage

to backfill it; new messages are indexed live either way. Set
`sessions.cjk_fts: false` in `~/.kenxcode/config.yaml` to disable. Override
the .so location with `KENXCODE_FTS5_CJK_SO`.
