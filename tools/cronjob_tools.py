"""
Cron job management tools for KenXCode Agent.
...
# uses 30s) — comfortably below the 1800s default KENXCODE_AGENT_TIMEOUT.
_CRON_RUN_HEARTBEAT_INTERVAL = 10.0

# Hard ceiling on how long the heartbeat keeps the parent watchdog at bay.
# The child cron run has its own inactivity watchdog (KENXCODE_CRON_TIMEOUT,
# default 600s) that bounds a wedged job, but with KENXCODE_CRON_TIMEOUT=0
...
#      saying "cat ~/.kenxcode/.env" or "rm -rf /". `_scan_cron_prompt()` runs
...
#      `kenxcode-agent-dev` skill contains a security postmortem mentioning
#      `cat ~/.kenxcode/.env`, which tripped `read_secrets` and silently
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    origin_platform = get_session_env("KENXCODE_SESSION_PLATFORM")
    origin_chat_id = get_session_env("KENXCODE_SESSION_CHAT_ID")
    if origin_platform and origin_chat_id:
        ...
        return {
            ...
            "chat_name": get_session_env("KENXCODE_SESSION_CHAT_NAME") or None,
            "thread_id": thread_id,
            ...
            # send_message, which passes KENXCODE_SESSION_USER_ID to
...
def _local_delivery_notice(job: Dict[str, Any], user_deliver: Optional[str]) -> Optional[str]:
    ...
    ``KENXCODE_SESSION_PLATFORM``/``CHAT_ID`` is set for them), so a
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within KENXCODE_HOME/scripts/.
    ...
    # Only relative paths within ~/.kenxcode/scripts/ are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.kenxcode/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.kenxcode/scripts/ and use just the filename."
        )

    # Validate containment after resolution
    from tools.path_security import validate_within_dir

    scripts_dir = get_kenxcode_home() / "scripts"
...
Important safety rule: cron-run sessions should not recursively schedule more cron jobs.""",
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                "type": "string",
                "description": f"Optional path to a script that runs each tick. In the default mode its stdout is injected into the agent's prompt as context (data-collection / change-detection pattern). With no_agent=True, the script IS the job and its stdout is delivered verbatim (classic watchdog pattern). Relative paths resolve under {display_kenxcode_home()}/scripts/. ``.sh``/``.bash`` extensions run via bash, everything else via Python. On update, pass empty string to clear."
            },
            "no_agent": {
                "type": "boolean",
                "default": False,
...
def check_cronjob_requirements() -> bool:
    ...
    return (
        env_var_enabled("KENXCODE_INTERACTIVE")
        or env_var_enabled("KENXCODE_GATEWAY_SESSION")
        or env_var_enabled("KENXCODE_EXEC_ASK")
    )
...
registry.register(
    ...
    handler=lambda args, **kw: cronjob(
        ...
        # `kenxcode cron create/edit --model`, or hand-edited jobs). The agent
