...
One ``CDPSupervisor`` runs per KenXCode ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach KenXCode logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "kenxcode-dialog-bridge.invalid"
...
(() => {
  if (window.__kenxcodeDialogBridgeInstalled) return;
  window.__kenxcodeDialogBridgeInstalled = true;
  const ENDPOINT = "http://kenxcode-dialog-bridge.invalid/";
