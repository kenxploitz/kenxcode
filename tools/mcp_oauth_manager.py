...
`mcp_tool.py`, and `kenxcode_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# KenXCodeMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_kenxcode_provider_class() -> Optional[type]:
    ...
    class KenXCodeMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._kenxcode_server_name = server_name
            self._kenxcode_home = ""
            ...
            self._kenxcode_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`KenXCodeTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, KenXCodeTokenStorage)
            ...
            if (
                ...
                try:
                    ...
                    logger.debug(
                        ...
                        "failed (non-fatal): %s",
                        self._kenxcode_server_name, exc,
                    )
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._kenxcode_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import KenXCodeTokenStorage
                        if isinstance(storage, KenXCodeTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._kenxcode_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import KenXCodeTokenStorage
            if not isinstance(storage, KenXCodeTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``kenxcode mcp reauth``.
            ...
            back to ``kenxcode mcp reauth``.
            """
            try:
                if self._kenxcode_preregistered:
                    return
        ...
        async def async_auth_flow(self, request):  # type: ignore[override]
            ...
            try:
                await get_manager().invalidate_if_disk_changed(
                    self._kenxcode_server_name,
                    kenxcode_home=self._kenxcode_home,
                )
            except Exception as exc:  # pragma: no cover — defensive
                logger.debug(
    ...
    return KenXCodeMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_KENXCODE_PROVIDER_CLS: Optional[type] = _make_kenxcode_provider_class()
...
class MCPOAuthManager:
        ...
        with self._entries_lock:
            ...
            if entry.provider is None:
                ...
                if entry.provider is not None:
                    entry.provider._kenxcode_home = key[0]
    ...
    def _build_provider(
        ...
        Constructs :class:`KenXCodeMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _KENXCODE_PROVIDER_CLS is None:
            logger.warning(
        ...
        if (
            ...
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `kenxcode mcp login "
    ...
    def remove(
        ...
        *,
        ...
        Called by ``kenxcode mcp remove <name>`` and (indirectly) by
        ``kenxcode mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
            entry = self._entries.pop(self._key(server_name, kenxcode_home), None)

        from tools.mcp_oauth import remove_oauth_tokens
        remove_oauth_tokens(server_name, kenxcode_home=kenxcode_home)
        logger.info(
    ...
    def restore_entry(
        ...
        *,
        ...
        if entry is None:
            return
        with self._entries_lock:
            self._entries.setdefault(self._key(server_name, kenxcode_home), entry)
    ...
    def evict(
        ...
        *,
        ...
        with self._entries_lock:
            self._entries.pop(self._key(server_name, kenxcode_home), None)
    ...
    async def invalidate_if_disk_changed(
        ...
        *,
        ...
        entry = self._entries.get(self._key(server_name, kenxcode_home))
        if entry is None or entry.provider is None:
            return False

        async with entry.lock:
            tokens_path = _get_token_dir(kenxcode_home) / f"{_safe_filename(server_name)}.json"
            try:
