#!/usr/bin/env python3
"""Close a read-only agent terminal tab in the KenXCode desktop GUI.
...
``read_terminal`` it is gated on ``KENXCODE_DESKTOP`` so it never appears outside
the GUI.
"""
...
CLOSE_TERMINAL_SCHEMA = {
    ...
    "description": (
        ...
        "the KenXCode desktop GUI (the tabs mirroring terminal(background=true) runs). "
