...
transport, discovers their tools, and registers them into the kenxcode-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.kenxcode/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.kenxcode/logs/mcp-stderr.log), tagged with the
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach KenXCode' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    KenXCode explicitly tagged during dotenv loading, plus any variables
    ...
    an exception: users configured that backend specifically so KenXCode and
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            kenxcode_home = os.path.expanduser(
                os.getenv(
                    "KENXCODE_HOME", os.path.join(os.path.expanduser("~"), ".kenxcode")
                )
            )
            candidates = [
                os.path.join(kenxcode_home, "node", "bin", resolved_command),
                ...
                # image (which the KenXCode Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → KenXCode MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that KenXCode gateways know how to render.
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the KenXCode document cache; returns a marker with
...
def _unwrap_exception_group(exc: BaseException) -> BaseException:
    ...
    Adapted from :func:`kenxcode_cli.mcp_config._unwrap_exception_group` with
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through KenXCode' existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (KENXCODE_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit KENXCODE_SESSION_PLATFORM, so the
    ...
    def _make_logging_callback(self):
        ...
        server into KenXCode' logging (agent.log via kenxcode_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "it is not installed. Run `kenxcode setup` to install MCP support, "
                "then retry."
            )
        ...
        # ungraceful exit of this KenXCode process (kill -9, crash, force-quit)
                ...
                if (
                    ...
                    post_resp = await client.post(
                        ...
                        content=(
                            ...
                            '"capabilities":{},'
                            '"clientInfo":{"name":"kenxcode-probe",'
                            '"version":"0.1"}}}'
                        ),
                    )
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through KenXCode' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until KenXCode is fully
...
def _handle_auth_error_and_retry(
    ...
    return tool_error(
        ...
        f"Run `kenxcode mcp login {server_name}` (or delete the tokens "
        f"file under ~/.kenxcode/mcp-tokens/ and restart). Do NOT retry "
...
# ---------------------------------------------------------------------------
# Advisory file lock that prevents N concurrent KenXCode processes (e.g.
    ...
    try:
        ...
        if _MCP_DISCOVERY_LOCK_PATH is None:
            _MCP_DISCOVERY_LOCK_PATH = str(
                get_kenxcode_home() / ".mcp-discovery.lock"
            )
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local KENXCODE_HOME override into ``coro``.
    ...
    try:
        ...
        home_override = get_kenxcode_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_kenxcode_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local KENXCODE_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_kenxcode_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the KenXCode config file.
    ...
    ``os.environ`` (which includes ``~/.kenxcode/.env`` loaded at startup).
    """
    try:
        from kenxcode_cli.config import load_config
        from utils import env_var_enabled as _env_enabled

        if _env_enabled("KENXCODE_SAFE_MODE"):
            return {}
        config = load_config()
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through KenXCode' MEDIA: tag convention and out to messaging
            ...
            # KenXCode' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves KenXCode's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. KenXCode uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the KenXCode registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``kenxcode tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the KenXCode registry.
...
def _kill_orphaned_mcp_children(
    ...
    survivors, avoiding shared-resource collisions when multiple kenxcode
