...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("KENXCODE_YOLO_MODE", ""))
...
# os.environ["KENXCODE_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export KENXCODE_INTERACTIVE.
_kenxcode_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "kenxcode_interactive",
    default=None,
)


def set_kenxcode_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context (thread or asyncio task).

    Use this instead of mutating ``os.environ["KENXCODE_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``KENXCODE_INTERACTIVE`` env var for legacy callers.
    """
    return _kenxcode_interactive_ctx.set("1" if interactive else "")


def reset_kenxcode_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_kenxcode_interactive_context`."""
    _kenxcode_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``KENXCODE_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _kenxcode_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("KENXCODE_INTERACTIVE")
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("KENXCODE_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("KENXCODE_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("KENXCODE_SESSION_PLATFORM", "") or ""
...
def _is_cron_approval_context() -> bool:
    ...
    try:
        ...
        return is_truthy_value(get_session_env("KENXCODE_CRON_SESSION", ""))
    except Exception:
        return env_var_enabled("KENXCODE_CRON_SESSION")


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set KENXCODE_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind KENXCODE_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds KENXCODE_SESSION_PLATFORM via
    ...
    if _is_cron_approval_context():
        return False
    if env_var_enabled("KENXCODE_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $KENXCODE_HOME, or by the resolved absolute
# active profile home path such as /home/kenxcode/.kenxcode/config.yaml. The
# resolved-absolute form is folded into the ~/.kenxcode/ patterns at detection
...
# go stale when KENXCODE_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_KENXCODE_ENV_PATH = (
    r'(?:~\/\.kenxcode/|'
    r'(?:\$home|\$\{home\})/\.kenxcode/|'
    r'(?:\$kenxcode_home|\$\{kenxcode_home\})/)'
    r'\.env\b'
)
# ~/.kenxcode/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _KENXCODE_ENV_PATH; matches the KENXCODE_HOME override form as
# well as ~/.kenxcode/.
_KENXCODE_CONFIG_PATH = (
    r'(?:~\/\.kenxcode/|'
    r'(?:\$home|\$\{home\})/\.kenxcode/|'
    r'(?:\$kenxcode_home|\$\{kenxcode_home\})/)'
    r'config\.yaml\b'
...
_SENSITIVE_WRITE_TARGET = (
    ...
    rf'{_KENXCODE_ENV_PATH}|'
    rf'{_KENXCODE_CONFIG_PATH}|'
    rf'{_SHELL_RC_FILES}|'
...
def _save_blocked_payload(command: str) -> Optional[str]:
    ...
    try:
        ...
        script_dir = get_kenxcode_home() / "cache" / "blocked-scripts"
        ...
        path.write_text(
            "#!/bin/bash\n"
            "# Auto-saved by KenXCode: this command exceeded the inline command\n"
...
DANGEROUS_PATTERNS = [
    ...
    # `kenxcode` and `gateway` (e.g. `kenxcode -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\bkenxcode\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart kenxcode gateway (kills running agents)"),
    (r'\bkenxcode\s+update\b', "kenxcode update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `kenxcode gateway restart`
    ...
    # a lifecycle command past the guard — same treatment as the `kenxcode ...
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart kenxcode-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart kenxcode-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(kenxcode|gateway|cli\.py)\b', "kill kenxcode/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill kenxcode` but not
    # `kill -9 $(pgrep -f kenxcode)` because the substitution is opaque
    ...
    # the `kenxcode gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.kenxcode.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(kenxcode|ai\.kenxcode)\b', "stop/restart kenxcode launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/KenXCode file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.kenxcode/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a KenXCode-managed security file (~/.kenxcode/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_KENXCODE_CONFIG_PATH}|{_KENXCODE_ENV_PATH})', "in-place edit of KenXCode config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_KENXCODE_CONFIG_PATH}|{_KENXCODE_ENV_PATH})', "in-place edit of KenXCode config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_KENXCODE_CONFIG_PATH}|{_KENXCODE_ENV_PATH})', "in-place edit of KenXCode config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.kenxcode/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / KENXCODE_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) KenXCode home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\kenxcode), so folding the user home
    # first would eat the prefix the KenXCode-home fold needs.
    command = _rewrite_resolved_kenxcode_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / KENXCODE_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.kenxcode``); the matched
...
def _rewrite_resolved_kenxcode_home(command: str) -> str:
    """Rewrite the resolved absolute KenXCode home prefix to ``~/.kenxcode/``.

    Resolves the active ``KENXCODE_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.kenxcode/`` so the static ``_KENXCODE_CONFIG_PATH`` / ``_KENXCODE_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/kenxcode/.kenxcode/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$KENXCODE_HOME``. Matches both POSIX and Windows separators. No-op when the
    path can't be resolved or doesn't appear.
    """
    try:
        from kenxcode_constants import get_kenxcode_home
        home = get_kenxcode_home().expanduser()
        candidates = [
            str(home),
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.kenxcode")
...
def _mask_quoted_newlines(command: str) -> str:
    ...
    start. That made any multi-line quoted argument (``kenxcode send`` message
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one KenXCode ad-hoc temp script."""
    try:
    ...
    if os.path.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"kenxcode-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode is derived from KenXCode YOLO.
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["KENXCODE_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "KENXCODE_SPINNER_PAUSE" in os.environ:
            del os.environ["KENXCODE_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    """Return whether one exact session bypasses KenXCode approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``KENXCODE_YOLO_MODE`` (frozen at import time
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            KENXCODE_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    if not is_cli and not is_gateway:
        ...
        if _is_cron_approval_context():
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set KENXCODE_INTERACTIVE or "
                "KENXCODE_GATEWAY_SESSION to answer the prompt.",
        ...
        logger.warning(
            "%s (pattern: %s): %s — set KENXCODE_INTERACTIVE or "
            "KENXCODE_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("KENXCODE_EXEC_ASK"):
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``KENXCODE_INTERACTIVE``) fails CLOSED — a plugin-
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("KENXCODE_EXEC_ASK")
        ...
        if _is_cron_approval_context():
            if _get_cron_approval_mode() == "deny":
                ...
                try:
                    ...
                    try:
                        from kenxcode_cli.config import load_config_readonly as _load_cfg
