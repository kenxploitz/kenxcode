...
Edit with `kenxcode config edit` or `kenxcode config set section.key value`.
Full reference: https://kenxcode-agent.nousresearch.com/docs/user-guide/configuration
...
`kenxcode config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `kenxcode tools` (interactive) or `kenxcode tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_KENXCODE_CORE_TOOLS` is the default bundle most platforms inherit).
