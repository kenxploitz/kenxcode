...
## Layer 1 — Is this thing a KenXCode plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the KenXCode process**. Its LLM calls go through KenXCode's already-configured provider. | Already uses Portal if user's KenXCode is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside KenXCode. That LLM call is already authenticated
via KenXCode's provider config. The plugin's own server auth (e.g.
...
What KenXCode Agent has that *feels* like OAuth — `kenxcode login --provider nous`
opening a browser, user signs in, token lands in `~/.kenxcode/auth.json` — is a
**KenXCode-specific browser flow**. Under the hood it produces a credential
KenXCode uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read KenXCode's existing Portal credential out of `~/.kenxcode/auth.json`.
...
rides on the credential KenXCode already holds.

Where this could live in KenXCode:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for KenXCode itself.
...
1. First decide: KenXCode plugin (runs inside KenXCode) or separate app? If plugin,
   it already uses Portal via KenXCode's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   KenXCode agent itself") is fully supported. KenXCode treats agent
...
pitch in the Tool Gateway announcement is specifically about KenXCode Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when KenXCode is configured to use Portal as its provider. It is
