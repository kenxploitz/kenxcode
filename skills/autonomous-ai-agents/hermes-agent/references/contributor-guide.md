...
For occasional contributors and PR authors. Full developer docs: https://kenxcode-agent.nousresearch.com/docs/developer-guide/
...
Config: `~/.kenxcode/config.yaml` (settings), `~/.kenxcode/.env` (API keys) — both under `$KENXCODE_HOME` when it is set.
...
`_KENXCODE_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_kenxcode_home()` for paths,
never hardcode `~/.kenxcode`. For custom/local-only tools, write a plugin in
`~/.kenxcode/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `kenxcode_cli/commands.py`
...
- Tests auto-redirect `KENXCODE_HOME` to temp dirs — never touch real `~/.kenxcode/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/kenxcode_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_kenxcode_home()` from `kenxcode_constants` for all paths (profile-safe)
