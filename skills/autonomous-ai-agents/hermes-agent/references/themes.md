# Themes / Skins — Author a KenXCode Color Theme

Author a KenXCode **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`kenxcode_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.kenxcode/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for KenXCode itself.
...
- Write access to the KenXCode home dir — `~/.kenxcode` by default, or `$KENXCODE_HOME`
  / the active profile's dir. Skins live in `<kenxcode-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `kenxcode config set`).
...
3. `write_file` it to `<kenxcode-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `kenxcode config set display.skin default` (or they can `/skin default`).
...
```
kenxcode skin set <key> <hex>      # e.g. kenxcode skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `kenxcode skin set background "#08201f"` changes
only the background; `kenxcode skin use <name>` / `kenxcode skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.kenxcode`** when a profile is active — resolve the real home
  from `$KENXCODE_HOME` first, falling back to `~/.kenxcode`.
...
- **Never hand-edit `config.yaml` to activate.** Use `kenxcode config set
...
- **You apply it, not the user.** `kenxcode config set display.skin <name>` is
...
- `read_file` the written `<kenxcode-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `kenxcode config get display.skin` and confirm it reports `<name>`.
