...
Write plugins for the KenXCode desktop app: statusbar items, layout panes,
...
system (`~/.kenxcode/plugins/`) is otherwise documented separately.
...
- The KenXCode desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$KENXCODE_HOME/desktop-plugins/` (usually
  `~/.kenxcode/desktop-plugins/`).

## How to Run

1. Create `$KENXCODE_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.kenxcode/...` by default, or `~/.kenxcode/profiles/<profile>/...` under a
...
The ONLY import surface is `@kenxcode/plugin-sdk` (plus `react` /
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.kenxcode/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@kenxcode/plugin-sdk`, `react`, and
