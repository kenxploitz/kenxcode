...
Author widget apps for the KenXCode TUI (`kenxcode --tui`): glanceable ambient
...
- The TUI must be in use (`kenxcode --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.kenxcode/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.kenxcode/logs/tui_gateway_crash.log` if a widget never appears.
