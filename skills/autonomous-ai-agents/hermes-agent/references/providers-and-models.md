...
Set via `kenxcode model` (picker) or `kenxcode setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://kenxcode-agent.nousresearch.com/docs/integrations/providers
...
| nous | OAuth device code | `kenxcode auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `kenxcode auth add openai-codex` |
| qwen-oauth | OAuth | `kenxcode auth add qwen-oauth` |
| minimax-oauth | OAuth | `kenxcode auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`kenxcode auth`).
Fallback chain when the primary fails: `kenxcode fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`kenxcode_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   kenxcode config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
