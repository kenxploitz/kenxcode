...
Ledger path resolution (first wins):
  --ledger PATH
  $KENXCODE_CITATION_LEDGER
  $KENXCODE_HOME/cache/citations/ledger.json
"""
...
from _kenxcode_home import get_kenxcode_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("KENXCODE_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_kenxcode_home() / "cache" / "citations" / "ledger.json"
