#!/usr/bin/env bash
# GitHub environment detection helper for KenXCode Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _kenxcode_env="${KENXCODE_HOME:-$HOME/.kenxcode}/.env"; [ -f "$_kenxcode_env" ] && grep -q "^GITHUB_TOKEN=" "$_kenxcode_env" 2>/dev/null; then
