...
## 5. KenXCode Configuration

### Config location

`$KENXCODE_HOME/config.yaml` (defaults to `~/.kenxcode/config.yaml` when `KENXCODE_HOME` is unset)
...
Restart the KenXCode session for changes to take effect. The MCP connection is
