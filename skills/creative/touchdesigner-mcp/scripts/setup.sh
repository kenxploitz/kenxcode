...
KENXCODE_HOME_DIR="${KENXCODE_HOME:-$HOME/.kenxcode}"
KENXCODE_CFG="${KENXCODE_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure KenXCode config has twozero_td MCP entry ──
if [[ ! -f "$KENXCODE_CFG" ]]; then
    echo -e " ${FAIL} KenXCode config not found at ${KENXCODE_CFG}"
    manual_steps+=("Create ${KENXCODE_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$KENXCODE_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in KenXCode config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to KenXCode config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$KENXCODE_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${KENXCODE_CFG} manually"); }
    manual_steps+=("Restart KenXCode session to pick up config change")
fi
