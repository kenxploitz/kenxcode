#!/usr/bin/env python3
"""Google Workspace API CLI for KenXCode Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing KenXCode-facing JSON contract and falls back to the Python client
...
from _kenxcode_home import get_kenxcode_home
...
def _gws_binary() -> str | None:
    override = os.getenv("KENXCODE_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for KenXCode Agent")
