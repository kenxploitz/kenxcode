"""
KenXCode MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (KenXCode-specific extra)

Usage:
    kenxcode mcp serve
    kenxcode mcp serve --verbose
...
logger = logging.getLogger("kenxcode.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using KENXCODE_HOME."""
    try:
        from kenxcode_constants import get_kenxcode_home
        return get_kenxcode_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("KENXCODE_HOME", Path.home() / ".kenxcode")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from kenxcode_constants import get_kenxcode_home
        directory_file = get_kenxcode_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("KENXCODE_HOME", Path.home() / ".kenxcode")
...
class EventBridge:
    ...
    This is the KenXCode equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _establish_baseline(self) -> None:
        ...
        try:
            from kenxcode_constants import get_kenxcode_home
            db_file = get_kenxcode_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("KENXCODE_HOME", Path.home() / ".kenxcode")) / "state.db"
        try:
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the KenXCode MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "kenxcode",
        instructions=(
            "KenXCode Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the KenXCode MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
