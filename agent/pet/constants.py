...
row taxonomy is inferred from the atlas shape so KenXCode can render both legacy
...
# rows (1536x1872), while older KenXCode/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are KenXCode' activity state names. They are not always identical to the
...
# Legacy KenXCode/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical KenXCode activity names -> accepted row-name aliases in descending
