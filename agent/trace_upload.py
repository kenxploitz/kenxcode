"""Upload a KenXCode session transcript to Hugging Face as an agent trace.

KenXCode stores sessions in its own SQLite store (``kenxcode_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``kenxcode trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through KenXCode' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "kenxcode-traces"
_KENXCODE_VERSION = "kenxcode-agent"
...
# ---------------------------------------------------------------------------
# Conversion: KenXCode OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses KenXCode' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render KenXCode conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.kenxcode/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `kenxcode trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/kenxcode-traces`` dataset. Returns a
