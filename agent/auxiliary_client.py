...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.kenxcode/auth.json active provider)
...
from kenxcode_constants import OPENROUTER_BASE_URL
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_ca_cert`` / ``ssl_verify`` config and the ``KENXCODE_CA_BUNDLE`` /
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `kenxcode update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # KenXCode owns auxiliary retry + provider/model fallback policy (the
    ...
    # KenXCode sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let KenXCode control the budget; explicit callers can still
    # override via kwargs.
...
def _run_protected_sync_provider_call(
    ...
    threading.Thread(
        ...
        name="kenxcode-protected-aux-provider",
        daemon=True,
    ).start()
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before KenXCode triggers summarization.  Higher values delay
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://kenxcode-agent.nousresearch.com",
    "X-Title": "KenXCode Agent",
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``KENXCODE_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``KENXCODE_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("KENXCODE_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("KENXCODE_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "KenXCodeAgent",
}
...
from kenxcode_cli import __version__ as _KENXCODE_VERSION

_AI_GATEWAY_HEADERS = {
    "HTTP-Referer": "https://kenxcode-agent.nousresearch.com",
    "X-Title": "KenXCode Agent",
    "User-Agent": f"KenXCodeAgent/{_KENXCODE_VERSION}",
}
...
# in lockstep with kenxcode_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    ...
    Computed at call time so a hot-reloaded ``kenxcode_cli.__version__`` is
...
_AUTH_JSON_PATH = get_kenxcode_home() / "auth.json"
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (KenXCode Agent)",
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "nous":
        ...
        from kenxcode_cli.auth import _nous_inference_env_override
...
def _nous_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("KENXCODE_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        #   - ``_``-prefixed keys: private KenXCode plumbing (_reasoning_config
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``kenxcode_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.kenxcode/auth.json for an active Nous provider.
...
def _nous_api_key(provider: dict) -> str:
    ...
    from kenxcode_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from kenxcode_cli.auth import _agent_key_is_usable
...
def _resolve_nous_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from kenxcode_cli.auth import resolve_nous_runtime_credentials

        creds = resolve_nous_runtime_credentials(
            timeout_seconds=env_float("KENXCODE_NOUS_TIMEOUT_SECONDS", 15),
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``kenxcode auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``kenxcode_cli.auth``'s singleton runtime resolver for older
    ...
    try:
        from kenxcode_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from KenXCode auth store.
    ...
    try:
        from kenxcode_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from kenxcode_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from kenxcode_cli.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from kenxcode_cli.models import copilot_default_headers
...
def _aux_openrouter_settings() -> Tuple[bool, str]:
    ...
    """
    try:
        from kenxcode_cli.config import cfg_get, load_config_readonly
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    model = _NOUS_MODEL
    try:
        from kenxcode_cli.models import get_nous_recommended_aux_model
...
def _resolve_moa_aggregator(preset_name: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        from kenxcode_cli.config import load_config
        from kenxcode_cli.moa_config import resolve_moa_preset
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `kenxcode setup` or `kenxcode model` and enter a valid http(s) base URL."
        ) from exc
...
def _build_xai_oauth_aux_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    from tools.xai_http import kenxcode_xai_default_headers

    real_client = _create_openai_client(
        api_key=api_key,
        base_url=base_url,
        default_headers=kenxcode_xai_default_headers(),
    )
    return CodexAuxiliaryClient(real_client, model), model
...
def _try_azure_foundry(
    *,
    ...
    :func:`kenxcode_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from kenxcode_cli.runtime_provider import _resolve_azure_foundry_runtime
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second kenxcode
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``kenxcode config aux reset``)."""
...
def _nous_portal_account_has_fresh_paid_access() -> bool:
    ...
    try:
        from kenxcode_cli.nous_account import get_nous_portal_account_info
...
def _refresh_provider_credentials(provider: str) -> bool:
    ...
    try:
        ...
        if normalized == "openai-codex":
            from kenxcode_cli.auth import resolve_codex_runtime_credentials
...
def _fallback_destination(
    ...
    attached = getattr(fb_client, "_kenxcode_fallback_destination", None)
...
def _fallback_entry_api_key(entry: Dict[str, Any]) -> Optional[str]:
    ...
    gateway multiplexing (see ``kenxcode_cli.fallback_config.resolve_entry_api_key``).
    """
...
def _resolve_fallback_entry(entry: Dict[str, Any]) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if client is not None:
        try:
            client._kenxcode_fallback_destination = _fallback_destination_from_entry(
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into KenXCode' built-in discovery
...
def _resolve_auto(
    ...
    #    scenario where a user switches providers via `kenxcode model` but the
    #    old OPENAI_BASE_URL lingers in ~/.kenxcode/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: kenxcode model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.kenxcode/.env",
    ...
    if (main_provider and main_model
        ...
        if runtime_base_url and main_provider == "custom":
            ...
            try:
                from kenxcode_cli.runtime_provider import _get_named_custom_provider
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        from kenxcode_cli.copilot_auth import copilot_request_headers
        ...
        async_kwargs["default_headers"] = kenxcode_xai_default_headers()
    else:
    ...
    # See _create_openai_client: disable SDK-internal retries so KenXCode owns
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: kenxcode auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: kenxcode model)")
                return None, None
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: kenxcode model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: kenxcode doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("kenxcode_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            headers.update(kenxcode_xai_default_headers())
        else:
            # Fall back to profile.default_headers for providers that declare
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from kenxcode_cli.models import _should_use_copilot_responses_api
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`kenxcode_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def _build_call_kwargs(
    ...
    # Anthropic Messages adapters translate KenXCode reasoning into native
...
def call_llm(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional KenXCode reasoning config for direct model calls
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is not None:
                    ...
                    raise RuntimeError(
                        ...
                        f"variable, or switch to a different provider with `kenxcode model`."
                    )
