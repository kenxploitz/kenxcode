...
upstream prefix cache warm.  See ``kenxcode-agent-dev``'s
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``KENXCODE_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("KENXCODE_DESKTOP_TERMINAL")):
        return hint
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  KenXCode never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want KENXCODE_HOME persona while keeping
    ...
    # Pointer to the kenxcode-agent skill + docs for user questions about KenXCode itself.
    stable_parts.append(KENXCODE_AGENT_HELP_GUIDANCE)
    ...
    # KENXCODE_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base KenXCode, any interactive coding surface in a code
    ...
    # Active-profile hint — names the KenXCode profile the agent is running
    # under so it doesn't conflate ~/.kenxcode/skills/ (default profile) with
    # ~/.kenxcode/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        post_workspace_parts.append(
            "Active KenXCode profile: default. Other profiles (if any) live "
            "under " + str(get_kenxcode_home()) + "/profiles/<name>/. Each profile has its own "
        ...
        post_workspace_parts.append(
            f"Active KenXCode profile: {active_profile}. This session reads "
            f"and writes {get_kenxcode_home()}/profiles/{active_profile}/. The default "
            f"profile's data lives at {get_kenxcode_home()}/skills/, {get_kenxcode_home()}/plugins/, "
            f"{get_kenxcode_home()}/cron/, {get_kenxcode_home()}/memories/ — those belong to a "
    ...
    if not agent.skip_context_files:
        ...
        # (developing KenXCode). Every other surface (desktop chat panel,
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — KenXCode never rebuilds or reinjects parts of it
