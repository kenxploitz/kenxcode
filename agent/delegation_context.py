...
The parent KenXCode process may itself be a Kanban dispatcher worker with
KENXCODE_KANBAN_* variables in process env. delegate_task children run inside the
...
DELEGATED_CHILD_ENV_MARKER = "KENXCODE_DELEGATED_CHILD_CONTEXT"
...
def delegated_child_subprocess_env(
    ...
    parent dispatcher ``KENXCODE_KANBAN_*`` vars while losing the ContextVar in
