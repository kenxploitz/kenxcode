...
from kenxcode_constants import get_config_path, get_skills_dir, is_termux
...
# Org mirrors live under ~/.kenxcode/skills/_org/<org_id>/. Resolution is
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``KENXCODE_KANBAN_TASK`` / ``KENXCODE_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("KENXCODE_KANBAN_TASK") or os.getenv("KENXCODE_KANBAN_BOARD"):
            result = True
        else:
            try:
        ...
        # The KenXCode Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``kenxcode_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``KENXCODE_PLATFORM`` or
            ``KENXCODE_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("KENXCODE_PLATFORM")
        or get_session_env("KENXCODE_SESSION_PLATFORM")
    )
...
# which becomes the dominant cost of ``kenxcode`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.kenxcode/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``kenxcode`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against KENXCODE_HOME, not cwd
        if not p.is_absolute():
            p = (kenxcode_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.kenxcode/skills/`` first, then external.
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.kenxcode/skills/`` (including via symlinks) or configured
    ...
    # ~/.kenxcode/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: KenXCode can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    kenxcode = metadata.get("kenxcode") or {}
    if not isinstance(kenxcode, dict):
        kenxcode = {}
    return {
        "fallback_for_toolsets": kenxcode.get("fallback_for_toolsets", []),
        "requires_toolsets": kenxcode.get("requires_toolsets", []),
        "fallback_for_tools": kenxcode.get("fallback_for_tools", []),
        "requires_tools": kenxcode.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    kenxcode = metadata.get("kenxcode")
    if not isinstance(kenxcode, dict):
        return []
    raw = kenxcode.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes KenXCode metadata, VCS, virtualenv/dependency, cache, and skill
