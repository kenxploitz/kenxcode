...
projection into KenXCode' display, approval bridging, transcript projection into
...
"codex_app_server"`. KenXCode' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 KenXCode secret — gateway
        ...
        spawn_env = kenxcode_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("KENXCODE_KANBAN_TASK"):
            kanban_db = spawn_env.get("KENXCODE_KANBAN_DB")
            kanban_root = (
                os.path.dirname(kanban_db)
                if kanban_db
                else spawn_env.get(
                    "KENXCODE_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("KENXCODE_HOME", os.path.expanduser("~/.kenxcode")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "kenxcode",
        client_title: str = "KenXCode Agent",
