...
Every KenXCode request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to KenXCode Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=kenxcode-agent",
        "client=kenxcode-client-v<__version__>",
    ]

The version is sourced live from ``kenxcode_cli.__version__`` so it auto-aligns
...
``kenxcode_cli.__version__`` is the canonical source of truth.
"""
...
def _kenxcode_version() -> str:
    """Return the current KenXCode release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``kenxcode_cli`` cannot be imported (should
...
def kenxcode_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=kenxcode-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=kenxcode-client-v{_kenxcode_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a KenXCode session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    KenXCode conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.kenxcode/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific KenXCode
    ...
    """
    tags = ["product=kenxcode-agent", kenxcode_client_tag()]
