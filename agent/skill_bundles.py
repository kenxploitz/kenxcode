...
Storage
-------
Bundles live in ``~/.kenxcode/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``KENXCODE_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``kenxcode bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under KENXCODE_HOME.

    Honors ``KENXCODE_BUNDLES_DIR`` for tests; falls back to
    ``<KENXCODE_HOME>/skill-bundles``.
    """
    override = os.environ.get("KENXCODE_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_kenxcode_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `kenxcode bundles` CLI subcommand.
# ---------------------------------------------------------------------------
