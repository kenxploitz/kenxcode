#!/usr/bin/env python3
"""Canvas LMS API CLI for KenXCode Agent.
...
def _check_config():
    ...
    if missing:
        kenxcode_env = os.path.join(
            os.environ.get("KENXCODE_HOME", os.path.expanduser("~/.kenxcode")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {kenxcode_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for KenXCode Agent"
    )
