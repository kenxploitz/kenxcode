...
The design is **KenXCode-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
kenxcode skills install official/security/unbroker
```

Then start a new KenXCode session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a KenXCode session:
...
- **PII handling.** Dossiers live under the KenXCode home directory (`0600`, optionally
