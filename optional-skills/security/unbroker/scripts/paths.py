...
All per-subject data lives under PDD_DATA_DIR (default: $KENXCODE_HOME/unbroker),
which is the same trust boundary KenXCode uses for .env and OAuth tokens.
"""
...
def kenxcode_home() -> Path:
    return Path(os.environ.get("KENXCODE_HOME") or (Path.home() / ".kenxcode"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else kenxcode_home() / "unbroker"
