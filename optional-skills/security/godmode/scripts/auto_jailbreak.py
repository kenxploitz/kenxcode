...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("KENXCODE_HOME", os.path.expanduser("~/.kenxcode")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("KENXCODE_HOME", Path.home() / ".kenxcode")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# KenXCode config paths
# ═══════════════════════════════════════════════════════════════════

KENXCODE_HOME = Path(os.getenv("KENXCODE_HOME", Path.home() / ".kenxcode"))
CONFIG_PATH = KENXCODE_HOME / "config.yaml"
PREFILL_PATH = KENXCODE_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/KenXCode models — already uncensored, just needs clean prompt
    "kenxcode": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "kenxcode" in model_lower or "nous" in model_lower:
        return "kenxcode"
...
def _get_current_model() -> tuple:
    """Read current model and provider from KenXCode config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.kenxcode/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart KenXCode for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart KenXCode for changes to take effect.")
