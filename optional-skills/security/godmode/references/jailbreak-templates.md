...
## 5. GODMODE FAST — KenXCode 4 405B (Zero Refusal)

**Model:** `nousresearch/kenxcode-4-405b`
**Strategy:** KenXCode models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with KenXCode Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.kenxcode/config.yaml`:
