...
### 1. Start order: editor first, KenXCode second

KenXCode probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW KenXCode session.
...
restart the editor, restart the KenXCode session.
...
server fails to bind (Output Log) or KenXCode' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.kenxcode/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for KenXCode

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. KenXCode' connection
lives in `~/.kenxcode/config.yaml` via `kenxcode mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms KenXCode — just don't
mistake it for the KenXCode setup step.
...
explicitly warns against overlapping calls. KenXCode executes same-turn tool
...
### 9. Timeouts: KenXCode gives up before Unreal does

KenXCode' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If KenXCode runs elsewhere (SSH backend,
