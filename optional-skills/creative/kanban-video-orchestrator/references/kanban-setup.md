...
the actual `setup.sh` that creates the project workspace, configures KenXCode
...
2. **Create profiles** — `kenxcode profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.kenxcode/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `kenxcode kanban create` assigned to the director
...
```bash
kenxcode profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.kenxcode/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.kenxcode/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most KenXCode
...
Each profile gets a `SOUL.md` at `~/.kenxcode/profiles/<name>/SOUL.md` that
...
```bash
kenxcode kanban create "Direct production of <video title>" \
    --assignee director \
...
the KenXCode `.env` (`${KENXCODE_HOME:-$HOME/.kenxcode}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _kenxcode_env="${KENXCODE_HOME:-$HOME/.kenxcode}/.env"
    if grep -q "^${var}=" "$_kenxcode_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_kenxcode_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_kenxcode_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY kenxcode ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY kenxcode OPENROUTER_API_KEY || exit 1
# ...
```
