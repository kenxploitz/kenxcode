...
Maps each role archetype to the KenXCode skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public kenxcode-agent
...
## KenXCode skills relevant to video production

### Visual / rendering skills (`kenxcode-agent/skills/creative/`)
...
### Design / pre-production skills (`kenxcode-agent/skills/creative/`)
...
### Audio / media skills (`kenxcode-agent/skills/creative/` + `skills/media/`)
...
These are **not** KenXCode skills but external CLIs / APIs that profiles invoke.
...
## Built-in KenXCode tools for media review

These are native KenXCode tools — not invoked via terminal but through their own
...
key is present in `${KENXCODE_HOME:-~/.kenxcode}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every KenXCode profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${KENXCODE_HOME:-~/.kenxcode}/.env` → environment variable.
...
When a new KenXCode-public video skill ships:
