...
kenxcode kanban watch --tenant <project-slug>

# Snapshot of the board
kenxcode kanban list --tenant <project-slug>
kenxcode kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
kenxcode kanban stats --tenant <project-slug>

# Visual dashboard (browser)
kenxcode dashboard

# Inspect a specific task (includes comments + events)
kenxcode kanban show <task-id>

# Follow a single task's event stream
kenxcode kanban tail <task-id>
```

Verify available subcommands with `kenxcode kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `kenxcode kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `kenxcode kanban block <id>` then `kenxcode kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `kenxcode kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `kenxcode kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `kenxcode profile list` (profile exists?), `kenxcode status` (gateway/dispatcher up?), and verify tenant. |
...
kenxcode kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
kenxcode kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(kenxcode kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
kenxcode kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
kenxcode kanban block <task-id>
kenxcode kanban archive <task-id>

# Diagnose what happened
kenxcode kanban show <task-id>      # task body, comments, recent events
kenxcode kanban tail <task-id>      # follow the live event stream
kenxcode kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `kenxcode kanban create` for the director
...
while true; do
    clear
    kenxcode kanban list --tenant <slug>
    echo "---"
    kenxcode kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `kenxcode kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `kenxcode profile list` and start
