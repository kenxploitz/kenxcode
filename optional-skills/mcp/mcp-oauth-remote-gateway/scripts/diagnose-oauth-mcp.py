...
Usage:
  ...
  <server_name> matches the files in $KENXCODE_HOME/mcp-tokens/<server>.json etc.
...
def _kenxcode_home():
    # Prefer KenXCode' own resolver (profile-safe); fall back to env then ~/.kenxcode.
    try:
        from kenxcode_constants import get_kenxcode_home
        return str(get_kenxcode_home())
    except Exception:
        return os.environ.get("KENXCODE_HOME") or os.path.expanduser("~/.kenxcode")


def _tokens_dir():
    return os.path.join(_kenxcode_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "kenxcode-diag", "version": "1.0"}},
        },
        headers={
