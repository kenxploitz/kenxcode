#!/usr/bin/env python3
"""OpenClaw -> KenXCode migration helper.

This script migrates the parts of an OpenClaw user footprint that map cleanly
into KenXCode Agent, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the OpenClaw persona file into KenXCode.",
    },
    "workspace-agents": {
    ...
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into KenXCode memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into KenXCode memories.",
    },
    "messaging-settings": {
        "label": "Messaging settings",
        "description": "Import KenXCode-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of KenXCode-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
        "label": "Command allowlist",
        "description": "Merge OpenClaw exec approval patterns into KenXCode command_allowlist.",
    },
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.kenxcode/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.kenxcode/tts/.",
    },
    "discord-settings": {
        "label": "Discord settings",
        "description": "Import Discord bot token and allowlist into KenXCode .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into KenXCode .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into KenXCode .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into KenXCode .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into KenXCode .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into KenXCode config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into KenXCode config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared OpenClaw skills from ~/.openclaw/skills/ into KenXCode.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into KenXCode MEMORY.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from OpenClaw into KenXCode config.yaml.",
    },
    "plugins-config": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'kenxcode cron'.",
    },
    "hooks-config": {
    ...
    "agent-config": {
        ...
        "description": "Import agent defaults (compaction, context, thinking) into KenXCode config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "session-config": {
        ...
        "description": "Import session reset policies (daily/idle) into KenXCode session_reset config.",
    },
    "full-providers": {
        "label": "Full model provider definitions",
        "description": "Import custom model providers (baseUrl, apiType, headers) into KenXCode custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into KenXCode config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into KenXCode config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into KenXCode config.yaml approvals section.",
    },
    "memory-backend": {
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ConfigReadError(
            ...
            f"({exc}). Fix it with `kenxcode config edit` (or move it aside), then "
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`kenxcode config edit` (or move it aside), then re-run the migration."
        )
    return data
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    ...
    ``~/.kenxcode/config.yaml`` into a dotfiles repo or profile package.
    """
    if yaml is None:
        raise RuntimeError("PyYAML is required to update KenXCode config.yaml")
    ensure_parent(path)
...
# ── Brand rewriting ─────────────────────────────────────────
# Replace OpenClaw brand names with KenXCode in migrated text so that
# memory entries, user profiles, SOUL.md, and workspace instructions
# read as self-referential to the new agent identity.
#
# Case-preserving: ``OpenClaw`` → ``KenXCode`` (prose), but lowercase matches
# like ``openclaw`` → ``kenxcode`` (so filesystem paths like ``~/.openclaw``
# become ``~/.kenxcode`` — the real KenXCode home — not the broken ``~/.KenXCode``).
_REBRAND_PATTERNS: List[Tuple[re.Pattern, str]] = [
    (re.compile(r'\bOpen[\s-]?Claw\b', re.IGNORECASE), 'KenXCode'),
    (re.compile(r'\bClawdBot\b', re.IGNORECASE), 'KenXCode'),
    (re.compile(r'\bMoltBot\b', re.IGNORECASE), 'KenXCode'),
]
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``KenXCode`` but maps ``openclaw`` → ``kenxcode`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.kenxcode/config.yaml`` (the real KenXCode home) instead of the broken
...
def rebrand_text(text: str) -> str:
    """Replace OpenClaw / ClawdBot / MoltBot brand names with KenXCode.
...
def parse_existing_memory_entries(path: Path) -> List[str]:
    """Parse a DESTINATION KenXCode memory store (memories/MEMORY.md, USER.md).
...
def write_report(output_dir: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# OpenClaw -> KenXCode Migration Report",
        "",
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-kenxcode/apply.ts "blocked by earlier
    ...
    # Option ids that mutate the KenXCode config.yaml file.  Once any one of
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-kenxcode/plan.ts warnings[].
    ...
    def _build_next_steps(self, summary: Dict[str, int]) -> List[str]:
        ...
        if summary.get("migrated", 0) > 0:
            ...
            steps.append(
                "Start a new KenXCode session (or /reset) to pick up the imported config."
            )
        if summary.get("conflict", 0) > 0:
            steps.append(
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "KenXCode config.yaml does not exist yet")
            return
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # that path will be stale after migration and would cause the KenXCode
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No KenXCode-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted KenXCode-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in {"file", "exec"}:
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: kenxcode config set",
                        )
                    continue
        ...
        for oc_key, kenxcode_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and kenxcode_key not in secret_additions:
                secret_additions[kenxcode_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, kenxcode_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and kenxcode_key not in secret_additions:
                        secret_additions[kenxcode_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        kenxcode_config = load_yaml_file(destination)
        current_model = kenxcode_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = kenxcode_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                kenxcode_config["model"] = {"default": model_str}
            dump_yaml_file(destination, kenxcode_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(provider, str) and provider in {"elevenlabs", "openai", "edge", "microsoft"}:
            # OpenClaw renamed "edge" to "microsoft"; KenXCode still uses "edge"
        ...
        kenxcode_config = load_yaml_file(destination)
        existing_tts = kenxcode_config.get("tts", {})
        ...
        if self.execute:
            ...
            kenxcode_config["tts"] = merged_tts
            dump_yaml_file(destination, kenxcode_config)
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct KenXCode destination; archived for manual review")
        ...
        partially_extracted = [
            ("openclaw.json", "Selected KenXCode-compatible values were extracted; raw OpenClaw config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected KenXCode-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        kenxcode_cfg_path = self.target_root / "config.yaml"
        kenxcode_cfg = load_yaml_file(kenxcode_cfg_path)
        existing_mcp = kenxcode_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            if name in existing_mcp and not self.overwrite:
                self.record("mcp-servers", f"mcp.servers.{name}", f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in KenXCode config")
                continue

            kenxcode_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                kenxcode_srv["command"] = srv["command"]
                if srv.get("args"):
                    kenxcode_srv["args"] = srv["args"]
                if srv.get("env"):
                    kenxcode_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    kenxcode_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                kenxcode_srv["url"] = srv["url"]
                if srv.get("headers"):
                    kenxcode_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    kenxcode_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                kenxcode_srv["enabled"] = False
            if srv.get("timeout"):
                kenxcode_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                kenxcode_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                kenxcode_srv["tools"] = {}
                if tools_cfg.get("include"):
                    kenxcode_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    kenxcode_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
            sampling = srv.get("sampling")
            if sampling and isinstance(sampling, dict):
                kenxcode_srv["sampling"] = {
                    k: v for k, v in {
            ...
            existing_mcp[name] = kenxcode_srv
            added += 1
        ...
        if added > 0 and self.execute:
            self.maybe_backup(kenxcode_cfg_path)
            kenxcode_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(kenxcode_cfg_path, kenxcode_cfg)
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'kenxcode cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = kenxcode_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> KenXCode reasoning_effort
        ...
        if compaction:
            compression = kenxcode_cfg.get("compression") or {}
            ...
            if compaction.get("model"):
                aux = kenxcode_cfg.setdefault("auxiliary", {})
                aux_comp = aux.setdefault("compression", {})
                aux_comp["model"] = compaction["model"]
            kenxcode_cfg["compression"] = compression
            changes = True

        # Map humanDelay
        human_delay = defaults.get("humanDelay") or {}
        if human_delay:
            hd = kenxcode_cfg.get("human_delay") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'kenxcode gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("KENXCODE_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        kenxcode_cfg_path = self.target_root / "config.yaml"
        kenxcode_cfg = load_yaml_file(kenxcode_cfg_path)
        sr = kenxcode_cfg.get("session_reset") or {}
        changes = False
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        kenxcode_cfg_path = self.target_root / "config.yaml"
        kenxcode_cfg = load_yaml_file(kenxcode_cfg_path)
        custom_providers = kenxcode_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(kenxcode_cfg_path)
            kenxcode_cfg["custom_providers"] = custom_providers
            dump_yaml_file(kenxcode_cfg_path, kenxcode_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map Discord-specific settings to KenXCode config
        discord_cfg = channels.get("discord") or {}
        if discord_cfg:
            kenxcode_cfg_path = self.target_root / "config.yaml"
            kenxcode_cfg = load_yaml_file(kenxcode_cfg_path)
            discord_kenxcode = kenxcode_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_kenxcode["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_kenxcode["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                kenxcode_cfg["discord"] = discord_kenxcode
                dump_yaml_file(kenxcode_cfg_path, kenxcode_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        kenxcode_cfg_path = self.target_root / "config.yaml"
        kenxcode_cfg = load_yaml_file(kenxcode_cfg_path)
        browser_kenxcode = kenxcode_cfg.get("browser") or {}
        changed = False

        # Map fields that have KenXCode equivalents
        if browser.get("cdpUrl"):
            browser_kenxcode["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_kenxcode["headless"] = browser["headless"]
            changed = True

        if changed:
            kenxcode_cfg["browser"] = browser_kenxcode
            if self.execute:
                self.maybe_backup(kenxcode_cfg_path)
                dump_yaml_file(kenxcode_cfg_path, kenxcode_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            kenxcode_mode = mode_map.get(mode, "manual")
            kenxcode_cfg.setdefault("approvals", {})["mode"] = kenxcode_mode
            if self.execute:
                self.maybe_backup(kenxcode_cfg_path)
                dump_yaml_file(kenxcode_cfg_path, kenxcode_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{kenxcode_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        if not self.output_dir:
            return
        notes = [
            "# OpenClaw -> KenXCode Migration Notes",
            "",
        ...
        if archived:
            notes.extend([
                ...
                "direct 1:1 mapping in KenXCode. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing KenXCode Config Not Overwritten)",
                "",
                "These items already existed in your KenXCode config. Re-run with",
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the KenXCode agent discovers these",
            "directories, it may read/write to them instead of the KenXCode state, causing",
            "confusion (e.g., cron jobs reading a different todo list than interactive sessions).",
            "",
            "**Strongly recommended:** Run `kenxcode claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `kenxcode claw cleanup` to fix it.",
            "",
            "## KenXCode-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `kenxcode claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `kenxcode setup` to configure any remaining settings",
            "- Run `kenxcode mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `kenxcode cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `kenxcode cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `kenxcode gateway install` if you need the gateway service",
            "- Review `~/.kenxcode/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into KenXCode Agent.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=os.environ.get("KENXCODE_HOME") or str(Path.home() / ".kenxcode"), help="KenXCode home directory")
    parser.add_argument(
        "--workspace-target",
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing KenXCode targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of KenXCode-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
...
def main() -> int:
    ...
    print()
    print("  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║   OpenClaw -> KenXCode Migration   [{mode_label:>8s}]   ║")
    print("  ╠══════════════════════════════════════════════════════╣")
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.kenxcode/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.kenxcode/config.yaml")
        print("    2. Run: kenxcode mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: kenxcode cron")
