...
$KENXCODE_OSINT_CACHE/icij/ (default: ~/.cache/kenxcode-osint/icij/) and search
...
def _cache_dir() -> Path:
    base = os.environ.get("KENXCODE_OSINT_CACHE")
    if base:
        return Path(base) / "icij"
    return Path.home() / ".cache" / "kenxcode-osint" / "icij"
...
def _download(dest: Path, force: bool = False) -> Path:
    ...
    req = urllib.request.Request(
        BULK_URL,
        headers={"User-Agent": "kenxcode-agent osint-investigation skill"},
    )
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $KENXCODE_OSINT_CACHE/icij or ~/.cache/kenxcode-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",
