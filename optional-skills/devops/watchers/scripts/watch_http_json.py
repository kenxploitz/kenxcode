...
Usage (via cron with --no-agent):
    ...
    kenxcode cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$KENXCODE_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "KenXCode-Watcher/1.0"})
