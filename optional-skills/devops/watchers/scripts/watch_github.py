...
Usage (via cron with --no-agent):
    ...
    kenxcode cron create kenxcode-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$KENXCODE_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name kenxcode-issues --repo NousResearch/kenxcode-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the KenXCode .env file
(``${KENXCODE_HOME:-~/.kenxcode}/.env``) to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "KenXCode-Watcher/1.0",
    }
