# OpenAI-Compatible API Server for KenXCode Agent
...
SSE streaming. By exposing this endpoint, kenxcode-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  kenxcode-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.kenxcode/.env`
2. Run `kenxcode gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with kenxcode-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns kenxcode-agent as a model) |
...
Authorization: Bearer kenxcode-api-key-here
...
But kenxcode-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "kenxcode-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured kenxcode-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of kenxcode-agent's core prompt
...
Once implemented, kenxcode-agent works as a drop-in backend for:
