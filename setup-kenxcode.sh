...
# 5. Symlinks the 'kenxcode' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ KenXCode Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/kenxcode-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink kenxcode into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up kenxcode command..."

KENXCODE_BIN="$SCRIPT_DIR/venv/bin/kenxcode"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$KENXCODE_BIN" "$COMMAND_LINK_DIR/kenxcode"
echo -e "${GREEN}✓${NC} Symlinked kenxcode → $COMMAND_LINK_DISPLAY_DIR/kenxcode"
...
# ============================================================================
# Seed bundled skills into ~/.kenxcode/skills/
# ============================================================================

KENXCODE_SKILLS_DIR="${KENXCODE_HOME:-$HOME/.kenxcode}/skills"
mkdir -p "$KENXCODE_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.kenxcode/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$KENXCODE_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  kenxcode status        # Check configuration"
if is_termux; then
    echo "  kenxcode gateway       # Run gateway in foreground"
else
    echo "  kenxcode gateway install # Install gateway service (messaging + cron)"
fi
echo "  kenxcode cron list     # View scheduled jobs"
echo "  kenxcode doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m kenxcode_cli.main setup
fi
