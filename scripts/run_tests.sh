#!/usr/bin/env bash
# Canonical test runner for kenxcode-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.kenxcode/...)
#
# Usage:
...
# (KENXCODE_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.kenxcode/kenxcode-agent/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.kenxcode/kenxcode-agent/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${KENXCODE_PYTHON:-}" ] && [ -x "$KENXCODE_PYTHON" ] \
    && "$KENXCODE_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: KENXCODE_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `kenxcode` binary rather
  # than the devShell hook.
  PYTHON="$KENXCODE_PYTHON"
  echo "▶ no local venv — using Nix dev venv via KENXCODE_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and KENXCODE_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.kenxcode/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.kenxcode"
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${KENXCODE_RUN_SLOW_PET_TESTS:+KENXCODE_RUN_SLOW_PET_TESTS="$KENXCODE_RUN_SLOW_PET_TESTS"} \
  ${KENXCODE_E2E_BROWSER:+KENXCODE_E2E_BROWSER="$KENXCODE_E2E_BROWSER"} \
