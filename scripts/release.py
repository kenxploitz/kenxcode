...
VERSION_FILE = REPO_ROOT / "kenxcode_cli" / "__init__.py"
...
LEGACY_AUTHOR_MAP = {
    ...
    "pi@kenxcode.local": "Elektrofussel",  # PR #61675 salvage (honcho: defaultHost + private-range local URL detection)
    ...
    "kavi@local.kenxcode": "kavioavio",  # Issue #46544 / PR #47705 evolution (smart DENY exact-operation owner override)
    ...
    "sunsky.lau@gmail.com": "liuhao1024",  # PR #56993 salvage (gateway: process-level KENXCODE_HOME for pid/lock/status identity files; #56986)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (kenxcode journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "kenxcode.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.kenxcode/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "brett@personalfinancelab.com": "brett539",  # PR #49369 salvage (cap Telegram initialize() with asyncio.wait_for(KENXCODE_TELEGRAM_INIT_TIMEOUT, default 30s) per attempt so an unreachable fallback-IP connect chain can't block gateway startup indefinitely; add WARNING progress logs before DoH discovery and each connect attempt)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows kenxcode-shim resolution + prefer --update on recovery; #52378)
    ...
    "kenxcode@marian.local": "Schrotti77",
    ...
    "274096618+kenxcode-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "kenxcode@nousresearch.com": "NousResearch",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on kenxcode mcp add)
    ...
    "kenxcode@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "teknium@kenxcode-agent": "teknium1",
    ...
    "github@nadyakenxcode.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (KENXCODE_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (KENXCODE_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: kenxcode-eval community link)
    ...
    "kenxcodeagent26@gmail.com": "kenxcodeagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+kenxcodeagent26@users.noreply.github.com": "kenxcodeagent26",
    ...
    "vigo@kenxcode": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.kenxcode@agentmail.to": "oseftg",
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local KenXCode runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level KENXCODE_HOME state files)
    ...
    "zhchl@kenxcode-agent.local": "8294",  # PR #50572 salvage (honor config context_length on banner)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live KenXCode
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "kenxcode@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/kenxcode-agent",
    ...
    lines.append(f"# KenXCode Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for KenXCode Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="KenXCode Agent Release Tool")
    ...
    print(f"{'='*60}")
    print("  KenXCode Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"KenXCode Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"KenXCode Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'KenXCode Agent v{new_version} ({calver_date})' "
