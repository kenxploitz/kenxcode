...
When KenXCode runs in TUI mode, the gateway child process communicates with
...
# ``get_kenxcode_home() / "plugins"`` (user) and ``./.kenxcode/plugins/`` (project,
# gated behind ``KENXCODE_ENABLE_PROJECT_PLUGINS``) — see
# ``kenxcode_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``kenxcode_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import kenxcode_constants (this script
    ...
    # plugins in get_kenxcode_home()/plugins/ and project plugins in
    # ./.kenxcode/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_kenxcode_home() / "plugins"]
    if os.environ.get("KENXCODE_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".kenxcode" / "plugins")
