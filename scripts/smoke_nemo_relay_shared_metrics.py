"""Run a real KenXCode CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_kenxcode_executable(kenxcode_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "kenxcode",
        Path(".venv") / "Scripts" / "kenxcode.exe",
    ):
        candidate = kenxcode_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("kenxcode")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "KenXCode executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="KenXCode source checkout containing .venv/bin/kenxcode",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated KENXCODE_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    expected_model = {
        "name": "kenxcode.model_call.count",
        "dimensions": {
            "call_role": "primary",
            "locality": "local",
            "model_family": "gpt",
    ...
    if by_name["kenxcode.model_call.count"] != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['kenxcode.model_call.count']}"
        )
    expected_start = {
    ...
    if by_name["kenxcode.task_run.started"] != expected_start:
        raise AssertionError(
            f"Unexpected task start: {by_name['kenxcode.task_run.started']}"
        )
    terminal = by_name["kenxcode.task_run.finished"]
...
def _validate_package(outbox: Path, schema_path: Path) -> tuple[Path, dict[str, Any]]:
    ...
    try:
        ...
        raise RuntimeError(
            "The KenXCode development environment requires jsonschema"
        ) from exc
    ...
    if metrics["kenxcode.model_call.count"]["dimensions"] != {
        "call_role": "primary",
        "locality": "local",
        "model_family": "gpt",
        "outcome": "success",
        ...
        raise AssertionError(
            f"Unexpected model metric: {metrics['kenxcode.model_call.count']}"
        )
    terminal = metrics["kenxcode.task_run.finished"]
...
def main() -> int:
    args = _arguments()
    kenxcode_repo = args.kenxcode_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    kenxcode = _resolve_kenxcode_executable(kenxcode_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="kenxcode-relay-shared-metrics-"))
    home = root / "kenxcode-home"
    ...
    (root / "kenxcode.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "kenxcode.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"KenXCode exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    if not _ModelHandler.requests:
        raise AssertionError("KenXCode did not call the local model endpoint")
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("KenXCode model request did not contain the prompt canary")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("KenXCode did not print the mock model response")
    ...
    package_path, package = _validate_package(
        ...
        / "observability"
        / "schemas"
        / "kenxcode.shared_metrics.v1.schema.json",
    )

    print("KenXCode -> NeMo Relay shared-metrics smoke test passed")
