#!/usr/bin/env python3
"""Live test harness for KenXCode Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any kenxcode imports.
ORIGINAL_HOME = os.environ.get("KENXCODE_HOME")
ORIGINAL_AUTH = Path.home() / ".kenxcode" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.kenxcode/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.kenxcode/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="kenxcode_ts_live_"))
    kenxcode_home = home_dir / ".kenxcode"
    kenxcode_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, kenxcode_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".kenxcode" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, kenxcode_home / ".env")
        ...
        load_kenxcode_dotenv(kenxcode_home=str(Path.home() / ".kenxcode"))
    ...
    (kenxcode_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
...
def reset_module_state():
    """Drop cached modules so the new KENXCODE_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "kenxcode_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["KENXCODE_HOME"] = str(home)
...
def main():
    ...
    # Restore original KENXCODE_HOME
    if ORIGINAL_HOME is not None:
        os.environ["KENXCODE_HOME"] = ORIGINAL_HOME
    else:
