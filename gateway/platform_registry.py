...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``kenxcode setup`` display).
    ...
    # built-ins).  Used by ``kenxcode gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `kenxcode`
        # invocation -- including plain `kenxcode chat`, which never touches any
        ...
        # `kenxcode setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``kenxcode setup``/``gateway status``, channel
