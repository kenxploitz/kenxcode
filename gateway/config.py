...
# dashboard's pre-write mutation validation (kenxcode_cli/web_server.py) so
...
class PlatformConfig:
    ...
    # "KenXCode is thinking…"). Platforms with textless indicators (Discord,
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_kenxcode_home() / "sessions")
    ...
    # gateway behaves exactly as before — single KENXCODE_HOME, no profile stamping.
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.kenxcode/config.yaml (primary user-facing config)
    3. ~/.kenxcode/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_kenxcode_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through kenxcode_cli.config.load_config, so without this a managed
            ...
            # (e.g. via `kenxcode config set gateway.<key> ...`, which naturally
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `kenxcode config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict) and isinstance(gateway_section, dict):
                ...
                # ``kenxcode config set gateway.streaming.*``
