...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚡ *KenXCode Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to KENXCODE_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/kenxcode), this function
    mirrors the bridge source to a writable KENXCODE_HOME location and returns that
    ...
    # Try KENXCODE_HOME location first
    kenxcode_home = get_kenxcode_home()
    kenxcode_home_bridge = kenxcode_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to KENXCODE_HOME if needed
    if kenxcode_home_bridge.exists():
        return kenxcode_home_bridge

    # Mirror the bridge source to KENXCODE_HOME
    try:
        kenxcode_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            kenxcode_home_bridge,
            dirs_exist_ok=False,
        )
        return kenxcode_home_bridge
    except Exception:
