...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.kenxcode to the active profile home."""
    ...
    if raw.startswith("~/.kenxcode/"):
        return kenxcode_home / raw.removeprefix("~/.kenxcode/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return kenxcode_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under KENXCODE_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from kenxcode_constants import get_kenxcode_home

    scripts_root = (get_kenxcode_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.kenxcode" or raw_text.startswith("~/.kenxcode/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__kenxcode_ignore__") is True
        ):
            return False, None
