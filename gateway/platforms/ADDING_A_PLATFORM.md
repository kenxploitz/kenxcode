...
There are two ways to add a platform to the KenXCode gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.kenxcode/plugins/` (or under `plugins/platforms/`
...
**zero changes to core KenXCode code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `kenxcode gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `kenxcode_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the KenXCode core.
...
And add it to the `kenxcode-gateway` composite:

```python
"kenxcode-gateway": {
    "includes": [..., "kenxcode-your-platform"]
}
```
...
## 12. Status Display (`kenxcode_cli/status.py`)
...
## 13. Gateway Setup Wizard (`kenxcode_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ kenxcode_cli/ toolsets.py \
  --include="*.py" -l | sort -u
