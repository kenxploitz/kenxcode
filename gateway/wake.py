...
* Stateless request/response adapters (the API server,
  ...
  ``X-KenXCode-Session-Id`` key real gateway/HQ turns run under
  ...
  server with the raw session id in the ``X-KenXCode-Session-Id`` header — the
...
async def deliver_wake(
    ...
    *,
    ...
    ``session_id`` is the RAW session id (the ``X-KenXCode-Session-Id`` value /
...
async def _self_post_chat_completion(
    ...
    Session continuation via ``X-KenXCode-Session-Id`` is 403-gated on
    ...
    if not api_key:
        raise RuntimeError(
            ...
            "X-KenXCode-Session-Id is rejected (403) on an unauthenticated API "
    ...
    payload = {
        "model": str(getattr(adapter, "_model_name", "") or "kenxcode-agent"),
