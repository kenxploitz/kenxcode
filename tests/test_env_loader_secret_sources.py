"""Tests for the secret-source tracking in ``kenxcode_cli.env_loader``.

These cover the small public surface that lets `kenxcode model` / `kenxcode setup`
...
from kenxcode_cli import env_loader  # noqa: E402
...
def test_cold_profile_hydration_seeds_op_env_bootstrap(tmp_path, monkeypatch):
    ...
    load_kenxcode_dotenv() reads <home>/.op.env for OP_SERVICE_ACCOUNT_TOKEN
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    """``load_kenxcode_dotenv()`` is called at module-import time from several
    hot modules (cli.py, kenxcode_cli/main.py, run_agent.py, ...).  The
    ...
    runs exactly once per KENXCODE_HOME per process.
    """
    ...
    # cli.py, kenxcode_cli/main.py, run_agent.py, trajectory_compressor.py,
