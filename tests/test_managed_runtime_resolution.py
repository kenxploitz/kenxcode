"""Guard: KenXCode-owned subprocesses must not resolve managed runtimes by bare PATH.

KenXCode installs runtimes for itself — ``uv`` at ``$KENXCODE_HOME/bin/uv``, Node at
``$KENXCODE_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in KenXCode's own
...
* when a system copy also exists, the one KenXCode does not own wins — which is
...
``iter_kenxcode_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes KenXCode provisions into KENXCODE_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not KenXCode-owned subprocess code: plugins ship their own
...
_ALLOWED: dict[tuple[str, str], str] = {
    ...
    ("kenxcode_cli/update_cmd.py", "npm"): (
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a KenXCode-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$KENXCODE_HOME/bin (uv) and $KENXCODE_HOME/node are not on an "
        ...
        "  node/npm -> kenxcode_constants.find_node_executable()\n"
        "  PATH env -> kenxcode_constants.iter_kenxcode_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(kenxcode_constants, helper))


def test_managed_uv_helpers_exist():
    from kenxcode_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
