...
from kenxcode_cli.profiles import _get_default_kenxcode_home
...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "kenxcode"
        assert config.workspace_id == "kenxcode"
...
class TestFromGlobalConfig:
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"kenxcode": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="kenxcode-agent"
        ):
            result = config.resolve_session_name("/home/user/kenxcode-agent/subdir")
        assert result == "kenxcode-agent"


class TestResolveConfigPath:
    def test_prefers_kenxcode_home_when_exists(self, tmp_path):
        kenxcode_home = tmp_path / "kenxcode"
        kenxcode_home.mkdir()
        local_cfg = kenxcode_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: KENXCODE_HOME points at ~/.kenxcode/profiles/<name>, so
        # _get_default_kenxcode_home() must resolve back to ~/.kenxcode — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".kenxcode"
        ...
        assert _get_default_kenxcode_home() == default_home
        assert result == default_cfg


class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "kenxcode_coder"
        assert profile_host_key("default") == "kenxcode"


    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"KENXCODE_HONCHO_HOST": "kenxcode.coder"}):
            assert resolve_active_host() == "kenxcode.coder"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False), patch(
            ...
            os.environ.pop("KENXCODE_HONCHO_HOST", None)
            # Temporarily remove kenxcode_cli.profiles to simulate import failure
            saved = sys.modules.get("kenxcode_cli.profiles")
            sys.modules["kenxcode_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "kenxcode"
            finally:
                if saved is not None:
                    sys.modules["kenxcode_cli.profiles"] = saved
                else:
                    sys.modules.pop("kenxcode_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="kenxcode_coder")
        assert config.host == "kenxcode_coder"
        assert config.workspace_id == "kenxcode"  # shared workspace
        assert config.ai_peer == "kenxcode_coder"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"kenxcode": {"enabled": True, "aiPeer": "kenxcode"}},
        }))
...
class TestGetHonchoClient:
    ...
    def test_timeout_change_triggers_client_rebuild(self):
        ...
        cfg_yaml = get_kenxcode_home() / "config.yaml"
    ...
    def test_managed_config_timeout_does_not_thrash_singleton(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KENXCODE_MANAGED_DIR", str(managed_dir))
...
class TestGetHonchoClientBaseUrlDoublePrefixFix:
    ...
    def test_local_base_url_with_v3_suffix_stripped(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("kenxcode_cli.config.load_config", return_value={}):
    ...
    def test_lan_default_host_empty_key_uses_local_placeholder(self, tmp_path):
        ...
        with patch.dict(os.environ, {}, clear=True), \
             patch("kenxcode_cli.profiles.get_active_profile_name", return_value="default"), \
