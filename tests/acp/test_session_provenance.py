...
def test_root_session_no_compression(db):
    ...
    assert prov["currentKenXCodeSessionId"] == "root1"
    assert prov["rootKenXCodeSessionId"] == "root1"
    assert prov["parentKenXCodeSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_kenxcode_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentKenXCodeSessionId"] == "old"
    assert prov["rootKenXCodeSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousKenXCodeSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootKenXCodeSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"kenxcode"}
    assert "sessionProvenance" in meta["kenxcode"]
    assert meta["kenxcode"]["sessionProvenance"]["currentKenXCodeSessionId"] == "root1"
