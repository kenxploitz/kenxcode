...
Keeps the ACP server tests offline: ``KenXCodeACPAgent._build_model_state``
calls ``kenxcode_cli.inventory.build_models_payload``, which (without this
...
def _offline_model_inventory(monkeypatch):
    ...
    import kenxcode_cli.inventory as inventory
