...
binary download are exercised manually by `kenxcode secrets bitwarden
...
def kenxcode_home(tmp_path, monkeypatch):
    """Point KenXCode at an isolated home directory."""
    home = tmp_path / ".kenxcode"
    home.mkdir()
    monkeypatch.setenv("KENXCODE_HOME", str(home))
    # Some modules cache get_kenxcode_home; clear if needed.
    import kenxcode_constants
    if hasattr(kenxcode_constants, "_KENXCODE_HOME_CACHE"):
        kenxcode_constants._KENXCODE_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(kenxcode_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from kenxcode_cli.env_loader import _apply_external_secret_sources
