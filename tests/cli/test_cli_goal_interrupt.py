...
minimal ``KenXCodeCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def kenxcode_home(tmp_path, monkeypatch):
    """Isolated KENXCODE_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".kenxcode"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("KENXCODE_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves KENXCODE_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal KenXCodeCLI stub with an active goal wired in."""
    from cli import KenXCodeCLI
    from kenxcode_cli.goals import GoalManager

    cli = KenXCodeCLI.__new__(KenXCodeCLI)
...
class TestInterruptAutoPause:
    ...
    def test_interrupted_turn_is_resumable(self, kenxcode_home):
        ...
        with patch("kenxcode_cli.goals.judge_goal"):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, kenxcode_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, kenxcode_home):
        ...
        src = inspect.getsource(KenXCodeCLI.chat)
