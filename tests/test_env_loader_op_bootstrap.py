...
1. ``load_kenxcode_dotenv()`` auto-loads ``~/.kenxcode/.op.env`` so the
...
def test_op_env_autoloads_bootstrap_token_in_cron_context(tmp_path, monkeypatch):
    ...
    env_loader.load_kenxcode_dotenv(kenxcode_home=home)
...
def _seed_openrouter_token(monkeypatch, dotenv_value, environ_value):
    ...
    monkeypatch.setattr(
        "kenxcode_cli.auth.is_source_suppressed", lambda _p, _s: False
    )
