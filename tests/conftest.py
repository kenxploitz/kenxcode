"""Shared fixtures for the kenxcode-agent test suite.
...
2. **Isolated KENXCODE_HOME.** KENXCODE_HOME points to a per-test tempdir so
   code reading ``~/.kenxcode/*`` via ``get_kenxcode_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".kenxcode"`` instead of the canonical
   ``get_kenxcode_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No KENXCODE_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox KENXCODE_HOME before ANY test module is imported ──────────────────
# `kenxcode_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_kenxcode_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.kenxcode/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes KENXCODE_HOME, but fixtures run
...
# the REAL KenXCode root — capture it BEFORE the sandbox rewires KENXCODE_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.kenxcode (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("KENXCODE_KANBAN_HOME", "").strip()
_PRE_SANDBOX_KENXCODE_HOME = os.environ.get("KENXCODE_HOME", "")
if not os.environ.get("KENXCODE_HOME"):
    _SESSION_KENXCODE_HOME = tempfile.mkdtemp(prefix="kenxcode-test-home-")
    os.environ["KENXCODE_HOME"] = _SESSION_KENXCODE_HOME
    atexit.register(shutil.rmtree, _SESSION_KENXCODE_HOME, True)

#: KENXCODE_HOME as it stood when conftest was imported - i.e. before any test
...
KENXCODE_HOME_AT_CONFTEST_IMPORT = os.environ.get("KENXCODE_HOME", "")
...
# KENXCODE_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_KENXCODE_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``kenxcode_cli.voice.speak_text`` — real synthesis,
    ...
    "KENXCODE_VOICE_TTS",
    "KENXCODE_YOLO_MODE",
    "KENXCODE_INTERACTIVE",
    "KENXCODE_QUIET",
    "KENXCODE_TOOL_PROGRESS",
    "KENXCODE_TOOL_PROGRESS_MODE",
    "KENXCODE_MAX_ITERATIONS",
    "KENXCODE_SESSION_PLATFORM",
    "KENXCODE_SESSION_CHAT_ID",
    "KENXCODE_SESSION_CHAT_NAME",
    "KENXCODE_SESSION_CHAT_TYPE",
    "KENXCODE_SESSION_THREAD_ID",
    "KENXCODE_SESSION_SOURCE",
    "KENXCODE_SESSION_KEY",
    "KENXCODE_GATEWAY_SESSION",
    "KENXCODE_CRON_SESSION",
    "_KENXCODE_GATEWAY",
    "KENXCODE_PLATFORM",
    "KENXCODE_MODEL",
    "KENXCODE_INFERENCE_MODEL",
    "KENXCODE_INFERENCE_PROVIDER",
    "KENXCODE_TUI_PROVIDER",
    "KENXCODE_MANAGED",
    "KENXCODE_MANAGED_DIR",
    "KENXCODE_DEV",
    "KENXCODE_CONTAINER",
    "KENXCODE_EPHEMERAL_SYSTEM_PROMPT",
    "KENXCODE_TIMEZONE",
    "KENXCODE_REDACT_SECRETS",
    "KENXCODE_BACKGROUND_NOTIFICATIONS",
    "KENXCODE_EXEC_ASK",
    "KENXCODE_HOME_MODE",
    "KENXCODE_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.kenxcode/kanban.db instead of the per-test KENXCODE_HOME.
    "KENXCODE_KANBAN_DB",
    "KENXCODE_KANBAN_BOARD",
    "KENXCODE_KANBAN_HOME",
    "KENXCODE_KANBAN_WORKSPACES_ROOT",
    "KENXCODE_KANBAN_LOGS_ROOT",
    "KENXCODE_KANBAN_TASK",
    "KENXCODE_KANBAN_WORKSPACE",
    "KENXCODE_KANBAN_RUN_ID",
    "KENXCODE_KANBAN_CLAIM_LOCK",
    "KENXCODE_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    "KENXCODE_DELEGATED_CHILD_CONTEXT",
    "KENXCODE_TENANT",
    ...
    # otherwise-unrelated config tests away from the default "kenxcode" host.
    "KENXCODE_HONCHO_HOST",
    ...
    "KENXCODE_DASHBOARD_OAUTH_CLIENT_ID",
    "KENXCODE_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and KENXCODE_HOME to per-test tempdirs so code that
    reads ``~/.kenxcode/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral KENXCODE_* vars that could change test semantics.
    for name in _KENXCODE_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("KENXCODE_HONCHO_HOST", "kenxcode")

    # 3. Redirect KENXCODE_HOME to a per-test tempdir. Code that reads
    #    ``~/.kenxcode/*`` via ``get_kenxcode_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.kenxcode/*`` via
    #    ``Path.home() / ".kenxcode"`` instead of ``get_kenxcode_home()``
    #    is a bug to fix at the callsite.
    fake_kenxcode_home = tmp_path / "kenxcode_test"
    fake_kenxcode_home.mkdir()
    (fake_kenxcode_home / "sessions").mkdir()
    (fake_kenxcode_home / "cron").mkdir()
    (fake_kenxcode_home / "memories").mkdir()
    (fake_kenxcode_home / "skills").mkdir()
    monkeypatch.setenv("KENXCODE_HOME", str(fake_kenxcode_home))

    # 3b. kenxcode_state computes ``DEFAULT_DB_PATH = get_kenxcode_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from kenxcode_state import ...``) that
    ...
    kenxcode_state_mod = sys.modules.get("kenxcode_state")
    if kenxcode_state_mod is not None and hasattr(kenxcode_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            kenxcode_state_mod, "DEFAULT_DB_PATH", fake_kenxcode_home / "state.db"
        )
    ...
    monkeypatch.setenv("KENXCODE_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.kenxcode/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import kenxcode_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_kenxcode_home(_hermetic_environment):
...
# invocation), kanban writes silently pollute the real ~/.kenxcode. This autouse
...
# allow-list because test-level fixtures legitimately move KENXCODE_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session KENXCODE_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``KENXCODE_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) KenXCode root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_KENXCODE_HOME:
        # KENXCODE_HOME was genuinely set before the sandbox — honor it via the
        # normal resolver (it may be a profile dir whose root matters).
        from kenxcode_constants import get_default_kenxcode_root
        return get_default_kenxcode_root().resolve()
    # No pre-existing KENXCODE_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".kenxcode").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.kenxcode`` captured at import time. Hermetic tests that legitimately
    move KENXCODE_HOME to sibling tempdirs are unaffected.

    Only patches when ``kenxcode_cli.kanban_db`` is *already imported* — a
    ...
    """
    _kdb = sys.modules.get("kenxcode_cli.kanban_db")
    if _kdb is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import kenxcode_cli.kanban_db`
    ...
    def _guarded_connect(db_path=None, *args, **kwargs):
        ...
        raise RuntimeError(
            ...
            f"to the real ~/.kenxcode. See #69283."
        )
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local KenXCode home override redirects every later
    # ``get_kenxcode_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from kenxcode_constants import get_kenxcode_home_override, set_kenxcode_home_override

        if get_kenxcode_home_override() is not None:
            set_kenxcode_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal kenxcode config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``kenxcode-gateway`` process
...
# correlated with a ``tests/kenxcode_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> kenxcode-gateway``
...
def _wal_is_usable() -> bool:
    """True when KenXCode will actually put a database into WAL mode here.

    KenXCode refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running KenXCode can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a KenXCode managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``kenxcode_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_kenxcode_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_kenxcode_home`` fixture
    redirects ``KENXCODE_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.kenxcode/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``kenxcode_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["KENXCODE_VOICE_TTS"] = "1"``. The
...
#      ``kenxcode_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``kenxcode_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``kenxcode_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``kenxcode_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where KenXCode falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — KenXCode uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart kenxcode-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting kenxcode/python patterns are also blocked.
    """
    ...
    def _matches_kenxcode_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _KENXCODE_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_kenxcode_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch kenxcode-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m kenxcode_cli.main".
                if (
                    "kenxcode" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live kenxcode-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting kenxcode/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `kenxcode update` (or the
        # equivalent `python -m kenxcode_cli.main update`).  These commands
        ...
        if "update" in low and (
            # kenxcode update / kenxcode update --gateway / setsid bash -c ... kenxcode update
            ("kenxcode" in low and "update" in low.split())
            or
            # python -m kenxcode_cli.main update --gateway
            ("kenxcode_cli" in low and "update" in low.split())
            or
            # venv/bin/kenxcode update  (absolute path variant used in tests)
            (".venv/bin/kenxcode" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`kenxcode update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_KENXCODE_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import kenxcode_cli.voice as _voice
    except Exception:
