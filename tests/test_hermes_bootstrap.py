"""Tests for kenxcode_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every KenXCode entry point
(kenxcode, kenxcode-agent, kenxcode-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every KenXCode entry point imports kenxcode_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported kenxcode_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("kenxcode_bootstrap", None)
    import kenxcode_bootstrap  # noqa: WPS433
    return kenxcode_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the KenXCode CLI
...
class TestEntryPointsImportBootstrap:
    """Every KenXCode entry point must import kenxcode_bootstrap as its
    ...
    # Entry points that invoke KenXCode as a process.  Each one must
    # import kenxcode_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "kenxcode_cli/main.py",   # kenxcode CLI (console_script)
        "run_agent.py",          # kenxcode-agent (console_script)
        "acp_adapter/entry.py",  # kenxcode-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import kenxcode_bootstrap' and that
        ...
        half-finished ``kenxcode update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``kenxcode_bootstrap`` as a top-level module) leaves kenxcode
        ...
        """
        # Resolve relative to the kenxcode-agent repo root.  Tests live
        # at tests/test_kenxcode_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # kenxcode start even when kenxcode_bootstrap hasn't been
        ...
        assert first_import_name == "kenxcode_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'kenxcode_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import kenxcode_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing KenXCode's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("KENXCODE_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["KENXCODE_PYTHON_SRC_ROOT"] = env
            elif "KENXCODE_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["KENXCODE_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/kenxcode")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("KENXCODE_PYTHON_SRC_ROOT", None)
            else:
                os.environ["KENXCODE_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/kenxcode", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # KenXCode root.  The guard must relocate KenXCode to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/kenxcode"])
        assert result[0] == "/opt/kenxcode"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the KenXCode root.
        assert result.index("/opt/kenxcode") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["KENXCODE_PYTHON_SRC_ROOT"] = "/env/kenxcode"
            hb.harden_import_path()
            assert sys.path[0] == "/env/kenxcode"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("KENXCODE_PYTHON_SRC_ROOT", None)
