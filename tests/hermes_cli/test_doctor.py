...
import kenxcode_cli.doctor as doctor
import kenxcode_cli.gateway as gateway_cli
from kenxcode_cli import doctor as doctor_mod
from kenxcode_cli.doctor import _has_provider_env_config
...
class TestDoctorPlatformHints:
    ...
    def test_sqlite_upgrade_hint_recreates_docker_containers(self, monkeypatch):
        ...
        assert "docker pull nousresearch/kenxcode-agent:latest" in hint
        assert "recreate all KenXCode containers" in hint
        assert "kenxcode update" not in hint

    def test_sqlite_upgrade_hint_keeps_git_runtime_repair(self):
        hint = doctor._sqlite_upgrade_hint("git")

        assert "run `kenxcode update`" in hint
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `kenxcode doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "KENXCODE_HOME", kenxcode_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_marks_kanban_available_only_when_missing_worker_env_gate(self, monkeypatch):
        ...
        monkeypatch.delenv("KENXCODE_KANBAN_TASK", raising=False)
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("KENXCODE_KANBAN_TASK", "probe")
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_kenxcode_home(self, tmp_path, provider=""):
        """Create a minimal KENXCODE_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_kenxcode_home(tmp_path, provider)
        monkeypatch.setattr(doctor_mod, "KENXCODE_HOME", home)
        ...
        try:
            from kenxcode_cli import auth as _auth_mod
...
def test_run_doctor_accepts_kenxcode_provider_ids_that_catalog_aliases(
...
def _run_doctor_with_managed_agent_browser(monkeypatch, tmp_path, runnable):
    ...
    KenXCode-managed node bin (~/.kenxcode/node/bin), not on PATH or in
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        """Point doctor at the temp KENXCODE_HOME.

        ``run_doctor`` reads the module-level ``KENXCODE_HOME`` constant (cached
        at import time), NOT the env var — so ``setenv("KENXCODE_HOME")`` alone
        leaves doctor probing the REAL ~/.kenxcode. On a dev machine with a
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from kenxcode_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("kenxcode_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "kenxcode_cli.doctor", raising=False)
        from kenxcode_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale KENXCODE_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        kenxcode_home = tmp_path / ".kenxcode"
        kenxcode_home.mkdir(parents=True)
        (kenxcode_home / "config.yaml").write_text(
            f"agent:\n  max_turns: {cfg_turns}\n", encoding="utf-8"
        )
        env_lines = ["OPENAI_API_KEY=sk-test\n"]
        if ghost is not None:
            env_lines.append(f"KENXCODE_MAX_ITERATIONS={ghost}\n")
        (kenxcode_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "KENXCODE_HOME", kenxcode_home)
        monkeypatch.setattr(doctor_mod, "get_kenxcode_home", lambda: kenxcode_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("KENXCODE_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("KENXCODE_MAX_ITERATIONS", raising=False)
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale KENXCODE_MAX_ITERATIONS" in out
        env_after = (kenxcode_home / ".env").read_text(encoding="utf-8")
        assert "KENXCODE_MAX_ITERATIONS" not in env_after
...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def test_kenxcode_tool_progress_warning_says_unsupported_since_floor(self):
        """KENXCODE_TOOL_PROGRESS lost its last consumer (the retired v3→4
        ...
        findings = dict(
            doctor_mod.collect_deprecated_env_vars({"KENXCODE_TOOL_PROGRESS": "true"})
        )
        assert "ignored/unsupported since config floor v12" in findings["KENXCODE_TOOL_PROGRESS"]
        # The MODE variant is still read by the gateway fallback → keeps the
        # plain deprecation wording.
        mode = dict(
            doctor_mod.collect_deprecated_env_vars({"KENXCODE_TOOL_PROGRESS_MODE": "all"})
        )
        assert mode["KENXCODE_TOOL_PROGRESS_MODE"] == "display.tool_progress in config.yaml"

    def _run_doctor_with_config(self, monkeypatch, tmp_path, *, config_yaml: str, env_text: str = ""):
        kenxcode_home = tmp_path / ".kenxcode"
        kenxcode_home.mkdir(parents=True)
        (kenxcode_home / "config.yaml").write_text(config_yaml, encoding="utf-8")
        env_body = env_text if env_text else "OPENAI_API_KEY=sk-test\n"
        (kenxcode_home / ".env").write_text(env_body, encoding="utf-8")
    ...
    def test_report_does_not_count_as_blocking_issue(self, monkeypatch, tmp_path, capsys):
        ...
        findings = doctor_mod.report_deprecated_config_and_env(
            ...
            {"KENXCODE_TOOL_PROGRESS_MODE": "verbose"},
        )
        out = capsys.readouterr().out
        assert len(findings) == 2
        assert "Deprecated: delegation.max_async_children" in out
        assert "Deprecated: KENXCODE_TOOL_PROGRESS_MODE" in out
