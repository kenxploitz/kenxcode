...
def kenxcode_home(tmp_path, monkeypatch):
    ...
    # /etc/kenxcode on the dev/CI box can't influence the test.
    monkeypatch.setenv("KENXCODE_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(kenxcode_home, monkeypatch):
    from kenxcode_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(kenxcode_home, monkeypatch):
    from kenxcode_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_kenxcode_dotenv(kenxcode_home=str(home))
