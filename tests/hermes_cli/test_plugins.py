"""Tests for the KenXCode plugin system (kenxcode_cli.plugins)."""
...
from kenxcode_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<kenxcode_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<kenxcode_home>/plugins/``; we derive
    ``<kenxcode_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <KENXCODE_HOME>/config.yaml.
        # Config is always read from KENXCODE_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        kenxcode_home_str = os.environ.get("KENXCODE_HOME")
        if kenxcode_home_str:
            kenxcode_home = Path(kenxcode_home_str)
        else:
            kenxcode_home = base.parent
        kenxcode_home.mkdir(parents=True, exist_ok=True)
        cfg_path = kenxcode_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_plugin_can_register_and_invoke_middleware(self, tmp_path, monkeypatch):
        plugins_dir = tmp_path / "kenxcode_test" / "plugins"
        _make_plugin_dir(
            plugins_dir,
            "mw_plugin",
            register_body=(
        ...
        monkeypatch.setenv("KENXCODE_HOME", str(tmp_path / "kenxcode_test"))
    ...
    def test_middleware_helpers_skip_no_listener_work(self, monkeypatch):
        ...
        monkeypatch.setattr("kenxcode_cli.plugins.get_plugin_manager", lambda: manager)
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under kenxcode_plugins.<name>."""
        plugins_dir = tmp_path / "kenxcode_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("KENXCODE_HOME", str(tmp_path / "kenxcode_test"))

        # Clean up any prior namespace module
        sys.modules.pop("kenxcode_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "kenxcode_plugins.ns_plugin" in sys.modules
    ...
    def test_user_memory_plugin_auto_coerced_to_exclusive(self, tmp_path, monkeypatch):
        ...
        kenxcode_home = tmp_path / "kenxcode_test"
...
class TestPreToolCallBlocking:
    ...
    def test_block_message_returned_for_valid_directive(self, monkeypatch):
        monkeypatch.setattr(
            "kenxcode_cli.plugins.invoke_hook",
...
class TestPreToolCallDirective:
    ...
    def test_approve_without_message_is_valid(self, monkeypatch):
        ...
        from kenxcode_cli.plugins import get_pre_tool_call_directive
        monkeypatch.setattr(
            "kenxcode_cli.plugins.invoke_hook",
...
class TestResolvePreToolBlock:
    ...
    def test_approve_passes_plugin_rule_key_to_gate(self, monkeypatch):
        from kenxcode_cli.plugins import resolve_pre_tool_block
...
class TestGetPreVerifyContinueMessage:
    ...
    def test_none_when_no_hooks(self, monkeypatch):
        monkeypatch.setattr("kenxcode_cli.plugins.invoke_hook", lambda hook_name, **kwargs: [])
    ...
    def test_forwards_scope_signals_to_hooks(self, monkeypatch):
        ...
        monkeypatch.setattr("kenxcode_cli.plugins.invoke_hook", capture)
...
class TestPluginContext:
    ...
    def test_register_tool_override_blocked_without_operator_opt_in(self, tmp_path, monkeypatch):
        ...
        from kenxcode_cli.plugins import PluginToolOverrideError
        ...
        try:
            ...
            from kenxcode_cli.plugins import PluginContext, PluginManifest
    ...
    def test_register_tool_override_blocked_via_delayed_callback(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            import sys as _sys
            mod = _sys.modules.get("kenxcode_plugins.delayed_override_plugin")
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        """
        import kenxcode_cli.plugins as plugins_mod
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `kenxcode plugins list`. Attribution now counts what each plugin's own
...
class TestPluginCommands:
    ...
    def test_register_command_empty_name_rejected(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="kenxcode_cli.plugins"):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        kenxcode_home = tmp_path / "kenxcode_test"
        plugins_dir = kenxcode_home / "plugins"
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("kenxcode_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("kenxcode_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("kenxcode_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("kenxcode_cli.plugins.PluginContext.dispatch_tool.__module__", "kenxcode_cli.plugins"):
...
class TestPluginDebugLogging:
    """KENXCODE_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("KENXCODE_PLUGINS_DEBUG", raising=False)
        from kenxcode_cli import plugins as plugins_mod
...
class TestPluginContextProfileName:
    """ctx.profile_name resolves from KENXCODE_HOME in every context."""
    ...
    def test_default_profile(self, tmp_path, monkeypatch):
        """KENXCODE_HOME at the root resolves to 'default'."""
    ...
    def test_named_profile(self, tmp_path, monkeypatch):
        """KENXCODE_HOME under profiles/<name> resolves to that name."""
        prof = tmp_path / ".kenxcode" / "profiles" / "coder"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("KENXCODE_HOME", str(prof))
