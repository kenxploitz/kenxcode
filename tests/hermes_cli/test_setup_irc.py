"""Tests for IRC gateway configuration via `kenxcode setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh KenXCode install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new KenXCode install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#kenxcode")
            monkeypatch.setenv("IRC_NICKNAME", "kenxcode-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `kenxcode setup gateway` experience with IRC present."""
