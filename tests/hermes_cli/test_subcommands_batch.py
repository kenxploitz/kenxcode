...
def test_config_get_unset_subcommands_parse():
    """`kenxcode config get/unset` parse key args (and --json for get)."""
    parser = argparse.ArgumentParser(prog="kenxcode")
...
# ── deprecated `kenxcode login` fails gracefully, not with argparse error ────
#
# `kenxcode login` is a removed command; its handler (`login_command` in
# `kenxcode_cli/auth.py`) prints a deprecation notice pointing at `kenxcode auth` /
# `kenxcode model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `kenxcode --help`.
