"""Regression tests for #34107 — Docker UID/GID handling in ensure_kenxcode_home.

When KenXCode runs in Docker with ``KENXCODE_UID=1000`` / ``KENXCODE_GID=911``,
the entrypoint chowns the top-level ``KENXCODE_HOME`` once at startup. But
...
The fix is a ``_chown_to_kenxcode_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_kenxcode_uid_gid
# ---------------------------------------------------------------------------


class TestResolveKenXCodeUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("KENXCODE_UID", "1000")
        monkeypatch.setenv("KENXCODE_GID", "911")
        from kenxcode_cli.config import _resolve_kenxcode_uid_gid
...
# ---------------------------------------------------------------------------
# _chown_to_kenxcode_uid
# ---------------------------------------------------------------------------


class TestChownToKenXCodeUid:
    def test_calls_os_chown_when_both_set(self, tmp_path, monkeypatch):
        monkeypatch.setenv("KENXCODE_UID", "1000")
        monkeypatch.setenv("KENXCODE_GID", "911")
        from kenxcode_cli import config as cfg

        d = tmp_path / "subdir"
        d.mkdir()

        with patch.object(cfg.os, "chown") as mock_chown:
            cfg._chown_to_kenxcode_uid(d)
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_kenxcode_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("KENXCODE_UID", raising=False)
        monkeypatch.delenv("KENXCODE_GID", raising=False)
        from kenxcode_cli import config as cfg
