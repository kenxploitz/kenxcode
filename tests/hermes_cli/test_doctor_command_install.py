"""Tests for the Command Installation check in kenxcode doctor."""
...
import kenxcode_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal KENXCODE_HOME + PROJECT_ROOT for doctor tests."""
    ...
    kenxcode_bin = venv_bin_dir / "kenxcode"
    kenxcode_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    kenxcode_bin.chmod(0o755)
    ...
    return home, project, kenxcode_bin
...
class TestDoctorCommandInstallation:
    ...
    def test_fix_repairs_wrong_symlink(self, monkeypatch, tmp_path):
        home, project, kenxcode_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create a symlink pointing to wrong target
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "kenxcode"
        wrong_target = tmp_path / "wrong_kenxcode"
        ...
        assert cmd_link.resolve() == kenxcode_bin.resolve()
