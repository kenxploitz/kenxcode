...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (KENXCODE_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via KENXCODE_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in KENXCODE_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from kenxcode_cli.providers import KENXCODE_OVERLAYS
        assert "bedrock" in KENXCODE_OVERLAYS
