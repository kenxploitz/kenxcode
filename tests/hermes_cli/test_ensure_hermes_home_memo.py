"""ensure_kenxcode_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".kenxcode"
    second = tmp_path / "b" / ".kenxcode"

    monkeypatch.setenv("KENXCODE_HOME", str(first))
    cfg.ensure_kenxcode_home()

    # Profile switch: KENXCODE_HOME moves → the new path is ensured too.
    monkeypatch.setenv("KENXCODE_HOME", str(second))
    cfg.ensure_kenxcode_home()
