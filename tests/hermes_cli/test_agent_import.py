"""Tests for kenxcode_cli.agent_import — ``kenxcode import-agent``.

Covers: source detection, Claude Code and Codex parsing, mapping into the
real KenXCode stores (memories/MEMORY.md, config.yaml command_allowlist /
...
Uses the profile_env fixture pattern from tests/kenxcode_cli/test_profiles.py:
Path.home() and KENXCODE_HOME are redirected to tmp_path so nothing touches
the real ~/.kenxcode.
"""
...
from kenxcode_cli.agent_import import (
    ENTRY_DELIMITER,
    AgentImporter,
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and KENXCODE_HOME (profile_env pattern)
# ---------------------------------------------------------------------------

@pytest.fixture()
def profile_env(tmp_path, monkeypatch):
    """Isolated environment: Path.home() -> tmp_path, KENXCODE_HOME -> tmp/.kenxcode."""
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    default_home = tmp_path / ".kenxcode"
    default_home.mkdir(exist_ok=True)
    monkeypatch.setenv("KENXCODE_HOME", str(default_home))
    return tmp_path


@pytest.fixture()
def kenxcode_home(profile_env):
    return profile_env / ".kenxcode"
...
def run_import(agent, source, kenxcode_home, execute, overwrite=False):
    return AgentImporter(
        agent=agent,
        source_root=source,
        target_root=kenxcode_home,
        execute=execute,
        overwrite=overwrite,
    ).run()
...
class TestDetection:
    ...
    def test_unsupported_agent_raises(self, kenxcode_home, tmp_path):
        with pytest.raises(ValueError):
            AgentImporter("cursor", tmp_path, kenxcode_home)
...
class TestDryRun:
    def test_claude_dry_run_writes_nothing(self, claude_tree, kenxcode_home):
        before = snapshot_tree(kenxcode_home)
        report = run_import("claude-code", claude_tree, kenxcode_home, execute=False)
        assert snapshot_tree(kenxcode_home) == before
        assert report["dry_run"] is True
        assert report["summary"]["imported"] > 0


    def test_dry_run_and_real_run_plan_same_items(self, claude_tree, kenxcode_home):
        preview = run_import("claude-code", claude_tree, kenxcode_home, execute=False)
        applied = run_import("claude-code", claude_tree, kenxcode_home, execute=True)
...
class TestClaudeCodeImport:
    @pytest.fixture()
    def report(self, claude_tree, kenxcode_home):
        return run_import("claude-code", claude_tree, kenxcode_home, execute=True)


    def test_allowlist_lands_in_config_yaml(self, report, kenxcode_home):
        config = yaml.safe_load((kenxcode_home / "config.yaml").read_text())
...
class TestCodexImport:
    @pytest.fixture()
    def report(self, codex_tree, kenxcode_home):
        return run_import("codex", codex_tree, kenxcode_home, execute=True)


    def test_mcp_servers_from_config_toml(self, report, kenxcode_home):
        config = yaml.safe_load((kenxcode_home / "config.yaml").read_text())
    ...
    def test_skill_copied(self, report, kenxcode_home):
        assert (kenxcode_home / "skills" / "codex-imports" / "db-migrate" / "SKILL.md").exists()
...
class TestSecretsNeverImported:
    def test_no_secret_values_anywhere_claude(self, claude_tree, kenxcode_home):
        run_import("claude-code", claude_tree, kenxcode_home, execute=True)
        blob = "".join(
            p.read_text(encoding="utf-8", errors="replace")
            for p in kenxcode_home.rglob("*") if p.is_file()
        )
        assert "ghp_SECRET123" not in blob
        assert "Bearer abc123" not in blob
        assert "sk-ant-SUPERSECRET" not in blob

    def test_no_secret_values_anywhere_codex(self, codex_tree, kenxcode_home):
        run_import("codex", codex_tree, kenxcode_home, execute=True)
        blob = "".join(
            p.read_text(encoding="utf-8", errors="replace")
            for p in kenxcode_home.rglob("*") if p.is_file()
        )
    ...
    def test_stripped_secrets_reported(self, claude_tree, kenxcode_home):
        report = run_import("claude-code", claude_tree, kenxcode_home, execute=True)
    ...
    def test_non_secret_header_kept(self, claude_tree, kenxcode_home):
        run_import("claude-code", claude_tree, kenxcode_home, execute=True)
        config = yaml.safe_load((kenxcode_home / "config.yaml").read_text())
...
class TestMalformedInputs:
    def test_bad_settings_json_reports_error(self, profile_env, kenxcode_home):
        root = profile_env / ".claude"
        root.mkdir()
        (root / "settings.json").write_text("{not json!!", encoding="utf-8")
        (root / "CLAUDE.md").write_text("- still importable\n", encoding="utf-8")
        report = run_import("claude-code", root, kenxcode_home, execute=True)
        ...
        assert "still importable" in (
            kenxcode_home / "memories" / "MEMORY.md").read_text()


    def test_empty_tree_all_skipped(self, profile_env, kenxcode_home):
        root = profile_env / ".codex"
        root.mkdir()
        report = run_import("codex", root, kenxcode_home, execute=True)
        assert report["summary"]["imported"] == 0
...
class TestMergeSemantics:
    ...
    def test_existing_mcp_server_conflicts_without_overwrite(
            self, claude_tree, kenxcode_home):
        (kenxcode_home / "config.yaml").write_text(
    ...
    def test_existing_skill_conflicts_without_overwrite(
            self, claude_tree, kenxcode_home):
        dest = kenxcode_home / "skills" / "claude-code-imports" / "deploy-helper"
    ...
    def test_reimport_is_idempotent_for_memory(self, claude_tree, kenxcode_home):
        run_import("claude-code", claude_tree, kenxcode_home, execute=True)
        first = (kenxcode_home / "memories" / "MEMORY.md").read_text()
        report = run_import("claude-code", claude_tree, kenxcode_home, execute=True)
        assert (kenxcode_home / "memories" / "MEMORY.md").read_text() == first
...
class TestExistingMemoryStorePreserved:
    ...
    @pytest.fixture()
    def seeded_home(self, kenxcode_home):
        memory = kenxcode_home / "memories" / "MEMORY.md"
...
class TestExistingConfigPreserved:
    ...
    @pytest.fixture()
    def config_path(self, kenxcode_home):
        return kenxcode_home / "config.yaml"
    ...
    def test_malformed_config_is_left_byte_identical(
            self, claude_tree, kenxcode_home, config_path):
    ...
    def test_malformed_config_records_an_error_not_an_import(
        ...
        for kind in CONFIG_WRITING_KINDS:
            ...
            assert "kenxcode config edit" in reason
    ...
    def test_unreadable_config_is_left_byte_identical(
        ...
        try:
            report = run_import("claude-code", claude_tree, kenxcode_home,
                                execute=True)
            statuses = {
                i["status"]
                for kind in CONFIG_WRITING_KINDS
    ...
    def test_import_preserves_every_pre_existing_config_key(
        ...
        assert merged["model"] == "kenxcode-4-405b"
    ...
    def test_config_write_is_atomic_and_leaves_no_temp_files(
        ...
        leftovers = [p.name for p in kenxcode_home.glob(".tmp*")]
        assert leftovers == []

    def test_symlinked_config_stays_a_symlink(
            self, claude_tree, kenxcode_home, tmp_path, config_path):
    ...
    def test_failed_write_does_not_truncate_the_existing_config(
            self, kenxcode_home, config_path, monkeypatch):
        """An interrupted dump leaves the previous file complete."""
        from kenxcode_cli import agent_import
...
class TestCliWiring:
    ...
    def test_command_dry_run_via_cli_writes_nothing(
            self, claude_tree, kenxcode_home, capsys):
        ...
        assert not (kenxcode_home / "memories" / "MEMORY.md").exists()
        assert not (kenxcode_home / "skills" / "claude-code-imports").exists()
        config_text = (kenxcode_home / "config.yaml").read_text(encoding="utf-8") \
            if (kenxcode_home / "config.yaml").exists() else ""
