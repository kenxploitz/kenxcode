...
Regression guard for the bug where `kenxcode model` saved `provider: upstage`
correctly but, on re-entry, showed a different provider as active. Root cause:
`kenxcode_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageOverlay:
    def test_overlay_exists(self):
        ...
        assert "upstage" in KENXCODE_OVERLAYS
        overlay = KENXCODE_OVERLAYS["upstage"]
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `kenxcode_cli/main.py` (cmd_model): read
