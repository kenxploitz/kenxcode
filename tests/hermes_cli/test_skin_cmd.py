"""`kenxcode skin set` — deterministic single-color tweak of the active skin.
...
from kenxcode_cli import skin_cmd
from kenxcode_constants import get_kenxcode_home


def _skins():
    d = get_kenxcode_home() / "skins"
    d.mkdir(parents=True, exist_ok=True)
    return d


def _activate(name: str) -> None:
    (get_kenxcode_home() / "config.yaml").write_text(f"display:\n  skin: {name}\n", encoding="utf-8")
...
def test_set_forks_a_builtin_without_inventing_a_background():
    ...
    assert (get_kenxcode_home() / "config.yaml").read_text().find("default-custom") != -1
