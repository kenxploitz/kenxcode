"""Tests for kenxcode_cli.tools_config platform tool persistence."""
...
def test_all_invalid_platform_toolsets_logs_runtime_warning(caplog):
    ...
    (e.g. 'kenxcode' instead of 'kenxcode-cli') must warn at resolve time so an
    already-corrupted config is caught at runtime, not just during migration."""
    import kenxcode_cli.tools_config as _tc
    ...
    config = {"platform_toolsets": {"cli": ["kenxcode"]}}

    with caplog.at_level(logging.WARNING, logger="kenxcode_cli.tools_config"):
        _get_platform_tools(config, "cli")

    warnings = [r.getMessage() for r in caplog.records if r.levelno >= logging.WARNING]
    assert any("#38798" in m and "kenxcode" in m for m in warnings), warnings
...
def test_valid_platform_toolsets_no_runtime_warning(caplog):
    ...
    config = {"platform_toolsets": {"cli": ["kenxcode-cli"]}}
...
def test_partially_valid_platform_toolsets_no_runtime_warning(caplog):
    ...
    config = {"platform_toolsets": {"cli": ["kenxcode-cli", "bogus"]}}
...
# via the platform's native composite. The composite ``kenxcode-discord``
...
def test_discord_toolsets_do_not_leak_to_other_platforms():
    ...
    config = {"platform_toolsets": {"telegram": ["kenxcode-telegram", "discord"]}}
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/NousResearch/kenxcode-agent/issues/1247
    """
    config = {
        "platform_toolsets": {
    ...
    with patch("kenxcode_cli.tools_config.save_config"):
...
def test_first_install_nous_auto_configures_video_gen(monkeypatch):
    ...
    monkeypatch.setattr("kenxcode_cli.nous_subscription.managed_nous_tools_enabled", lambda: True)
    config = {
        "model": {"provider": "nous"},
    ...
    monkeypatch.setattr(
        "kenxcode_cli.tools_config._prompt_toolset_checklist",
        lambda *args, **kwargs: {"video_gen"},
    )
    monkeypatch.setattr("kenxcode_cli.tools_config.save_config", lambda config: None)
    monkeypatch.setattr(
        "kenxcode_cli.tools_config._get_enabled_platforms",
        lambda: ["cli"],
    )
    monkeypatch.setattr(
    ...
    configured = []
    monkeypatch.setattr(
        "kenxcode_cli.tools_config._configure_toolset",
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from kenxcode_cli.tools_config import PLATFORMS
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """kenxcode-gateway includes list should cover all messaging platforms."""
        from kenxcode_cli.tools_config import PLATFORMS
        from toolsets import TOOLSETS

        gateway_includes = set(TOOLSETS["kenxcode-gateway"]["includes"])
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from kenxcode_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from kenxcode_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/NousResearch/kenxcode-agent/issues/6901
    """
    config = {
...
class TestImagegenBackendRegistry:
    ...
    def test_fal_backend_registered(self):
        from kenxcode_cli.tools_config import IMAGEGEN_BACKENDS
    ...
    def test_image_gen_providers_tagged_with_fal_backend(self):
        ...
        from kenxcode_cli.tools_config import TOOL_CATEGORIES
...
class TestImagegenModelPicker:
    ...
    def test_picker_writes_chosen_model_to_config(self):
        from kenxcode_cli.tools_config import _configure_imagegen_model
        config = {}
        # Force _prompt_choice to pick index 1 (second-in-ordered-list).
        with patch("kenxcode_cli.tools_config._prompt_choice", return_value=1):
    ...
    def test_picker_with_gpt_image_does_not_prompt_quality(self):
        ...
        config = {}
        with patch("kenxcode_cli.tools_config._prompt_choice", side_effect=fake_prompt):
            _configure_imagegen_model("fal", config)
    ...
    def test_picker_repairs_corrupt_config_section(self):
        ...
        with patch("kenxcode_cli.tools_config._prompt_choice", return_value=0):
            _configure_imagegen_model("fal", config)
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    them twice — otherwise `kenxcode tools` → "reconfigure existing" shows
    the same toolset two rows in a row.
    """
    from kenxcode_cli.tools_config import _get_effective_configurable_toolsets
...
#    reported as added/removed by `kenxcode tools` ──────────────────────────




def test_kanban_not_reported_as_removed_in_diff():
    """Reproduces the false-signal bug: `kenxcode tools` printed ``- kanban``
...
def test_vision_picker_custom_endpoint(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("KENXCODE_HOME", str(tmp_path))
    import kenxcode_cli.tools_config as tc
...
# `kenxcode tools post-setup <key>` child. On Windows each console child (npm,
...
# ── Toolsets that shipped after a platform's last `kenxcode tools` save ────────
#
# Saving the picker (or one toggle in the desktop Toolsets UI) replaces a
# platform's composite (``[kenxcode-cli]``) with a frozen explicit list, and
...
def test_saved_list_gains_toolsets_that_shipped_after_it_was_written():
    ...
    on_composite = _get_platform_tools(
        {"platform_toolsets": {"cli": ["kenxcode-cli"]}},
        "cli",
...
def test_platforms_whose_composite_excludes_it_are_left_narrow():
    ...
    composite (kenxcode-acp, kenxcode-webhook) that never carried the toolset."""
    from toolsets import TOOLSETS, resolve_toolset

    narrow = [
        platform
        for platform in ("acp", "webhook")
        if f"kenxcode-{platform}" in TOOLSETS
        and not any(
            set(resolve_toolset(ts, include_registry=False))
            <= set(resolve_toolset(f"kenxcode-{platform}"))
