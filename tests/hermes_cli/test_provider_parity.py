...
the SAME provider universe as ``kenxcode model`` (the CLI/TUI picker).
...
def test_every_kenxcode_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `kenxcode model` universe."""
    ...
    assert not missing, (
        "providers shown in `kenxcode model` but not configurable in the desktop "
