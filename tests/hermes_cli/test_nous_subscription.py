...
from kenxcode_cli.nous_account import NousPortalAccountInfo, NousToolAccessInfo
from kenxcode_cli import nous_subscription as ns
...
def _capture_checklist(monkeypatch, *, selected_idx):
    ...
    import kenxcode_cli.setup as setup_mod

    monkeypatch.setattr(setup_mod, "prompt_checklist", _fake_checklist, raising=False)
    monkeypatch.setattr(
        "kenxcode_cli.config.save_config", lambda cfg: None, raising=False
    )
    return captured
...
# ---------------------------------------------------------------------------
# ensure_nous_portal_access — inline login gate for `kenxcode tools`
# ---------------------------------------------------------------------------
...
def test_has_agent_browser_resolves_via_kenxcode_managed_node_path(monkeypatch, tmp_path):
    """The managed-Node rung: a runnable agent-browser under the KenXCode Node
    ...
    monkeypatch.setattr(
        "kenxcode_constants.with_kenxcode_node_path", lambda: {"PATH": str(managed_dir)}
    )
    monkeypatch.setattr(
        "kenxcode_constants.agent_browser_runnable",
