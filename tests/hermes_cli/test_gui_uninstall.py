"""Tests for kenxcode_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import kenxcode_cli.gui_uninstall as gu


def _make_agent(kenxcode_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = kenxcode_home / "kenxcode-agent"
    (agent_root / "kenxcode_cli").mkdir(parents=True)
    (agent_root / "kenxcode_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(kenxcode_home: Path) -> None:
    """Create the source-built GUI artifacts a `kenxcode desktop` run produces."""
    desktop = kenxcode_home / "kenxcode-agent" / "apps" / "desktop"
    ...
    (kenxcode_home / "kenxcode-agent" / "node_modules").mkdir(parents=True)
    (kenxcode_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(kenxcode_home: Path) -> None:
    (kenxcode_home / "config.yaml").write_text("x: 1\n")
    (kenxcode_home / ".env").write_text("KEY=secret\n")
    (kenxcode_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(kenxcode_home)
    _make_gui_build(kenxcode_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(kenxcode_home)
    ...
    assert summary["kenxcode_home"] == str(kenxcode_home)
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import kenxcode_cli.uninstall as uninstall
