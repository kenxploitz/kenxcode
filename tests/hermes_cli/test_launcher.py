"""Tests for the top-level `./kenxcode` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./kenxcode` should use `kenxcode_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "kenxcode"
    called = []

    fake_main_module = types.ModuleType("kenxcode_cli.main")

    def fake_main():
        called.append("kenxcode_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "kenxcode_cli.main", fake_main_module)
    ...
    assert called == ["kenxcode_cli.main"]
