...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_kenxcode_home", lambda: kenxcode_home)
    monkeypatch.setattr(uninstall, "_is_default_kenxcode_home", lambda home: False)
    ...
    assert str(kenxcode_home) in output
    assert project_root.exists()
    assert kenxcode_home.exists()
