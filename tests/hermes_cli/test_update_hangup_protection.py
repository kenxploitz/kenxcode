"""Tests for SIGHUP protection and stdout mirroring in ``kenxcode update``.

Covers ``_UpdateOutputStream``, ``_install_hangup_protection``, and
``_finalize_update_output`` in ``kenxcode_cli/main.py``.  These exist so
that ``kenxcode update`` survives a terminal disconnect mid-install
...
class TestInstallHangupProtection:
    ...
    def test_wraps_stdout_and_stderr_with_mirror(self, tmp_path, monkeypatch):
        ...
        import kenxcode_cli.config as _cfg
        if hasattr(_cfg, "_KENXCODE_HOME_CACHE"):
            _cfg._KENXCODE_HOME_CACHE = None  # type: ignore[attr-defined]
        ...
        try:
            ...
            assert "kenxcode update started" in contents
        finally:
    ...
    def test_non_fatal_if_log_setup_fails(self, monkeypatch):
        """If get_kenxcode_home() raises, stdio must be left untouched but SIGHUP still handled."""
        prev_out, prev_err = sys.stdout, sys.stderr
        ...
        monkeypatch.setattr(
            "kenxcode_cli.config.get_kenxcode_home", _boom, raising=True
        )
