"""Tests for kenxcode_cli._early_recovery — the dependency-light bootstrap
repair that runs BEFORE kenxcode_cli.main's third-party imports (#57828 / #58004).

Covers:
- entry-point lifecycle: a broken core import (dotenv) crashes the import of
  kenxcode_cli.main WITHOUT early recovery, and imports fine when recovery runs
...
from kenxcode_cli import _early_recovery as er
...
def _run_lifecycle_subprocess(tmp_path: Path, *, repair: bool) -> subprocess.CompletedProcess:
    ...
    script.write_text(
        textwrap.dedent(
            ...
            import kenxcode_cli._early_recovery as er
            ...
            import kenxcode_cli.main  # noqa: F401
    ...
    env = {
        **os.environ,
        ...
        "KENXCODE_HOME": str(kenxcode_home),
    }
...
def test_broken_dotenv_crashes_main_import_without_repair(tmp_path):
    """Negative control: the shadow really breaks importing kenxcode_cli.main,
...
def test_early_recovery_module_is_stdlib_only(tmp_path):
    ...
    script.write_text(
        textwrap.dedent(
            ...
            STDLIB = set(sys.stdlib_module_names) | {"kenxcode_cli"}
            ...
            import kenxcode_cli._early_recovery  # noqa: F401
