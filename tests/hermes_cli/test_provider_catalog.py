"""Tests for the unified provider catalog (kenxcode_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``kenxcode model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from kenxcode_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
