...
On hosts with many profile-backed ``kenxcode-gateway*.service`` units,
``kenxcode update`` used to wrap the entire per-scope unit loop in a single
...
class TestFleetRestartTimeoutIsolation:
    def test_timeout_on_middle_unit_continues_remaining_units(self):
        units = [
            "kenxcode-gateway-xiaomo1",
            "kenxcode-gateway-xiaomo2",
            "kenxcode-gateway-xiaomo3",
            "kenxcode-gateway-xiaomo4",
            "kenxcode-gateway-xiaomo5",
            "kenxcode-gateway-xiaomo6",
            "kenxcode-gateway-xiaomo7",
            "kenxcode-gateway",
        ]
        restarted: list[str] = []
        failed: list[str] = []
        ...
        def process_unit(svc_name: str) -> None:
            if svc_name == "kenxcode-gateway-xiaomo5":
        ...
        assert failed == ["kenxcode-gateway-xiaomo5"]
        assert restarted == [
            "kenxcode-gateway-xiaomo1",
            "kenxcode-gateway-xiaomo2",
            "kenxcode-gateway-xiaomo3",
        ...
        assert timeout_cmds == [
            ["systemctl", "--user", "--no-ask-password", "restart", "kenxcode-gateway-xiaomo5"]
        ]
    ...
    def test_non_gateway_units_in_list_output_are_ignored(self):
        ...
        _for_each_systemd_gateway_unit(
            "\n".join(
                [
                    ...
                    "kenxcode-gateway-coder.service loaded active running",
        ...
        assert seen == ["kenxcode-gateway-coder"]
    ...
    def test_process_errors_other_than_timeout_still_propagate(self):
        ...
        with pytest.raises(RuntimeError, match="not a timeout"):
            _for_each_systemd_gateway_unit(
                _list_units_stdout(["kenxcode-gateway"]),
                process_unit=process_unit,
...
class TestIncompleteFleetRestartWarning:
    def test_warns_with_exact_unrestarted_units(self, capsys):
        _warn_incomplete_gateway_fleet_restart(
            ["kenxcode-gateway-xiaomo5", "kenxcode-gateway-xiaomo6", "kenxcode-gateway-xiaomo5"]
        )
        out = capsys.readouterr().out
        assert "Update incomplete" in out
        assert out.count("kenxcode-gateway-xiaomo5") == 1
        assert "kenxcode-gateway-xiaomo6" in out
