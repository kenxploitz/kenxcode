...
kenxcode_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `kenxcode config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_kenxcode_home = get_kenxcode_home()`` at import time (module
    singleton), so monkeypatching KENXCODE_HOME after import doesn't move it.
    ...
    this only matters for tests that swap KENXCODE_HOME.)
    """
    import cli

    cli._kenxcode_home = home
