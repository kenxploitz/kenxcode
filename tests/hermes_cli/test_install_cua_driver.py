...
* Be supported-platform-only — no-op silently elsewhere so ``kenxcode update``
...
class TestInstallCuaDriverUpgrade:
    def test_upgrade_on_unsupported_platform_is_silent_noop(self):
        from kenxcode_cli import tools_config
...
class TestRequireConfirmedUpdate:
    """`kenxcode update` passes require_confirmed_update=True: the full
    ...
    Explicit `kenxcode computer-use install --upgrade` keeps the old
    ...
    def test_explicit_upgrade_still_falls_through_on_indeterminate(self):
        # `kenxcode computer-use install --upgrade` (default flag): the old
...
class TestUpdateCheckTimeoutDefaults:
    ...
    full reinstall fall-through during `kenxcode update`.
    """
