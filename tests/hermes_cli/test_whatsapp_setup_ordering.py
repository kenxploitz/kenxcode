...
Before the fix, ``kenxcode whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``kenxcode gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    kenxcode = home / ".kenxcode"
    kenxcode.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("KENXCODE_HOME", str(kenxcode))
...
def _env_value(kenxcode_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from kenxcode_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("kenxcode_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``kenxcode whatsapp`` with an existing paired session and
