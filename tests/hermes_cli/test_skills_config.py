"""Tests for kenxcode_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestGetDisabledSkills:
    def test_empty_config(self):
        from kenxcode_cli.skills_config import get_disabled_skills
...
class TestSaveDisabledSkills:
    @patch("kenxcode_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from kenxcode_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    ...
    @patch("kenxcode_cli.config.load_config")
    ...
    @patch("kenxcode_cli.config.load_config")
    @patch.dict("os.environ", {"KENXCODE_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_explicit_platform_param(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("KENXCODE_PLATFORM", raising=False)
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """KENXCODE_SESSION_PLATFORM should be used when KENXCODE_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_kenxcode_platform_takes_precedence(self, tmp_path, monkeypatch):
        """KENXCODE_PLATFORM should win over KENXCODE_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
        ...
        monkeypatch.setenv("KENXCODE_HOME", str(tmp_path))
        monkeypatch.setenv("KENXCODE_PLATFORM", "telegram")
        monkeypatch.setenv("KENXCODE_SESSION_PLATFORM", "discord")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from kenxcode_cli.skills_config import _get_categories
        skills = [
