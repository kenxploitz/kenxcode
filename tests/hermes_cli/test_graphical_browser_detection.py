"""Tests for `_can_open_graphical_browser()` in kenxcode_cli.auth.

Guards the fix for the May 2026 report where `kenxcode auth add xai-oauth`
...
from kenxcode_cli.auth import _can_open_graphical_browser
...
def _force_platform_linux(monkeypatch):
    monkeypatch.setattr("kenxcode_cli.auth.sys.platform", "linux")
