"""Tests for kenxcode_cli.security_advisories.
...
import kenxcode_cli.security_advisories as adv
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect KENXCODE_HOME so banner cache and config writes are sandboxed."""
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full kenxcode_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "kenxcode_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
            "kenxcode_cli.config.save_config",
