"""Tests for the ``kenxcode send`` CLI subcommand.

Covers the argument parsing / stdin / file / list behavior of
``kenxcode_cli.send_cmd``. The underlying ``send_message_tool`` is stubbed so
...
from kenxcode_cli import send_cmd
...
def test_load_kenxcode_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    ``kenxcode send`` from a fresh shell cannot resolve the home channel
    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``kenxcode config set``) lives
    ...
    kenxcode_home.mkdir()
    (kenxcode_home / ".env").write_text("SOME_TOKEN=abc123\n")
    ...
    # Force get_kenxcode_home() to re-resolve under the patched env.
    from importlib import reload

    import kenxcode_cli.config as _hc_config
    reload(_hc_config)

    send_cmd._load_kenxcode_env()
