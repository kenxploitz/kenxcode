...
from kenxcode_cli.copilot_auth import _try_gh_cli_token
...
from kenxcode_cli.auth import PROVIDER_REGISTRY as _REGISTRY
...
_EXTRA_ENV_VARS = (
    ...
    "KENXCODE_COPILOT_ACP_COMMAND", "COPILOT_CLI_PATH",
    "KENXCODE_COPILOT_ACP_ARGS", "COPILOT_ACP_BASE_URL",
)
...
def _clear_provider_env(monkeypatch):
    ...
    monkeypatch.setattr("kenxcode_cli.auth._load_auth_store", lambda: {})
...
class TestResolveApiKeyProviderCredentials:
    ...
    def test_try_gh_cli_token_uses_homebrew_path_when_not_on_path(self, monkeypatch):
        monkeypatch.setattr("kenxcode_cli.copilot_auth.shutil.which", lambda command: None)
        monkeypatch.setattr(
            "kenxcode_cli.copilot_auth.os.path.isfile",
            lambda path: path == "/opt/homebrew/bin/gh",
        )
        monkeypatch.setattr(
            "kenxcode_cli.copilot_auth.os.access",
        ...
        monkeypatch.setattr("kenxcode_cli.copilot_auth.subprocess.run", _fake_run)
...
class TestRuntimeProviderResolution:
    ...
    def test_runtime_copilot_uses_gh_cli_token(self, monkeypatch):
        monkeypatch.setattr("kenxcode_cli.copilot_auth._try_gh_cli_token", lambda: "gho_cli_secret")
        from kenxcode_cli.runtime_provider import resolve_runtime_provider
    ...
    def test_runtime_copilot_uses_responses_for_gpt_5_4(self, monkeypatch):
        monkeypatch.setattr("kenxcode_cli.copilot_auth._try_gh_cli_token", lambda: "gho_cli_secret")
        monkeypatch.setattr(
            "kenxcode_cli.runtime_provider._get_model_config",
            lambda: {"provider": "copilot", "default": "gpt-5.4"},
        )
        monkeypatch.setattr(
            "kenxcode_cli.models.fetch_github_model_catalog",
    ...
    def test_runtime_copilot_acp_uses_process_runtime(self, monkeypatch):
        monkeypatch.setattr("kenxcode_cli.auth.shutil.which", lambda command: f"/usr/local/bin/{command}")
        monkeypatch.setenv("KENXCODE_COPILOT_ACP_ARGS", "--acp --stdio --debug")
...
class TestHasAnyProviderConfigured:
    ...
    def test_claude_code_creds_ignored_on_fresh_install(self, monkeypatch, tmp_path):
        """Claude Code credentials should NOT skip the wizard when KenXCode is unconfigured."""
        from kenxcode_cli import config as config_module
        from kenxcode_cli.auth import PROVIDER_REGISTRY
        kenxcode_home = tmp_path / ".kenxcode"
        kenxcode_home.mkdir()
        monkeypatch.setattr(config_module, "get_env_path", lambda: kenxcode_home / ".env")
        monkeypatch.setattr(config_module, "get_kenxcode_home", lambda: kenxcode_home)
        monkeypatch.setattr("kenxcode_cli.copilot_auth.resolve_copilot_token", lambda: ("", ""))
        ...
        monkeypatch.setattr("kenxcode_cli.auth.get_auth_status", lambda _pid: {})
        ...
        from kenxcode_cli.main import _has_any_provider_configured
    ...
    def test_config_provider_counts(self, monkeypatch, tmp_path):
        ...
        kenxcode_home = tmp_path / ".kenxcode"
        kenxcode_home.mkdir()
        config_file = kenxcode_home / "config.yaml"
...
class TestKimiCodeCredentialAutoDetect:
    ...
    def test_non_kimi_providers_unaffected(self, monkeypatch):
        ...
        monkeypatch.setattr("kenxcode_cli.auth.detect_zai_endpoint", lambda *a, **kw: None)
...
class TestZaiEndpointAutoDetect:
    ...
    def test_env_override_skips_probe(self, monkeypatch):
        ...
        monkeypatch.setattr("kenxcode_cli.auth.detect_zai_endpoint", _never_called)
        creds = resolve_api_key_provider_credentials("zai")
...
class TestKimiMoonshotModelListIsolation:
    ...
    def test_moonshot_list_excludes_coding_plan_only_models(self):
        from kenxcode_cli.main import _PROVIDER_MODELS
...
class TestHuggingFaceModels:
    ...
    def test_model_lists_match(self):
        ...
        from kenxcode_cli.main import _PROVIDER_MODELS as main_models
        from kenxcode_cli.models import _PROVIDER_MODELS as models_models
...
class TestNovitaProvider:
    ...
    def test_novita_pricing_cache(self, monkeypatch):
        ...
        from kenxcode_cli import models as models_mod
...
def _deepinfra_cache_isolation(monkeypatch):
    ...
    """
    import kenxcode_cli.models as _models_mod
...
class TestFetchDeepInfraModels:
    ...
    def test_returns_filtered_models_on_success(self, monkeypatch):
        ...
        import kenxcode_cli.models as models
        monkeypatch.setattr(
            models, "_urlopen_model_catalog_request", lambda *a, **kw: _Resp()
        )
        from kenxcode_cli.models import _fetch_deepinfra_models
...
class TestDeepInfraTagFiltering:
    ...
    def test_filters_by_surface_tag_and_handles_rollout_states(self, monkeypatch):
        ...
        from kenxcode_cli.models import _fetch_deepinfra_models_by_tag
        import kenxcode_cli.models as _m
...
class TestDeepInfraProviderProfile:
    ...
    def test_profile_registered_with_alias_and_aux(self):
        ...
        from kenxcode_cli.auth import PROVIDER_REGISTRY, resolve_provider
        from kenxcode_cli.config import OPTIONAL_ENV_VARS
        from kenxcode_cli.models import CANONICAL_PROVIDERS
