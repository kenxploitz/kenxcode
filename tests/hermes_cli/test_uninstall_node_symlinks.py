"""Tests for kenxcode_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $KENXCODE_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the KenXCode-managed node dir.
"""
...
def _make_kenxcode_node(kenxcode_home: Path) -> Path:
    """Create a fake $KENXCODE_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = kenxcode_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    kenxcode_home = fake_home / ".kenxcode"
    _make_kenxcode_node(kenxcode_home)
    ...
    removed = uninstall.remove_node_symlinks(kenxcode_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    kenxcode_home = fake_home / ".kenxcode"
    node_bin = _make_kenxcode_node(kenxcode_home)
