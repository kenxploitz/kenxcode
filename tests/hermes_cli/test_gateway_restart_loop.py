...
Covers:
- Defense 1: gateway stop/restart refuse when _KENXCODE_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "kenxcode  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "KENXCODE GATEWAY RESTART",           # uppercase
    ])
    def test_kenxcode_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.kenxcode.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.kenxcode/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l kenxcode-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.kenxcode.gateway.restart-once.plist",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.kenxcode.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.kenxcode/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated kenxcode note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        "kenxcode cron list",
        "kenxcode update",
        "kenxcode config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `kenxcode gateway start` is benign — starting a gateway from inside a
        ...
        "kenxcode gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway kenxcode
        # services must not be falsely blocked (the old `.*kenxcode` matched any
        # kenxcode token).
        "launchctl unload ai.kenxcode.update-checker.plist",
        "launchctl restart ai.kenxcode.daemon",
        ...
        "systemctl restart kenxcode-meta.service",
        "systemctl restart kenxcode-cron-helper",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_kenxcode_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade kenxcode then run kenxcode gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_kenxcode_gateway_once.sh). The script must live under
        # KENXCODE_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("KENXCODE_HOME", str(tmp_path / ".kenxcode"))
        scripts_dir = tmp_path / ".kenxcode" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    """Verify kenxcode gateway stop/restart refuse when _KENXCODE_GATEWAY=1."""

    def test_stop_refuses_inside_gateway(self, monkeypatch):
        monkeypatch.setenv("_KENXCODE_GATEWAY", "1")
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_KENXCODE_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _KENXCODE_GATEWAY=1.

    Issue #37453: systemctl --user restart kenxcode-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart kenxcode-gateway",
        "systemctl --user restart kenxcode-gateway",
        "systemctl stop kenxcode-gateway.service",
        "kenxcode gateway restart",
        "launchctl kickstart gui/501/ai.kenxcode.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.kenxcode.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.kenxcode/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.kenxcode.gateway.restart-once.plist",
        "pkill -f kenxcode.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart kenxcode-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.kenxcode.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-kenxcode label: label-independent detection is the point
        # (#62891 second reproduction used `ai.kenxcode.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.kenxcode.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.kenxcode.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.kenxcode.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-kenxcode systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run kenxcode gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("kenxcode gateway stop\n")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfekenxcode gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under KENXCODE_HOME/scripts (the
        ...
        (scripts_dir / "restart.sh").write_text(
            "launchctl kickstart -k gui/501/ai.kenxcode.gateway\n"
        )
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("kenxcode gateway restart")\n')
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    kenxcode_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run kenxcode gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KENXCODE_HOME", str(tmp_path / ".kenxcode"))
        (tmp_path / ".kenxcode").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run kenxcode gateway restart nightly",
        ))
