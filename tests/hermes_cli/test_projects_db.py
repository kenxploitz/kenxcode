"""Tests for the per-profile Projects store (kenxcode_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="KenXCode Agent", folders=["/tmp/kenxcode"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "kenxcode-agent"
    assert proj.name == "KenXCode Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/kenxcode"
    assert [f.path for f in proj.folders] == ["/tmp/kenxcode"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "kenxcode-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' KENXCODE_HOME.
