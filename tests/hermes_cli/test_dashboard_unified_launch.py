...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "kenxcode_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # KENXCODE_HOME.  For a standard install (KENXCODE_HOME unset) that root is
        # the platform-native default (~/.kenxcode), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from kenxcode_constants import get_default_kenxcode_root
        assert env.get("KENXCODE_HOME") == str(get_default_kenxcode_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (KENXCODE_DESKTOP=1) must NOT
