...
The cache exists to keep `kenxcode tools` → "All Platforms" fast: every
...
def test_remove_env_value_invalidates_cache(tmp_path, monkeypatch):
    ...
    from kenxcode_cli import config as config_mod
    ...
    monkeypatch.setattr(config_mod, "ensure_kenxcode_home", lambda: None)
