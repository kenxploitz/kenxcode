...
def test_discover_all_plugins_includes_entrypoint_plugins(monkeypatch, tmp_path):
    ...
    dist = SimpleNamespace(
        version="0.1.0",
        metadata={"Summary": "Karpathy-style LLM Wikis for KenXCode"},
    )
    entry_point = SimpleNamespace(
        name="wiki",
        value="adapters.kenxcode.cli_plugin",
        group="kenxcode_agent.plugins",
        dist=dist,
    )

    monkeypatch.setattr(plugins_cmd, "_plugins_dir", lambda: user_dir)
    monkeypatch.setattr(
        "kenxcode_cli.plugins.get_bundled_plugins_dir",
        lambda: bundled_dir,
    )
    monkeypatch.setattr(
    ...
    assert entries == [
        (
            "wiki",
            "0.1.0",
            "Karpathy-style LLM Wikis for KenXCode",
            "entrypoint",
            "adapters.kenxcode.cli_plugin",
            "wiki",
        )
    ]
