...
class TestSlackNativeSlashes:
    ...
    def test_telegram_parity(self):
        ...
        # Commands deliberately routed through /kenxcode <command> on Slack only
        # (Slack's 50-slash cap) are expected to be absent from native slashes.
        via_kenxcode_norm = {_norm(n) for n in _SLACK_VIA_KENXCODE_ONLY}
        missing = (tg_norm - slack_norm) - reserved_norm - via_kenxcode_norm
        assert not missing, (
...
class TestSlackAppManifest:
    """Generated Slack app manifest (used by `kenxcode slack manifest`)."""
...
class TestSubcommandCompletion:
    ...
    def test_tools_enable_skips_already_listed(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "kenxcode_cli.tools_config._get_plugin_toolset_keys",
            lambda: set(),
        )
...
from kenxcode_cli.commands import discord_skill_commands_by_category  # noqa: E402
...
class TestPluginCommandEnumeration:
    ...
    def _patch_plugin_commands(self, monkeypatch, commands):
        """Monkeypatch kenxcode_cli.plugins.get_plugin_commands() to a fixed dict."""
        from kenxcode_cli import plugins as _plugins_mod
