"""Tests for Codex auth — tokens stored in KenXCode auth store (~/.kenxcode/auth.json)."""
...
def _setup_kenxcode_auth(kenxcode_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the KenXCode auth store."""
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    ...
    _setup_kenxcode_auth(kenxcode_home, access_token="")
...
def test_resolve_codex_runtime_credentials_falls_back_to_pool_when_singleton_empty(tmp_path, monkeypatch):
    ...
    (kenxcode_home / "auth.json").write_text(json.dumps(auth_store))
    monkeypatch.setenv("KENXCODE_HOME", str(kenxcode_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((kenxcode_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``kenxcode auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``kenxcode auth add openai-codex``.
    ...
    (kenxcode_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `kenxcode auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``kenxcode auth add openai-codex`` for accounts B and C
    ...
    (kenxcode_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``kenxcode auth add openai-codex`` (account B and account C).
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to KenXCode auth store, not ~/.codex/."""
    kenxcode_home = tmp_path / "kenxcode"
    ...
    _save_codex_tokens({"access_token": "kenxcode-at", "refresh_token": "kenxcode-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches KenXCode store
    assert not (codex_home / "auth.json").exists()

    # KenXCode auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "kenxcode-at"


def test_resolve_returns_kenxcode_auth_store_source(tmp_path, monkeypatch):
    kenxcode_home = tmp_path / "kenxcode"
    _setup_kenxcode_auth(kenxcode_home)
    monkeypatch.setenv("KENXCODE_HOME", str(kenxcode_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "kenxcode-auth-store"
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run kenxcode auth".
    """
    from kenxcode_cli.auth import (
    ...
    assert "kenxcode auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from kenxcode_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from kenxcode_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def _patch_httpx_post(monkeypatch, responses):
    """Patch kenxcode_cli.auth.httpx.Client so .post() returns queued responses."""
    seq = iter(responses)
    ...
    monkeypatch.setattr("kenxcode_cli.auth.httpx.Client", lambda *a, **k: _FakeClient())
