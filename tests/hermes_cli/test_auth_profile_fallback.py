...
When ``KENXCODE_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.kenxcode/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.kenxcode            (has its own auth.json fixture)
    * Profile     -> tmp_path/.kenxcode/profiles/coder   (active, KENXCODE_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"KenXCode is not
...
def test_auth_lock_reentrancy_is_scoped_after_profile_context_switch(profile_env):
    ...
    import kenxcode_cli.auth as auth
    ...
    with auth._auth_store_lock():
        ...
        token = set_kenxcode_home_override(profile_b)
        try:
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (KENXCODE_HOME != ~/.kenxcode, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    kenxcode_home = tmp_path / "classic"
    kenxcode_home.mkdir()
