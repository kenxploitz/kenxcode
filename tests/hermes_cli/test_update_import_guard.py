"""Tests for the post-update *import* guard in ``kenxcode update``.
...
from kenxcode_cli import update_cmd
...
def test_syntax_guard_passes_but_import_guard_catches_skew(monkeypatch, tmp_path):
    ...
    # NOTE: patch update_cmd's global, not kenxcode_main's. Both modules expose
    ...
    syntax_ok, _, _ = kenxcode_main._validate_critical_files_syntax(tmp_path)
    ...
    ok, module, error = kenxcode_main._validate_critical_modules_import(tmp_path)
    assert ok is False
...
def test_import_guard_passes_on_consistent_tree(monkeypatch, tmp_path):
    ...
    assert kenxcode_main._validate_critical_modules_import(tmp_path) == (True, None, None)
...
def test_import_guard_ignores_non_import_errors(monkeypatch, tmp_path):
    ...
    ok, _, _ = kenxcode_main._validate_critical_modules_import(tmp_path)
    assert ok is True
...
def test_hint_fires_for_first_party_import_error():
    ...
    assert any("kenxcode update" in line for line in hint)
...
def test_import_guard_prefers_the_project_venv_interpreter(monkeypatch, tmp_path):
    """``kenxcode update`` can run under a different Python than the install's.
...
@pytest.mark.parametrize("modname", ["agents", "agentops", "toolsets_x", "kenxcodex"])
...
def test_probe_and_hint_share_one_first_party_definition():
    ...
    `cli` was first-party to the hint but not the probe, and `kenxcodex`
    (third-party) matched the probe's loose `startswith("kenxcode")`. A user
    ...
    """
    from kenxcode_constants import FIRST_PARTY_MODULE_ROOTS, is_first_party_module
    ...
    for lookalike in ("agents", "agentops", "toolsets_x", "kenxcodex"):
