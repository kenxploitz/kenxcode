...
``kenxcode_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``kenxcode --help``, ``kenxcode version``,
``kenxcode logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``kenxcode --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from kenxcode_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["kenxcode", "--help"]
    buf = io.StringIO()
    try:
...
def test_deferred_platform_cli_resolution_targets_matching_platform():
    ...
    Photon registers ``kenxcode photon`` only when its adapter module is
...
def test_deferred_platform_loader_registers_cli_command_before_parser_table():
    ...
    helper calls it. This regression exercises the full chain kenxcode-sweeper
    asked for:
    ...
    from kenxcode_cli.plugins import PluginContext, PluginManager, PluginManifest
    ...
    def _fake_loader():
        ...
        # top-level kenxcode <name> CLI command via PluginContext.
