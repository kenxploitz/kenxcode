...
kenxcode_cli.config.load_config, so the managed overlay has to be wired into each.
...
def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # kenxcode_time checks an env override first; ensure it's unset so config wins.
    monkeypatch.delenv("KENXCODE_TIMEZONE", raising=False)
    monkeypatch.delenv("TZ", raising=False)
    _seed(home, managed, user="timezone: America/New_York\n", mgd="timezone: Asia/Tokyo\n")
    import kenxcode_time

    assert kenxcode_time._resolve_timezone_name() == "Asia/Tokyo"
...
def test_gateway_env_bridge_honors_managed(homes, monkeypatch):
    ...
    every turn (KENXCODE_TIMEZONE, KENXCODE_REDACT_SECRETS, KENXCODE_MAX_ITERATIONS,
    ...
    # writing KENXCODE_TIMEZONE = cfg["timezone"]. Prove the overlay flips the value.
    import yaml
