...
def _client():
    ...
    # Keep the state DB under the isolated KENXCODE_HOME for any handler that
    # touches it.
...
class TestMcpEndpoints:
    ...
    def _setup(self, _isolate_kenxcode_home):
    ...
    def test_http_bearer_auth_separates_secret_from_config(
        ...
        config_text = (kenxcode_home / "config.yaml").read_text()
        env_text = (kenxcode_home / ".env").read_text()
    ...
    def test_http_oauth_mode_is_persisted_for_existing_auth_flow(self):
        ...
        from kenxcode_cli.mcp_config import _get_mcp_servers
...
class TestCredentialPoolEndpoints:
    ...
    def test_env_seeded_delete_stays_deleted(self):
        ...
        load_pool() re-seeds from ~/.kenxcode/.env on every call, so removing
        just the pool row silently reverts on the next dashboard refresh.
        The endpoint must mirror `kenxcode auth remove`: clean up the backing
        source and suppress (provider, source).
        """
        from agent.credential_pool import load_pool
        from kenxcode_cli.auth import is_source_suppressed
    ...
    def test_post_readd_lifts_suppression(self):
        ...
        Mirrors `kenxcode auth add`, which clears every suppression for the
...
class TestMemoryEndpoints:
    ...
    def _setup(self, _isolate_kenxcode_home):
        ...
        (get_kenxcode_home() / "memories").mkdir(parents=True, exist_ok=True)
    ...
    def test_reset_targets(self):
        ...
        mem = get_kenxcode_home() / "memories"
...
class TestPairingEndpoints:
    ...
    def test_pairing_is_isolated_per_profile(self):
        ...
        (get_kenxcode_home() / "profiles" / "work").mkdir(parents=True, exist_ok=True)
...
class TestWebhookEndpoints:
    ...
    def test_enable_platform_starts_gateway_restart(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_kenxcode_action", fake_spawn_action)
...
class TestSystemStatsEndpoint:
    ...
    def test_stats_shape(self):
        ...
        for key in ("os", "arch", "hostname", "python_version", "kenxcode_version"):
    ...
    def test_sources_lists_configured_hubs(self, monkeypatch):
        ...
        class _Src:
            ...
            def search(self, q, limit=10):
                return [_FakeMeta("kenxcode-index/featured-skill", "trusted")]

        def _fake_router():
            srcs = [_Src("official"), _Src("github")]
            # kenxcode-index source advertises availability + featured search.
            idx = _Src("kenxcode-index")
        ...
        assert {"official", "github", "kenxcode-index"} <= ids
...
class TestSkillsHubPreviewEndpoint:
    ...
    def test_preview_returns_skill_md_text(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "kenxcode_cli.skills_hub._resolve_source_meta_and_bundle",
...
class TestUpdateCheckEndpoint:
    """``GET /api/kenxcode/update/check`` reports availability without applying.

    Powers the dashboard's check-before-you-update flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/kenxcode/update`` runs ``kenxcode update``.
    """
    ...
    def test_git_install_reports_behind_count(self, monkeypatch):
        ...
        import kenxcode_cli.banner as banner

        monkeypatch.setattr(banner, "check_for_updates", lambda: 5)

        r = self.client.get("/api/kenxcode/update/check")
        assert r.status_code == 200
        body = r.json()
        assert {
            "install_method",
    ...
    def test_managed_runtime_dashboard_is_not_applyable(self, monkeypatch):
        ...
        body = self.client.get("/api/kenxcode/update/check").json()
...
class TestDebugShareEndpoint:
    ...
    def test_redact_false_is_honored(self, monkeypatch):
        import kenxcode_cli.debug as dbg
        ...
        monkeypatch.setattr("kenxcode_cli.dump.run_dump", lambda a: None)
...
class TestToolsConfigEndpoints:
    ...
    the dashboard surface that replicates the `kenxcode tools` configurator."""
...
# ---------------------------------------------------------------------------
# _spawn_kenxcode_action env scrubbing (#52470)
# ---------------------------------------------------------------------------

def test_spawn_kenxcode_action_scrubs_gateway_loop_guard_env(monkeypatch, tmp_path):
    """The dashboard runs inside the gateway, so os.environ has
    _KENXCODE_GATEWAY=1. Spawned actions (e.g. `gateway restart`) must NOT inherit
    ...
    ws._spawn_kenxcode_action(["gateway", "restart"], "gateway-restart")

    assert "_KENXCODE_GATEWAY" not in captured["env"]
    assert captured["env"]["KENXCODE_NONINTERACTIVE"] == "1"
