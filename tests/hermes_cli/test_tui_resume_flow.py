...
def main_mod(monkeypatch):
    import kenxcode_cli.main as mod
...
def test_termux_skips_bundled_skill_sync_when_stamp_fresh(monkeypatch, tmp_path, main_mod):
    ...
    monkeypatch.setattr(main_mod, "get_kenxcode_home", lambda: tmp_path)
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        """
        import kenxcode_cli.oneshot as oneshot
        from kenxcode_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """kenxcode -z bypasses KenXCodeCLI, but recall still needs SessionDB."""
    from kenxcode_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "kenxcode_state", mod("kenxcode_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "kenxcode_cli.config",
        mod("kenxcode_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "kenxcode_cli.models",
        mod("kenxcode_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
        "kenxcode_cli.runtime_provider",
        mod(
            "kenxcode_cli.runtime_provider",
    ...
    monkeypatch.setitem(
        sys.modules,
        "kenxcode_cli.tools_config",
        mod("kenxcode_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["KENXCODE_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nous/kenxcode-test", provider="nous", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["KENXCODE_MODEL"] == "nous/kenxcode-test"
    assert env["KENXCODE_INFERENCE_MODEL"] == "nous/kenxcode-test"
    assert env["KENXCODE_TUI_PROVIDER"] == "nous"
    assert env["KENXCODE_INFERENCE_PROVIDER"] == "nous"
    assert env["KENXCODE_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["KENXCODE_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("kenxcode-tui-active-session-")
...
def test_make_tui_argv_dev_prebuilds_kenxcode_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("KENXCODE_TUI_DIR", raising=False)
