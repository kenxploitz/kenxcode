...
class TestGoogleWorkspaceCredentialFiles:
    ...
    def test_entries_are_registered_when_files_exist(self, tmp_path):
        ...
        kenxcode_home.mkdir()
        (kenxcode_home / "google_token.json").write_text("{}")
        (kenxcode_home / "google_client_secret.json").write_text("{}")
        ...
        try:
            ...
            assert "/root/.kenxcode/google_token.json" in container_paths
            assert "/root/.kenxcode/google_client_secret.json" in container_paths
        finally:
            clear_credential_files()
