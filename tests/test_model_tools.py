...
class TestHandleFunctionCall:
    ...
    def test_post_tool_call_receives_non_negative_integer_duration_ms(self):
        ...
        with (
            ...
            patch("kenxcode_cli.plugins.has_hook", return_value=True),
            patch("kenxcode_cli.plugins.invoke_hook") as mock_invoke_hook,
        ):
    ...
    def test_tool_request_and_execution_middleware_wrap_registry_dispatch(self, monkeypatch):
        ...
        monkeypatch.setattr("kenxcode_cli.plugins.invoke_middleware", fake_invoke_middleware)
        ...
        monkeypatch.setattr("kenxcode_cli.plugins.has_hook", lambda name: True)
...
class TestPreToolCallBlocking:
    ...
    def test_blocked_tool_returns_error_and_skips_dispatch(self, monkeypatch):
        ...
        monkeypatch.setattr("kenxcode_cli.plugins.invoke_hook", fake_invoke_hook)
        monkeypatch.setattr("kenxcode_cli.plugins.has_hook", lambda name: True)
        monkeypatch.setattr("model_tools.registry.dispatch", fake_dispatch)
    ...
    def test_relay_rewrite_is_visible_to_pre_tool_authorization(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "kenxcode_cli.observability.relay_runtime.apply_tool_request_intercepts",
            rewrite,
        )
        monkeypatch.setattr("kenxcode_cli.plugins.invoke_hook", fake_invoke_hook)
        monkeypatch.setattr("kenxcode_cli.plugins.has_hook", lambda name: True)
...
class TestDisabledToolsetsPlatformBundle:
    """Regression test for #33924: disabling a platform bundle (kenxcode-*)
    must not remove core tools from other enabled toolsets."""

    def test_disabling_platform_bundle_preserves_core_tools(self):
        """Disabling kenxcode-yuanbao should not strip core tools from kenxcode-telegram."""
        from model_tools import get_tool_definitions

        tools_telegram = get_tool_definitions(
            enabled_toolsets=["kenxcode-telegram"],
            quiet_mode=True,
        )
        tools_telegram_no_yuanbao = get_tool_definitions(
            enabled_toolsets=["kenxcode-telegram"],
            disabled_toolsets=["kenxcode-yuanbao"],
            quiet_mode=True,
        )
        ...
        assert names_telegram == names_no_yuanbao, (
            f"Tools lost after disabling kenxcode-yuanbao: "
            f"{names_telegram - names_no_yuanbao}"
        )

    def test_disabling_platform_bundle_removes_own_tools(self):
        """Disabling kenxcode-discord should remove discord-specific tools."""
        from model_tools import get_tool_definitions

        tools = get_tool_definitions(
            enabled_toolsets=["kenxcode-discord"],
            disabled_toolsets=["kenxcode-discord"],
            quiet_mode=True,
        )
    ...
    def test_bundle_non_core_tools_unknown_falls_back(self):
        ...
        assert bundle_non_core_tools("kenxcode-does-not-exist") == set()
...
class TestDisabledToolsetsPostureToolset:
    ...
    not own -- same non-core-delta subtraction as kenxcode-* bundles (#33924) --
