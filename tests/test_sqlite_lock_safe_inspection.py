"""POSIX advisory locks must survive KenXCode' own database inspection.
...
KenXCode used to byte-probe live databases in several places (kanban's
post-commit page-count check, the zeroed-state.db detector run on every
SessionDB construction, backup header verification). Under `kenxcode sessions
...
out across KenXCode' inspection calls.
"""
...
from kenxcode_cli.sqlite_safe_read import (
...
def clean_registry():
    ...
    import kenxcode_cli.sqlite_safe_read as mod
...
def test_failed_close_keeps_connection_tracked(tmp_path, clean_registry):
    ...
    class ControllableConnection(sqlite3.Connection):
        def close(self):
            if getattr(self, "_kenxcode_fail_close", False):
    ...
    conn._kenxcode_fail_close = True
    ...
    conn._kenxcode_fail_close = False
    conn.close()
    assert not has_live_connection(db)
    assert read_header_bytes_preopen(db, length=16) is not None
...
def test_probe_and_connect_do_not_race(tmp_path, clean_registry, monkeypatch):
    ...
    """
    import kenxcode_cli.sqlite_safe_read as ssr
