...
def kenxcode_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves KENXCODE_HOME.
...
@pytest.fixture()
def server(kenxcode_home, monkeypatch):
    ...
    # Pin config resolution to the isolated KENXCODE_HOME. Sibling test
    ...
    # module-level ``_kenxcode_home = get_kenxcode_home()`` snapshot freezes
    ...
    monkeypatch.setattr(mod, "_kenxcode_home", kenxcode_home)
...
def test_moa_bare_returns_usage(server, session, kenxcode_home):
    _write_moa_config(kenxcode_home, """
moa:
