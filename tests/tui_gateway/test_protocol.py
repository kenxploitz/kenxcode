...
def server():
    ...
    # e.g. kenxcode_cli.active_sessions would bind the mocked get_kenxcode_home
    ...
    with patch.dict("sys.modules", {
        "kenxcode_constants": MagicMock(get_kenxcode_home=MagicMock(return_value="/tmp/kenxcode_test")),
        "kenxcode_cli.env_loader": MagicMock(),
        "kenxcode_cli.banner": MagicMock(),
        "kenxcode_state": MagicMock(),
    }):
        import importlib
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `KENXCODE_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("KENXCODE_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("KENXCODE_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_config_roundtrip(server, tmp_path):
    server._kenxcode_home = tmp_path
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/kenxcode-agent-dev": {"name": "kenxcode-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "kenxcode-agent-dev", "session_id": sid},
        })
...
def test_skin_live_switch_end_to_end(server, tmp_path, monkeypatch):
    """Real config + skin files: activating a skin (as `kenxcode config set` does)
    ...
    import kenxcode_cli.skin_engine as skin_engine

    (tmp_path / "skins").mkdir()
    (tmp_path / "skins" / "midnight.yaml").write_text(
        "name: midnight\ndescription: t\ncolors:\n  banner_title: '#00ffcc'\n  background: '#001010'\n"
    )
    monkeypatch.setattr(skin_engine, "get_kenxcode_home", lambda: tmp_path)
    ...
    # Activate midnight, as `kenxcode config set display.skin midnight` would.
