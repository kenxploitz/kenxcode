...
def server():
    ...
    with patch.dict(
        "sys.modules",
        {
            "kenxcode_constants": MagicMock(
                get_kenxcode_home=MagicMock(return_value="/tmp/kenxcode_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'kenxcode-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'kenxcode-release' patched"
    }
