"""Harness: dashboard opt-in via KENXCODE_DASHBOARD.

Today (tini): dashboard starts once when KENXCODE_DASHBOARD=1; if it crashes
...
Every ``docker exec`` here runs as the unprivileged ``kenxcode`` user
...
def test_dashboard_not_running_by_default(
    ...
    """Without KENXCODE_DASHBOARD, no dashboard process should be running."""
    start_container(built_image, container_name, cmd="sleep 60")
    r = docker_exec(container_name, "pgrep", "-f", "kenxcode dashboard")
    # pgrep exits non-zero when no match found
    assert r.returncode != 0, (
        "Dashboard should not be running without KENXCODE_DASHBOARD"
    )
...
# whenever `KENXCODE_DASHBOARD_HOST` was non-loopback, silently disabling
...
"""
    ...
    probe = (
        "/opt/kenxcode/.venv/bin/python - <<'PY'\n"
        f"{py_program}"
        "PY"
    )
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    provider activates whenever ``KENXCODE_DASHBOARD_OAUTH_CLIENT_ID`` is
    set).
    ...
    start_container(
        ...
        "KENXCODE_DASHBOARD_HOST=0.0.0.0",
        "KENXCODE_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
        cmd="sleep 120",
    )
    ...
    assert "nous" in provider_names, (
        ...
        f"KENXCODE_DASHBOARD_OAUTH_CLIENT_ID is set. Got: {payload!r}"
    )
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and KENXCODE_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_no_longer_bypasses_gate(
    ...
    """``KENXCODE_DASHBOARD_INSECURE=1`` NO LONGER disables the auth gate
