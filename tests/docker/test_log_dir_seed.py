"""Runtime smoke test for Docker $KENXCODE_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the kenxcode user after container boot.
...
registered later runs its log service as the dropped kenxcode user and
...
def test_logs_gateways_seeded_and_kenxcode_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by kenxcode after boot."""
    ...
    # Both must be owned by kenxcode
    r = docker_exec_sh(
        container_name,
    ...
    assert "logs=kenxcode" in r.stdout, (
        f"logs/ not owned by kenxcode: {r.stdout}"
    )
    assert "gateways=kenxcode" in r.stdout, (
        f"logs/gateways/ not owned by kenxcode: {r.stdout}"
    )
