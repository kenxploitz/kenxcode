...
Build the real image and verify: a config.yaml present in $KENXCODE_HOME
is migrated by docker_config_migrate.py on boot, running as the kenxcode
user.
"""
...
def test_config_migration_runs_on_boot(
    ...
    """A config.yaml in $KENXCODE_HOME must be migrated on boot by
    docker_config_migrate.py, running as the kenxcode user."""
    # Start container
    ...
    assert "EXISTS" in r.stdout, (
        f"config.yaml not found in $KENXCODE_HOME: {r.stdout}"
    )

    # Verify the migration script exists in the image
    r = docker_exec_sh(
        container_name,
        "test -f /opt/kenxcode/scripts/docker_config_migrate.py && "
    ...
    # Verify config.yaml is owned by kenxcode (migration ran as kenxcode)
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/config.yaml',
        timeout=10,
    )
    assert r.stdout.strip() == "kenxcode", (
        f"config.yaml not owned by kenxcode (migration may have run as root): "
        f"{r.stdout.strip()}"
    )
