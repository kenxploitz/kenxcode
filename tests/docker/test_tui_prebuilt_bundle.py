...
package-lock.json. Without KENXCODE_TUI_DIR set, ``_make_tui_argv`` falls
...
The fix is ``ENV KENXCODE_TUI_DIR=/opt/kenxcode/ui-tui`` in the Dockerfile, which
...
def _exec_py(image: str, py: str) -> str:
    """Run a Python snippet inside the image as the kenxcode user, return stdout."""
    inner = (
        "source /opt/kenxcode/.venv/bin/activate && "
        "cd /opt/kenxcode && "
        f"python3 -c {shlex.quote(py)}"
    )
    # Drop to the kenxcode user (UID 10000) so we exercise the same path the
    # dashboard PTY child runs as — not root.
    cmd = [
        "docker", "run", "--rm", "--entrypoint", "su", image,
        "kenxcode", "-s", "/bin/bash", "-c", inner,
    ]
...
def test_kenxcode_tui_dir_env_is_set(built_image: str) -> None:
    """KENXCODE_TUI_DIR must point at the prebuilt bundle dir in the image."""
    r = subprocess.run(
        ["docker", "run", "--rm", "--entrypoint", "sh", built_image,
         "-c", 'printf "%s" "$KENXCODE_TUI_DIR"'],
        capture_output=True, text=True, timeout=60,
    )
    assert r.returncode == 0, r.stderr[-2000:]
    assert r.stdout.strip() == "/opt/kenxcode/ui-tui", (
        f"KENXCODE_TUI_DIR={r.stdout.strip()!r} (expected /opt/kenxcode/ui-tui)"
    )
...
def test_prebuilt_bundle_present_and_no_runtime_install(built_image: str) -> None:
    ...
    py = (
        ...
        "from kenxcode_cli.main import _tui_need_npm_install, _find_bundled_tui, _make_tui_argv\n"
        "ui = Path('/opt/kenxcode/ui-tui')\n"
    ...
    # With KENXCODE_TUI_DIR set, _make_tui_argv returns the prebuilt path BEFORE
