...
Override the image with ``KENXCODE_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("KENXCODE_TEST_IMAGE", "kenxcode-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``KENXCODE_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("KENXCODE_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged kenxcode user
# ---------------------------------------------------------------------------
#
# Background: every KenXCode runtime path inside the container drops to UID
# 10000 (the ``kenxcode`` user) via ``s6-setuidgid kenxcode``. ``docker exec``
...
# fails (PermissionError swallowed) for kenxcode, so a test that ran as root
...
# helpers below run every probe under ``-u kenxcode`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: kenxcode).
