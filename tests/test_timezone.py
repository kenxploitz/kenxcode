"""
Tests for timezone support (kenxcode_time module + integration points).
...
import kenxcode_time


def _reset_kenxcode_time_cache():
    """Reset the kenxcode_time module cache (replacement for removed reset_cache)."""
    kenxcode_time._cached_tz = None
    kenxcode_time._cached_tz_name = None
    kenxcode_time._cache_resolved = False


# =========================================================================
# kenxcode_time.now() — core helper
# =========================================================================

class TestKenXCodeTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_kenxcode_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["KENXCODE_TIMEZONE"] = "Asia/Kolkata"
        result = kenxcode_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["KENXCODE_TIMEZONE"] = "UTC"
        result = kenxcode_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["KENXCODE_TIMEZONE"] = "Europe/London"
        tz = kenxcode_time.get_timezone()
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When KENXCODE_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) KENXCODE_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("KENXCODE_TIMEZONE=" + os.environ.get("KENXCODE_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "KENXCODE_TIMEZONE=NOT_SET" in result["output"], (
            "KENXCODE_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When KENXCODE_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("KENXCODE_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=kenxcode_tz) which shifted
        absolute time when system-local tz != KenXCode tz.  The fix interprets
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when KenXCode tz
        ...
        # Use a KenXCode timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _kenxcode_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["KENXCODE_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_kenxcode_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of KenXCode timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["KENXCODE_TIMEZONE"] = "US/Eastern"
        _reset_kenxcode_time_cache()
