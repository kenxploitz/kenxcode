...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure KENXCODE_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", kenxcode_home=str(tmp_path / ".kenxcode"))
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KENXCODE_HOME", str(kenxcode_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", kenxcode_home=str(kenxcode_home))
        return p
