...
Run from the PR worktree:
    ...
    cd ~/.kenxcode/kenxcode-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.kenxcode/kenxcode-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.kenxcode/kenxcode-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a kenxcode-agent checkout"
)
...
# Isolated KENXCODE_HOME for the config write.
