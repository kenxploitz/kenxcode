...
def test_openviking_provider_config_loader_uses_readonly_config(monkeypatch):
    ...
    config = openviking_module._load_kenxcode_openviking_config()
...
def test_post_setup_existing_profile_picker_validates_and_links_saved_profile(tmp_path, monkeypatch):
    ...
    from kenxcode_cli import memory_setup
    ...
    OpenVikingMemoryProvider().post_setup(str(kenxcode_home), config)
...
def test_tool_add_resource_rejects_kenxcode_credential_file_upload(tmp_path, monkeypatch):
    import agent.file_safety as fs

    kenxcode_home = tmp_path / "kenxcode_home"
    kenxcode_home.mkdir()
    auth_json = kenxcode_home / "auth.json"
...
def test_viking_client_delete_uses_identity_headers(monkeypatch):
    ...
    assert captured["kwargs"]["headers"]["X-OpenViking-Actor-Peer"] == "kenxcode"
...
# ---------------------------------------------------------------------------
# on_session_switch — flush + commit + rotate behavior (kenxcode-agent#28296)
# ---------------------------------------------------------------------------
...
def test_sync_turn_captures_session_id_before_worker_runs():
    ...
    assert captured_payloads == [{
        "messages": [
            ...
            {"role": "assistant", "parts": [{"type": "text", "text": "a"}], "peer_id": "kenxcode"},
        ]
    }]
...
def test_session_needs_commit_guard_wins_over_stale_turn_count():
    """Regression for kenxcode-agent#28296 review (M3): once a session is marked
...
def test_concurrent_providers_claim_unlocked_pending_owner_once(
    ...
    for provider in providers:
        ...
        provider._kenxcode_home = str(tmp_path)
