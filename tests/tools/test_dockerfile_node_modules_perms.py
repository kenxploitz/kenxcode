...
Older images made /opt/kenxcode/ui-tui and /opt/kenxcode/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV KENXCODE_TUI_DIR=/opt/kenxcode/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R kenxcode:kenxcode /opt/kenxcode/ui-tui" not in text
    assert "chown -R kenxcode:kenxcode /opt/kenxcode/node_modules" not in text
