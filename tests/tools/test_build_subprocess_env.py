...
def test_no_scrub_inherit_profile_home_bridges_context_override(tmp_path):
    ...
    token = set_kenxcode_home_override(str(tmp_path))
    try:
        env = build_subprocess_env(
    ...
    assert env["KENXCODE_HOME"] == str(tmp_path)
...
def test_e2e_child_sees_kenxcode_home_and_no_planted_secret(tmp_path, monkeypatch):
    """A real child spawned with a factory-built env must see KENXCODE_HOME
    ...
    code = (
        "import os, json; "
        "print(json.dumps({'home': os.environ.get('KENXCODE_HOME'), "
    ...
    assert result["home"] == str(kenxcode_home)
