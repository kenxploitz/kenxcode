...
UDS socket lifecycle, kenxcode_tools generation, timeout enforcement,
...
from tools.code_execution_tool import (
    ...
    execute_code,
    generate_kenxcode_tools_module,
...
class TestKenXCodeToolsGeneration(unittest.TestCase):
    def test_generates_all_allowed_tools(self):
        src = generate_kenxcode_tools_module(list(SANDBOX_ALLOWED_TOOLS))
        for tool in SANDBOX_ALLOWED_TOOLS:
            self.assertIn(f"def {tool}(", src)


    def test_empty_list_generates_nothing(self):
        src = generate_kenxcode_tools_module([])
    ...
    def test_file_transport_uses_tempfile_fallback_for_rpc_dir(self):
        src = generate_kenxcode_tools_module(["terminal"], transport="file")
        self.assertIn("import json, os, shlex, tempfile, threading, time", src)
        self.assertIn("os.path.join(tempfile.gettempdir(), \"kenxcode_rpc\")", src)
        self.assertNotIn('os.environ.get("KENXCODE_RPC_DIR", "/tmp/kenxcode_rpc")', src)
    ...
    def test_uds_transport_serializes_concurrent_calls(self):
        ...
        src = generate_kenxcode_tools_module(["terminal"], transport="uds")
...
class TestExecuteCodeRemoteTempDir(unittest.TestCase):
    def test_execute_remote_uses_backend_temp_dir_for_sandbox(self):
        ...
        self.assertIn("mkdir -p /data/data/com.termux/files/usr/tmp/kenxcode_exec_", mkdir_cmd)
        self.assertIn("KENXCODE_RPC_DIR=/data/data/com.termux/files/usr/tmp/kenxcode_exec_", run_cmd)
        self.assertIn("rm -rf /data/data/com.termux/files/usr/tmp/kenxcode_exec_", cleanup_cmd)
        self.assertNotIn("mkdir -p /tmp/kenxcode_exec_", mkdir_cmd)

    def test_timezone_shell_quoted_in_remote_execution(self):
        """KENXCODE_TIMEZONE must be shell-quoted in remote env_prefix to prevent injection."""
        class FakeEnv:
            def __init__(self):
                self.commands = []
        ...
        with patch("tools.code_execution_tool._load_config",
             ...
             patch.dict(os.environ, {"KENXCODE_TIMEZONE": malicious_tz}):
            result = json.loads(_execute_remote("print('hello')", "task-1", ["terminal"]))
...
class TestExecuteCode(unittest.TestCase):
    ...
    def test_repo_root_modules_are_importable(self):
        ...
        result = self._run('import kenxcode_constants; print(kenxcode_constants.__file__)')
        self.assertEqual(result["status"], "success")
        self.assertIn("kenxcode_constants.py", result["output"])

    def test_single_tool_call(self):
        """Script calls terminal and prints the result."""
        code = """
from kenxcode_tools import terminal
...
"""
    ...
    def test_shell_quote_helper(self):
        ...
        code = """
from kenxcode_tools import shell_quote
...
"""
    ...
    def test_retry_helper_all_fail(self):
        ...
        code = """
from kenxcode_tools import retry
def always_fail():
...
class TestEnvVarFiltering(unittest.TestCase):
    ...
    def test_kenxcode_rpc_socket_injected(self):
        child_env = self._get_child_env()
        self.assertIn("KENXCODE_RPC_SOCKET", child_env)


    def test_timezone_injected_when_set(self):
        env_backup = os.environ.copy()
        try:
            os.environ["KENXCODE_TIMEZONE"] = "America/New_York"
            child_env = self._get_child_env()
    ...
    def test_timezone_not_set_when_empty(self):
        env_backup = os.environ.copy()
        try:
            os.environ.pop("KENXCODE_TIMEZONE", None)
            child_env = self._get_child_env()
            if "TZ" in child_env:
...
class TestRpcTokenAuthorization(unittest.TestCase):
    ...
    request without the matching KENXCODE_RPC_TOKEN must be rejected before
    ...
    def test_generated_module_sends_token(self):
        """The generated kenxcode_tools module reads KENXCODE_RPC_TOKEN and sends it."""
        src = generate_kenxcode_tools_module(["terminal"], transport="uds")
        self.assertIn("KENXCODE_RPC_TOKEN", src)
