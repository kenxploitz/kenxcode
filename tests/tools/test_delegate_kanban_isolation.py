...
# cwd. Without an explicit PYTHONPATH the child resolves ``kenxcode_cli`` /
...
def _make_running_kanban_task(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("KENXCODE_PROFILE", "parent-worker")
    monkeypatch.setenv("KENXCODE_KANBAN_WORKSPACE", str(workspace))
    monkeypatch.setenv("KENXCODE_KANBAN_ATTACHMENTS_ROOT", str(attachments_root))
    ...
    monkeypatch.setenv("KENXCODE_KANBAN_RUN_ID", str(run_id))
...
def test_delegated_child_context_suppresses_env_gated_kanban_tools(monkeypatch, tmp_path):
    ...
    The parent process may be a dispatcher worker with KENXCODE_KANBAN_TASK set;
    the child is only a subagent, not the run owner.
    """
    monkeypatch.setenv("KENXCODE_KANBAN_TASK", "t_parent")
    monkeypatch.setenv("KENXCODE_KANBAN_RUN_ID", "123")
    home = tmp_path / ".kenxcode"
    home.mkdir()
    monkeypatch.setenv("KENXCODE_HOME", str(home))
...
def test_delegate_child_execute_code_env_bridges_contextvar_and_scrubs_kanban(
    ...
    ``os.environ`` and therefore never wrote KENXCODE_DELEGATED_CHILD_CONTEXT into
    the sandbox env.
    """
    home = tmp_path / ".kenxcode"
    home.mkdir()
    ...
    monkeypatch.setenv("KENXCODE_KANBAN_TASK", "t_parent")
    monkeypatch.setenv("KENXCODE_KANBAN_RUN_ID", "123")
    monkeypatch.setenv("KENXCODE_KANBAN_DB", str(home / "kanban.db"))
    monkeypatch.setenv("KENXCODE_KANBAN_WORKSPACE", str(tmp_path / "parent-workspace"))
    monkeypatch.setenv("KENXCODE_KANBAN_CLAIM_LOCK", "lock")
    monkeypatch.delenv("KENXCODE_DELEGATED_CHILD_CONTEXT", raising=False)
    ...
    with delegated_child_context():
        env = _scrub_child_env(
            dict(os.environ),
            is_passthrough=lambda k: k.startswith("KENXCODE_KANBAN_"),
            is_windows=False,
        )

    assert os.environ.get("KENXCODE_DELEGATED_CHILD_CONTEXT") is None
    assert env["KENXCODE_HOME"] == str(home)
    assert env["KENXCODE_DELEGATED_CHILD_CONTEXT"] == "1"
    assert "KENXCODE_KANBAN_TASK" not in env
    assert "KENXCODE_KANBAN_RUN_ID" not in env
    assert "KENXCODE_KANBAN_DB" not in env
    assert "KENXCODE_KANBAN_WORKSPACE" not in env
    assert "KENXCODE_KANBAN_CLAIM_LOCK" not in env
...
def test_delegate_child_kanban_cli_cannot_delete_parent_board(
    ...
    code = (
        "from kenxcode_cli import kanban; "
        "import argparse; "
