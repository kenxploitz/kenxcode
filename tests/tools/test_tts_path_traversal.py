...
def test_output_path_rejects_kenxcode_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    kenxcode_home = tmp_path / "kenxcode-home"
    kenxcode_home.mkdir()
    monkeypatch.setattr(file_safety, "_kenxcode_home_path", lambda: kenxcode_home)
    monkeypatch.setattr(file_safety, "_kenxcode_root_path", lambda: kenxcode_home)

    target = kenxcode_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    kenxcode_home = tmp_path / "kenxcode-home"
    token_dir = kenxcode_home / "mcp-tokens"
