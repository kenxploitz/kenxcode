...
class TestKenXCodeTokenStorage:
    def test_roundtrip_tokens(self, tmp_path, monkeypatch):
        monkeypatch.setenv("KENXCODE_HOME", str(tmp_path))
        storage = KenXCodeTokenStorage("test-server")
    ...
    def test_token_file_created_with_0o600(self, tmp_path, monkeypatch):
        ...
        """
        monkeypatch.setenv("KENXCODE_HOME", str(tmp_path))
        storage = KenXCodeTokenStorage("perm-test-server")
    ...
    def test_corrupt_tokens_returns_none(self, tmp_path, monkeypatch):
        monkeypatch.setenv("KENXCODE_HOME", str(tmp_path))
        storage = KenXCodeTokenStorage("bad-server")
...
class TestPathTraversal:
    ...
    def test_dots_and_slashes_sanitized(self, tmp_path, monkeypatch):
        monkeypatch.setenv("KENXCODE_HOME", str(tmp_path))
        storage = KenXCodeTokenStorage("../../../etc/passwd")
...
def test_build_oauth_auth_preserves_server_url_path():
    ...
    ``OAuthContext.get_authorization_base_url``; KenXCode must not pre-strip.
    """
    ...
    with patch.object(mcp_oauth, "_OAUTH_AVAILABLE", True), \
         ...
         patch.object(mcp_oauth, "KenXCodeTokenStorage") as mock_storage_cls:
