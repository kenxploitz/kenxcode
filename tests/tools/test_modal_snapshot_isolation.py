...
def _restore_tool_modules():
    original_kenxcode_home = os.environ.get("KENXCODE_HOME")
    original_modules = {
        name: module
        for name, module in sys.modules.items()
        if name == "tools"
        or name.startswith("tools.")
        or name == "kenxcode_cli"
        or name.startswith("kenxcode_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_kenxcode_home is None:
            os.environ.pop("KENXCODE_HOME", None)
        else:
            os.environ["KENXCODE_HOME"] = original_kenxcode_home
        _reset_modules(("tools", "kenxcode_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    kenxcode_cli = types.ModuleType("kenxcode_cli")
    kenxcode_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["kenxcode_cli"] = kenxcode_cli
    kenxcode_home = tmp_path / "kenxcode-home"
    os.environ["KENXCODE_HOME"] = str(kenxcode_home)
    sys.modules["kenxcode_cli.config"] = types.SimpleNamespace(
        get_kenxcode_home=lambda: kenxcode_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="kenxcode-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": kenxcode_home / "modal_snapshots.json",
