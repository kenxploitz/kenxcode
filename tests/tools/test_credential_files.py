...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        kenxcode_home.mkdir()
        (kenxcode_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"KENXCODE_HOME": str(kenxcode_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(kenxcode_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.kenxcode/token.json"
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        kenxcode_home = tmp_path / ".kenxcode"
        kenxcode_home.mkdir()
        (kenxcode_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        kenxcode_home = tmp_path / ".kenxcode"
        skills_dir = kenxcode_home / "skills"
        ...
        assert mounts[0]["container_path"] == "/root/.kenxcode/skills"


    def test_custom_container_base(self, tmp_path):
        kenxcode_home = tmp_path / ".kenxcode"
        (kenxcode_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"KENXCODE_HOME": str(kenxcode_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.kenxcode")

        assert mounts[0]["container_path"] == "/home/user/.kenxcode/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.kenxcode/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.kenxcode/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape KENXCODE_HOME."""
        monkeypatch.setenv("KENXCODE_HOME", str(tmp_path / ".kenxcode"))
        (tmp_path / ".kenxcode").mkdir()

        # Create a sensitive file one level above kenxcode_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside kenxcode_home
    ...
    def test_nested_subdir_inside_kenxcode_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of KENXCODE_HOME must be allowed."""
        kenxcode_home = tmp_path / ".kenxcode"
        kenxcode_home.mkdir()
        subdir = kenxcode_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside KENXCODE_HOME pointing outside must be rejected."""
        kenxcode_home = tmp_path / ".kenxcode"
        kenxcode_home.mkdir()
        monkeypatch.setenv("KENXCODE_HOME", str(kenxcode_home))

        # Create a sensitive file outside kenxcode_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside kenxcode_home pointing outside
        symlink = kenxcode_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes KENXCODE_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, kenxcode_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape KENXCODE_HOME."""
        kenxcode_home = tmp_path / ".kenxcode"
        kenxcode_home.mkdir()
        monkeypatch.setenv("KENXCODE_HOME", str(kenxcode_home))
        ...
        sensitive.write_text("{}")
        self._write_config(kenxcode_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(kenxcode_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside KENXCODE_HOME via config must still mount."""
        kenxcode_home = tmp_path / ".kenxcode"
        kenxcode_home.mkdir()
        monkeypatch.setenv("KENXCODE_HOME", str(kenxcode_home))

        (kenxcode_home / "oauth.json").write_text("{}")
        self._write_config(kenxcode_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        kenxcode_home = tmp_path / ".kenxcode"
        kenxcode_home.mkdir()
        (kenxcode_home / "cache" / "documents").mkdir(parents=True)
        (kenxcode_home / "cache" / "audio").mkdir(parents=True)
        (kenxcode_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("KENXCODE_HOME", str(kenxcode_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.kenxcode/cache/documents" in paths
        assert "/root/.kenxcode/cache/audio" in paths
        assert "/root/.kenxcode/cache/videos" in paths
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_kenxcode_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        kenxcode_home = tmp_path / ".kenxcode"
        kenxcode_home.mkdir()
        # Use legacy dir name with content — get_kenxcode_dir prefers
        # populated old over new.
        legacy_doc = kenxcode_home / "document_cache"
        legacy_img = kenxcode_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(kenxcode_home / "document_cache") in host_paths
        assert str(kenxcode_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.kenxcode/cache/documents" in container_paths
        assert "/root/.kenxcode/cache/images" in container_paths
    ...
    def test_images_upload_dir_is_mounted(self, tmp_path, monkeypatch):
        ...
        Desktop / clipboard / PDF uploads land in ``KENXCODE_HOME/images``, not
        ...
        """
        kenxcode_home = tmp_path / ".kenxcode"
        (kenxcode_home / "images").mkdir(parents=True)
        monkeypatch.setenv("KENXCODE_HOME", str(kenxcode_home))

        mounts = get_cache_directory_mounts()
        by_container = {m["container_path"]: m["host_path"] for m in mounts}
        assert "/root/.kenxcode/images" in by_container
        assert by_container["/root/.kenxcode/images"] == str(kenxcode_home / "images")
    ...
    def test_images_upload_file_maps_into_container(self, tmp_path, monkeypatch):
        ...
        kenxcode_home = tmp_path / ".kenxcode"
        (kenxcode_home / "images").mkdir(parents=True)
        upload = kenxcode_home / "images" / "upload_20260722_181019_1.png"
        upload.write_bytes(bytes.fromhex("89504e470d0a1a0a"))
        monkeypatch.setenv("KENXCODE_HOME", str(kenxcode_home))

        assert (
            map_cache_path_to_container(str(upload))
            == "/root/.kenxcode/images/upload_20260722_181019_1.png"
        )
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        kenxcode_home = tmp_path / ".kenxcode"
        img_dir = kenxcode_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.kenxcode/cache/images/generated.png"
        )
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(kenxcode_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        kenxcode_home = tmp_path / ".kenxcode"
        doc_dir = kenxcode_home / "cache" / "documents"
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — KENXCODE_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping KENXCODE_HOME, but every master
    ...
    def test_master_credential_store_is_refused(self, tmp_path, rel_path):
        ...
        with patch.dict(os.environ, {"KENXCODE_HOME": str(home)}):
    ...
    def test_per_service_token_still_mounts(self, tmp_path):
        ...
        assert [m["container_path"] for m in mounts] == [
            "/root/.kenxcode/google_token.json"
        ]
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.kenxcode/google_token.json" in paths
        assert "/root/.kenxcode/.env" not in paths
