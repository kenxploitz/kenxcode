...
``kenxcode_tools.py`` and ``script.py`` into a temp dir, and those files
...
def _legacy_posix_scrubber(source_env, is_passthrough):
    ...
    Deliberately updated for #27303 (the broad ``KENXCODE_`` prefix was dropped
...
class TestPosixEquivalence:
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        # KENXCODE_* handling (#27303): only the operational allowlist passes;
        # every other KENXCODE_* is dropped (the broad prefix was removed).
        "KENXCODE_HOME": "/home/alice/.kenxcode",        # allowlisted → kept
        "KENXCODE_PROFILE": "default",                 # allowlisted → kept
        "KENXCODE_INTERACTIVE": "1",                   # not allowlisted → dropped
        "KENXCODE_BASE_URL": "https://api.internal",   # not allowlisted → dropped
        "KENXCODE_KANBAN_DB": "postgres://u:p@h/db",   # not allowlisted → dropped
...
# ``kenxcode_tools.py`` stub, and the LLM's ``script.py``.  Both contain
...
class TestSandboxWritesUtf8:
    ...
    def test_stub_and_script_writes_specify_utf8(self):
        """Both ``kenxcode_tools.py`` and ``script.py`` writes in
    ...
    def test_stub_source_roundtrips_through_utf8(self):
        ...
        """
        from tools.code_execution_tool import generate_kenxcode_tools_module
        import tempfile, ast
        stub = generate_kenxcode_tools_module(
    ...
    def test_windows_default_encoding_would_have_failed(self):
        ...
        *without* ``encoding="utf-8"`` would corrupt the file.  If this
        ...
        stub = generate_kenxcode_tools_module(["terminal"], transport="uds")
...
# let the child import kenxcode_tools, a user script that printed non-ASCII
