...
class GetSpillConfigTests(unittest.TestCase):
    def test_defaults_when_no_config(self):
        with patch.object(hos, "load_config", create=True, return_value={}):
            ...
            pass
        with patch("kenxcode_cli.config.load_config", return_value={}):
    ...
    def test_load_config_exception_is_swallowed(self):
        with patch("kenxcode_cli.config.load_config", side_effect=RuntimeError("bad")):
            cfg = hos.get_spill_config()
        self.assertEqual(cfg["max_chars"], hos.DEFAULT_MAX_CHARS)
        self.assertTrue(cfg["enabled"])
...
class SpillIfOversizedTests(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="kenxcode-spill-test-")
    ...
    def test_default_directory_uses_kenxcode_home(self):
        """When no directory override, spill under KENXCODE_HOME/hook_outputs."""
        test_home = tempfile.mkdtemp(prefix="kenxcode-home-")
        try:
            with patch.dict(os.environ, {"KENXCODE_HOME": test_home}):
                # Also patch get_kenxcode_home to the env var to mirror production.
            ...
            # ~/.kenxcode/hook_outputs depending on get_kenxcode_home behaviour.
            candidates = [
                Path(test_home) / "hook_outputs" / "sess",
                Path(os.path.expanduser("~/.kenxcode/hook_outputs/sess")),
            ]
