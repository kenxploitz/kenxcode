...
class TestBackendSelection:
    ...
    ``kenxcode tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
...
class TestFirecrawlEnvResolution:
    """Verify Firecrawl reads env values from kenxcode_cli.config.get_env_value,
    not just os.getenv.  This catches the regression reported in #40190 where
    values stored in ~/.kenxcode/.env were invisible to the provider."""
...
class TestSiblingProvidersEnvResolution:
    ...
    ~/.kenxcode/.env), not bare os.getenv. Parametrized over the four
