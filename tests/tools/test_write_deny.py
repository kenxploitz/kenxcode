...
class TestWriteDenyExactPaths:
    ...
    def test_kenxcode_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.kenxcode/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "kenxcode_root"
        ...
        # Sanity check: KENXCODE_HOME does point to the profile dir, not the root.
        from kenxcode_constants import get_kenxcode_home, get_default_kenxcode_root
        assert get_kenxcode_home() == profile_home
        assert get_default_kenxcode_root() == root
...
class TestWriteAllowed:
    ...
    def test_kenxcode_control_files_requested_writable(self):
