...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad KENXCODE_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: KENXCODE_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_kenxcode_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "KENXCODE_HOME": "/h", "KENXCODE_PROFILE": "p",
        "KENXCODE_CONFIG": "/c.yaml", "KENXCODE_ENV": "/e",
        "KENXCODE_DELEGATED_CHILD_CONTEXT": "1",
        # other KENXCODE_* → dropped (broad prefix removed)
        "KENXCODE_BASE_URL": "https://x", "KENXCODE_INTERACTIVE": "1",
        "KENXCODE_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "KENXCODE_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "KENXCODE_BASE_URL", "KENXCODE_INTERACTIVE", "KENXCODE_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped KENXCODE_* vars."""
    import logging
