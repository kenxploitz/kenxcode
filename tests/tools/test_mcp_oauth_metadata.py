...
Covers:
- :class:`KenXCodeTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.KenXCodeMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _KENXCODE_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# KenXCodeTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = KenXCodeTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("KENXCODE_HOME", str(tmp_path))
        storage = KenXCodeTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (KenXCodeMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: KenXCodeTokenStorage, **context_attrs):
    ...
    provider = _KENXCODE_PROVIDER_CLS.__new__(_KENXCODE_PROVIDER_CLS)
    provider._kenxcode_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KENXCODE_HOME", str(tmp_path))
        storage = KenXCodeTokenStorage("mgr-srv")
        ...
        with patch.object(
            _KENXCODE_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KENXCODE_HOME", str(tmp_path))
        storage = KenXCodeTokenStorage("flow-srv")
        ...
        with patch.object(
            _KENXCODE_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",
