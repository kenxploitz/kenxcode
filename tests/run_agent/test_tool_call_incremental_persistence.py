...
def _make_agent():
    kenxcode_home = Path(tempfile.mkdtemp(prefix="kenxcode-test-home-"))
    (kenxcode_home / "logs").mkdir(parents=True, exist_ok=True)
    with (
        patch(
        ...
        patch("run_agent._kenxcode_home", kenxcode_home),
