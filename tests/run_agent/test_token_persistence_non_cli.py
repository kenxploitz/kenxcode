...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    kenxcode_state = ModuleType("kenxcode_state")
    kenxcode_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "kenxcode_state", kenxcode_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "KenXCode"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "KenXCode"
