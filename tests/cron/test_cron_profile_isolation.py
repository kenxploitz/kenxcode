...
profile's KENXCODE_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.kenxcode/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active KENXCODE_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever KENXCODE_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    KENXCODE_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import kenxcode_constants

    monkeypatch.setattr(
        kenxcode_constants, "_get_platform_default_kenxcode_home", lambda: root
    )
    monkeypatch.setenv("KENXCODE_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile KENXCODE_HOME (<root>/profiles/<name>), the cron store
    resolves to <profile>/cron, NOT the shared <root>/cron."""
    root = tmp_path / "kenxcode_home"
    ...
    assert kenxcode_constants.get_kenxcode_home().resolve() == profile_home.resolve()
    assert kenxcode_constants.get_default_kenxcode_root().resolve() == root.resolve()

    # cron/jobs.py computes KENXCODE_DIR from get_kenxcode_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.KENXCODE_DIR.resolve() == profile_home.resolve()
        assert (
