...
class TestRunJobTerminalCwd:
    ...
    def _install_stubs(monkeypatch, observed: dict):
        ...
        # Bypass the real provider resolver — it reads ~/.kenxcode and credentials.
        from kenxcode_cli import runtime_provider as _rtp
        monkeypatch.setattr(
            _rtp,
        ...
        monkeypatch.setenv("KENXCODE_CRON_TIMEOUT", "0")

        # run_job calls load_dotenv(~/.kenxcode/.env, override=True), which will
