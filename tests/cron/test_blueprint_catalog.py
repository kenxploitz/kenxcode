...
the shared /blueprint command handler (kenxcode_cli/blueprint_cmd.py), and
the docs generator. Uses an isolated KENXCODE_HOME for anything that touches the
cron job store.
"""
...
class TestRenderers:
    ...
    def test_deeplink_shape(self):
        ...
        assert url.startswith("kenxcode://blueprint/morning-brief?")
