...
These exercise the real store against a temp KENXCODE_HOME (no mocks) per the
...
def temp_home(tmp_path, monkeypatch):
    """Isolated KENXCODE_HOME so jobs.json doesn't touch the real store."""
    monkeypatch.setenv("KENXCODE_HOME", str(tmp_path))
    # cron.jobs caches no home at import; get_kenxcode_home() reads the env live.
    yield tmp_path
