...
(/opt/kenxcode/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable KENXCODE_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_kenxcode_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable KENXCODE_HOME."""
    ...
    monkeypatch.setattr(
        "kenxcode_constants.get_kenxcode_home", lambda: kenxcode_home
    )
    ...
    expected = kenxcode_home / "scripts" / "whatsapp-bridge"
