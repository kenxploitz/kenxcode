...
closure the PR changed, against a real temp ``KENXCODE_HOME``.
"""
...
def _fake_switch_result():
    ...
    from kenxcode_cli.model_switch import ModelSwitchResult
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "kenxcode_cli.model_switch.list_picker_providers",
    ...
    monkeypatch.setattr(
        "kenxcode_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(gateway_run, "_kenxcode_home", kenxcode_home)
    _stub_picker_dependencies(monkeypatch)
    # save_config writes to ``get_kenxcode_home() / config.yaml`` — point it here.
    monkeypatch.setattr("kenxcode_constants.get_kenxcode_home", lambda: kenxcode_home)
    monkeypatch.setattr("kenxcode_cli.config.get_kenxcode_home", lambda: kenxcode_home)
    return cfg_path
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    monkeypatch.setattr(gateway_run, "_kenxcode_home", default_home)
    _stub_picker_dependencies(monkeypatch)
