...
def patch_startup_side_effects(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("kenxcode_cli.plugins.discover_plugins", lambda: None)
...
async def test_start_gateway_does_not_start_cron_after_aborted_startup(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kenxcode_logging.setup_logging", lambda kenxcode_home, mode: None)
