...
KENXCODE_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env KENXCODE_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``kenxcode config get terminal.backend`` still says local.
    """
    kenxcode_home = tmp_path / ".kenxcode"
    kenxcode_home.mkdir()
    (kenxcode_home / "config.yaml").write_text(
    ...
    (kenxcode_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
