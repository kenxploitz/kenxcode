...
def test_get_flush_dir_uses_get_kenxcode_home(tmp_path, monkeypatch):
    """Flush dir must use get_kenxcode_home(), not hardcoded Path.home()."""
    import gateway.shutdown_flush as mod

    captured = {}

    def fake_get_kenxcode_home():
        from pathlib import Path
        captured["called"] = True
        return tmp_path

    monkeypatch.setattr(
        "kenxcode_constants.get_kenxcode_home", fake_get_kenxcode_home
    )
