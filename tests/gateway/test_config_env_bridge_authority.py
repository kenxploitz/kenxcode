...
Guards against the 60-vs-500 bug where a stale `.env KENXCODE_MAX_ITERATIONS=60`
...
def _run_gateway_import(kenxcode_home: Path, initial_env: dict[str, str]) -> dict[str, str]:
    ...
    script = textwrap.dedent(
        ...
        for k in (
            ...
            "KENXCODE_AGENT_TIMEOUT",
            "KENXCODE_AGENT_TIMEOUT_WARNING",
            "KENXCODE_SESSION_STALL_TIMEOUT",
            "KENXCODE_GATEWAY_BUSY_INPUT_MODE",
            "KENXCODE_GATEWAY_BUSY_TEXT_MODE",
            "KENXCODE_GATEWAY_PLATFORM_CONNECT_TIMEOUT",
            "KENXCODE_TIMEZONE",
        ):
            v = os.environ.get(k)
            if v is not None:
...
@pytest.fixture
def kenxcode_home(tmp_path: Path) -> Path:
    home = tmp_path / ".kenxcode"
    home.mkdir()
    return home


def test_config_gateway_timeout_wins_over_stale_env(kenxcode_home: Path) -> None:
    """Every agent.* bridge key must be config-authoritative, not .env-authoritative."""
    _write_config(kenxcode_home, agent_cfg={
        "gateway_timeout": 1800,
        "gateway_timeout_warning": 900,
        "session_stall_timeout": 300,
    })
    _write_env(kenxcode_home, {
        "KENXCODE_AGENT_TIMEOUT": "60",
        "KENXCODE_AGENT_TIMEOUT_WARNING": "30",
        "KENXCODE_SESSION_STALL_TIMEOUT": "15",
    })

    env = _run_gateway_import(kenxcode_home, initial_env={})

    assert env.get("KENXCODE_AGENT_TIMEOUT") == "1800"
    assert env.get("KENXCODE_AGENT_TIMEOUT_WARNING") == "900"
    assert env.get("KENXCODE_SESSION_STALL_TIMEOUT") == "300"


def test_config_platform_connect_timeout_supplies_env_when_unset(kenxcode_home: Path) -> None:
    ...
    _write_config(kenxcode_home, gateway_cfg={"platform_connect_timeout": 90})

    env = _run_gateway_import(kenxcode_home, initial_env={})

    assert env.get("KENXCODE_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "90"


def test_env_platform_connect_timeout_wins_over_config(kenxcode_home: Path) -> None:
    """Unlike the agent.*/display.*/timezone bridges (config-authoritative),
    KENXCODE_GATEWAY_PLATFORM_CONNECT_TIMEOUT is the manual-override escape hatch:
    ...
    env = _run_gateway_import(
        kenxcode_home,
        initial_env={"KENXCODE_GATEWAY_PLATFORM_CONNECT_TIMEOUT": "120"},
    )

    assert env.get("KENXCODE_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "120"
