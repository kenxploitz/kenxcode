...
class TestDefaultInteractionDispatch:
    ...
    async def test_update_prompt_click_writes_response_file(self, tmp_path, monkeypatch):
        """update_prompt:y click writes 'y' to ~/.kenxcode/.update_response."""
        adapter = self._make_adapter()
        kenxcode_home = tmp_path / "kenxcode_home"
        kenxcode_home.mkdir()
        monkeypatch.setattr(
            "kenxcode_constants.get_kenxcode_home",
            lambda: kenxcode_home,
        )
        ...
        response = kenxcode_home / ".update_response"
