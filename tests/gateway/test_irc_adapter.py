...
class TestIRCAdapterInit:
    ...
    def test_init_from_config_extra(self, monkeypatch):
        ...
        assert adapter.nickname == "kenxcode"
        assert adapter.channel == "#kenxcode-dev"
...
class TestIRCAdapterMessageParsing:
    ...
    async def test_handle_addressed_channel_message(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG #test :kenxcode: hello there")
    ...
    async def test_ctcp_action_converted(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG kenxcode :\x01ACTION waves\x01")
        assert len(dispatched) == 1
    ...
    async def test_unauthorized_user_blocked(self, monkeypatch):
        ...
        adapter = IRCAdapter(cfg)
        adapter._current_nick = "kenxcode"
        ...
        await adapter._handle_line(":eve!u@host PRIVMSG #test :kenxcode: hello")
        assert len(dispatched) == 0
...
class TestIRCStandaloneSend:
    ...
    async def test_standalone_send_completes_handshake_and_sends_privmsg(self, monkeypatch):
        ...
        monkeypatch.setenv("IRC_NICKNAME", "kenxcodebot")
        monkeypatch.setenv("IRC_USE_TLS", "false")

        # Server greets us with 001 RPL_WELCOME, then nothing for QUIT drain.
        conn = _FakeIRCConnection([b":server 001 kenxcodebot-cron :Welcome"])
        ...
        assert any(line.startswith("NICK kenxcodebot-cron") for line in sent_lines)
        assert any(line.startswith("USER kenxcodebot-cron 0 * :KenXCode Agent (cron)")
