...
def _write_home(tmp_path: Path, sessions_cfg: dict, env_text: str = "") -> Path:
    ...
    (kenxcode_home / ".env").write_text(env_text, encoding="utf-8")
    return kenxcode_home


def test_cjk_fts_bridged_from_config(tmp_path, monkeypatch):
    home = _write_home(tmp_path, {"cjk_fts": False})
    monkeypatch.setattr(gateway_run, "_kenxcode_home", home)
    monkeypatch.setenv("KENXCODE_CJK_FTS", "1")
    gateway_run._reload_runtime_env_preserving_config_authority()
    assert os.environ["KENXCODE_CJK_FTS"] == "False"
