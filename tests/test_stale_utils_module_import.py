...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.kenxcode/kenxcode-agent/utils.py)
...
2. ``kenxcode update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``kenxcode_cli/main.py`` (the ``kenxcode update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``kenxcode_constants`` after the
