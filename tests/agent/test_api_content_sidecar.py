...
class TestPrologueStamping:
    ...
    def test_no_stamp_without_injections(self):
        agent = _FakeAgent()
        with patch("kenxcode_cli.plugins.invoke_hook", return_value=[]):
            ctx = _build(agent)
...
@pytest.fixture()
def wire_env():
    """Mock provider + isolated KENXCODE_HOME + a shared SessionDB.
    ...
    test_home = tempfile.mkdtemp(prefix="kenxcode_api_content_")
    os.makedirs(os.path.join(test_home, ".kenxcode"))
    prev_home = os.environ.get("KENXCODE_HOME")
    os.environ["KENXCODE_HOME"] = os.path.join(test_home, ".kenxcode")
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["KENXCODE_HOME"] = prev_home
