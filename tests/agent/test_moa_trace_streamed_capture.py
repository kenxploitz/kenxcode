...
path via ``kenxcode chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp KENXCODE_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point KENXCODE_HOME at a temp dir and turn moa.save_traces on."""
    kenxcode_home = tmp_path / ".kenxcode"
    kenxcode_home.mkdir()
    monkeypatch.setenv("KENXCODE_HOME", str(kenxcode_home))

    # save_moa_turn reads config via kenxcode_cli.config.load_config; stub it to
    ...
    return kenxcode_home / "moa-traces"
