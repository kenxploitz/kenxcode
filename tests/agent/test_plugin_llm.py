...
""",
        ...
        from kenxcode_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        kenxcode_home = tmp_path / ".kenxcode"
        kenxcode_home.mkdir()
        (kenxcode_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("KENXCODE_HOME", str(kenxcode_home))
        from kenxcode_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from kenxcode_cli.plugins import PluginContext, PluginManifest, PluginManager
