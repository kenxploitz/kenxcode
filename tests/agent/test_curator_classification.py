...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"kenxcode-agent-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "kenxcode-agent-dev", "declared": True},
        },
    )
    ...
    assert e["into"] == "kenxcode-agent-dev"
...
# `kenxcode curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `kenxcode curator pin <umbrella>` in the rename block so
