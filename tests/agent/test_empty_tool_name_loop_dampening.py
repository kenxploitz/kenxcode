...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated KENXCODE_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="kenxcode_e2e_47967_")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("kenxcode_"):
            del sys.modules[mod]
