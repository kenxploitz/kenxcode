...
@pytest.fixture
def kenxcode_home_with_config(tmp_path, monkeypatch):
    """Isolated ``~/.kenxcode/`` with a config.yaml referencing one external dir."""
...
def test_cache_invalidates_on_mtime_change(kenxcode_home_with_config):
    """A config.yaml edit invalidates the cache on the next call."""
    _home, external, config = kenxcode_home_with_config
...
def test_cache_key_is_per_config_path(tmp_path, monkeypatch):
    """Two different KENXCODE_HOMEs keep separate cache entries."""
    home_a = tmp_path / "home_a" / ".kenxcode"
    ...
    home_b = tmp_path / "home_b" / ".kenxcode"
    ...
    monkeypatch.setenv("KENXCODE_HOME", str(home_a))
    assert get_external_skills_dirs() == [ext_a.resolve()]

    monkeypatch.setenv("KENXCODE_HOME", str(home_b))
