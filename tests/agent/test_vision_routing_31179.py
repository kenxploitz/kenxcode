...
def isolated_home(monkeypatch):
    """Temp KENXCODE_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="kenxcode_test_31179_")
    kenxcode_home = os.path.join(test_home, ".kenxcode")
    os.makedirs(kenxcode_home)
...
def _fresh_modules():
    """Drop cached kenxcode modules so each test reloads against current env."""
    _drop_reload_targets()
