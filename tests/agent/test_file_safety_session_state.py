...
manipulation. KenXCode keeps canonical conversation history in state.db and may
...
def fake_homes(tmp_path, monkeypatch):
    """Point KENXCODE_HOME at a temp profile dir.

    Uses the real env-var resolution chain (get_kenxcode_home /
    get_default_kenxcode_root) instead of monkeypatching private helpers —
    ...
    KENXCODE_HOME=<root>/profiles/<name> makes get_default_kenxcode_root()
