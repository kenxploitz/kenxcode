...
def test_coding_prompt_preserves_legacy_workspace_order(monkeypatch):
    ...
    monkeypatch.setattr(system_prompt, "KENXCODE_AGENT_HELP_GUIDANCE", "HELP")
    monkeypatch.setattr(system_prompt, "STEER_CHANNEL_NOTE", "STEER")
    monkeypatch.setattr(system_prompt, "get_kenxcode_home", lambda: Path("/kenxcode"))

    expected_profile = (
        "Active KenXCode profile: default. Other profiles (if any) live "
        "under /kenxcode/profiles/<name>/. Each profile has its own skills/, "
    ...
    with (
        ...
        patch("kenxcode_time.now", return_value=datetime(2026, 1, 2)),
    ):
...
class TestTelegramRichMessagesHint:
    ...
    def test_base_hint_without_rich_messages(self, monkeypatch):
        ...
        with patch("kenxcode_cli.config.load_config_readonly") as mock_cfg:
