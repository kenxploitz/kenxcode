...
def kenxcode_home(monkeypatch):
    """Isolate KENXCODE_HOME for ``reload_skills`` tests.

    Rather than popping cache-bearing modules from ``sys.modules``,
    we monkeypatch the module-level ``KENXCODE_HOME`` / ``SKILLS_DIR``
    constants in place so the isolation is local to this fixture's scope.
    """
    td = tempfile.mkdtemp(prefix="kenxcode-reload-skills-")
    monkeypatch.setenv("KENXCODE_HOME", td)
    home = Path(td)
    ...
    monkeypatch.setattr(_st, "KENXCODE_HOME", home, raising=False)
...
class TestReloadSkillsHelper:
    ...
    def test_detects_removed_skill_carries_description(self, kenxcode_home):
        from agent.skill_commands import reload_skills

        skill_dir = _write_skill(kenxcode_home / "skills", "demo", "soon to be gone")
    ...
    def test_does_not_invalidate_prompt_cache_snapshot(self, kenxcode_home):
