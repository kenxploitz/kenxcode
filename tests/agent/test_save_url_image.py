...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate KENXCODE_HOME under tmp_path."""
    monkeypatch.setenv("KENXCODE_HOME", str(tmp_path / ".kenxcode"))
    (tmp_path / ".kenxcode").mkdir()

    # Force the constants/image cache helpers to re-read KENXCODE_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("kenxcode_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_kenxcode_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under KENXCODE_HOME — gateway cleanup
