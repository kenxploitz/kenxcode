...
``kenxcode_subprocess_env`` / ``_sanitize_subprocess_env``, which the factory
...
``KENXCODE_HOME`` or secret-scrub propagation.
...
SCAN_DIRS = ("agent", "kenxcode_cli", "tools", "gateway", "cron", "tui_gateway")
SCAN_ROOT_FILES = ("cli.py", "kenxcode_constants.py")
...
ALLOWED_RAW_SPAWN_ENV_FILES = {
    # THE owner module: _sanitize_subprocess_env / kenxcode_subprocess_env /
