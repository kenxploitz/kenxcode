...
to a generic "Your API key was rejected... run kenxcode setup" message, which is
...
def test_nous_401_guidance_strings_present():
    ...
    # Must give a concrete re-auth command, not a generic "kenxcode setup".
    assert "kenxcode portal" in source
