...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("kenxcode.gateway.up"), _M("kenxcode.gateway.active_agents"),
        _M("kenxcode.gateway.busy"), _M("kenxcode.gateway.drainable"),
        _M("kenxcode.gateway.restart_requested"),
        _M("kenxcode.platform.up"), _M("kenxcode.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("kenxcode.cron.scheduler.heartbeat_age_seconds"),
        _M("kenxcode.cron.scheduler.last_success_age_seconds"),
        _M("kenxcode.cron.scheduler.catch_up_occurrences"),
        _M("kenxcode.cron.jobs.enabled"), _M("kenxcode.cron.jobs.running"),
        _M("kenxcode.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "KenXCode Agent-owned Relay transport health" in text
