...
from kenxcode_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(kenxcode_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <kenxcode_home>/plugins/<name> and opt it in."""
    plugin_dir = kenxcode_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("KENXCODE_HOME", str(tmp_path / "kenxcode_empty"))
