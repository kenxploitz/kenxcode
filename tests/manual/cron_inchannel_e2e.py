...
Uses a throwaway KENXCODE_HOME so it never touches ~/.kenxcode. No real names.
"""
...
def _fresh_home():
    """Point KENXCODE_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_kenxcode_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["KENXCODE_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER KENXCODE_HOME is set.
...
# a different get_kenxcode_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"KENXCODE_HOME (throwaway): {HOME}")
