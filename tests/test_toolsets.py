...
class TestToolsetConsistency:
    ...
    def test_kenxcode_platforms_share_core_tools(self):
        """All kenxcode-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on kenxcode-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["kenxcode-cli", "kenxcode-telegram", "kenxcode-discord", "kenxcode-whatsapp", "kenxcode-slack", "kenxcode-signal", "kenxcode-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_kenxcode_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("kenxcode-whatsapp")
