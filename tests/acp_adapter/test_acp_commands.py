...
from acp_adapter.server import KenXCodeACPAgent
...
class FakeAgent:
    def __init__(self):
        ...
        self.enabled_toolsets = ["kenxcode-acp"]
...
def test_acp_real_agent_gets_session_db_for_recall(monkeypatch):
    ...
    monkeypatch.setitem(
        sys.modules,
        "kenxcode_cli.config",
        mod("kenxcode_cli.config", load_config=lambda: {"model": {"default": "m", "provider": "p"}}),
    )
    monkeypatch.setitem(
        sys.modules,
