...
The ``kenxcode update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m kenxcode_cli.main gateway run`` — image name
    ``pythonw``, not ``kenxcode.exe`` — so the ``taskkill /IM kenxcode.exe`` guard
    ...
    # The kenxcode.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM kenxcode.exe' in text
