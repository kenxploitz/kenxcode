"""KenXCode middleware contract helpers.
...
OBSERVER_SCHEMA_VERSION = "kenxcode.observer.v1"
MIDDLEWARE_SCHEMA_VERSION = "kenxcode.middleware.v1"
...
def apply_llm_request_middleware(
    ...
    **context: Any,
    ...
    provider kwargs before KenXCode sends them.
    """
...
def _invoke_middleware(kind: str, **kwargs: Any) -> List[Any]:
    from kenxcode_cli.plugins import invoke_middleware

    return invoke_middleware(kind, **middleware_payload(**kwargs))


def _has_middleware(kind: str) -> bool:
    from kenxcode_cli.plugins import has_middleware
