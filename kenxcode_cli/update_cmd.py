"""KenXCode update pipeline — extracted from ``kenxcode_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``kenxcode_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``kenxcode_cli.main`` reference — so existing call sites and test monkeypatches
that target ``kenxcode_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``kenxcode_cli.main``.
...
Imports are one-way: ``kenxcode_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``kenxcode_cli.main`` reference.

    Lets callers keep patching ``kenxcode_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``kenxcode_cli.main`` -> ``kenxcode_cli.update_cmd`` stays one-way
    at import time.
    """
    from kenxcode_cli import main
...
def _reload_updated_runtime_modules() -> None:
    ...
    ``kenxcode update`` keeps running in the pre-pull Python process. After a
...
# Critical files that KenXCode must be able to import immediately after an
...
_UPDATE_CRITICAL_FILES = (
    "kenxcode_cli/main.py",
    "kenxcode_cli/config.py",
    "kenxcode_cli/__init__.py",
    "kenxcode_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
    "kenxcode_constants.py",
)
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``kenxcode`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="kenxcode-syntax-check-") as tmpdir:
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    ``_venv_core_imports_healthy``): ``kenxcode update`` can be driven by a
    ...
    """
    from kenxcode_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from kenxcode_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('kenxcode_'):\n"
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``kenxcode update --gateway`` so interactive prompts (stash restore,
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `kenxcode update`.
    ...
    print("  Preview now:  kenxcode curator run --dry-run")
    print("  Pause it:     kenxcode curator pause")
    print(
        "  Docs:         https://kenxcode-agent.nousresearch.com/docs/user-guide/features/curator"
    )

def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `kenxcode update`.
    ...
    try:
        ...
        return
    db_path = get_kenxcode_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `kenxcode sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    kenxcode sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  kenxcode sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``kenxcode update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``kenxcode update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: kenxcode curator status)"
    )
...
def _finish_dashboard_update_cleanup(node_failures: list[str]) -> None:
    ...
    print("    kenxcode dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``kenxcode_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.kenxcode-update-staging"
    backup = f"{dst}.kenxcode-update-old"
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `kenxcode update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``kenxcode_constants.py``
...
def _update_via_zip(args):
    """Update KenXCode Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `kenxcode update "
            f"--branch {branch}`, or update against main with `kenxcode update`."
        )
        _m().sys.exit(1)
    zip_url = (
        f"https://github.com/NousResearch/kenxcode-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="kenxcode-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"kenxcode-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for kenxcode-agent itself
        ...
        # GitHub ZIPs extract to kenxcode-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"kenxcode-agent-{branch}")
        ...
        print(
            "  Re-run `kenxcode update` to retry; if the agent won't start, "
            "reinstall from https://kenxcode-agent.nousresearch.com"
        )
        _m().sys.exit(1)
    finally:
    ...
    from kenxcode_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    if not import_ok:
        ...
        print("  Re-run `kenxcode update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: kenxcode skills list-modified  "
    ...
    try:
        from kenxcode_cli.model_catalog import seed_cache_from_checkout
    ...
    try:
        from kenxcode_cli.backup import _quick_snapshot_root, verify_sqlite_integrity

        _state_path = get_kenxcode_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_kenxcode_home())
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "kenxcode-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if KenXCode behaves unexpectedly.")
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes kenxcode completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but KenXCode couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but KenXCode couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if KenXCode behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but KenXCode couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but KenXCode couldn't drop "
...
OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/kenxcode-agent.git",
    "git@github.com:NousResearch/kenxcode-agent.git",
    "https://github.com/NousResearch/kenxcode-agent",
    "git@github.com:NousResearch/kenxcode-agent",
}

OFFICIAL_REPO_URL = "https://github.com/NousResearch/kenxcode-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_kenxcode_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_kenxcode_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official KenXCode repository.")
        print("  This means you may miss updates from NousResearch/kenxcode-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/kenxcode-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/kenxcode-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``kenxcode update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from kenxcode_constants import get_default_kenxcode_root

    default_home = get_default_kenxcode_root()
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "kenxcode.exe"
    lines = ["✗ Another kenxcode.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close KenXCode Desktop, exit any open `kenxcode` REPLs, and")
    lines.append("  stop the gateway (`kenxcode gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `kenxcode update --force` if you've already")
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``kenxcode update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `kenxcode update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `kenxcode update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in KenXCode' editable-install extras or
    ...
    try:
        from kenxcode_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from kenxcode_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$KENXCODE_HOME/bin/uv``, but on Termux the official
    ...
    """
    from kenxcode_cli.managed_uv import resolve_uv
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `kenxcode update` is exactly the step
...
def _npm_lockfile_changed(kenxcode_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `kenxcode
    ...
    try:
        ...
        cache_file = kenxcode_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(kenxcode_root: Path) -> None:
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from kenxcode_constants import is_wsl
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `kenxcode update`.")
            failed = ["repo root"]
            if any(
    ...
    # KenXCode profile using this checkout. Keep one per-checkout cache under the
    # shared KenXCode root rather than rerunning npm once per named profile.
    shared_kenxcode_root = get_default_kenxcode_root()
    if not _m()._npm_lockfile_changed(shared_kenxcode_root):
    ...
    # need desktop during `kenxcode update`, so we install root-only first
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `kenxcode update`.")
        return list(labels)
    ...
    nixos_env = with_kenxcode_node_path(_m()._nixos_build_env())
    ...
    # hung. The chatty npm-deprecation noise during `kenxcode update` comes from
    ...
    if ws_result.returncode == 0:
        _record_npm_lockfile_hash(shared_kenxcode_root)
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.kenxcode/logs/update.log`` only, never the terminal.

    During ``kenxcode update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``kenxcode update --check``: fetch and report without installing.
    ...
    """
    from kenxcode_cli.config import detect_install_method, recommended_update_command_for_method
    method = detect_install_method(_m().PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``kenxcode update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from kenxcode_cli.config import format_docker_update_message
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``kenxcode update`` without requiring a reinstall.  The
    ...
    ``kenxcode`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/kenxcode.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v kenxcode'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/kenxcode, code at /usr/local/lib/kenxcode-agent).
    fhs_link = Path("/usr/local/bin/kenxcode")
    ...
    path_comment = (
        "# KenXCode Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _ensure_acp_launcher() -> None:
    """Self-heal: install a ``kenxcode-acp`` launcher next to the ``kenxcode`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``kenxcode update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``kenxcode-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report KenXCode as not installed even when it is.

    The shim simply delegates to the sibling ``kenxcode`` launcher with the
    ...
    ``kenxcode-acp.exe`` already resolves) and wherever a ``kenxcode-acp`` is
    already present next to the ``kenxcode`` command.  Unwritable directories
    (e.g. ``/usr/local/bin`` as non-root) are skipped silently.  Idempotent.
    """
    if _m().sys.platform == "win32":
        return
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        kenxcode_cmd = bin_dir / "kenxcode"
        acp_cmd = bin_dir / "kenxcode-acp"
        try:
            if not (kenxcode_cmd.is_file() or kenxcode_cmd.is_symlink()):
                continue
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of KENXCODE_HOME under
      ``backups/`` (restorable via ``kenxcode import``; the #48200 wrong-path
    ...
    snapshot_id = None
    try:
        from kenxcode_cli.backup import (
            _quick_snapshot_root,
            create_quick_snapshot,
            verify_sqlite_integrity,
        )

        # NOTE: this function later does `from kenxcode_constants import
        # get_kenxcode_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from kenxcode_cli.config import get_kenxcode_home as _get_home
    ...
    try:
        from kenxcode_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_kenxcode_home so the user sees ~/.kenxcode/...
    try:
        from kenxcode_constants import get_kenxcode_home, display_kenxcode_home

        home = get_kenxcode_home()
        try:
            display_path = f"{display_kenxcode_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  kenxcode import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``kenxcode update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``kenxcode update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run kenxcode from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.kenxcode-bootstrap-complete`,
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".kenxcode-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The kenxcode.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m kenxcode_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``kenxcode
    ...
    for proc in proc_iter:
        ...
        # this venv's path, or a `-m kenxcode_cli.main ...` invocation tied to
        # this install (install root in the cmdline or as the working dir).
        if not is_holder and venv_prefix in cmdline_low:
            is_holder = True
        if not is_holder and "kenxcode_cli.main" in cmdline_low:
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other KenXCode processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← KenXCode Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
    ...
    lines.append(
        "  Close the KenXCode desktop app / other KenXCode terminals, then re-run:"
    )
    lines.append("    kenxcode update")
    lines.append("  (or use `kenxcode update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``kenxcode update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from kenxcode_cli._scan_venv_blockers import _is_pausable_gateway
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    kenxcode.exe concurrent-instance guard does not see them. They still import
    ...
    try:
        ...
        from kenxcode_cli.gateway import (
    ...
    print("→ Stopping Windows gateway process(es) before updating KenXCode...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m kenxcode_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: kenxcode gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``kenxcode gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from kenxcode_cli import gateway_windows
        from kenxcode_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``kenxcode-gateway*.service`` from ``systemctl list-units``.
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        if not unit.startswith("kenxcode-gateway"):
            continue
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``kenxcode update`` never touched them, so installs created before the
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``kenxcode update`` sees a clean tree instead of autostashing every run.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``kenxcode update``, which ships with the checkout
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚡ Updating KenXCode Agent...")
    print()

    # On Windows, abort early if another kenxcode.exe is holding the venv shim
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_kenxcode_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `kenxcode serve` backend) will keep .pyd
    ...
    # the kenxcode.exe shim guard above, but its lock probe only checks the shim
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://kenxcode-agent.nousresearch.com/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all KenXCode windows/gateways and re-run: kenxcode update")
            else:
                print("✓ Already up to date!")
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running KenXCode gateways, Desktop backends, or other "
        ...
        # orphan merge-conflict markers in kenxcode_cli/config.py bricked
        # every user who ran ``kenxcode update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``kenxcode
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``kenxcode update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_kenxcode_home added to kenxcode_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # marker survives and the next ``kenxcode`` launch finishes the install
        ...
        # symbols in kenxcode_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `kenxcode` again "
        ...
        if not import_ok:
            ...
            print("    Run `kenxcode update` again — if it persists, reinstall:")
            print("    https://kenxcode-agent.nousresearch.com")

        node_failures = _update_node_dependencies()
        _m()._build_web_ui(_m().PROJECT_ROOT / "web")

        # Rebuild the desktop app if the source tree changed since the last
        # build.  ``kenxcode desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``kenxcode desktop`` shouldn't be forced into a full
        # Electron build by ``kenxcode update``.
        ...
        if (desktop_dir / "package.json").exists() and _m()._resolve_node_runtime_npm() and has_desktop_app:
            ...
            # `kenxcode desktop --build-only` subprocess re-imports the whole
            ...
            if _skip_desktop_build:
                ...
                _desktop_build_cmd = [sys.executable, "-m", "kenxcode_cli.main", "desktop", "--build-only"]
                ...
                # debuggable.
                #
                # Start the build subprocess with the KenXCode-managed Node on PATH:
                # when `kenxcode update` runs inside the desktop updater chain
                # (Desktop → kenxcode-setup → kenxcode update), the shell PATH
                # customizations are lost, so a bare-PATH child would fail with
                # `node: not found` before cmd_gui can self-heal.
                from kenxcode_constants import with_kenxcode_node_path

                _build_env = with_kenxcode_node_path()
                build_result = _m()._run_logged_subprocess(_desktop_build_cmd, cwd=_m().PROJECT_ROOT, env=_build_env)
                if build_result.returncode != 0:
                    build_result = _m()._run_logged_subprocess(_desktop_build_cmd, cwd=_m().PROJECT_ROOT, env=_build_env)
                if build_result.returncode != 0:
                    print("  ⚠ Desktop build failed (non-fatal; run `kenxcode desktop` to retry)")
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_kenxcode_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # current — so copy it straight over ~/.kenxcode/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit KENXCODE_HOME so
        # it is not affected by sync_skills()'s module-level KENXCODE_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's KENXCODE_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from kenxcode_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'kenxcode config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: kenxcode config migrate")
            else:
                print()
                print("Skipped. Run 'kenxcode config migrate' later to configure.")
        else:
        ...
        # it and warn loudly.
        try:
            from kenxcode_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `kenxcode curator status`. Self-stamps after printing so it
        ...
        # Self-heal the kenxcode-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve KenXCode on PATH without
        # a reinstall.  No-op on Windows and when already present.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("kenxcode-acp launcher self-heal failed: %s", e)
        ...
        # don't have it. Tying the refresh to ``kenxcode update`` gives users a
        ...
        try:
            ...
            if (
                ...
                # driver) keeps the installed version — `kenxcode update`
                # must stay fast; `kenxcode computer-use install --upgrade`
        ...
        # When running as ``kenxcode update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_kenxcode_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... kenxcode-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... kenxcode-gateway*)
            ...
            try:
                from kenxcode_cli.gateway import _get_restart_exit_wait_budget
            ...
            # Discover all kenxcode-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: kenxcode gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `kenxcode update` restart it automatically, allow\n"
                        ...
                        # path in `kenxcode gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: kenxcode gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: kenxcode -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy KenXCode gateway unit files are still installed.
        # When both kenxcode.service (from a pre-rename install) and the
        # current kenxcode-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `kenxcode update` surfaces the issue until the user migrates.
        try:
            from kenxcode_cli.gateway import (
                has_legacy_kenxcode_units,
                _find_legacy_kenxcode_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_kenxcode_units():
                print()
                print("⚠ Legacy KenXCode gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_kenxcode_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (kenxcode.service) fight the current")
                print("  kenxcode-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    kenxcode gateway migrate-legacy")
                print()
        ...
        print("  kenxcode model              # Select provider and model")
