"""Windows ConPTY bridge for the `kenxcode dashboard` chat tab.

Drop-in counterpart to ``kenxcode_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``kenxcode_cli.web_server`` consumes: ``spawn``, ``read``,
