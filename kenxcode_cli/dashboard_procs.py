"""Dashboard process-hygiene helpers — extracted from ``kenxcode_cli/main.py``.
...
``_detect_concurrent_kenxcode_instances``) are lifted verbatim. References to
helpers that STAY in ``kenxcode_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``kenxcode_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``kenxcode_cli.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``kenxcode_cli.main`` reference (call-time; keeps patches working)."""
...
def _scan_dashboard_processes(
    *,
    ...
    ``kenxcode dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``kenxcode update`` replaces files on disk, the running
    ...
    ``kenxcode-dashboard.service`` systemd unit.  Managed units are restarted
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the KenXCode Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``kenxcode serve`` as
    ...
    environment variable ``KENXCODE_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        ...
        # The headless backend (`kenxcode serve`) is the same long-lived server
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "kenxcode.*dashboard"`) keeps us consistent
            # with `kenxcode_cli.gateway._scan_gateway_pids` and avoids the
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``kenxcode dashboard`` / ``kenxcode serve`` processes.

    Called at the end of ``kenxcode update`` (default ``reason``) and also
    from ``kenxcode dashboard --stop`` (which overrides ``reason``).  The
    ...
    When ``restart_managed`` is true (the ``kenxcode update`` path), a detected
    ``kenxcode-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``kenxcode-serve.service``) has its owning unit
    ...
    # When the KenXCode Desktop Electron app spawns this dashboard as a
    # backend child, it sets KENXCODE_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("KENXCODE_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    # caller asked for restarts (the `kenxcode update` path) — `--stop` must
    ...
    #    remote backend (kenxcode serve) under Restart=on-failure never comes
...
def _detect_concurrent_kenxcode_instances(
    ...
    The KenXCode Desktop Electron app spawns ``kenxcode.EXE`` as a backend child,
    so during ``kenxcode update`` the user-invoked process and the desktop's
    ...
    shims (``kenxcode.exe`` / ``kenxcode-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``kenxcode update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``kenxcode.exe``) is a separate process from the Python
    ...
    for shim in _m()._kenxcode_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated kenxcode.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``kenxcode update``
    ...
    #      kenxcode.exe sitting *under* a non-KenXCode parent (e.g. a KenXCode
