"""Cross-process mutual exclusion for in-flight KenXCode updates.

Three different surfaces can start an update of the same install tree:

* ``kenxcode update`` from a terminal,
* the dashboard's Update button (``POST /api/kenxcode/update`` →
  ``_spawn_kenxcode_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``kenxcode-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``kenxcode update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <KENXCODE_HOME>/.kenxcode-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``kenxcode update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("KenXCode is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``kenxcode-setup`` binary under ``~/.kenxcode`` is only refreshed by a full
  installer run (``copy_self_to_kenxcode_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("KenXCode is still running") on every
  GUI update forever, with no KenXCode process actually running.
"""
...
MARKER_NAME = ".kenxcode-update-in-progress"

# Set by an orchestrating updater (the Tauri `kenxcode-setup --update` flow) to
# its own pid before spawning `kenxcode update` as a child stage. The parent
...
HANDOFF_PID_ENV = "KENXCODE_UPDATE_HANDOFF_PID"
...
# to show "KenXCode is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* KenXCode home (never the context-local profile override):
    the Rust updater resolves ``$KENXCODE_HOME`` or the platform default, and the
    ...
    """
    from kenxcode_constants import get_process_kenxcode_home

    return get_process_kenxcode_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``kenxcode update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``kenxcode-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another KenXCode update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `kenxcode update` as a stage): we run under ITS claim
