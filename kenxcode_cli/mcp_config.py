"""
MCP Server Management CLI — ``kenxcode mcp`` subcommand.

Implements ``kenxcode mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.kenxcode/config.yaml under the ``mcp_servers`` key.
"""
...
from kenxcode_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_kenxcode_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from kenxcode_cli.cli_output import prompt as _shared_prompt
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.kenxcode/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``kenxcode mcp login`` to distinguish a genuine authentication
    ...
    """
    try:
        from tools.mcp_oauth import KenXCodeTokenStorage
        return KenXCodeTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # kenxcode_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  kenxcode mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  kenxcode mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  kenxcode mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_kenxcode_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: kenxcode mcp test " + name)
        return
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(f"Saved '{name}' to {display_kenxcode_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `kenxcode mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  kenxcode mcp add <name> --url <endpoint>')
        _info('  kenxcode mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    landed before reporting success. Shared by ``kenxcode mcp login`` and
    ``kenxcode mcp reauth`` so both behave identically for a single server.
    """
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `kenxcode mcp remove` + `kenxcode mcp add` to reconfigure auth.")
        return False
    ...
    #
    # force_interactive_oauth: `kenxcode mcp login` is *explicitly* user-
    # initiated even when stdin isn't a TTY (KenXCode desktop / agent-
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `kenxcode mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    ``kenxcode mcp reauth <name>`` re-auths a single server (same as ``login``).
    ``kenxcode mcp reauth --all`` discovers every ``auth: oauth`` server in
    ...
    if not name:
        ...
        _info("Usage: kenxcode mcp reauth <name>   |   kenxcode mcp reauth --all")
        return
    if name not in servers:
        _error(f"Server '{name}' not found in config.")
        if servers:
...
# ─── kenxcode mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'kenxcode mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``kenxcode mcp`` subcommands."""
    ...
    if action == "picker":
        from kenxcode_cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from kenxcode_cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from kenxcode_cli.mcp_picker import install_by_name
        import sys as _sys
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `kenxcode plugin`.
        from kenxcode_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("kenxcode mcp                                    Open the catalog picker (default)")
        _info("kenxcode mcp catalog                            List Nous-approved MCPs")
        _info("kenxcode mcp install <name>                     Install a catalog MCP")
        _info("kenxcode mcp serve                              Run as MCP server")
        _info("kenxcode mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("kenxcode mcp add <name> --command <cmd>         Add a stdio server")
        _info("kenxcode mcp add <name> --preset <preset>       Add from a known preset")
        _info("kenxcode mcp remove <name>                      Remove a server")
        _info("kenxcode mcp list                               List configured servers")
        _info("kenxcode mcp test <name>                        Test connection")
        _info("kenxcode mcp configure <name>                   Toggle tools")
        _info("kenxcode mcp login <name>                       Re-authenticate OAuth")
        _info("kenxcode mcp reauth <name> | --all              Re-auth one or all OAuth servers")
        print()
