"""
Configuration management for KenXCode Agent.

Config files are stored in ~/.kenxcode/ for easy access:
- ~/.kenxcode/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.kenxcode/.env         - API keys and secrets

This module provides:
- kenxcode config          - Show current configuration
- kenxcode config edit     - Open config in editor
- kenxcode config get      - Print a resolved configuration value
- kenxcode config set      - Set a specific value
- kenxcode config unset    - Remove a user configuration value
- kenxcode config wizard   - Re-run setup wizard
"""
...
from kenxcode_cli.route_identity import normalize_route_base_url
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``kenxcode config set`` (which rewrites
    ...
    ``config.yaml`` in place: kenxcode never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.kenxcode/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``kenxcode logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  kenxcode config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` KenXCode makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. KenXCode itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``kenxcode update``, the TUI build.
...
#   on every plugin install / ``kenxcode update``.
...
# * ``KENXCODE_HOME`` / ``KENXCODE_PROFILE`` / ``KENXCODE_CONFIG`` /
#   ``KENXCODE_ENV`` — KenXCode runtime location flags. Writing these into
...
#
# IMPORTANT: ``KENXCODE_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (KENXCODE_LANGFUSE_PUBLIC_KEY,
# KENXCODE_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # KenXCode runtime location — never via dashboard env writer.
    # NOT a KENXCODE_* blanket: integration credentials (KENXCODE_GEMINI_*,
    # KENXCODE_LANGFUSE_*, KENXCODE_SPOTIFY_*, ...) ARE allowed.
    "KENXCODE_HOME", "KENXCODE_PROFILE", "KENXCODE_CONFIG", "KENXCODE_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or KenXCode runtime location "
            "(KENXCODE_HOME, KENXCODE_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.kenxcode/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # KENXCODE_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # Activation is via plugins.enabled (opt-in through `kenxcode plugins enable
    # observability/langfuse` or `kenxcode tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "KENXCODE_LANGFUSE_ENV",
    "KENXCODE_LANGFUSE_RELEASE",
    "KENXCODE_LANGFUSE_SAMPLE_RATE",
    "KENXCODE_LANGFUSE_MAX_CHARS",
    "KENXCODE_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so different
    # profiles can use different ACP backends without cross-leak.
    "KENXCODE_ACP_AUTH_METHOD",
    "KENXCODE_ACP_AUTO_APPROVE",
    "KENXCODE_COPILOT_ACP_COMMAND",
    "KENXCODE_COPILOT_ACP_ARGS",
    "COPILOT_CLI_PATH",
...
from kenxcode_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
# from `nix run` / `nix profile install` (which don't set KENXCODE_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("KENXCODE_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_kenxcode_home() / ".managed"
...
def is_managed() -> bool:
    """Check if KenXCode is running in package-manager-managed mode.

    Two signals: the KENXCODE_MANAGED env var (set by the systemd service),
    or a .managed marker file in KENXCODE_HOME (set by the NixOS activation
...
_NIX_UPDATE_MSG = (
    "Update KenXCode through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``kenxcode_cli/`` — i.e. the git checkout for source
    installs, ``/opt/kenxcode`` inside the published image. It is a property of
    the running interpreter, NOT of ``$KENXCODE_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how KenXCode was installed: 'docker', 'nix', 'nixos', 'git', or 'unknown'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$KENXCODE_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. KENXCODE_MANAGED env / .managed marker (NixOS managed mode)
    4. /nix/store/ path detection -> 'nix' (nix run / nix profile install)
    5. .git directory presence -> 'git'
    6. Fallback -> 'unknown'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.kenxcode``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$KENXCODE_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.kenxcode:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$KENXCODE_HOME``, a
    ...
    ``docker`` and ``kenxcode update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/kenxcode-agent`` image bakes a ``docker``
        stamp into ``/opt/kenxcode`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $KENXCODE_HOME.
    try:
    ...
    #    host install whose shared $KENXCODE_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``kenxcode update``.
    try:
        method = (
            (get_kenxcode_home() / ".install_method")
    ...
    # detect Nix installs that don't set KENXCODE_MANAGED (e.g. ``nix run``,
...
def _running_in_container() -> bool:
    """Thin wrapper around ``kenxcode_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$KENXCODE_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/kenxcode`` in the published image, which instead bakes the stamp at
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method in {"nix", "nixos"}:
        return _NIX_UPDATE_MSG
    if method == "docker":
        return "docker pull nousresearch/kenxcode-agent:latest"
    return "kenxcode update"
...
# Long-form text for ``kenxcode update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# kenxcode_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side KenXCode, it doesn't update
...
✗ ``kenxcode update`` doesn't apply inside the Docker container.

KenXCode Agent runs as a published image (nousresearch/kenxcode-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/kenxcode-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate kenxcode-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/kenxcode-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/kenxcode-agent/tags
  • Your config and session history live under ``$KENXCODE_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``kenxcode update`` inside Docker.
...
def format_managed_message(action: str = "modify this KenXCode installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("KENXCODE_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this KenXCode installation is managed by NixOS "
            f"(KENXCODE_MANAGED={env_hint}).\n"
            "Edit services.kenxcode-agent.settings in your configuration.nix and run:\n"
            "  sudo nixos-rebuild switch"
        )

    return (
        f"Cannot {action}: this KenXCode installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall KenXCode."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from KENXCODE_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, kenxcode_bin
    or None if container mode is not active, we're already inside the
    container, or KENXCODE_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("KENXCODE_DEV") == "1":
        return None
    ...
    container_mode_file = get_kenxcode_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "kenxcode-agent")
    exec_user = info.get("exec_user", "kenxcode")
    kenxcode_bin = info.get("kenxcode_bin", "/data/current-package/bin/kenxcode")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "kenxcode_bin": kenxcode_bin,
    }
...
# Re-export from kenxcode_constants — canonical definition lives there.
from kenxcode_constants import get_kenxcode_home, get_process_kenxcode_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_kenxcode_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_kenxcode_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_kenxcode_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the KENXCODE_UID / KENXCODE_GID env vars set by Docker deployments.

    Docker containers running KenXCode commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level KENXCODE_HOME once, but
    subdirectories created at runtime by ``ensure_kenxcode_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("KENXCODE_UID", "").strip()
    gid_str = os.environ.get("KENXCODE_GID", "").strip()
    try:
...
def _chown_to_kenxcode_uid(path) -> None:
    """Chown ``path`` to ``KENXCODE_UID:KENXCODE_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_kenxcode_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_kenxcode_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the kenxcode group can
    share state with the gateway service.

    The mode can be overridden via the KENXCODE_HOME_MODE environment variable
    (e.g. KENXCODE_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse KENXCODE_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``KENXCODE_UID``/``KENXCODE_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("KENXCODE_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_kenxcode_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When KenXCode runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("KENXCODE_CONTAINER") or os.environ.get("KENXCODE_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set KENXCODE_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into KENXCODE_HOME, upgrading legacy empty templates.
...
# ensure_kenxcode_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_KENXCODE_HOME_ENSURED: set = set()


def ensure_kenxcode_home():
    """Ensure ~/.kenxcode directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``KENXCODE_HOME`` we only re-run
    ...
    ``get_kenxcode_home()`` and therefore re-run for the new path.
    """
    home = get_kenxcode_home()
    key = str(home)

    if key in _KENXCODE_HOME_ENSURED and home.is_dir():
        return
    # Named profiles must be created explicitly (e.g. ``kenxcode profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old KENXCODE_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_kenxcode_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
    ...
    _KENXCODE_HOME_ENSURED.add(key)


def _ensure_kenxcode_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"KENXCODE_HOME {home} does not exist. "
...
from kenxcode_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `kenxcode config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.kenxcode.config`` entries, then checks
    ...
    try:
        ...
        # should never break `kenxcode update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but KenXCode' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older KenXCode versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``kenxcode_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (kenxcode_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (kenxcode_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (kenxcode_cli/setup.py)
    "known_plugin_toolsets", # written/read by kenxcode_cli/tools_config.py toolset-save flow
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'kenxcode setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — KenXCode won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # scalars are bridged into os.environ (gateway/run.py, kenxcode send) so
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'kenxcode doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        ...
        hint_path = display_kenxcode_home()
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "kenxcode update / kenxcode -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from kenxcode_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ kenxcode config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in kenxcode_cli.config_migrations as a
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from kenxcode_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: kenxcode config set <key> <value>")
    ...
    # `kenxcode update`, but only the version bump is persisted.
    ...
    # metadata.kenxcode.config in their SKILL.md frontmatter.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.kenxcode/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_kenxcode_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``kenxcode config set``
    ...
    backends) — while display paths (``kenxcode status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _kenxcode_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.kenxcode/config.yaml as-is, without merging defaults or migrating.
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_kenxcode_home``, tui profile override, multi-profile probes).
    """
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.kenxcode/config.yaml.
    ...
    (which change ``KENXCODE_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/kenxcode/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_kenxcode_dotenv()
...
#   openai-codex (OAuth — kenxcode auth) — OpenAI Codex
#   nous         (OAuth — kenxcode auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.kenxcode/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset KenXCode writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.kenxcode/.env.
    ...
    interactive menu render (`kenxcode tools`, `kenxcode setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `kenxcode tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.kenxcode/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.kenxcode/.env."""
    if is_managed():
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``KENXCODE_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"KENXCODE_CUSTOM_{slug}_API_KEY" if slug else "KENXCODE_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.kenxcode/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.kenxcode/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to KenXCode — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known KenXCode vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.kenxcode/.env``, scope-aware.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.kenxcode/.env`` over ``os.environ``.

    Used for KenXCode-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚡ KenXCode Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from kenxcode_cli.auth import get_anthropic_key
    ...
    # Warn on stale KENXCODE_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("KENXCODE_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale KENXCODE_MAX_ITERATIONS={_env_ghost} "
                f"(run 'kenxcode doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  kenxcode config edit     # Edit config file", Colors.DIM))
    print(color("  kenxcode config set <key> <value>", Colors.DIM))
    print(color("  kenxcode setup           # Run setup wizard", Colors.DIM))
    print()
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `kenxcode cron list`, then pin the "
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``kenxcode config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``kenxcode config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``kenxcode config set --force``.
    """
    if is_managed():
    ...
    if config_path.exists():
        try:
            ...
            print(
                ...
                f"  (kenxcode config edit will open it in your editor.)",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    # ``kenxcode config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``kenxcode config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    kenxcode config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    kenxcode config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
    _set_nested(user_config, key, value)
    # Normalize the api_base → base_url alias at set-time too (issue #8919),
    # so a fresh `kenxcode config set model.api_base ...` lands on the canonical
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_kenxcode_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
    ...
    # — e.g. `kenxcode config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `kenxcode config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: kenxcode config get <key> [--json]")
            print()
            print("Examples:")
            print("  kenxcode config get model")
            print("  kenxcode config get terminal.backend")
            print("  kenxcode config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: kenxcode config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  kenxcode config set model anthropic/claude-sonnet-4")
            print("  kenxcode config set terminal.backend docker")
            print("  kenxcode config set OPENROUTER_API_KEY sk-or-...")
            print()
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: kenxcode config unset <key>")
            print()
            print("Examples:")
            print("  kenxcode config unset model")
            print("  kenxcode config unset terminal.backend")
            print("  kenxcode config unset OPENROUTER_API_KEY")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'kenxcode config migrate' to add them")
        ...
        print("  kenxcode config           Show current configuration")
        print("  kenxcode config edit      Open config in editor")
        print("  kenxcode config get <key>          Print a resolved config value")
        print("  kenxcode config set <key> <value>   Set a config value")
        print("  kenxcode config unset <key>        Remove a config value")
        print("  kenxcode config check     Check for missing/outdated config")
        print("  kenxcode config migrate   Update config with new options")
        print("  kenxcode config path      Show config file path")
        print("  kenxcode config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``kenxcode config`` UI so users
