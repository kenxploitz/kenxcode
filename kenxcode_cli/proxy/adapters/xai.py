...
# xAI's public API is OpenAI-compatible for the endpoints KenXCode commonly
# uses. The Responses endpoint is included because KenXCode' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via KenXCode-managed OAuth credentials."""

    auth_hint = "kenxcode auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`kenxcode auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`kenxcode auth reset xai-oauth` or re-authenticate with "
                    "`kenxcode auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `kenxcode auth add xai-oauth --type oauth`."
            )
