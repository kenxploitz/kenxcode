"""``kenxcode logs`` — view and filter KenXCode log files.
...
Usage examples::
    ...
    kenxcode logs                    # last 50 lines of agent.log
    kenxcode logs -f                 # follow agent.log in real time
    kenxcode logs errors             # last 50 lines of errors.log
    kenxcode logs gateway -n 100    # last 100 lines of gateway.log
    kenxcode logs gui -f            # follow gui.log (dashboard/pty/ws)
    kenxcode logs desktop -f        # follow desktop.log (Electron app boot/backend)
    kenxcode logs --level WARNING    # only WARNING+ lines
    kenxcode logs --session abc123   # filter by session ID substring
    kenxcode logs --component tools  # only tool-related lines
    kenxcode logs --since 1h         # lines from the last hour
    kenxcode logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_kenxcode_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when KenXCode runs — try 'kenxcode chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_kenxcode_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_kenxcode_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_kenxcode_home()}/logs/")
        return

    print(f"Log files in {display_kenxcode_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'kenxcode chat' to generate logs)")
