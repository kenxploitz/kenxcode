"""
Unified tool configuration for KenXCode Agent.

`kenxcode tools` and `kenxcode setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.kenxcode/config.yaml under
...
from kenxcode_cli.nous_subscription import (
    MANAGED_FEATURE_COVERAGE_CATEGORY,
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from kenxcode_cli.nous_account import format_nous_portal_entitlement_message
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``kenxcode tools post-setup <key>`` child. On Windows, every console child
    ...
    :func:`kenxcode_cli._subprocess_compat.windows_hide_flags`) suppresses it
...
from kenxcode_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _KENXCODE_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `kenxcode tools` → Video Generation, which walks
...
# `kenxcode tools` → X (Twitter) Search setup walks users through credential
# setup. The tool's check_fn means the schema still won't appear to the
# model if the credential later goes missing or expires.
_DEFAULT_OFF_TOOLSETS = {"homeassistant", "spotify", "discord", "discord_admin", "video", "video_gen", "x_search", "a2a"}


# Config-only capabilities: they appear in `kenxcode tools` for provider/API-key
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from kenxcode_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `kenxcode tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``kenxcode tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from kenxcode_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``kenxcode tools`` checklist actually offers
    for ``platform``.
    ...
    so ``kenxcode tools`` never claims to add or remove a toolset the user was
...
from kenxcode_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "KenXCode routes X searches through xAI's built-in x_search "
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set KENXCODE_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "KENXCODE_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "KENXCODE_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "KENXCODE_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "KENXCODE_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "KENXCODE_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `kenxcode model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("KENXCODE_CUA_DRIVER_CMD", "").strip() or "cua-driver"
...
def _cua_driver_env() -> dict:
    """cua-driver child env with the KenXCode telemetry policy applied.
...
def _pip_install(
    ...
    *,
    ...
    # Managed uv first: $KENXCODE_HOME/bin is never on PATH, so a bare which()
    # misses the uv KenXCode installed and prefers a system one when both exist.
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``kenxcode update`` and
      by ``kenxcode computer-use install --upgrade``.
    ...
    falling through to the full upstream installer. ``kenxcode update`` sets
    ...
    a further ~600s wait on Windows). ``kenxcode computer-use install
    ...
    ``kenxcode update`` already prints a contextual line before its update
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `kenxcode update` calls this
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `kenxcode computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
    ...
    # caller: `kenxcode update` (require_confirmed_update=True) keeps the
    ...
    # `kenxcode computer-use install --upgrade` falls through and re-runs the
    # installer as before.
    ...
    if binary:
        ...
        if _state is None and require_confirmed_update:
            ...
            _print_info(
                "    Force a refresh with: kenxcode computer-use install --upgrade"
            )
            return True
...
def _run_cua_driver_installer(
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    try:
        # When not verbose (e.g. `kenxcode update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `kenxcode update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.kenxcode/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            ...
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / KenXCode process.")
            return True
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # Managed Node first — $KENXCODE_HOME/node is not on PATH, so a bare
        # which() reports "no npm" on installs whose only Node is the one
        # KenXCode installed for exactly this toolchain.
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_kenxcode_home()}/kenxcode-agent && npm install --workspaces=false")
                if result.stderr:
            ...
            _print_warning("    Node.js not found - browser tools require: npm install (in kenxcode-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/kenxcode-agent:latest"
            )
            return
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_info("    Change via stt.local.model in ~/.kenxcode/config.yaml")
            else:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.kenxcode/config.yaml")
        ...
        # Run the full `kenxcode auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.kenxcode/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from kenxcode_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: kenxcode auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `kenxcode auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: kenxcode auth spotify")
        except Exception as exc:
        ...
        try:
            from kenxcode_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: kenxcode plugins enable observability/langfuse")
        _print_info("    Restart KenXCode for tracing to take effect.")
        _print_info("    Verify: kenxcode plugins list")
        ...
        try:
            ...
            _print_info("    Run later: kenxcode auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `kenxcode auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: kenxcode auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: kenxcode auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``kenxcode tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``kenxcode tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: kenxcode tools post-setup <key>")
        return 2
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``kenxcode-discord``, which contains those
...
#:
#: Saving ``kenxcode tools`` (or one toggle in the desktop Toolsets UI) replaces
...
#: has touched the picker, while everyone still on ``[kenxcode-cli]`` inherits it
...
def _enable_recently_shipped_toolsets(
    ...
    Either way of saying no outlives this: unchecking in ``kenxcode tools``
    ...
    default_ts = plat_info["default_toolset"] if plat_info else f"kenxcode-{platform}"
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # narrow composites (kenxcode-acp, kenxcode-webhook) stay narrow.
...
def _get_platform_tools(
    ...
    *,
    ...
    # ``kenxcode-discord``) is an opt-in to the platform's native default-off
    ...
    if toolset_names is None or not isinstance(toolset_names, list):
        ...
        if plat_info:
            ...
            default_ts = f"kenxcode-{platform}"
    ...
    # "kenxcode-cli" (which include all _KENXCODE_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[kenxcode-cli, spotify]`` after enabling Spotify via ``kenxcode
        ...
        # (e.g. "kenxcode-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``kenxcode tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `kenxcode tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"kenxcode-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("kenxcode-")}
    ...
    # shipped as a bundled plugin but user must opt in via `kenxcode tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `kenxcode tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by kenxcode tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # is invalid (e.g. a migration or hand-edit left `kenxcode` instead of
    # `kenxcode-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `kenxcode update`/`kenxcode doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `kenxcode tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (kenxcode-cli, kenxcode-telegram, etc.)
    ...
    # Opening `kenxcode tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `kenxcode tools` gets a silent no-op
...
def _agent_browser_installed() -> bool:
    ...
    from kenxcode_cli.nous_subscription import _local_browser_runnable

    # The install hook runs in a spawned ``kenxcode tools post-setup`` process,
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from kenxcode_cli.nous_subscription import _has_agent_browser
...
# Kept in sync with the dashboard selects (kenxcode_cli/web_server.py
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `kenxcode model`).
    if managed_feature:
        from kenxcode_cli.nous_subscription import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``kenxcode model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    substrate — so this picker lists exactly what the ``kenxcode model`` aux-task
    ...
    """
    try:
        from kenxcode_cli.inventory import (
    ...
    if not providers:
        _print_warning(
            ...
            "with `kenxcode model`, then re-run this."
        )
        return
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `kenxcode model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `kenxcode tools` and `kenxcode setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚡ KenXCode Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://kenxcode-agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from kenxcode_constants import display_kenxcode_home
    print(color(f"  Tool configuration saved to {display_kenxcode_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'kenxcode' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, kenxcode mcp
