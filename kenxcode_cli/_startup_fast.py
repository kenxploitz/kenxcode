...
This module is imported by ``kenxcode_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no kenxcode_cli.config,
...
``kenxcode_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(kenxcode_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect KENXCODE_HOME?"""
    active_profile = os.path.join(kenxcode_root, "active_profile")
    try:
...
def _resolved_home() -> str:
    kenxcode_home = os.environ.get("KENXCODE_HOME", "").strip()
    if kenxcode_home:
        return kenxcode_home
    return os.path.join(os.path.expanduser("~"), ".kenxcode")
...
def container_mode_may_be_active() -> bool:
    ...
    kenxcode_home = os.environ.get("KENXCODE_HOME", "").strip()
    if kenxcode_home:
        if os.path.exists(os.path.join(kenxcode_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(kenxcode_home)))
        return (
            parent_name != "profiles"
            and active_profile_may_override_home(kenxcode_home)
        )

    default_home = os.path.join(os.path.expanduser("~"), ".kenxcode")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect KENXCODE_HOME.
    """
...
def print_fast_version_info() -> None:
    from kenxcode_cli import __release_date__, __version__

    print(f"KenXCode Agent v{__version__} ({__release_date__})")
    ...
    print("Run 'kenxcode version' for update status.")


def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``kenxcode --version`` before the heavy import wall.

    Termux keeps its historical contract (also accepts the ``version``
    subcommand + the KENXCODE_TERMUX_DISABLE_FAST_CLI escape hatch). Everywhere
    ...
    if is_termux and os.environ.get("KENXCODE_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
