"""MCP picker — interactive `kenxcode mcp picker` (also the default `kenxcode mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``kenxcode mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `kenxcode plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from kenxcode_cli.cli_output import prompt_yes_no
from kenxcode_cli.curses_ui import curses_single_select
from kenxcode_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new KenXCode session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from kenxcode_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `kenxcode mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: kenxcode mcp install <name>    Picker: kenxcode mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their KenXCode is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer KenXCode — run `kenxcode update` "
...
def show_catalog() -> None:
    """`kenxcode mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`kenxcode mcp picker` (and default `kenxcode mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`kenxcode mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `kenxcode mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
