"""
KenXCode Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with kenxcode-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.kenxcode/plugins/<name>/``
3. **Project plugins** – ``./.kenxcode/plugins/<name>/`` (opt-in via
   ``KENXCODE_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``kenxcode_agent.plugins``
   entry-point group.
...
from kenxcode_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE


def get_bundled_plugins_dir() -> Path:
    """Locate the bundled ``plugins/`` directory.

    Honours ``KENXCODE_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("KENXCODE_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``KENXCODE_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.kenxcode/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("KENXCODE_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When KENXCODE_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other KenXCode loggers.
    """
    ...
    logger.debug(
        "KENXCODE_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. KenXCode' shipped guidance lives in the evidence-based
    ...
    # Kanban task lifecycle hooks. Fired by kenxcode_cli.kanban_db when a task
    ...
    # separate `kenxcode -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `kenxcode kanban dispatch`),
...
ENTRY_POINTS_GROUP = "kenxcode_agent.plugins"

_NS_PARENT = "kenxcode_plugins"
...
class PluginManifest:
    ...
    #              in ~/.kenxcode/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``kenxcode plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        """Return the active KenXCode profile name (e.g. ``"default"``).

        Derived from ``KENXCODE_HOME`` via
        :func:`kenxcode_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.kenxcode/profiles/<name>``, or ``"custom"`` when
        ``KENXCODE_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with KenXCode core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``kenxcode honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``kenxcode <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`kenxcode_cli.dashboard_auth.DashboardAuthProvider`. Used by
    ...
    def register_secret_source(self, source) -> None:
        ...
        sources run during ``load_kenxcode_dotenv()`` startup — after
        ``~/.kenxcode/.env`` loads, before KenXCode reads credentials — when
    ...
    def register_slack_action_handler(
        ...
        KenXCode' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``kenxcode model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: kenxcode_cli.main imports plugins indirectly
        from kenxcode_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.kenxcode/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        if self._discovered and not force:
            return
        if env_var_enabled("KENXCODE_SAFE_MODE"):
            logger.info("KENXCODE_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to kenxcode_cli/. Two layouts are
        ...
        # 2. User plugins (~/.kenxcode/plugins/)
        user_dir = get_kenxcode_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.kenxcode/plugins/)
        if _env_enabled("KENXCODE_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".kenxcode" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set KENXCODE_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with kenxcode and must
            ...
            # all ~20 of them added several seconds to every `kenxcode`
            # invocation — including plain `kenxcode chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform KenXCode ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `kenxcode plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``kenxcode plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`kenxcode plugins
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `kenxcode plugins list`.
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``kenxcode_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``kenxcode_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="kenxcode-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``kenxcode tools`` TUI so plugin-provided toolsets appear
