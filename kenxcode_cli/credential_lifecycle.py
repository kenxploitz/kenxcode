"""Unified provider-credential lifecycle across every store KenXCode reads.

A provider API key can live in up to THREE stores at once:

    1. ``~/.kenxcode/.env``                     — the canonical secret store
    2. ``~/.kenxcode/auth.json`` →
       ``credential_pool.<provider>[*]``      — env-seeded pool entries
       (``source == "env:<VAR>"``) persisted by the pool loader
    3. ``~/.kenxcode/config.yaml``              — inline mirrors written by the
...
def purge_env_credential_references(
    ...
    # Make the removal sticky the same way `kenxcode auth remove` does: a
...
def save_provider_env_credential(env_var: str, value: str) -> Dict[str, Any]:
    ...
    re-add through the UI behaves like ``kenxcode auth add``.
    """
    from kenxcode_cli.config import load_env, save_env_value
    ...
    # that reads this var.
    try:
        from kenxcode_cli.auth import unsuppress_credential_source
...
def remove_provider_env_credential(env_var: str) -> Dict[str, Any]:
    ...
    """
    from kenxcode_cli.config import load_env, remove_env_value
