"""Shell completion script generation for kenxcode CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_kenxcode_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# KenXCode Agent bash completion
# Add to ~/.bashrc:
#   eval "$(kenxcode completion bash)"

_kenxcode_profiles() {{
    local profiles_dir="$HOME/.kenxcode/profiles"
...
_kenxcode_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_kenxcode_profiles)" -- "$cur"))
        return
    fi
...
complete -F _kenxcode_completion kenxcode
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef kenxcode
# KenXCode Agent zsh completion
# Add to ~/.zshrc:
...
_kenxcode_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.kenxcode/profiles" ]]; then
        profiles+=($HOME/.kenxcode/profiles/*(N/:t))
    fi
...
_kenxcode() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_kenxcode_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'kenxcode command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# KenXCode Agent fish completion",
        "# Add to your config:",
        "#   kenxcode completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __kenxcode_profiles",
        "    echo default",
        "    if test -d $HOME/.kenxcode/profiles",
        "        for d in $HOME/.kenxcode/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c kenxcode -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c kenxcode -f -s p -l profile"
        " -d 'Profile name' -xa '(__kenxcode_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__kenxcode_profiles)' -d 'Profile name'"
                )
