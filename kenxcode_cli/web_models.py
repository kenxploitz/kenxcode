"""Pydantic request/response models for the KenXCode dashboard web server.

Extracted verbatim from ``kenxcode_cli/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from kenxcode_cli.web_server import ConfigUpdate`` keep working.
"""
...
class ModelAssignment(BaseModel):
    ...
    # ``kenxcode model`` custom flow collects.
    api_key: str = ""
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `kenxcode import`. The spawned action runs with
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`kenxcode -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the KENXCODE_HOME
