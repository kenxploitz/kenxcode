...
Pure display functions with no KenXCodeCLI state dependency.
"""
import json
import logging
import os
...
def cprint(text: str):
    ...
    try:
        ...
        # absent stdout (pythonw.exe, CI, `kenxcode ... > file`) raises
...
from kenxcode_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
# (e.g. nix-built kenxcode — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/NousResearch/kenxcode-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/nousresearch/kenxcode-agent"
...
def check_for_updates() -> Optional[int]:
    """Check whether a KenXCode update is available.

    Two paths: if ``KENXCODE_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = kenxcode_home / ".update_check"
    embedded_rev = os.environ.get("KENXCODE_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # KENXCODE_REVISION (that's nix-only). Returning None makes both the Rich
    # banner (build_welcome_banner) and the Ink badge (branding.tsx, guarded
    # on `typeof === 'number' && > 0`) show nothing. The dashboard's REST
    # `/api/kenxcode/update/check` endpoint short-circuits docker the same way
    # (web_server.py); mirror that here so the banner/TUI surfaces agree.
    try:
        from kenxcode_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $KENXCODE_HOME/kenxcode-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = kenxcode_home / "kenxcode-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active KenXCode git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$KENXCODE_HOME/kenxcode-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``kenxcode_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from kenxcode_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/NousResearch/kenxcode-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    KenXCode checkout. Cached per-process. Release URL always points at the
    canonical NousResearch/kenxcode-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"KenXCode Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else KENXCODE_CADUCEUS
    except Exception:
        _bskin = None
    ...
    if (provider or "").strip().lower() == "moa":
        ...
        if not (model or "").strip() or (model or "").strip().lower() == "unknown":
            ...
            left_lines.append(
                ...
                f"[dim {dim}]— run /model or kenxcode setup[/]"
            )
        else:
    ...
    if os.getenv("KENXCODE_YOLO_MODE"):
    ...
    try:
        from kenxcode_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from kenxcode_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built kenxcode; we know an update
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else KENXCODE_AGENT_LOGO
        console.print(_logo)
        console.print()
