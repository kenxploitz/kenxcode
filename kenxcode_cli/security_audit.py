"""On-demand supply-chain audit for KenXCode Agent installs.

Scans three surfaces a KenXCode user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The KenXCode venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.kenxcode/plugins``
...
def _discover_plugins(kenxcode_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.kenxcode/plugins``.
...
def _discover_components(
    *,
    ...
    home = kenxcode_home or Path(get_kenxcode_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `kenxcode security audit`."""
    ...
    components = _discover_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, kenxcode_home=home
    )
