...
Why this exists
---------------
``kenxcode_cli/web_server.py`` owns all dashboard runtime state: the ephemeral
...
``APIRouter`` modules under ``kenxcode_cli/web_routers/`` need those helpers, but
...
thin proxy that resolves ``kenxcode_cli.web_server.<name>`` *at call time*.  This
...
def _server():
    """Return the live ``kenxcode_cli.web_server`` module (imported on demand)."""
    mod = sys.modules.get("kenxcode_cli.web_server")
    if mod is None:  # pragma: no cover - routers are only mounted by web_server
        import kenxcode_cli.web_server as mod  # type: ignore[no-redef]
    return mod
