...
def _root() -> Path:
    ...
    # helper's token, including custom KENXCODE_HOME layouts.
    return get_default_kenxcode_root() / "desktop-ssh"
...
def process_state(pid: int, creation_time_ns: int, kenxcode_path: str, spawn_nonce: str) -> dict[str, Any]:
    import psutil
    _nonce(spawn_nonce)
    try:
    ...
    expected = os.path.normcase(os.path.abspath(kenxcode_path))
    arg0 = os.path.normcase(os.path.abspath(argv[0]))
    # argv[0] is either the kenxcode exe directly, or (normal case) the base Python
    ...
    # `-c <bootstrap that runs kenxcode_cli.main>`; also accept a plain `-m` launch.
    ...
    launches_module = (
        argv[1:3] == ["-m", "kenxcode_cli.main"]
        or (len(argv) > 2 and argv[1] == "-c" and "kenxcode_cli.main" in argv[2])
    )
...
def terminate_owned(pid: int, creation_time_ns: int, kenxcode_path: str, spawn_nonce: str) -> bool:
    state = process_state(pid, creation_time_ns, kenxcode_path, spawn_nonce)
...
def _resolve_direct_interpreter(python_entry: str) -> tuple[str, list[str]]:
    ...
    Also resolves kenxcode_cli's own location and prepends its parent, because the
    ...
    # the kenxcode_cli package parent must be explicit -- the launcher resolves it via
    ...
    if not root or not os.path.isdir(root):
        raise ValueError("could not locate the kenxcode_cli package")
    return base, py_path
...
def spawn_backend(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    configured_path = str(payload["kenxcodePath"])
    if not os.path.isabs(configured_path):
        raise ValueError("KenXCode path must be absolute")
    kenxcode_path = os.path.abspath(configured_path)
    ...
    venv_dir = os.path.dirname(kenxcode_path)
    python_entry = os.path.join(venv_dir, "python.exe")
    if not os.path.isfile(python_entry):
        raise ValueError("KenXCode Python runtime was not found")
    ...
    bootstrap = (
        ...
        "runpy.run_module('kenxcode_cli.main',run_name='__main__',alter_sys=True)"
    )
...
def inspect_kenxcode(kenxcode_path: str) -> dict[str, Any]:
    path = os.path.abspath(kenxcode_path)
    if not os.path.isabs(kenxcode_path) or not os.path.isfile(path):
        raise ValueError("KenXCode path is not an executable file")
...
def dispatch(argv: list[str]) -> Any:
    ...
    if operation == "probe":
        import platform
        return {"os": "Windows", "arch": platform.machine(), "kenxcodeHome": str(get_default_kenxcode_root()), "python": sys.executable}
    ...
    if operation == "inspect" and len(argv) == 2:
        return inspect_kenxcode(argv[1])
