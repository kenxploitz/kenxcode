...
Usage:
    kenxcode pairing list              # Show all pending + approved users
    kenxcode pairing approve <platform> <request-id|code>  # Approve a pairing request
    kenxcode pairing revoke <platform> <user_id> # Revoke user access
    kenxcode pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle kenxcode pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: kenxcode pairing {list|approve|revoke|clear-pending}")
        print("Run 'kenxcode pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: kenxcode pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.kenxcode/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'kenxcode pairing list' to see pending requests.\n")
