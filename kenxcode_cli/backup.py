"""
Backup and import commands for kenxcode CLI.

`kenxcode backup` creates a zip archive of the entire ~/.kenxcode/ directory
(excluding the kenxcode-agent repo and transient files).

`kenxcode import` restores from a backup zip, overlaying onto the current
KENXCODE_HOME root.
"""
...
from kenxcode_constants import get_default_kenxcode_root, get_kenxcode_home, display_kenxcode_home
...
# ``kenxcode-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/kenxcode-agent/``
...
# KENXCODE_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "kenxcode-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under KENXCODE_HOME) —
...
# File names that ``kenxcode import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE KENXCODE_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside KENXCODE_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to kenxcode root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``kenxcode-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/kenxcode-agent/`` — must be preserved.
        if part == "kenxcode-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside KENXCODE_HOME.
...
# to the user as a hung `kenxcode update` (#70553 follow-up). Sessions databases
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # kenxcode_cli.sqlite_safe_read). Verification targets snapshots and
...
def run_backup(args) -> None:
    """Create a zip backup of the KenXCode home directory."""
    kenxcode_root = get_default_kenxcode_root()

    if not kenxcode_root.is_dir():
        print(f"Error: KenXCode home directory not found at {kenxcode_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"kenxcode-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"kenxcode-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_kenxcode_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(kenxcode_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(kenxcode_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``kenxcode-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "kenxcode-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(kenxcode_root)
    ...
    # outside KENXCODE_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_kenxcode_home()}."
        )
    ...
    if not errors:
        print(f"\nRestore with: kenxcode import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a KenXCode backup.
    ...
    # Look for telltale files that a kenxcode home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a KenXCode backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.kenxcode/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a kenxcode dir name
        if prefix in {".kenxcode", "kenxcode"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_kenxcode_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (kenxcode_root / "config.yaml").exists()
        has_env = (kenxcode_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has KenXCode configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under KENXCODE_HOME.
            ...
            target = kenxcode_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(kenxcode_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_kenxcode_home()}."
            )
        ...
        profiles_dir = kenxcode_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # kenxcode_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: kenxcode profile list  (after installing kenxcode)")

        # Guidance
        print()
        if not (kenxcode_root / "kenxcode-agent").is_dir():
            print("Note: The kenxcode-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: kenxcode update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  kenxcode -p {pname} gateway install")

        print("Done. Your KenXCode configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and kenxcode backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to KENXCODE_HOME).
...
# — `kenxcode update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $KENXCODE_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside KENXCODE_HOME and the entry is silently
...
def _quick_snapshot_root(kenxcode_home: Optional[Path] = None) -> Path:
...
def create_quick_snapshot(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: kenxcode snapshot list"
        )
        logger.error(
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(kenxcode_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside KENXCODE_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``kenxcode update``.
    ...
    Args:
        ...
        kenxcode_home: Override for the KenXCode home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(kenxcode_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for kenxcode backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_kenxcode_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, kenxcode_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``kenxcode_root`` to ``out_path``.
...
def _pre_update_backup_dir(kenxcode_home: Optional[Path] = None) -> Path:
    home = kenxcode_home or get_kenxcode_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of KENXCODE_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<KENXCODE_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``kenxcode update``) should continue even if the backup fails.
    """
    kenxcode_root = kenxcode_home or get_default_kenxcode_root()
    if not kenxcode_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(kenxcode_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, kenxcode_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `kenxcode claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of KENXCODE_HOME under ``backups/`` before a
    ``kenxcode claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``kenxcode import <archive>``.  Writes to
    ``<KENXCODE_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `kenxcode import` and the
