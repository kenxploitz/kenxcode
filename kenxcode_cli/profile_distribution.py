"""Profile distributions — shareable, packaged KenXCode profiles via git.

A distribution is a KenXCode profile published as a git repository (or
...
* ``kenxcode profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``kenxcode skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``kenxcode profile``, not a parallel tree):

    kenxcode profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "kenxcode_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".kenxcode_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "kenxcode-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            kenxcode_requires=str(data.get("kenxcode_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.kenxcode_requires:
            out["kenxcode_requires"] = self.kenxcode_requires
        if self.author:
...
def check_kenxcode_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires KenXCode {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``kenxcode profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a KenXCode profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a KenXCode distribution."
        )

    # Version check up-front so we fail fast
    check_kenxcode_requires(manifest.kenxcode_requires, kenxcode_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.kenxcode).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="kenxcode_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `kenxcode profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`kenxcode profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="kenxcode_dist_update_") as tmp:
        plan = plan_install(
