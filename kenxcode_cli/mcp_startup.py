...
def start_background_mcp_discovery(*, logger, thread_name: str) -> None:
    ...
    with _mcp_discovery_lock:
        ...
        # Capture the caller's context-local KENXCODE_HOME override (profile
        ...
        def _discover() -> None:
            token = None
            try:
                from kenxcode_constants import set_kenxcode_home_override
            ...
            try:
                ...
                if token is not None:
                    try:
                        from kenxcode_constants import reset_kenxcode_home_override
...
def _resolve_discovery_timeout(
    ...
    When ``single_query`` is True (``kenxcode -z "..."`` / ``-q``), the larger
    ...
    try:
        from kenxcode_cli.config import load_config, DEFAULT_CONFIG
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``kenxcode dashboard``).  Those processes populate THIS module's
...
def ensure_mcp_discovery_before_agent_build(
    *,
    ...
    Non-interactive first turns (``chat -q``, ``kenxcode -z``) can construct
