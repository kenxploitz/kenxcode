"""`kenxcode checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.kenxcode/checkpoints/``.  Actions:

    kenxcode checkpoints               # same as `status`
    kenxcode checkpoints status        # total size, project count, breakdown
    kenxcode checkpoints list          # per-project checkpoint counts + workdir
    kenxcode checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    kenxcode checkpoints clear [-f]    # nuke the entire base (asks first)
    kenxcode checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    kenxcode checkpoints
    kenxcode checkpoints prune --retention-days 3 --max-size-mb 200
    kenxcode checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: kenxcode checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``kenxcode checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `kenxcode checkpoints` → status
