...
the tool registry or launching KenXCode (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``kenxcode-cli`` to the non-existent ``kenxcode``. ``resolve_toolset('kenxcode')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``kenxcode-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``kenxcode`` instead of ``kenxcode-cli``),
    ...
    for platform, raw in platform_toolsets.items():
        ...
        for name in names:
            ...
            if is_valid_toolset(name):
                valid_count += 1
                continue
            suggestion = f"kenxcode-{platform}"
            hint = (
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `kenxcode tools` to reconfigure."
        )
    return warnings
