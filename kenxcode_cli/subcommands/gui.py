"""``kenxcode gui`` subcommand parser.
...
def build_gui_parser(subparsers, *, cmd_gui: Callable) -> None:
    ...
    gui_parser = subparsers.add_parser(
        ...
        description=(
            "Launch the KenXCode Electron desktop app. By default this installs "
    ...
    gui_parser.add_argument(
        "--ignore-existing",
        action="store_true",
        help="Force Desktop to ignore any kenxcode CLI already on PATH during backend resolution",
    )
    gui_parser.add_argument(
        "--kenxcode-root",
        help="Override the KenXCode source root used by Desktop (sets KENXCODE_DESKTOP_KENXCODE_ROOT)",
    )
    gui_parser.add_argument(
        "--cwd",
        help="Initial project directory for Desktop chat sessions (sets KENXCODE_DESKTOP_CWD)",
    )
    gui_parser.add_argument(
        "--skip-build",
        action="store_true",
