...
Profile-aware location: ``$KENXCODE_HOME/logs/dashboard-auth.log``.
...
from ``kenxcode_constants`` or other kenxcode_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$KENXCODE_HOME/logs/dashboard-auth.log``.

    Uses ``kenxcode_constants.get_kenxcode_home()`` (a leaf module — no import
    ...
    return get_kenxcode_home() / "logs" / "dashboard-auth.log"
