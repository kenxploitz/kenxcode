...
description: "Diagnose and fix common KenXCode cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`kenxcode gateway` for foreground, or `kenxcode gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `kenxcode cron tick`.
...
```bash
date
kenxcode cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.kenxcode/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.kenxcode/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.kenxcode/.env` |
...
| `local` | Write access to `~/.kenxcode/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `kenxcode cron list` for updated `last_error` field (if available).
...
```bash
kenxcode skills list
```

Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `kenxcode skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `kenxcode skills list`.
...
2. `~/.kenxcode/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `kenxcode cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the KenXCode config directory). Verify:
```bash
ls ~/.kenxcode/scripts/your-script.py   # Must exist
kenxcode cron edit <job_id> --script ~/.kenxcode/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `kenxcode skills install <skill-name>`.
...
```bash
ps aux | grep kenxcode
# Kill duplicate processes, keep only one
```

### Check 4: Permissions on jobs.json

Jobs are stored in `~/.kenxcode/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.kenxcode/cron/jobs.json
chmod 600 ~/.kenxcode/cron/jobs.json   # Your user should own it
```
...
```bash
kenxcode cron list                    # Show all jobs, states, next_run times
kenxcode cron run <job_id>            # Schedule for next tick (for testing)
kenxcode cron edit <job_id>           # Fix configuration issues
kenxcode logs                         # View recent KenXCode logs
kenxcode skills list                  # Verify installed skills
```
...
1. Run the job with `kenxcode cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.kenxcode/logs/agent.log` for scheduler messages and `~/.kenxcode/logs/errors.log` for warnings
3. Open an issue at [github.com/NousResearch/kenxcode-agent](https://github.com/NousResearch/kenxcode-agent) with:
