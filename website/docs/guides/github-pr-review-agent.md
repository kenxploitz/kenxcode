...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  KenXCode Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to KenXCode instantly when PRs are opened or updated.
:::
...
- **KenXCode Agent installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  kenxcode gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.kenxcode/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure KenXCode can access GitHub. Start a chat:
...
```
Run: gh pr list --repo NousResearch/kenxcode-agent --state open --limit 3
```
...
Still in the chat, ask KenXCode to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo NousResearch/kenxcode-agent
```
...
A skill gives KenXCode consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.kenxcode/skills/code-review
```

Create `~/.kenxcode/skills/code-review/SKILL.md`:
...
Verify it loaded — start `kenxcode` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach KenXCode your team's standards:
...
2. Teach KenXCode your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
kenxcode gateway status    # Is the gateway running?
kenxcode cron list         # Is the job enabled?
```
