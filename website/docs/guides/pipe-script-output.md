...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `kenxcode send`."
---

# Pipe Script Output to Messaging Platforms

`kenxcode send` is a small, scriptable CLI that pushes a message to any
messaging platform KenXCode is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | kenxcode send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `kenxcode
...
kenxcode send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
kenxcode send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
kenxcode send --list

# Filter by platform
kenxcode send --list telegram
```
...
Any platform KenXCode ships adapters for works as a target:
...
`kenxcode send` resolves the message body in this order:

1. **Positional argument** — `kenxcode send --to telegram "hi"`
2. **`--file PATH`** — `kenxcode send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | kenxcode send --to telegram`

When stdin is a TTY (no pipe), KenXCode does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  kenxcode send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `kenxcode send` reuses your KenXCode config, the same script works on
any host where KenXCode is installed — no need to export bot tokens into
...
`kenxcode send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  kenxcode send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | kenxcode send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.kenxcode/bin/kenxcode send \
...
```bash
./train.py --epochs 200 && \
  kenxcode send --to telegram "training done" || \
  kenxcode send --to telegram "training failed (exit $?)"
```
...
kenxcode send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(kenxcode send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `kenxcode send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `kenxcode send` calls
the platform's REST endpoint directly using credentials from
`~/.kenxcode/.env` and `~/.kenxcode/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `kenxcode gateway start` and retry.
...
# Machine-readable
kenxcode send --list --json
```

The listing is built from `~/.kenxcode/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`kenxcode gateway
...
| Approach | Multi-platform | Reuses KenXCode creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `kenxcode send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`kenxcode send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `kenxcode send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `kenxcode send` shares with cron delivery.
