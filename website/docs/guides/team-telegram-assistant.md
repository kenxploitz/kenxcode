...
This tutorial walks you through setting up a Telegram bot powered by KenXCode Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **KenXCode Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.kenxcode/.env`

:::tip
A $5/month VPS is plenty for running the gateway. KenXCode itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team KenXCode Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_kenxcode_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by KenXCode Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.kenxcode/.env`:
...
```
[Gateway] Starting KenXCode Gateway...
...
# View live logs
journalctl --user -u kenxcode-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `kenxcode gateway install` to update the plist.
:::
...
```bash
kenxcode gateway stop && kenxcode gateway start
```
...
kenxcode pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.kenxcode/.env`:
...
Control how much detail the bot shows when using tools. In `~/.kenxcode/config.yaml`:
...
Customize how the bot communicates by editing `~/.kenxcode/SOUL.md`:

For a full guide, see [Use SOUL.md with KenXCode](/guides/use-soul-with-kenxcode).
...
```bash
# From the CLI
kenxcode cron list          # View all scheduled jobs
kenxcode cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u kenxcode-gateway` (Linux) or `~/.kenxcode/logs/gateway.log` (macOS) |
| Cron job output | `~/.kenxcode/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.kenxcode/cron/jobs.json` |
| Pairing data | `~/.kenxcode/pairing/` |
| Session history | `~/.kenxcode/sessions/` |
