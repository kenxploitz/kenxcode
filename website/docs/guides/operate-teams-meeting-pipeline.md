...
```bash
kenxcode teams-pipeline validate
```
...
```bash
kenxcode teams-pipeline token-health
kenxcode teams-pipeline token-health --force-refresh
```
...
```bash
kenxcode teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
kenxcode teams-pipeline maintain-subscriptions
kenxcode teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: KenXCode cron (recommended if you already run the KenXCode gateway)

KenXCode ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.kenxcode/scripts/`. First create the script:

```bash
mkdir -p ~/.kenxcode/scripts
cat > ~/.kenxcode/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec kenxcode teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.kenxcode/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
kenxcode cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/kenxcode-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=KenXCode Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=kenxcode
EnvironmentFile=/etc/kenxcode/env
ExecStart=/usr/local/bin/kenxcode teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/kenxcode-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run KenXCode Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now kenxcode-teams-pipeline-maintain.timer
systemctl list-timers kenxcode-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/kenxcode teams-pipeline maintain-subscriptions >> /var/log/kenxcode/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.kenxcode/.env` at the top of a wrapper script that crontab calls.
...
```bash
kenxcode teams-pipeline subscriptions   # should show expirationDateTime advanced
kenxcode teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
kenxcode teams-pipeline list
kenxcode teams-pipeline list --status failed
kenxcode teams-pipeline show <job-id>
```

### Replay a stored job

```bash
kenxcode teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
kenxcode teams-pipeline fetch --meeting-id <meeting-id>
kenxcode teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `kenxcode teams-pipeline maintain-subscriptions --dry-run`
- inspect `kenxcode teams-pipeline list --status failed`
...
- run `kenxcode teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `kenxcode teams-pipeline run`
...
- [ ] `kenxcode teams-pipeline validate` returns an OK snapshot
- [ ] `kenxcode teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (KenXCode cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
