...
title: "Desktop Plugin SDK (@kenxcode/plugin-sdk)"
description: "Extend the native KenXCode Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [KenXCode Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `KenXCodePlugin`.
It imports one module — `@kenxcode/plugin-sdk` — and gets everything: the app's
...
`$KENXCODE_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across KenXCode. This page is the **native
desktop app** (`kenxcode desktop`) SDK — the `@kenxcode/plugin-sdk` module and
`$KENXCODE_HOME/desktop-plugins/`. The **web dashboard** (`kenxcode dashboard`) has
its own, unrelated plugin system on `window.__KENXCODE_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a KenXCode Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$KENXCODE_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

Both take the same `KenXCodePlugin` contract, appear in **Settings → Plugins**, and
...
Create `$KENXCODE_HOME/desktop-plugins/hello/plugin.js` (that's `~/.kenxcode/...`
by default, or `~/.kenxcode/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.kenxcode/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@kenxcode/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, KenXCode' }),
      jsx('div', {
...
The only importable specifiers are `@kenxcode/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `KenXCodePlugin`:

```ts
interface KenXCodePlugin {
...
interface PluginContext {
  ...
  /** Plugin-scoped JSON persistence (keys live under `kenxcode.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@kenxcode/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@kenxcode/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@kenxcode/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@kenxcode/plugin-sdk'
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@kenxcode/plugin-sdk'
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@kenxcode/plugin-sdk'
...
```javascript
import { queryClient } from '@kenxcode/plugin-sdk'
...
`dashboard/` subfolder of a regular KenXCode plugin and declare it in a
`manifest.json`:
...
kenxcode-agent codebase directly (`kenxcode_state`, `kenxcode_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.kenxcode/`)
...
(`kenxcode.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `KenXCodePlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@kenxcode/plugin-sdk` alias.
2. It's still lint-fenced to `@kenxcode/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@kenxcode/plugin-sdk`, `react`,
...
| Plugin contract | `KenXCodePlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginRestOptions`, `Contribution` |
...
### Agents: the `kenxcode-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`kenxcode-desktop-plugins`** skill — it carries the same contract as this page in
...
`$KENXCODE_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `kenxcode logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@kenxcode/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.kenxcode/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.kenxcode/logs/errors.log` for
