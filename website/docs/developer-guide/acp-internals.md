...
The ACP adapter wraps KenXCode' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
kenxcode acp / kenxcode-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.kenxcode/.env
  -> configure stderr logging
  -> construct KenXCodeACPAgent
...
- `allow_once` -> KenXCode `once`
- `allow_always` -> KenXCode `always`
- reject options -> KenXCode `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps KenXCode tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["kenxcode-acp"])
...
Instead it reuses KenXCode' runtime resolver:

- `acp_adapter/auth.py`
- `kenxcode_cli/runtime_provider.py`

So ACP advertises and uses the currently configured KenXCode provider/credentials. It also always advertises a terminal setup auth method (`kenxcode-setup`, args `--setup`) so first-run ACP clients can open KenXCode' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.kenxcode/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `kenxcode-acp` toolset definition
- `kenxcode_cli/main.py` — `kenxcode acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `kenxcode-acp` script
