...
title: "Build a KenXCode Plugin"
description: "Step-by-step guide to building a complete KenXCode plugin with tools, hooks, data files, and skills"
---

# Build a KenXCode Plugin

This guide walks through building a complete KenXCode plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
KenXCode has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](/user-guide/features/tts#voice-message-transcription-stt) — set `KENXCODE_LOCAL_STT_COMMAND` to an argv-tokenized template |
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.kenxcode/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `kenxcode skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
Plugins that integrate **someone else's product or project** — observability/metrics backends, vendor SaaS connectors, analytics dashboards, paid-service tie-ins — are built and distributed as **standalone plugin repos**, not merged into `NousResearch/kenxcode-agent`. Users install them into `~/.kenxcode/plugins/` or via a pip entry point; everything in this guide works the same way from a standalone repo. This is a coupling-and-maintenance decision (the core moves fast and we don't own your backend), not a quality bar — a plugin can be excellent and still belong in its own repo. Promote it in the Nous Research Discord `#plugins-skills-and-skins` channel. See [CONTRIBUTING.md](https://github.com/NousResearch/kenxcode-agent/blob/main/CONTRIBUTING.md) for the policy.
:::
...
```bash
mkdir -p ~/.kenxcode/plugins/calculator
cd ~/.kenxcode/plugins/calculator
```
...
This tells KenXCode: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
4. **Accept `**kwargs`:** KenXCode may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `kenxcode my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but KenXCode continues fine
...
If your plugin doesn't show up — or shows up but isn't loading — set `KENXCODE_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:

```bash
KENXCODE_PLUGINS_DEBUG=1 kenxcode plugins list
```
...
The same logs are always written to `~/.kenxcode/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
kenxcode logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `kenxcode plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout** — must be `~/.kenxcode/plugins/<plugin-name>/plugin.yaml` (flat) or `~/.kenxcode/plugins/<category>/<plugin-name>/plugin.yaml` (one level of category nesting, max). Anything deeper is ignored.
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.kenxcode/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.kenxcode/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `kenxcode plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing KenXCode into installing arbitrary packages — only specs KenXCode itself ships are eligible |
...
auditable in `~/.kenxcode/logs/agent.log`. Plugins load after built-in
...
Per-hook context is capped at `10,000` characters by default. Anything above the cap is written to `$KENXCODE_HOME/hook_outputs/<session_id>/<uuid>.txt` and replaced with a head/tail preview plus the saved path. The model can read the full content via `read_file` or `terminal` if it genuinely needs it. This keeps a runaway plugin from inflating every subsequent turn's prompt and blowing out the prompt cache prefix. Tune in `config.yaml`:
...
hooks:
  output_spill:
    ...
    # directory: null      # default: $KENXCODE_HOME/hook_outputs
```
...
- **The system prompt is KenXCode's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
Plugins can add their own `kenxcode <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for kenxcode my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: kenxcode my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for kenxcode my-plugin."""
...
After registration, users can run `kenxcode my-plugin status`, `kenxcode my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `kenxcode name` in a terminal |
...
`ctx._cli_ref` is only populated in an **interactive CLI** session. It is `None` in the gateway, in non-interactive `kenxcode chat -q` runs, and in **kanban-spawned worker sessions** — so any plugin logic that reaches through `_cli_ref` silently no-ops in exactly those contexts. Two stable, session-agnostic APIs cover what hooks actually need:

- **`ctx.profile_name`** — the active profile name (e.g. `"default"`, or the assignee profile in a kanban worker). Derived from `KENXCODE_HOME`, so it works everywhere with no `_cli_ref` dependency.
...
For running a full `kenxcode <subcommand>` (e.g. `kenxcode kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "kenxcode kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive KenXCode from a hook.
...
KenXCode has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.kenxcode/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `kenxcode gateway status` without SDK instantiation.
...
KenXCode also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into KenXCode without any Python plugin. Declare them in `~/.kenxcode/config.yaml`:
...
KenXCode connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.kenxcode/hooks/<name>/`:
...
kenxcode skills search my-workflow --source myorg/skills-repo
kenxcode skills install myorg/skills-repo/my-workflow
```
...
```toml
# pyproject.toml
[project.entry-points."kenxcode_agent.plugins"]
my-plugin = "my_plugin_package"
```

```bash
pip install kenxcode-plugin-calculator
# Plugin auto-discovered on next kenxcode startup
```
...
```python
# Wrong — will break if KenXCode passes extra context
def handler(args):
    ...
