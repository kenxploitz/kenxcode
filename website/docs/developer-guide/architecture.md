...
sidebar_position: 1
title: "Architecture"
description: "KenXCode Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of KenXCode Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ kenxcode_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # KenXCodeCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── kenxcode_state.py           # SQLite session/state database with FTS5
├── kenxcode_constants.py       # KENXCODE_HOME, profile-aware paths
...
│
├── kenxcode_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `kenxcode` subcommands (large file)
...
│   ├── skills_config.py      # kenxcode skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # kenxcode tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # kenxcode gateway start/stop
│
...
```text
User input → KenXCodeCLI.process_input()
...
Three discovery sources: `~/.kenxcode/plugins/` (user), `.kenxcode/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `kenxcode plugins` or `config.yaml`.
...
Exposes KenXCode as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`kenxcode -p <name>`) gets its own KENXCODE_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
