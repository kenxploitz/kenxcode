...
KenXCode Agent uses a SQLite database (`~/.kenxcode/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `kenxcode_state.py`


## Architecture Overview

```
~/.kenxcode/state.db (SQLite, WAL mode)
...
Abridged — see `SCHEMA_SQL` in `kenxcode_state.py` for the full current column list
...
indexed columns — see `SCHEMA_SQL` in `kenxcode_state.py` for the exact SQL.
...
Multiple kenxcode processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.kenxcode/state.db
...
Default path: `~/.kenxcode/state.db`

This is derived from `kenxcode_constants.get_kenxcode_home()` which resolves to
`~/.kenxcode/` by default, or the value of `KENXCODE_HOME` environment variable.
