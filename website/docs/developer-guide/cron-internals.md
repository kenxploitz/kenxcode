...
description: "How KenXCode stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `kenxcode_cli/cron.py` | CLI `kenxcode cron` subcommands |
...
Jobs are stored in `~/.kenxcode/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `kenxcode cron` commands are run or during active CLI sessions.
...
```python
# ~/.kenxcode/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.kenxcode/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `kenxcode cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `kenxcode cron` CLI provides direct job management:

```bash
kenxcode cron list                    # Show all jobs
kenxcode cron create                  # Interactive job creation (alias: add)
kenxcode cron edit <job_id>           # Edit job configuration
kenxcode cron pause <job_id>          # Pause a running job
kenxcode cron resume <job_id>         # Resume a paused job
kenxcode cron run <job_id>            # Trigger immediate execution
kenxcode cron remove <job_id>         # Delete a job
```
