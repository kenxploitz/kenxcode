...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to KenXCode Agent — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to KenXCode Agent! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying KenXCode core? Start with [Build a KenXCode Plugin](../developer-guide/plugins/index.md)
- Building a new built-in core tool for KenXCode itself? Start with [Adding Tools](./adding-tools.md)
...
You can also run a fully isolated KenXCode instance (throwaway KENXCODE_HOME, separate Electron
userData, distinct Electron app name to avoid the single-instance lock):

```bash
scripts/dev-sandbox.sh python -m kenxcode_cli.main
scripts/dev-sandbox.sh --persistent python -m kenxcode_cli.main desktop  # state survives restarts, but lives in the worktree :)
```
...
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.kenxcode/.env
```
...
- **Profile-safe paths**: Never hardcode `~/.kenxcode` — use `get_kenxcode_home()` from `kenxcode_constants` for code paths and `display_kenxcode_home()` for user-facing messages. See [AGENTS.md](https://github.com/NousResearch/kenxcode-agent/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
...
By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/NousResearch/kenxcode-agent/blob/main/LICENSE).
