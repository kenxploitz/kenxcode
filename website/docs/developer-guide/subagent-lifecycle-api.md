...
Plugins can launch and supervise fresh KenXCode child sessions without importing
...
active agent turn fails closed with `No active KenXCode parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until KenXCode can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; KenXCode's existing unsafe-tool block remains enforced.
