...
| Variable | Meaning |
|----------|---------|
| `KENXCODE_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `KENXCODE_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `KENXCODE_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by KenXCode itself — they're private to these helpers. The variables KenXCode *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `kenxcode --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_kenxcode_root)" || { echo "htui: not in a KenXCode checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$KENXCODE_MAIN_CHECKOUT/.venv/bin/python" -m kenxcode_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `KENXCODE_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_kenxcode_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `KENXCODE_TUI_DIR` are mutually exclusive
`KENXCODE_TUI_DIR` points KenXCode at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `kenxcode --tui --dev` exits with an error. Run `unset KENXCODE_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_kenxcode_root)" || { echo "hgui: not in a KenXCode checkout" >&2; return 1; }
  deps="${KENXCODE_GUI_DEPS_CHECKOUT:-$KENXCODE_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _kenxcode_link_deps "$desktop" "$deps/apps/desktop"
    _kenxcode_link_deps "$root" "$deps"
  else
  ...
  trap '_kenxcode_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    KENXCODE_DESKTOP_KENXCODE_ROOT="$root" \
    KENXCODE_DESKTOP_PYTHON="$KENXCODE_MAIN_CHECKOUT/.venv/bin/python" \
    KENXCODE_DESKTOP_IGNORE_EXISTING=1 \
    KENXCODE_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `KENXCODE_DESKTOP_KENXCODE_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `kenxcode`. |
| `KENXCODE_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `KENXCODE_DESKTOP_IGNORE_EXISTING` | Ignores any `kenxcode` on `PATH` so it can't shadow the worktree. |
| `KENXCODE_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real KenXCode checkout.
_kenxcode_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/kenxcode_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_kenxcode_gui_cleanup() {
  local root="$1"
  [[ -n "$root" ]] && pkill -TERM -f "${root}/apps/desktop/node_modules/electron" 2>/dev/null
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'kenxcode_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `kenxcode --tui --dev` and the `KENXCODE_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/NousResearch/kenxcode-agent/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `KENXCODE_*` variable KenXCode reads
