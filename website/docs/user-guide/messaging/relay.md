---
sidebar_position: 30
title: "KenXCode Relay"
description: "Connect KenXCode to messaging platforms through a relay connector that owns the platform credentials — enrollment, capabilities, config, and troubleshooting"
---

# KenXCode Relay (Connector)
...
KenXCode Relay is not a chat platform itself — it is a **connector system** that
...
1. Resolves a fresh Nous Portal access token from your existing login
   (`~/.kenxcode/auth.json`) — this proves which Nous org (tenant) you own. If
...
3. The connector verifies the token (signature, single-use, tenant match),
   ...
   once.
4. Persists the credentials into `~/.kenxcode/.env`:
...
`kenxcode gateway enroll` refuses to run in managed/hosted installs — there the
...
|---------|-------|---------|
| `GATEWAY_RELAY_URL` | env (`~/.kenxcode/.env`) | Connector relay WebSocket URL. Presence enables the relay platform. |
...
token. Re-login with `kenxcode auth add nous` (or `kenxcode setup`) and retry.
...
startup. Restart it (`kenxcode gateway restart`).
