...
Connect KenXCode to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
KenXCode supports two WeCom integration modes:
...
> Run `kenxcode gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. KenXCode decrypts the message, queues it for the agent
...
(Use `kenxcode gateway start` only after `kenxcode gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in KenXCode and the token the
...
in `~/.kenxcode/.env` values around `=` will also break signature checks. After
fixing, restart `kenxcode gateway run`.
...
Check `kenxcode gateway run` logs for the bound host/port. If the adapter bound to
