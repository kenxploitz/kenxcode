...
A **profile distribution** packages a complete KenXCode agent — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## What this means

Before distributions, sharing a KenXCode agent meant sending someone:
...
```bash
kenxcode profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `kenxcode profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running KenXCode in 2026, that's already true.
...
- **You just want to back up a profile on your own machine.** Use [`kenxcode profile export` / `import`](../reference/profile-commands.md#kenxcode-profile-export) — that's what those are for.
...
:::caution
**KenXCode does not control git.** The file exclusions described on this page are applied by the **installer** when someone runs `kenxcode profile install` or `kenxcode profile update`. They are **not** applied when you run `git add` or `git commit`.
:::
...
```bash
kenxcode profile create research-bot
research-bot setup                    # configure model, API keys
# Edit ~/.kenxcode/profiles/research-bot/SOUL.md
...
Create `~/.kenxcode/profiles/research-bot/distribution.yaml`:

```yaml
name: research-bot
version: 1.0.0
description: "Autonomous research assistant with arXiv and web tools"
kenxcode_requires: ">=0.12.0"
author: "Your Name"
license: "MIT"
...
Create `~/.kenxcode/profiles/research-bot/.gitignore` with at minimum:
...
```bash
cd ~/.kenxcode/profiles/research-bot
git init
git add .
git commit -m "v1.0.0"
...
Recipients who run `kenxcode profile update research-bot` will pull the latest.
...
```bash
kenxcode profile install github.com/you/research-bot --alias
```
...
5. Copies distribution-owned files into `~/.kenxcode/profiles/research-bot/` (or wherever the manifest's `name` resolves). The [hard-excluded paths](#whats-not-in-a-distribution-ever) are stripped during this copy, even if the author accidentally left them in the repo.
...
```bash
# GitHub shorthand
kenxcode profile install github.com/you/research-bot

# Full HTTPS
kenxcode profile install https://github.com/you/research-bot.git

# SSH
kenxcode profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
kenxcode profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
kenxcode profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
kenxcode profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
kenxcode profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
kenxcode profile install github.com/acme/support-bot --name support-eu --alias
```
...
```
# Environment variables required by this KenXCode distribution.
...
```bash
cp ~/.kenxcode/profiles/research-bot/.env.EXAMPLE ~/.kenxcode/profiles/research-bot/.env
...
```bash
kenxcode profile info research-bot
```
...
`kenxcode profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
kenxcode profile update research-bot
```
...
```bash
kenxcode profile delete research-bot
```
...
Path:    ~/.kenxcode/profiles/research-bot
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `kenxcode profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
cd ~/.kenxcode/profiles/pr-reviewer
# ... build and tune ...
git init && git add . && git status   # confirm no secrets staged
...
# Each engineer
kenxcode profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `kenxcode profile update pr-reviewer` and everyone's on the new version within minutes.
...
cd ~/.kenxcode/profiles/polymarket-trader
# Write a solid README.md at the repo root — GitHub shows it on the repo page
git init && git add . && git status   # confirm no secrets staged
git commit -m "v1.0"
git tag v1.0.0
# Publish to a public GitHub repo
git remote add origin https://github.com/you/kenxcode-polymarket-trader.git
git push -u origin main --tags

# Anyone
kenxcode profile install github.com/you/kenxcode-polymarket-trader --alias
```
...
You built KenXCode-on-top — maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
# Each on-call
kenxcode profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
kenxcode profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `kenxcode profile info <name>` and hold off on updates until you're ready.
:::
...
kenxcode profile info research-bot | grep Version
...
```yaml
# ~/.kenxcode/profiles/research-bot/local/my-overrides.yaml
...
kenxcode profile delete research-bot --yes
kenxcode profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
kenxcode profile update research-bot --force-config --yes
```
...
kenxcode profile install github.com/yourname/forked-research-bot --alias

# Iterate locally in ~/.kenxcode/profiles/forked-research-bot/
...
kenxcode profile install ~/.kenxcode/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
kenxcode profile delete research-bot-test --yes
kenxcode profile install ~/.kenxcode/profiles/research-bot --name research-bot-test
```
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `kenxcode -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `kenxcode profile` — not a parallel command tree.
...
- Reserved profile names (`kenxcode`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`kenxcode profile export` / `import`](../reference/profile-commands.md#kenxcode-profile-export) — local backup / restore (not distribution)
- [Using SOUL with KenXCode](../guides/use-soul-with-kenxcode.md) — authoring personalities
