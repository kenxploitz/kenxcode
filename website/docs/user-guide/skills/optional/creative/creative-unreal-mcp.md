...
| | |
|---|---|
| Source | Optional — install with `kenxcode skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the KenXCode MCP catalog. The
...
Two halves, in this order: the editor side must be up before KenXCode connects.
...
### One-time, KenXCode side

    kenxcode mcp install unreal-engine
...
`~/.kenxcode/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for KenXCode — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. KenXCode connects
...
2. Start the KenXCode session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through KenXCode
they appear as:

| KenXCode tool | Purpose |
|---|---|
...
then happens at `kenxcode mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — KenXCode runs batched calls
...
- **Start order matters.** Editor + server up first, then the KenXCode
...
- **Timeouts on long operations.** KenXCode' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.kenxcode/config.yaml` for
