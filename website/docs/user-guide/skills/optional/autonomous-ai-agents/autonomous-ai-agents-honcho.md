---
title: "Honcho — Configure and troubleshoot Honcho memory for KenXCode"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for KenXCode"
---
...
Configure and troubleshoot Honcho memory for KenXCode.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `kenxcode skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for KenXCode

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every KenXCode profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/kenxcode#running-honcho-locally-with-kenxcode

### Verify

```bash
kenxcode honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this KenXCode profile's AI peer
...
Honcho models conversations as interactions between **peers**. KenXCode creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this KenXCode instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each KenXCode run |
| `global` | Single session across all directories |

Manual override: `kenxcode honcho map my-project-name`
...
Each KenXCode profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block kenxcode.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `kenxcode.coder` host block in `honcho.json`
...
```bash
kenxcode honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for KenXCode when Honcho memory is active.
...
Config file: `$KENXCODE_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `kenxcode honcho setup`. Ensure `memory.provider: honcho` is in `~/.kenxcode/config.yaml`.

### Memory not persisting across sessions
Check `kenxcode honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `kenxcode profile create <name> --clone`. For existing profiles: `kenxcode honcho sync`.
...
| Command | Description |
|---------|-------------|
| `kenxcode honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `kenxcode honcho status` | Show resolved config, connection test, peer info for active profile |
| `kenxcode honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `kenxcode honcho disable` | Disable Honcho for the active profile |
| `kenxcode honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `kenxcode honcho peers` | Show peer identities across all profiles |
| `kenxcode honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `kenxcode honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `kenxcode honcho sessions` | List known directory-to-session-name mappings |
| `kenxcode honcho map <name>` | Map current working directory to a Honcho session name |
| `kenxcode honcho identity` | Seed AI peer identity or show both peer representations |
| `kenxcode honcho sync` | Create host blocks for all KenXCode profiles that don't have one yet |
| `kenxcode honcho migrate` | Step-by-step migration guide from OpenClaw native memory to KenXCode + Honcho |
| `kenxcode memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `kenxcode memory status` | Show active memory provider and config |
| `kenxcode memory off` | Disable external memory provider |
