...
| | |
|---|---|
| Source | Optional — install with `kenxcode skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${KENXCODE_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${KENXCODE_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${KENXCODE_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client kenxcode
```
...
```bash
PUBLISH="${KENXCODE_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client kenxcode
```
...
```bash
DRIVE="${KENXCODE_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `kenxcode`)    |
