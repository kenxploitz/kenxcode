...
| | |
|---|---|
| Source | Optional — install with `kenxcode skills install official/productivity/canvas` |
...
4. Name the token (e.g., "KenXCode Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${KENXCODE_HOME:-~/.kenxcode}/.env`:
...
```bash
CANVAS="python $KENXCODE_HOME/skills/productivity/canvas/scripts/canvas_api.py"
