...
| | |
|---|---|
| Source | Optional — install with `kenxcode skills install official/productivity/shopify` |
...
4. Save to `${KENXCODE_HOME:-~/.kenxcode}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"KenXCode","productType":"Apparel","tags":["test"]}}'
```
