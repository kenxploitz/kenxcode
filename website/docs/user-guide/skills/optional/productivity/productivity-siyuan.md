...
| | |
|---|---|
| Source | Optional — install with `kenxcode skills install official/productivity/siyuan` |
...
3. Store it in `${KENXCODE_HOME:-~/.kenxcode}/.env`:
   ```
...
```yaml
# In ~/.kenxcode/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
