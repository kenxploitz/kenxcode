...
| | |
|---|---|
| Source | Optional — install with `kenxcode skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by KenXCode Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during KenXCode terminal calls (desktop app flow only)
...
### Service Account (recommended for KenXCode)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${KENXCODE_HOME:-~/.kenxcode}/.env` (the skill will prompt for this on first load).
...
## KenXCode Execution Pattern (desktop app flow)

KenXCode terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/kenxcode-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/kenxcode-op.sock"
