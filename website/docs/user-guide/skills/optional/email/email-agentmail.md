...
| | |
|---|---|
| Source | Optional — install with `kenxcode skills install official/email/agentmail` |
...
Add to `~/.kenxcode/config.yaml` (paste your actual key — MCP env vars are not expanded from .env):
```yaml
mcp_servers:
  agentmail:
    command: "npx"
...
1. Create a dedicated inbox:
   - Use `create_inbox` with a username (e.g. `kenxcode-agent`)
   - The agent gets address: `kenxcode-agent@agentmail.to`
2. Send an email:
...
```
1. create_inbox (username: "kenxcode-outreach")
...
```
kenxcode --toolsets mcp -q "Create an AgentMail inbox called test-agent and tell me its email address"
```
