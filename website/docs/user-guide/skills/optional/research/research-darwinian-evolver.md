...
| | |
|---|---|
| Source | Optional — install with `kenxcode skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), KenXCode Agent |
| License | MIT |
...
import upstream classes into KenXCode itself.
...
```bash
mkdir -p ~/.kenxcode/cache/darwinian-evolver && cd ~/.kenxcode/cache/darwinian-evolver
...
```bash
cd ~/.kenxcode/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.kenxcode/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.kenxcode/skills/research/darwinian-evolver
DE_DIR=~/.kenxcode/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside KenXCode core.
   Custom driver scripts under `~/.kenxcode/skills/...` are user-side and fine.
