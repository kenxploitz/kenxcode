...
| | |
|---|---|
| Source | Optional — install with `kenxcode skills install official/mcp/fastmcp` |
...
| Related skills | [`kenxcode-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-kenxcode-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into KenXCode or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to KenXCode. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.kenxcode/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.kenxcode/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is KenXCode integration, either:

- configure the server in `~/.kenxcode/config.yaml` using the `native-mcp` skill, or
...
### KenXCode cannot see the deployed server

The server-building part may be correct while the KenXCode config is not. Load the `native-mcp` skill and configure the server in `~/.kenxcode/config.yaml`, then restart KenXCode.
