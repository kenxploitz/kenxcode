...
Everything operator-facing is a `kenxcode teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `${KENXCODE_HOME:-~/.kenxcode}/.env`:
...
```bash
kenxcode teams-pipeline validate              # config snapshot — run first after any change
kenxcode teams-pipeline token-health          # Graph token status
kenxcode teams-pipeline token-health --force-refresh   # force a fresh token acquisition
kenxcode teams-pipeline list                  # recent meeting jobs
kenxcode teams-pipeline list --status failed  # only failed jobs
kenxcode teams-pipeline show <job-id>         # full detail of one job
kenxcode teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
kenxcode teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
kenxcode teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
kenxcode teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```

### Subscription management

```bash
kenxcode teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

kenxcode teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
kenxcode teams-pipeline delete-subscription <sub-id>
kenxcode teams-pipeline maintain-subscriptions            # renew near-expiry ones
kenxcode teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `kenxcode teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `kenxcode cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
