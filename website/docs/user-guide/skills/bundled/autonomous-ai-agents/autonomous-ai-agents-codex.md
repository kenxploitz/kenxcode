...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`kenxcode-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-kenxcode-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the KenXCode terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For KenXCode itself, `model.provider: openai-codex` uses KenXCode-managed Codex
OAuth from `~/.kenxcode/auth.json` after `kenxcode auth add openai-codex`. For the
...
## KenXCode Gateway Caveat

When invoking the Codex CLI from a KenXCode gateway/service context (for example,
