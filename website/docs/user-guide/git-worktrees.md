...
title: "Git Worktrees"
description: "Run multiple KenXCode agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

KenXCode Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with KenXCode so each session has a clean, isolated working directory.

## Why Use Worktrees with KenXCode?

KenXCode treats the **current working directory** as the project root:

- CLI: the directory where you run `kenxcode` or `kenxcode chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.kenxcode/config.yaml`
...
git worktree add ../repo-feature feature/kenxcode-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/kenxcode-experiment` checked out in that directory

Now you can `cd` into the new worktree and run KenXCode there:

```bash
cd ../repo-feature

# Start KenXCode in the worktree
kenxcode
```
...
git worktree add ../repo-experiment-a feature/kenxcode-a
git worktree add ../repo-experiment-b feature/kenxcode-b
```
...
Each KenXCode process:

- Works on its own branch (`feature/kenxcode-a` vs `feature/kenxcode-b`).
...
- KenXCode checkpoint data under `~/.kenxcode/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per KenXCode experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/kenxcode-checkpoints-docs`, `feature/kenxcode-refactor-tests`.
- **Commit frequently**
...
- **Avoid running KenXCode from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `kenxcode -w` (Automatic Worktree Mode)

KenXCode has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `kenxcode/kenxcode-<hash>`).
...
```bash
kenxcode -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `kenxcode -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each KenXCode session its own clean checkout.
