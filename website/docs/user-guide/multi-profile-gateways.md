...
If you only run one KenXCode agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more KenXCode agents that should all
...
(`ai.kenxcode.gateway-<name>.plist`) or systemd user service
(`kenxcode-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
kenxcode profile create coder
kenxcode profile create personal-bot
kenxcode profile create research
...
```bash
kenxcode config set gateway.multiplex_profiles true
kenxcode gateway restart
```

Equivalently, in the default profile's `~/.kenxcode/config.yaml`:
...
You do **not** run `kenxcode gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `kenxcode gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.kenxcode/profiles/coder/.env`; the default listener key is rejected.
...
home). `kenxcode status` reports the multiplexer and the profiles it serves;
`kenxcode status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/kenxcode-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: kenxcode-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    kenxcode gateway "$action"
  else
    kenxcode -p "$profile" gateway "$action"
  fi
}
...
```bash
kenxcode-gateways start      # start every configured profile
kenxcode-gateways stop       # stop every configured profile
kenxcode-gateways restart    # restart all
kenxcode-gateways status     # status across all
kenxcode-gateways list       # delegates to `kenxcode gateway list`
```

:::tip
The `default` profile is targeted with `kenxcode gateway <action>` (no `-p`),
not `kenxcode -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `kenxcode -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.kenxcode.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/kenxcode-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.kenxcode.gateway.plist` /
`kenxcode-gateway.service`.
...
```bash
# Default profile
tail -f ~/.kenxcode/logs/gateway.log
tail -f ~/.kenxcode/logs/gateway.error.log

# Named profile
tail -f ~/.kenxcode/profiles/<name>/logs/gateway.log
tail -f ~/.kenxcode/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.kenxcode/logs/gateway.log ~/.kenxcode/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
kenxcode logs -f                  # follow default profile
kenxcode -p coder logs -f         # follow one profile
kenxcode logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
kenxcode profile list             # profiles + model + gateway state
kenxcode-gateways status          # full status across every profile
launchctl list | grep kenxcode    # macOS — PIDs and labels
systemctl --user list-units 'kenxcode-gateway-*'   # Linux — units
```
...
```
~/.kenxcode/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.kenxcode/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
kenxcode config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.kenxcode/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=kenxcode --why="gateways running" \
  sleep infinity &
...
`kenxcode-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.kenxcode/.env ~/.kenxcode/profiles/*/.env
```

## Updating the code

`kenxcode update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `kenxcode gateway start` after a previous `kenxcode gateway stop`. The
...
```bash
ps -ef | grep "kenxcode_cli.*-p <profile>"
cat ~/.kenxcode/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.kenxcode.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.kenxcode.gateway-<profile>.plist

# Linux
systemctl --user restart kenxcode-gateway-<profile>.service
```

### Health check

```bash
kenxcode doctor                  # default profile
kenxcode -p <profile> doctor     # one profile
```
