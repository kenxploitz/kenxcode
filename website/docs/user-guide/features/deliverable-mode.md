...
When KenXCode Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.kenxcode/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.kenxcode/config.yaml` (switchable per session
via `/personality`).
...
If you use KenXCode' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.kenxcode/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. KenXCode Agent's deliverable
