...
chat completions — use your KenXCode-managed provider subscription as their
...
| Use case | "Use KenXCode as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. KenXCode stores
the refresh token in `~/.kenxcode/auth.json` — the same place all KenXCode
...
Model:      KenXCode-4-70B    # or KenXCode-4.3-36B, KenXCode-4-405B
```
...
interface in `kenxcode_cli/proxy/adapters/`.
...
If you see `not logged in`, run `kenxcode portal`. If you see
...
`kenxcode_cli/proxy/adapters/<provider>.py` and registering it in
