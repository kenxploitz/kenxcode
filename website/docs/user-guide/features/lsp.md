...
KenXCode runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. KenXCode
...
1. KenXCode captures a baseline of current diagnostics for the file.
...
…). KenXCode auto-detects the binary on PATH or in
`<KENXCODE_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point KenXCode at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<KENXCODE_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`kenxcode lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — KenXCode installs both packages together when you
run `kenxcode lsp install typescript` or auto-install fires on first
use.

## CLI

```
kenxcode lsp status          # service state + per-server install status
kenxcode lsp list            # registry, optionally --installed-only
kenxcode lsp install <id>    # eagerly install one server
kenxcode lsp install-all     # try every server with a known recipe
kenxcode lsp restart         # tear down running clients
kenxcode lsp which <id>      # print resolved binary path
```

`kenxcode lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <KENXCODE_HOME>/lsp/bin
...
When `install_strategy: auto`, KenXCode installs binaries into
`<KENXCODE_HOME>/lsp/bin/`. NPM packages land in
`<KENXCODE_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully KenXCode-owned and is
...
**`kenxcode lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<KENXCODE_HOME>/lsp/bin/`. Run
`kenxcode lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `kenxcode lsp status`**
...
When `kenxcode lsp status` shows a `Backend warnings` section, install
...
`~/.kenxcode/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.kenxcode/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `kenxcode lsp restart` to clear the set;
