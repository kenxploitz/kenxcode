...
KenXCode can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<KENXCODE_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), KenXCode renders the sprite at full fidelity when your
...
kenxcode pets list
kenxcode pets list cat

# Install a pet and make it active in one step
kenxcode pets install boba --select

# Preview / animate it in your terminal (Ctrl+C to stop)
kenxcode pets show

# Check your setup
kenxcode pets doctor
```

## `kenxcode pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `kenxcode pets list [query] [--limit N]` |
| List installed pets | `kenxcode pets list --installed` |
| Install a pet | `kenxcode pets install <slug> [--select] [--force]` |
| Set the active pet | `kenxcode pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `kenxcode pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `kenxcode pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `kenxcode pets off` |
| Remove an installed pet | `kenxcode pets remove <slug>` |
| Diagnose setup | `kenxcode pets doctor` |

`kenxcode pets show` flags:
...
Beyond installing pre-made pets from the gallery, KenXCode can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `kenxcode pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `kenxcode tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `KENXCODE_PET_IMAGE_PROVIDER` env var (e.g. `KENXCODE_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while KenXCode is
...
Run `kenxcode pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; KenXCode uses its own
  profile-scoped `<KENXCODE_HOME>/pets/` instead — install through `kenxcode pets`.

## See also

- The [`kenxcode-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-kenxcode-agent.md)
