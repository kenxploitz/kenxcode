...
All skills live in **`~/.kenxcode/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point KenXCode at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
By default every profile is seeded with the bundled skill catalog, and each `kenxcode update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:

**At install time** (applies to the default `~/.kenxcode` profile):

```bash
curl -fsSL https://kenxcode-agent.nousresearch.com/install.sh | bash -s -- --no-skills
```

**At profile-create time** (named profiles):

```bash
kenxcode profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
kenxcode skills opt-out            # stop future seeding — nothing on disk is touched
kenxcode skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
kenxcode skills opt-in --sync      # undo: remove the marker and re-seed now
```

All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `kenxcode update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `kenxcode skills opt-in`) to re-enable.

:::note Safe by default
`kenxcode skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version KenXCode installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
:::
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling KenXCode to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.kenxcode/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
kenxcode chat --toolsets skills -q "What skills do you have?"
kenxcode chat --toolsets skills -q "Show me the axolotl skill"
```
...
description, the standard section order, KenXCode-tool framing, no invented
commands).
...
/home/user/.kenxcode/cache/chart-q4-2025.png
...
When a missing value is encountered, KenXCode asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `kenxcode setup` or `~/.kenxcode/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `kenxcode config migrate` prompts for unconfigured settings, and `kenxcode config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.kenxcode/skills/                  # Single source of truth
...
and `examples/`. Unreferenced repository files are not copied. KenXCode scans the
...
If you maintain skills outside of KenXCode — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell KenXCode to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.kenxcode/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.kenxcode/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the KenXCode process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, KenXCode ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.kenxcode/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.kenxcode/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `kenxcode bundles list`.
...
kenxcode bundles create research

# Overwrite an existing bundle
kenxcode bundles create backend-dev --skill ... --force

# Delete a bundle
kenxcode bundles delete backend-dev

# Re-scan ~/.kenxcode/skill-bundles/ and report changes
kenxcode bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.kenxcode/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.kenxcode/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
Staged writes survive restarts under `~/.kenxcode/pending/skills/` and are
...
```bash
kenxcode skills browse                              # Browse all hub skills (official first)
kenxcode skills browse --source official            # Browse only official optional skills
kenxcode skills search kubernetes                   # Search all sources
kenxcode skills search react --source skills-sh     # Search the skills.sh directory
kenxcode skills search https://mintlify.com/docs --source well-known
kenxcode skills inspect openai/skills/k8s           # Preview before installing
kenxcode skills install openai/skills/k8s           # Install with security scan
kenxcode skills install official/security/1password
kenxcode skills install skills-sh/vercel-labs/json-render/json-render-react --force
kenxcode skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
kenxcode skills list --source hub                   # List hub-installed skills
kenxcode skills check                               # Check installed hub skills for upstream updates
kenxcode skills update                              # Reinstall hub skills with upstream changes when needed
kenxcode skills audit                               # Re-scan all hub skills for security
kenxcode skills uninstall k8s                       # Remove a hub skill
kenxcode skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
kenxcode skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
kenxcode skills publish skills/my-skill --to github --repo owner/repo
kenxcode skills snapshot export setup.json          # Export skill config
kenxcode skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with KenXCode. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `kenxcode skills search <query> --source skills-sh`. KenXCode resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
KenXCode currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the KenXCode repository itself and install with built-in trust.
...
This is Vercel's public skills directory. KenXCode can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
kenxcode skills search https://mintlify.com/docs --source well-known
kenxcode skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
kenxcode skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

KenXCode can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
kenxcode skills install openai/skills/k8s
kenxcode skills tap add myorg/skills-repo
```
...
[Skills Hub](https://kenxcode-agent.nousresearch.com/docs) page — instead of a
tag-derived guess. This is generic: any tap that ships the file gets real
categorization, no KenXCode-side changes required.
...
- KenXCode source id: `clawhub`

#### 6. LobeHub (`lobehub`)

KenXCode can search and convert agent entries from LobeHub's public catalog into installable KenXCode skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- KenXCode source id: `lobehub`

#### 7. browse.sh (`browse-sh`)

KenXCode integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with KenXCode' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- KenXCode source id: `browse-sh`
- Trust level: `community`

```bash
kenxcode skills search airbnb --source browse-sh
kenxcode skills inspect browse-sh/airbnb.com/search-listings-ddgioa
kenxcode skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). KenXCode also fetches explicitly referenced files under `references/`, `templates/`, `scripts/`, `assets/`, and `examples/`, then scans and installs the complete bundle.

- KenXCode source id: `url`
- Identifier: the URL itself (no prefix needed)
- Scope: `SKILL.md` plus exact referenced support files in the allowlisted directories. KenXCode does not enumerate or copy unrelated files from the host.

```bash
kenxcode skills install https://sharethis.chat/SKILL.md
kenxcode skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
kenxcode skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `kenxcode skills update` re-fetches from the same URL automatically when you want to refresh.
...
`kenxcode skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with KenXCode | Always trusted |
...
```bash
kenxcode skills check          # Report which installed hub skills changed upstream
kenxcode skills update         # Reinstall only the skills with updates available
kenxcode skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other KenXCode users add with `kenxcode skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.kenxcode.tags`, `version`, `author`, `platforms`, `metadata.kenxcode.config`).
...
KenXCode discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.

#### Minimal tap example

```
my-org/kenxcode-skills
└── skills/
    └── deploy-runbook/
        └── SKILL.md
```
...
After pushing that to GitHub, any KenXCode user can subscribe and install:

```bash
kenxcode skills tap add my-org/kenxcode-skills
kenxcode skills search deploy
kenxcode skills install my-org/kenxcode-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.kenxcode/skills/.hub/taps.json`:
...
The `kenxcode skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `kenxcode skills tap list` shows the effective path per tap.
...
```bash
kenxcode skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.kenxcode/skills/.hub/taps.json` (created on demand).

## Bundled skill updates (`kenxcode skills reset`)

KenXCode ships with a set of bundled skills in `skills/` inside the repo. On install and on every `kenxcode update`, a sync pass copies those into `~/.kenxcode/skills/` and records a manifest at `~/.kenxcode/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, KenXCode recomputes the hash of your local copy and compares it to the origin hash:
...
kenxcode skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `KENXCODE_HOME`, so `kenxcode -p coder skills reset <name>` only affects that profile.
:::
