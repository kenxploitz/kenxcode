...
KenXCode Agent can drive your desktop — clicking, typing, scrolling,
...
```
kenxcode computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `kenxcode computer-use
status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `kenxcode tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or KenXCode app). `kenxcode computer-use doctor` will tell you which permission is missing. |
...
```
kenxcode -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.kenxcode/config.yaml`.

## Permission modes and logged-in browser profiles

KenXCode maps its existing approval UX onto cua-driver 0.10's immutable daemon
modes. There is no second permission toggle to keep in sync:

| KenXCode session | cua-driver mode | Human intervention | `existing_profile` |
|---|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal KenXCode approvals; Cua stops at its protected boundary | Refuses unless a certified protected host is available; KenXCode does not claim one today |
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit KenXCode risk acceptance; no runtime Cua prompts | Allowed within Cua's built-in, managed, and user policy ceilings |

The unrestricted daemon is private to that KenXCode session. Turning `/yolo`
...
daemon's mode or grants another KenXCode conversation the same authority.
...
## `kenxcode computer-use doctor` — your first triage stop

`kenxcode computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ kenxcode computer-use doctor
...
agent is acting here." Each KenXCode run declares its own cua-driver
**session id** (something like `kenxcode-3a7b9c14d2e8`); the cursor's
...
KenXCode intentionally keeps its skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the KenXCode-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the KenXCode skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **KenXCode
...
KenXCode applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.kenxcode/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. KenXCode applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  KenXCode agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the KenXCode agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. KenXCode warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive KenXCode from inside the RDP / console
...
```
KENXCODE_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
KENXCODE_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **KenXCode disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) KenXCode sets
...
When it's on, `kenxcode computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point KenXCode at a binary you built from source instead
of the published release. KenXCode resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`KENXCODE_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for KenXCode testing (see notes).
...
### Option B — point KenXCode straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`KENXCODE_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
KENXCODE_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
KENXCODE_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm KenXCode is using your build

- `kenxcode computer-use status` prints the resolved binary path and
  version.
- `kenxcode computer-use doctor` confirms the binary is reachable and
...
- **KenXCode spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  KenXCode YOLO, KenXCode instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no KenXCode
  change needed either way.
- **Local builds skip the version check.** KenXCode warns when the
...
**First action when anything's off: run `kenxcode computer-use doctor`.**
...
Run `kenxcode computer-use install` to fetch the cua-driver binary, or
run `kenxcode tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `kenxcode computer-use doctor` will
...
- **KenXCode-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  KenXCode `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects KenXCode (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (KenXCode among them)
