...
description: "Connect KenXCode Agent to external tool servers via MCP — and control exactly which MCP tools KenXCode loads"
---

# MCP (Model Context Protocol)

MCP lets KenXCode Agent connect to external tool servers so the agent can use tools that live outside KenXCode itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted KenXCode to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native KenXCode tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want KenXCode to see

## Quick start

1. MCP support ships with the standard install — no extra step needed.

2. Add an MCP server to `~/.kenxcode/config.yaml`:
...
4. Ask KenXCode to use the MCP-backed capability.
...
KenXCode will discover the MCP server's tools and use them like any other tool.

## Catalog: one-click install for Nous-approved MCPs

KenXCode ships a curated catalog of MCP servers that Nous staff has reviewed
and merged. They're disabled by default — install only what you actually
want.

```bash
kenxcode mcp                # interactive picker (default)
kenxcode mcp catalog        # plain-text list, scriptable
kenxcode mcp install n8n    # install a catalog entry by name
```

The picker shows each entry with its current status:

```
n8n          available              Manage and inspect n8n workflows from KenXCode
...
`optional-mcps/` in the kenxcode-agent repo — presence in that directory means
Nous approval. There is no community submission tier; entries are added by
merging a PR.

Catalog entries can require:

- **API key** — KenXCode prompts at install time and writes the value to
  `~/.kenxcode/.env`. Non-secret values (base URLs) go to the same file.
- **OAuth** (remote MCP) — written as `auth: oauth` in your config; the MCP
  client opens a browser on first connection.
- **OAuth** (third-party provider like Google/GitHub) — KenXCode points you at
  `kenxcode auth <provider>` if you haven't authenticated already.

### Tool selection at install time

After credentials are configured, KenXCode probes the MCP server to list every
...
written (if not). Re-run `kenxcode mcp configure <name>` once the server is
reachable to refine.
...
the kenxcode-agent repo, so Nous has reviewed each entry before it shipped —
...
Manifests live at
[`optional-mcps/<name>/manifest.yaml`](https://github.com/NousResearch/kenxcode-agent/tree/main/optional-mcps)
...
PR adds an entry with a newer `manifest_version` than your installed KenXCode
understands, the picker will surface a warning (`⚠ '<name>' requires a newer
KenXCode`) for that entry instead of silently hiding it. Run `kenxcode update`
to install the latest KenXCode when you see that.
...
from environment variables (which include everything in `~/.kenxcode/.env`).
...
MCPs are never auto-updated. Re-run `kenxcode mcp install <name>` to refresh
after a KenXCode update if a manifest version changed.

To add an MCP to the catalog, open a PR against
[`optional-mcps/`](https://github.com/NousResearch/kenxcode-agent/tree/main/optional-mcps).
...
HTTP MCP servers are remote endpoints KenXCode connects to directly.
...
- you do not want KenXCode spawning a local subprocess for that integration

### OAuth-authenticated HTTP servers

Most hosted MCP servers (Linear, Sentry, Atlassian, Asana, Figma, Stripe, …) require OAuth 2.1 instead of a static bearer token. Set `auth: oauth` and KenXCode handles discovery, dynamic client registration, PKCE, token exchange, refresh, and step-up auth via the MCP Python SDK.

:::tip Figma remote MCP
Figma's hosted endpoint (`https://mcp.figma.com/mcp`) allowlists Dynamic Client Registration by **exact `client_name`** — bare `"KenXCode Agent"` 403s, while `"Claude Code"` and `"Codex"` succeed. KenXCode auto-sets `oauth.client_name: "Claude Code"` for `mcp.figma.com` so install/login works without a special trick:
...
Or: `kenxcode mcp install figma`, then `kenxcode mcp login figma`.
:::
...
On first connect, KenXCode prints an authorize URL, opens your browser when possible, and waits for the OAuth callback on a local loopback port. Tokens are cached at `~/.kenxcode/mcp-tokens/<server>.json` with 0o600 perms; subsequent runs reuse them silently until refresh fails.

**Remote / headless hosts.** When KenXCode runs on a different machine than your browser, the loopback callback can't reach your laptop. Two ways to complete the flow:

- **Paste-back (no setup):** on an interactive terminal KenXCode prints "Or paste the redirect URL here…" alongside the authorize URL. Open the URL in your browser, approve, copy the full URL the browser ends up on (the redirect will show a connection error — that's expected), paste it at the prompt. Bare `?code=…&state=…` query strings work too.
- **SSH port forward:** `ssh -N -L <port>:127.0.0.1:<port> user@host` in a separate terminal, then let the redirect flow normally.
- **Proxied callback (`redirect_uri`):** when a public HTTPS endpoint forwards to the host (e.g. a Tailscale Funnel or reverse proxy pointed at the callback port), set `oauth.redirect_uri` and the browser redirect reaches KenXCode on its own — no tunnel or paste needed:
...
For fully headless gateways (messaging bot, no interactive terminal at all), the optional [`mcp-oauth-remote-gateway` skill](../skills/optional/mcp/mcp-mcp-oauth-remote-gateway.md) walks the agent through completing the flow manually and writing tokens where KenXCode expects them.
...
See [OAuth over SSH / Remote Hosts](../../guides/oauth-over-ssh.md#mcp-servers) for the full walkthrough, including DCR-less servers (e.g. Slack), pre-registered `client_id`/`client_secret`, scope customization, and re-auth via `kenxcode mcp login <server>`.

**Pitfall — providers that don't support automatic registration (Google Drive, Atlassian).** Some servers reject the dynamic client registration step (RFC 7591) that bare `auth: oauth` relies on — Google's official Drive server (`https://drivemcp.googleapis.com/mcp/v1`) returns a `400 Bad Request`, so no OAuth client is created and no token is acquired. The symptom is subtle: these servers also serve `tools/list` *without* auth, so `kenxcode mcp login` can list the tools and look like it worked, but every real tool call later times out. `kenxcode mcp login` now detects this (it checks that a token actually landed on disk) and tells you to supply your own OAuth client. Create one in the provider's console and add it to config:
...
Then run `kenxcode mcp login googledrive` — with the pre-registered client, KenXCode skips registration and runs the normal browser authorization flow.

**Pitfall — config auto-reload race.** When you edit `~/.kenxcode/config.yaml` from inside a running KenXCode session, the CLI auto-reloads MCP connections with a 30s timeout. That's not enough for an interactive OAuth flow. Add the entry, then run `kenxcode mcp login <server>` from a fresh terminal — it waits the full 5 minutes for you to complete auth.

## mTLS / client certificates

Remote HTTP MCP servers that require mutual TLS (client-certificate authentication) are supported via `client_cert` / `client_key`. KenXCode passes the resolved certificate to the underlying HTTP client for the TLS handshake.
...
KenXCode reads MCP config from `~/.kenxcode/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, KenXCode skips the server entirely |
...
For well-known MCP servers, `kenxcode mcp add` accepts a `--preset` flag that fills in the transport details so you don't have to look up the command and args. The preset only supplies defaults — anything else (env vars, headers, filtering) you pass on the same command line still wins.
...
kenxcode mcp add codex --preset codex
```
...
You can pick any local name (`kenxcode mcp add my-codex --preset codex` is fine); the preset only provides the `command`/`args` defaults.

## How KenXCode registers MCP tools

KenXCode prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — KenXCode sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, KenXCode also registers utility tools around MCP resources and prompts:
...
- KenXCode only registers resource utilities if the MCP session actually supports resource operations
- KenXCode only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to KenXCode, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, KenXCode skips the server completely and does not even attempt a connection.
...
You can also separately disable KenXCode-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, KenXCode does not create an empty runtime MCP toolset for that server.
...
KenXCode discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify KenXCode when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When KenXCode receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, KenXCode does not blindly pass your full shell environment.
...
Then verify your config and restart KenXCode.
...
Because KenXCode now only registers those wrappers when both are true:
...
When `supports_parallel_tool_calls` is `true`, KenXCode may execute multiple tools from that server at the same time within a single tool-call batch, just like it does for built-in read-only tools (web_search, read_file, etc.).
...
MCP servers can request LLM inference from KenXCode via the `sampling/createMessage` protocol. This allows an MCP server to ask KenXCode to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running KenXCode as an MCP server

In addition to connecting **to** MCP servers, KenXCode can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use KenXCode's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through KenXCode
- You want a single MCP server that bridges to all of KenXCode's connected messaging platforms at once
- You already have a running KenXCode gateway with connected platforms
...
Add KenXCode to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed KenXCode in a specific location:

```json
{
  "mcpServers": {
    "kenxcode": {
      "command": "/home/user/.kenxcode/kenxcode-agent/venv/bin/kenxcode",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a KenXCode-specific channel browser:
...
The MCP server includes a live event bridge that polls KenXCode's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
kenxcode mcp serve              # Normal mode
kenxcode mcp serve --verbose    # Debug logging on stderr
```

### How it works

The MCP server reads conversation data directly from KenXCode's session store — `~/.kenxcode/state.db` is the primary source, with `sessions.json` kept only as a legacy fallback. A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same internal send engine (`tools/send_message_tool.py`) that powers cron delivery and the `kenxcode send` CLI.
...
- The embedded `kenxcode mcp serve` exposes a **stdio-only** MCP server today. If you need an HTTP MCP server, run a separate adapter — or, much more commonly, use the MCP **client** side of KenXCode, which already speaks both stdio and HTTP (`url` + `headers` in `mcp_servers.yaml` / `config.yaml`; see [HTTP servers](#http-servers) above).
