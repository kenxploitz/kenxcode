...
Type `@` followed by a reference to inject content directly into your message. KenXCode expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- KenXCode env: `$KENXCODE_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$KENXCODE_HOME/skills/.hub/`
