...
KenXCode Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
kenxcode memory setup      # interactive picker + configuration
kenxcode memory status     # check what's active
kenxcode memory off        # disable external provider
```

You can also select the active memory provider via `kenxcode plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.kenxcode/config.yaml`:
...
When a memory provider is active, KenXCode automatically:
...
**Setup Wizard:**
```bash
kenxcode memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `kenxcode honcho setup` command still works (it now redirects to `kenxcode memory setup`), but is only registered after Honcho is selected as the active memory provider.
...
**Config:** `$KENXCODE_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$KENXCODE_HOME/honcho.json` > `~/.kenxcode/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/NousResearch/kenxcode-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/kenxcode).
...
:::tip Migrating from `kenxcode honcho`
If you previously used `kenxcode honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per KenXCode profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All KenXCode profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per KenXCode profile. Host key `kenxcode` → default; `kenxcode.<profile>` for others. |
...
```bash
kenxcode profile create coder --clone
```

`--clone` creates a `kenxcode.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.

### Existing profiles, backfill Honcho peers

```bash
kenxcode honcho sync
```

Scans every KenXCode profile, creates host blocks for any profile without one, inherits settings from the default `kenxcode` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `kenxcode memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/NousResearch/kenxcode-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/kenxcode).
...
kenxcode memory setup    # select "openviking"
# Or manually:
kenxcode config set memory.provider openviking
```

`kenxcode memory setup` can reuse or copy connection values from
`~/.openviking/ovcli.conf`. Manual setup uses the active profile's `.env` file;
for the default profile that is `~/.kenxcode/.env`, and for named profiles use
`~/.kenxcode/profiles/<profile>/.env`.
...
`OPENVIKING_AGENT` is KenXCode' peer ID in OpenViking for peer-scoped memories.
...
**Setup (Platform):**
```bash
kenxcode memory setup    # select "mem0" → "Platform"
# Or manually:
kenxcode config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.kenxcode/.env
```

**Setup (OSS):**
```bash
kenxcode memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
kenxcode memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
kenxcode memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
kenxcode memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
kenxcode memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.kenxcode/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.kenxcode/.env
```
...
**Config:** `$KENXCODE_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.kenxcode/.env`.
...
| `user_id` | `kenxcode-user` | User identifier |
| `agent_id` | `kenxcode` | Agent identifier |
...
**Switching modes:** Re-run `kenxcode memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
kenxcode memory setup    # select "hindsight"
# Or manually:
kenxcode config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.kenxcode/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p kenxcode ui start`

**Config:** `$KENXCODE_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `kenxcode` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between KenXCode Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/NousResearch/kenxcode-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
kenxcode memory setup    # select "holographic"
# Or manually:
kenxcode config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.kenxcode-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$KENXCODE_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
kenxcode memory setup    # select "retaindb"
# Or manually:
kenxcode config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.kenxcode/.env
```
...
# Then configure KenXCode
kenxcode memory setup    # select "byterover"
# Or manually:
kenxcode config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$KENXCODE_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [cloud API key](http://app.supermemory.ai/integrations?connect=kenxcode), or a [self-hosted server](https://supermemory.ai/docs/self-hosting/overview) |
...
**Setup:**
```bash
kenxcode memory setup    # select "supermemory"
# Or manually:
kenxcode config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.kenxcode/.env
```
...
Before running `kenxcode memory setup`, set `base_url` in
`$KENXCODE_HOME/supermemory.json`:
...
Then run `kenxcode memory setup` and enter the API key printed by the local
server. Configuring the endpoint first ensures the setup connection probe also
stays local.

**Config:** `$KENXCODE_HOME/supermemory.json`
...
| `container_tag` | `kenxcode` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `kenxcode-{identity}` → `kenxcode-coder`) to isolate memories per KenXCode profile
...
| **Requires** | `pip install kenxcode-memori` + `kenxcode-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install kenxcode-memori
kenxcode-memori install
kenxcode config set memory.provider memori
kenxcode memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `kenxcode-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$KENXCODE_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$KENXCODE_HOME/` so each profile has its own credentials
