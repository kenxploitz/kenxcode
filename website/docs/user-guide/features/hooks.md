...
KenXCode has four hook systems that run custom code at key lifecycle points:

| System | Registered via | Runs in | Use case |
|--------|---------------|---------|----------|
| **[Gateway hooks](#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/.kenxcode/hooks/` | Gateway only | Logging, alerts, webhooks |
...
| **[Shell hooks](#shell-hooks)** | `hooks:` block in `~/.kenxcode/config.yaml` pointing at shell scripts | CLI + Gateway | Drop-in scripts for blocking, auto-formatting, context injection |
| **[Outbound webhooks](#outbound-webhooks)** | `hooks.outbound:` list in `~/.kenxcode/config.yaml` | CLI + Gateway | Push signed lifecycle events to external HTTP endpoints — CI, dashboards, other agents |
...
Each hook is a directory under `~/.kenxcode/hooks/` containing two files:
...
LOG_FILE = Path.home() / ".kenxcode" / "hooks" / "my-hook" / "activity.log"
...
```yaml
# ~/.kenxcode/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.kenxcode/hooks/long-task-alert/handler.py
import os
import httpx
...
```yaml
# ~/.kenxcode/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.kenxcode/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".kenxcode" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/.kenxcode/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.kenxcode/hooks/session-webhook/handler.py
import httpx

WEBHOOK_URL = "https://your-service.example.com/kenxcode-events"
...
A popular pattern from the community: drop a Markdown checklist at `~/.kenxcode/BOOT.md`, and have the agent run it once every time the gateway starts. Useful for "on every boot, check overnight cron failures and ping me on Discord if anything failed," or "summarize the last 24h of deploy.log and post it to Slack #ops."

This tutorial shows how to build it yourself as a user-defined hook. KenXCode does not ship a built-in BOOT.md hook — you wire up exactly the behavior you want.

#### What we're building

1. A file at `~/.kenxcode/BOOT.md` with natural-language startup instructions.
...
Create `~/.kenxcode/BOOT.md`. Write it as if you were giving instructions to a human assistant:

```markdown
# Startup Checklist

1. Run `kenxcode cron list` and check if any scheduled jobs failed overnight.
...
```text
~/.kenxcode/hooks/boot-md/
├── HOOK.yaml
└── handler.py
```

**`~/.kenxcode/hooks/boot-md/HOOK.yaml`**

```yaml
name: boot-md
description: Run ~/.kenxcode/BOOT.md on gateway startup
events:
  - gateway:startup
```

**`~/.kenxcode/hooks/boot-md/handler.py`**

```python
"""Run ~/.kenxcode/BOOT.md on every gateway startup."""
...
BOOT_FILE = Path.home() / ".kenxcode" / "BOOT.md"
...
```bash
kenxcode logs --follow --level INFO | grep boot-md
```
...
Delete `~/.kenxcode/BOOT.md` to disable the checklist — the hook stays loaded but silently skips when the file isn't there.
...
**Where context is injected:** Always the **user message**, never the system prompt. This preserves the prompt cache — the system prompt stays identical across turns, so cached tokens are reused. The system prompt is KenXCode's territory (model guidance, tool enforcement, personality, skills). Plugins contribute context alongside the user's input.
...
Declare shell-script hooks in your `~/.kenxcode/config.yaml` and KenXCode will run them as subprocesses whenever the corresponding plugin-hook event fires — in both CLI and gateway sessions. No Python plugin authoring required.
...
Shell hooks are registered by calling `agent.shell_hooks.register_from_config(cfg)` at both CLI startup (`kenxcode_cli/main.py`) and gateway startup (`gateway/run.py`). They compose naturally with Python plugin hooks — both flow through the same dispatcher.

### Comparison at a glance

| Dimension | Shell hooks | [Plugin hooks](#plugin-hooks) | [Gateway hooks](#gateway-event-hooks) |
|-----------|-------------|-------------------------------|---------------------------------------|
| Declared in | `hooks:` block in `~/.kenxcode/config.yaml` | `register()` in a `plugin.yaml` plugin | `HOOK.yaml` + `handler.py` directory |
| Lives under | `~/.kenxcode/agent-hooks/` (by convention) | `~/.kenxcode/plugins/<name>/` | `~/.kenxcode/hooks/<name>/` |
...
Each time the event fires, KenXCode spawns a subprocess for every matching hook (matcher permitting), pipes a JSON payload to **stdin**, and reads **stdout** back as JSON.
...
{"action":   "block", "message": "Forbidden: rm -rf"}   // KenXCode-canonical
...
hooks:
  post_tool_call:
    - matcher: "write_file|patch"
      command: "~/.kenxcode/agent-hooks/auto-format.sh"
```

```bash
#!/usr/bin/env bash
# ~/.kenxcode/agent-hooks/auto-format.sh
payload="$(cat -)"
...
hooks:
  pre_tool_call:
    - matcher: "terminal"
      command: "~/.kenxcode/agent-hooks/block-rm-rf.sh"
      timeout: 5
```

```bash
#!/usr/bin/env bash
# ~/.kenxcode/agent-hooks/block-rm-rf.sh
payload="$(cat -)"
...
hooks:
  pre_llm_call:
    - command: "~/.kenxcode/agent-hooks/inject-cwd-context.sh"
```

```bash
#!/usr/bin/env bash
# ~/.kenxcode/agent-hooks/inject-cwd-context.sh
...
Claude Code's `UserPromptSubmit` event is intentionally not a separate KenXCode event — `pre_llm_call` fires at the same place and already supports context injection. Use it here.
...
hooks:
  subagent_stop:
    - command: "~/.kenxcode/agent-hooks/log-orchestration.sh"
```

```bash
#!/usr/bin/env bash
# ~/.kenxcode/agent-hooks/log-orchestration.sh
log=~/.kenxcode/logs/orchestration.log
...
Each unique `(event, command)` pair prompts the user for approval the first time KenXCode sees it, then persists the decision to `~/.kenxcode/shell-hooks-allowlist.json`. Subsequent runs (CLI or gateway) skip the prompt.

Three escape hatches bypass the interactive prompt — any one is sufficient:

1. `--accept-hooks` flag on the CLI (e.g. `kenxcode --accept-hooks chat`)
2. `KENXCODE_ACCEPT_HOOKS=1` environment variable
3. `hooks_auto_accept: true` in `~/.kenxcode/config.yaml`
...
**Script edits are silently trusted.** The allowlist keys on the exact command string, not the script's hash, so editing the script on disk does not invalidate consent. `kenxcode hooks doctor` flags mtime drift so you can spot edits and decide whether to re-approve.

#### Manual allowlisting

Manual allowlisting is useful for non-TTY or service-account deployments where an operator cannot answer the first-use prompt interactively. The allowlist file is `~/.kenxcode/shell-hooks-allowlist.json`, and the expected format is an `approvals` array. Each approval records the hook `event` and the exact `command` string:
...
```json
{
  "approvals": [
    {
      "event": "post_llm_call",
      "command": "/home/kenxcode/.kenxcode/hooks/my-hook.py"
    }
  ]
}
```

The command string must match the configured hook command exactly. A path-keyed object with a `sha256` field is not the expected format and will not approve the hook. Verify manual entries with `kenxcode hooks list`.

### The `kenxcode hooks` CLI

| Command | What it does |
|---------|--------------|
| `kenxcode hooks list` | Dump configured hooks with matcher, timeout, and consent status |
| `kenxcode hooks test <event> [--for-tool X] [--payload-file F]` | Fire every matching hook against a synthetic payload and print the parsed response |
| `kenxcode hooks revoke <command>` | Remove every allowlist entry matching `<command>` (takes effect on next restart) |
| `kenxcode hooks doctor` | For every configured hook: check exec bit, allowlist status, mtime drift, JSON output validity, and rough execution time |
...
- Keep scripts inside `~/.kenxcode/agent-hooks/` so the path is easy to audit.
- Re-run `kenxcode hooks doctor` after you pull a shared config to spot newly-added hooks before they register.
...
Outbound webhooks are the push-side mirror of the [inbound webhook platform](/user-guide/messaging/webhooks): inbound webhooks wake KenXCode when the world changes; outbound webhooks tell the world when KenXCode does something. Configure a list of HTTP endpoints and the lifecycle events they care about, and KenXCode POSTs a signed JSON payload to each endpoint whenever a matching event fires — no polling on the receiving end.
...
- Wake *another* KenXCode instance: point the URL at that instance's inbound webhook

### Configuration

Add a `hooks.outbound:` list to `~/.kenxcode/config.yaml`:

```yaml
hooks:
  outbound:
    - name: ci-notify                       # optional label for logs
      url: https://ci.example.com/kenxcode-events
      events: [on_session_end, subagent_stop]
      secret_env: KENXCODE_OUTBOUND_WEBHOOK_SECRET   # env var holding the HMAC secret
      timeout: 10                           # per-attempt seconds (1–60)

    - name: tool-monitor
      url: https://metrics.example.com/hooks/kenxcode
...
Secrets: prefer `secret_env` (the name of an environment variable, typically set in `~/.kenxcode/.env`) over an inline `secret:` literal, so the config file stays free of credentials. Entries without a secret are delivered unsigned (flagged as `UNSIGNED` by `kenxcode hooks list`).
...
| `X-KenXCode-Event` | The hook event name |
| `X-KenXCode-Delivery` | Unique id per delivery — same value as `delivery_id` in the body |
| `X-KenXCode-Signature-256` | `sha256=<hex>` — HMAC-SHA256 of the raw body, GitHub-style; only present when a secret is configured |
...
- **Dedupe** on `delivery_id` (or the matching `X-KenXCode-Delivery` header) — remember recently seen ids and skip duplicates. KenXCode retries failed deliveries once, so the same id can legitimately arrive twice.
...
- **No consent prompt.** Outbound targets execute no code on your machine — they receive data at a URL you configured. `KENXCODE_SAFE_MODE=1` still skips registration, same as plugins and shell hooks. Note that payloads include tool inputs and event metadata, so only point targets at endpoints you trust, and prefer `https://`.

`kenxcode hooks list` shows configured outbound targets alongside shell hooks, including whether each target is signed.
