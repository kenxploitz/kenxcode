...
Schedule tasks to run automatically with natural language or cron expressions. KenXCode exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
All of this is available to KenXCode itself through the `cronjob` tool, so you can create, pause, edit, and remove jobs by asking in plain language — no CLI required.

:::tip
**Which model does a cron job run on?** Resolution at fire time is: per-job pin → `cron.model` in `config.yaml` → the global default from `kenxcode model`.

- **Per-job pin** — set by *you* via the dashboard, `kenxcode cron create/edit --model … --provider …`, or by editing `~/.kenxcode/cron/jobs.json`. Once set, it sticks until you change it. The agent's `cronjob` tool cannot set or change per-job models — inference pins are user-owned.
- **`cron.model` / `cron.model_provider`** — a cron-fleet default: every unpinned job runs on this model, independent of your chat model. Set it once (`kenxcode config set cron.model <name>`) and switching your chat model with `kenxcode model` or `/model` never touches your cron fleet.
- **Global default** — only when neither of the above is set does a job follow `kenxcode model`. In this case KenXCode **snapshots** the provider and model at creation, and if the global default later changes the job **fails closed**: it skips the run, makes no inference call, and alerts you to pin the provider/model explicitly (#44585). This prevents an unattended job from silently inheriting a switch to a paid provider/model. Setting `cron.model` (or a per-job pin) is the deliberate way to route cron spend, and the drift guard does not engage for an axis covered by it. Operators who instead want unpinned jobs to track the changing global default can [disable the drift guard](#letting-unpinned-jobs-track-global-defaults).

`kenxcode setup --portal` is the lowest-friction option for unattended runs since OAuth refresh is automatic. See [Nous Portal](/integrations/nous-portal).
:::

:::warning
Cron-run sessions cannot recursively create more cron jobs. KenXCode disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
kenxcode cron create "every 2h" "Check server status"
kenxcode cron create "every 1h" "Summarize new feed items" --skill blogwatcher
kenxcode cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Ask KenXCode normally:

```text
Every morning at 9am, check Hacker News for AI news and send me a summary on Telegram.
```

KenXCode will use the unified `cronjob` tool internally.
...
```bash
kenxcode config set cron.model_drift_guard false
```
...
kenxcode cron create "every 1d at 09:00" \
...
```bash
kenxcode cron edit <job_id> --schedule "every 4h"
kenxcode cron edit <job_id> --prompt "Use the revised task"
kenxcode cron edit <job_id> --skill blogwatcher --skill maps
kenxcode cron edit <job_id> --add-skill maps
kenxcode cron edit <job_id> --remove-skill blogwatcher
kenxcode cron edit <job_id> --clear-skills
```
...
```bash
kenxcode cron list
kenxcode cron pause <job_id_or_name>
kenxcode cron resume <job_id_or_name>
kenxcode cron run <job_id_or_name>
kenxcode cron remove <job_id_or_name>
kenxcode cron edit <job_id_or_name> [...flags]
kenxcode cron status
kenxcode cron tick
```
...
```bash
kenxcode gateway install     # Install as a user service
sudo kenxcode gateway install --system   # Linux: boot-time system service for servers
kenxcode gateway             # Or run in foreground
...
On each tick KenXCode:

1. loads jobs from `~/.kenxcode/cron/jobs.json`
...
A file lock at `~/.kenxcode/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.

### Execution history

KenXCode records each claimed cron attempt in the profile-local
`~/.kenxcode/cron/executions.db` before executor or provider dispatch. Attempts
move through `claimed`, `running`, and one immutable terminal state:
`completed`, `failed`, or `unknown`. After restart, KenXCode marks an abandoned
...
Inspect recent attempts with `kenxcode cron runs [job-id] --limit 20` (alias:
...
| `"local"` | Save to local files only (`~/.kenxcode/cron/output/`) | Default on CLI |
...
If the agent's final response contains `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.kenxcode/cron/output/`), but no message is sent to the delivery target.
...
Or set the `KENXCODE_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 3600s default.
...
`.sh` / `.bash` files run under `bash` from `PATH` when available, otherwise `/bin/bash` (important on Windows Git Bash). Anything else runs under the current Python interpreter (`sys.executable`). Scripts must resolve inside `$KENXCODE_HOME/scripts/` — relative names, absolute paths, and `~`-prefixed paths are accepted when the resolved target stays in that directory; paths that escape it are rejected. Subprocess env is sanitized (`_sanitize_subprocess_env`): provider API credentials and other KenXCode-managed secrets are **not** inherited by cron scripts.

### The agent sets these up for you

The `cronjob` tool's schema exposes `no_agent` to KenXCode directly, so you can describe a watchdog in chat and let the agent wire it up:

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

KenXCode will write the check script to `~/.kenxcode/scripts/` via `write_file`, then call:
...
cronjob(
    action="create",
    prompt="Fetch the top 10 AI/ML stories from Hacker News. Save them to ~/.kenxcode/data/briefs/raw.md in markdown format with title, URL, and score.",
    schedule="0 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.kenxcode/data/briefs/raw.md. Score each story 1–10 for engagement potential and novelty. Output the top 5 to ~/.kenxcode/data/briefs/ranked.md.",
    schedule="30 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.kenxcode/data/briefs/ranked.md. Write 3 tweet drafts (hook + body + hashtags). Deliver to telegram:7976161601.",
    schedule="0 8 * * *",
...
- When Job 2 fires, KenXCode reads Job 1's most recent output from `~/.kenxcode/cron/output/{job1_id}/*.md`
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `kenxcode tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `kenxcode tools` cron-platform config wins; otherwise KenXCode falls back to the built-in defaults. This matters for cost control: carrying `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether KenXCode should even invoke the agent. Emit a final stdout line of the form:
...
```bash
#!/bin/bash
# ~/.kenxcode/scripts/feed-changed.sh
FEED="$HOME/data/feed.json"
STATE="$HOME/.kenxcode/scripts/.feed-changed.last"
...
```bash
#!/bin/bash
# ~/.kenxcode/scripts/flag-ready.sh
...
:::tip
KenXCode's own `~/.kenxcode/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::

Credit: this recipe set was prompted by @iankar8's exploration in [#2654](https://github.com/NousResearch/kenxcode-agent/pull/2654), which proposed adding sql/file/command triggers as a parallel mechanism. The `script` + `wakeAgent` gate already covers all three cases at $0, so the work landed as documentation instead.
...
Jobs are stored in `~/.kenxcode/cron/jobs.json`. Output from job runs is saved to `~/.kenxcode/cron/output/{job_id}/{timestamp}.md`.

:::tip
Ask the agent to manage jobs through the `cronjob` tool, `kenxcode cron edit`, or `/cron` — not by patching `jobs.json` directly. Direct edits can fail silently when [file write safety](../security.md#file-write-safety) blocks the path (for example when `KENXCODE_WRITE_SAFE_ROOT` is set), and the [file-mutation verifier](../configuration.md#file-mutation-verifier) footer is the authoritative signal that nothing was saved.
:::

Jobs may store `model` and `provider` as `null`. When those fields are omitted, KenXCode resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
