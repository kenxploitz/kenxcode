...
`~/.kenxcode/config.yaml`, `~/.kenxcode/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/kenxcode`:

```text
/etc/kenxcode/
├── config.yaml     # managed config layer (wins over ~/.kenxcode/config.yaml)
└── .env            # managed env layer (wins over ~/.kenxcode/.env + shell)
```
...
The location can be relocated with the `KENXCODE_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `KENXCODE_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by KenXCode.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export KENXCODE_MANAGED_DIR=/opt/org/kenxcode-policy
```

:::warning
A user who can set `KENXCODE_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `kenxcode doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/kenxcode/config.yaml` (managed) | `/etc/kenxcode/.env` (managed) |
| 2 | `~/.kenxcode/config.yaml` (user) | `~/.kenxcode/.env` (user) |
...
```bash
kenxcode config        # shows a header naming the managed source + the pinned keys
kenxcode doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, KenXCode refuses and names the source:

```bash
$ kenxcode config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/kenxcode/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `kenxcode config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/kenxcode

# Pin some config values for every user on this machine
sudo tee /etc/kenxcode/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/kenxcode/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/kenxcode
sudo chmod 0644 /etc/kenxcode/config.yaml /etc/kenxcode/.env
```

Changes take effect on the next KenXCode start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`kenxcode doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs KenXCode as `root`), managed scope is advisory.
