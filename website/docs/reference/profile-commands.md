...
This page covers all commands related to [KenXCode profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `kenxcode profile` without a subcommand shows help.
...
## `kenxcode profile use`

```bash
kenxcode profile use <name>
```

Sets `<name>` as the active profile. All subsequent `kenxcode` commands (without `-p`) will use this profile.
...
kenxcode profile use default
```

## `kenxcode profile create`

```bash
kenxcode profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `kenxcode profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `kenxcode update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.kenxcode`), use `kenxcode skills opt-out` / `kenxcode skills opt-in`. |
...
kenxcode profile create work2 --clone-from work

# Clone everything from a specific profile
kenxcode profile create work2-backup --clone-from work --clone-all
```

## `kenxcode profile describe`

```bash
kenxcode profile describe [<name>] [options]
```
...
kenxcode profile describe researcher

# Set it explicitly
kenxcode profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
kenxcode profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
kenxcode profile describe --all --auto
```

## `kenxcode profile delete`

```bash
kenxcode profile delete <name> [options]
```
...
```bash
kenxcode profile delete mybot
kenxcode profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.kenxcode`) cannot be deleted — use `kenxcode uninstall` to remove everything.
:::

## `kenxcode profile show`

```bash
kenxcode profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's KenXCode home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ kenxcode profile show work
Profile: work
Path:    ~/.kenxcode/profiles/work
...
## `kenxcode profile alias`

```bash
kenxcode profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your KenXCode installation.
...
```bash
kenxcode profile alias work
# Creates/updates ~/.local/bin/work

kenxcode profile alias work --name mywork
# Creates ~/.local/bin/mywork

kenxcode profile alias work --remove
# Removes the wrapper script
```

## `kenxcode profile rename`

```bash
kenxcode profile rename <old-name> <new-name>
```
...
```bash
kenxcode profile rename mybot assistant
# ~/.kenxcode/profiles/mybot → ~/.kenxcode/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `kenxcode profile export`

```bash
kenxcode profile export <name> [options]
```
...
```bash
kenxcode profile export work
# Creates work.tar.gz in the current directory

kenxcode profile export work -o ./work-2026-03-29.tar.gz
```

## `kenxcode profile import`

```bash
kenxcode profile import <archive> [options]
```
...
```bash
kenxcode profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

kenxcode profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`kenxcode profile export` / `import` are still the right commands for
...
### `kenxcode profile install`

```bash
kenxcode profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `kenxcode -p telemetry`). |
...
kenxcode profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
kenxcode profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
kenxcode profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
kenxcode profile install ./telemetry/
```

### `kenxcode profile update`

```bash
kenxcode profile update <name> [--force-config] [--yes]
```
...
### `kenxcode profile info`

```bash
kenxcode profile info <name>
```

Prints the profile's distribution manifest — name, version, required
KenXCode version, author, env var requirements, the source URL/path, and
...
`kenxcode profile list` also shows the distribution name and version in a
`Distribution` column, and `kenxcode profile show <name>` / `delete <name>`
...
kenxcode profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
kenxcode profile install https://github.com/your-org/internal-assistant.git
```
...
`kenxcode_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
KenXCode version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host KenXCode can clone from.
3. Tell recipients to run `kenxcode profile install <your-repo-url>`.
...
## `kenxcode -p` / `kenxcode --profile`

```bash
kenxcode -p <name> <command> [options]
kenxcode --profile <name> <command> [options]
```

Global flag to run any KenXCode command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
kenxcode -p work chat -q "Check the server status"
kenxcode --profile dev gateway start
kenxcode -p personal skills list
kenxcode -p work config edit
```

## `kenxcode completion`

```bash
kenxcode completion <shell>
```
...
- `kenxcode profile <TAB>` — subcommands (list, use, create, etc.)
- `kenxcode profile use <TAB>` — profile names
- `kenxcode -p <TAB>` — profile names
