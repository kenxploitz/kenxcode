...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with KenXCode Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working KenXCode setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [KenXCode Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="KenXCode Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up KenXCode for a team, bot, or always-on workflow
...
|---|---|---|
| I just want KenXCode working on my machine | `kenxcode setup` | Run a real chat and verify it responds |
| I already know my provider | `kenxcode model` | Save the config, then start chatting |
| I want a bot or always-on setup | `kenxcode gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `kenxcode model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `kenxcode model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if KenXCode cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install KenXCode Agent
### With the KenXCode Desktop installer on macOS or Windows (recommended)
To easily install the command-line and desktop applications, [download the KenXCode Desktop installer](https://kenxcode-agent.nousresearch.com/) from our website and run it.

### Without KenXCode Desktop:
For a command-line only install without KenXCode Desktop, run:
...
The single most important setup step. Use `kenxcode model` to walk through the choice interactively:
...
:::info Setup modes
On a fresh install, `kenxcode setup` offers three modes:
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `kenxcode update`. Re-enable anything later with `kenxcode tools`, seed skills with `kenxcode skills opt-in --sync`, or tune settings with `kenxcode setup agent`.
:::
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `kenxcode model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `kenxcode model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `kenxcode model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `kenxcode_cli/models.py` may change between releases) | `kenxcode model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `kenxcode model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `kenxcode model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `kenxcode model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `kenxcode model` (requires `copilot` CLI + `copilot login`) |
...
KenXCode Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `kenxcode model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

KenXCode separates secrets from normal config:

- **Secrets and tokens** → `~/.kenxcode/.env`
- **Non-secret settings** → `~/.kenxcode/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
kenxcode config set model anthropic/claude-opus-4.6
kenxcode config set terminal.backend docker
kenxcode config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
kenxcode            # classic CLI
kenxcode --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
KenXCode ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `kenxcode` vs `kenxcode --tui`.
:::
...
- KenXCode replies without error
...
```bash
kenxcode --continue    # Resume the most recent session
kenxcode -c            # Short form
```
...
```bash
kenxcode gateway setup    # Interactive platform configuration
```
...
- `kenxcode tools` — tune tool access per platform
- `kenxcode skills` — browse and install reusable workflows
...
```bash
kenxcode config set terminal.backend docker    # Docker isolation
kenxcode config set terminal.backend ssh       # Remote server
```

For Docker sandboxes, you can also enable the **egress credential-injection proxy** so the sandbox never sees your real API keys — only opaque proxy tokens that work exclusively from behind a local TLS-intercepting daemon. See [Egress proxy](../user-guide/egress/iron-proxy.md). Setup is `kenxcode egress setup && kenxcode egress start`; `kenxcode setup terminal` also points Docker users at it. Modal, SSH, Daytona, and Singularity are not wired yet.

### Voice mode

```bash
# From the KenXCode install directory (the curl installer placed it at
# ~/.kenxcode/kenxcode-agent on Linux/macOS or %LOCALAPPDATA%\kenxcode\kenxcode-agent on Windows):
cd ~/.kenxcode/kenxcode-agent
...
Skills are on-demand instruction documents that teach KenXCode how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

KenXCode ships with a catalog of bundled skills already installed in `~/.kenxcode/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
kenxcode skills browse                      # list everything available
kenxcode skills search kubernetes           # find skills by keyword
kenxcode skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `kenxcode skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let KenXCode ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.kenxcode/kenxcode-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| KenXCode opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `kenxcode model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `kenxcode gateway setup` and check `kenxcode gateway status` |
| `kenxcode --continue` can't find old session | Switched profiles or session never saved | Check `kenxcode sessions list` and confirm you're in the right profile |
...
| `kenxcode doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `kenxcode model`
3. `kenxcode setup`
4. `kenxcode sessions list`
5. `kenxcode --continue`
6. `kenxcode gateway status`
...
| Command | Description |
|---------|-------------|
| `kenxcode` | Start chatting |
| `kenxcode model` | Choose your LLM provider and model |
| `kenxcode tools` | Configure which tools are enabled per platform |
| `kenxcode setup` | Full setup wizard (configures everything at once) |
| `kenxcode doctor` | Diagnose issues |
| `kenxcode update` | Update to latest version |
| `kenxcode gateway` | Start the messaging gateway |
| `kenxcode --continue` | Resume last session |
