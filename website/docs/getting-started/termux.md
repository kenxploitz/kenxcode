...
description: "Run KenXCode Agent directly on an Android phone with Termux"
---

# KenXCode on Android with Termux
...
KenXCode Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop KenXCode from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

KenXCode now ships a Termux-aware installer path:
...
- links `kenxcode` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `kenxcode` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/kenxcode" "$PREFIX/bin/kenxcode"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `kenxcode` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.kenxcode/.env`.
...
### `kenxcode doctor` says ripgrep or Node is missing
