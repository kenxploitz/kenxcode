...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/kenxcode-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    kenxcode_meta = (fm.get("metadata") or {}).get("kenxcode") or {}
    tags = kenxcode_meta.get("tags") or []
    related = kenxcode_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `kenxcode skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with KenXCode Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "KenXCode ships with a large built-in skill library copied into `~/.kenxcode/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "KenXCode also syncs bundled skills on `kenxcode update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.kenxcode/skills/` tree, it is still shipped with KenXCode; restore it with `kenxcode skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with kenxcode-agent — install via kenxcode skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with kenxcode-agent under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
